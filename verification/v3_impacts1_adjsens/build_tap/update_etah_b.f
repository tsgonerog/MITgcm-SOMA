...
C
C  Differentiation of update_etah in reverse (adjoint) mode:
C   gradient     of useful results: etan etah detahdt
C   with respect to varying inputs: etan etah detahdt
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE UPDATE_ETAH_B(mytime, myiter, mythid)
      IMPLICIT NONE
C     == Global variables
CBOP
C    !ROUTINE: SIZE.h
...
C
            DO j=1,sny
              DO i=1,snx
                CALL PUSHREAL8(etah(i, j, bi, bj))
                etah(i, j, bi, bj) = etan(i, j, bi, bj) + (1.-
...
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
C
      IF (implicdiv2dflow .NE. 1.d0 .OR. (useobcs .AND. nonlinfreesurf 
     +    .GT. 0)) CALL EXCH_XY_RL_B(etah, etahb, mythid)
          ...
          IF (branch .EQ. 0) THEN
            DO j=sny+oly,1-oly,-1
              DO i=snx+olx,1-olx,-1
                CALL POPREAL8(etah(i, j, bi, bj))
                etanb(i, j, bi, bj) = etanb(i, j, bi, bj) + etahb(i, j, 
     +            bi, bj)
                etahb(i, j, bi, bj) = 0.D0
              ENDDO
            ENDDO
          ELSE
            DO j=sny,1,-1
              DO i=snx,1,-1
                ...
                etanb(i, j, bi, bj) = etanb(i, j, bi, bj) + etahb(i, j, 
     +            bi, bj)
                detahdtb(i, j, bi, bj) = detahdtb(i, j, bi, bj) + (1.-
     +            implicdiv2dflow)*deltatfreesurf*etahb(i, j, bi, bj)
                etahb(i, j, bi, bj) = 0.D0
              ENDDO
            ENDDO
          END IF
