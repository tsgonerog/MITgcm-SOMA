...
C
C  Differentiation of apply_forcing_u in reverse (adjoint) mode:
C   gradient     of useful results: surfaceforcingu gu_arr
C   with respect to varying inputs: surfaceforcingu
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE APPLY_FORCING_U_B(gu_arr, gu_arrb, imin, imax, jmin, 
     +                             jmax, k, bi, bj, mytime, myiter, 
     +                             mythid)
      IMPLICIT NONE
C     == Global data ==
CBOP
...
C     myThid    :: my Thread Id number
      ...
      REAL*8 gu_arrb(1-olx:snx+olx, 1-oly:sny+oly)
...
C     Ocean: Add momentum surface forcing (e.g., wind-stress) in surface level
      IF (k .EQ. ksurface) THEN
        DO j=sny+1,0,-1
          DO i=snx+1,1,-1
            surfaceforcingub(i, j, bi, bj) = surfaceforcingub(i, j, bi, 
     +        bj) + fofacmom*recip_drf(k)*recip_hfacw(i, j, k, bi, bj)*
     +        gu_arrb(i, j)
          ENDDO
        ENDDO
      ELSE IF (ksurface .EQ. -1) THEN
        DO j=0,sny+1
          DO i=1,snx+1
        ...
        DO j=sny+1,0,-1
          DO i=snx+1,1,-1
            ...
            IF (branch .NE. 0) surfaceforcingub(i, j, bi, bj) = 
     +          surfaceforcingub(i, j, bi, bj) + fofacmom*recip_drf(k)*
     +          recip_hfacw(i, j, k, bi, bj)*gu_arrb(i, j)
          ENDDO
        ENDDO
      END IF
      CALL POPCONTROL1B(branch)
...
C  Differentiation of apply_forcing_v in reverse (adjoint) mode:
C   gradient     of useful results: surfaceforcingv gv_arr
C   with respect to varying inputs: surfaceforcingv
C
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
CBOP
C     !ROUTINE: APPLY_FORCING_V
C     !INTERFACE:
      SUBROUTINE APPLY_FORCING_V_B(gv_arr, gv_arrb, imin, imax, jmin, 
     +                             jmax, k, bi, bj, mytime, myiter, 
     +                             mythid)
      IMPLICIT NONE
C     == Global data ==
...
C     myThid    :: my Thread Id number
      ...
      REAL*8 gv_arrb(1-olx:snx+olx, 1-oly:sny+oly)
      INTEGER imin, imax, jmin, jmax
      INTEGER k, bi, bj
      REAL*8 mytime
      INTEGER myiter
...
C     Ocean: Add momentum surface forcing (e.g., wind-stress) in surface level
      IF (k .EQ. ksurface) THEN
        DO j=sny+1,1,-1
          DO i=snx+1,0,-1
            surfaceforcingvb(i, j, bi, bj) = surfaceforcingvb(i, j, bi, 
     +        bj) + fofacmom*recip_drf(k)*recip_hfacs(i, j, k, bi, bj)*
     +        gv_arrb(i, j)
          ENDDO
        ENDDO
      ELSE IF (ksurface .EQ. -1) THEN
        ...
        DO j=sny+1,1,-1
          DO i=snx+1,0,-1
            CALL POPCONTROL1B(branch)
            IF (branch .NE. 0) surfaceforcingvb(i, j, bi, bj) = 
     +          surfaceforcingvb(i, j, bi, bj) + fofacmom*recip_drf(k)*
     +          recip_hfacs(i, j, k, bi, bj)*gv_arrb(i, j)
          ENDDO
        ENDDO
      END IF
      CALL POPCONTROL1B(branch)
...
C  Differentiation of apply_forcing_t in reverse (adjoint) mode:
C   gradient     of useful results: wvel theta salt qsw surfaceforcingt
C                tsurfcor gt_arr
C   with respect to varying inputs: wvel theta salt qsw surfaceforcingt
C                tsurfcor gt_arr
C
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
CBOP
...
C     !INTERFACE:
      SUBROUTINE APPLY_FORCING_T_B(gt_arr, gt_arrb, imin, imax, jmin, 
     +                             jmax, k, bi, bj, mytime, myiter, 
     +                             mythid)
      IMPLICIT NONE
C     == Global data ==
...
C     myThid    :: my Thread Id number
      ...
      REAL*8 gt_arrb(1-olx:snx+olx, 1-oly:sny+oly)
      INTEGER imin, imax, jmin, jmax
      INTEGER k, bi, bj
      REAL*8 mytime
      INTEGER myiter
...
C     kSurface  :: index of surface level
      ...
      REAL*8 tmpvarb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 tmpfac, delpi
      REAL*8 recip_cp
      REAL*8 swfracb(2)
...
C
      IF (fluidisair) THEN
        ...
        ksurface = 0
      ELSE IF (usingzcoords .AND. useshelfice) THEN
...
C     conversion from W/kg to K/s
        ...
        IF (k .EQ. 1) THEN
          CALL PUSHCONTROL1B(0)
        ELSE
          delpi = atm_cp*((rc(km)/atm_po)**atm_kappa-(rc(kc)/atm_po)**
     +      atm_kappa)
          CALL PUSHCONTROL1B(1)
        END IF
        IF (k .LT. nr) THEN
          CALL PUSHREAL8(delpi)
          delpi = atm_cp*((rc(kc)/atm_po)**atm_kappa-(rc(kp)/atm_po)**
     +      atm_kappa)
          CALL PUSHCONTROL1B(1)
        ELSE
          CALL PUSHCONTROL1B(0)
...
C     this is the oceanic pressure coordinate case
        ...
        END IF
      END IF
      CALL SWFRAC(2, onerl, swfracb, mytime, myiter, mythid)
      IF (k .EQ. km) swfracb(2) = 0.d0
      DO j=sny+1,0,-1
        DO i=snx+1,0,-1
          qswb(i, j, bi, bj) = qswb(i, j, bi, bj) - recip_hfacc(i, j, k
     +      , bi, bj)*recip_cp*mass2runit*recip_drf(k)*(swfracb(1)*maskc
     +      (i, j, k, bi, bj)-swfracb(2)*maskc(i, j, km, bi, bj))*
     +      gt_arrb(i, j)
        ENDDO
      ENDDO
      CALL POPCONTROL1B(branch)
      ...
      IF (branch .EQ. 0) THEN
        DO j=sny+1,0,-1
          DO i=snx+1,0,-1
            CALL POPCONTROL1B(branch)
            IF (branch .NE. 0) tsurfcorb = tsurfcorb + recip_drf(k)*
     +          recip_hfacc(i, j, k, bi, bj)*gt_arrb(i, j)
          ENDDO
        ENDDO
      END IF
      ...
      IF (branch .EQ. 0) THEN
        DO j=sny+1,0,-1
          DO i=snx+1,0,-1
            surfaceforcingtb(i, j, bi, bj) = surfaceforcingtb(i, j, bi, 
     +        bj) + recip_drf(k)*recip_hfacc(i, j, k, bi, bj)*gt_arrb(i
     +        , j)
          ENDDO
        ENDDO
      ELSE IF (branch .EQ. 1) THEN
        DO j=sny+1,0,-1
          DO i=snx+1,0,-1
            CALL POPCONTROL1B(branch)
            IF (branch .NE. 0) surfaceforcingtb(i, j, bi, bj) = 
     +          surfaceforcingtb(i, j, bi, bj) + recip_drf(k)*
     +          recip_hfacc(i, j, k, bi, bj)*gt_arrb(i, j)
          ENDDO
        ENDDO
      END IF
      ...
      IF (branch .EQ. 0) THEN
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            tmpvarb(ii2, ii1) = 0.D0
          ENDDO
        ENDDO
        DO j=sny+1,0,-1
          DO i=snx+1,0,-1
            tmpvarb(i, j) = tmpvarb(i, j) + tmpfac*recip_drf(k)*
     +        recip_hfacc(i, j, k, bi, bj)*gt_arrb(i, j)
          ENDDO
        ENDDO
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) THEN
          DO j=sny+1,0,-1
            DO i=snx+1,0,-1
              temp2 = salt(i, j, kp, bi, bj)
              temp1 = theta(i, j, kp, bi, bj)
              temp0 = salt(i, j, kc, bi, bj)
              temp = theta(i, j, kc, bi, bj)
              tempb0 = delpi*0.25d0*atm_rq*maskc(i, j, kp, bi, bj)*
     +          tmpvarb(i, j)
              wvelb(i, j, kp, bi, bj) = wvelb(i, j, kp, bi, bj) + (temp*
     +          temp0+temp1*temp2)*tempb0
              tempb = wvel(i, j, kp, bi, bj)*tempb0
              thetab(i, j, kc, bi, bj) = thetab(i, j, kc, bi, bj) + 
     +          temp0*tempb
              saltb(i, j, kc, bi, bj) = saltb(i, j, kc, bi, bj) + temp*
     +          tempb
              thetab(i, j, kp, bi, bj) = thetab(i, j, kp, bi, bj) + 
     +          temp2*tempb
              saltb(i, j, kp, bi, bj) = saltb(i, j, kp, bi, bj) + temp1*
     +          tempb
            ENDDO
          ENDDO
          CALL POPREAL8(delpi)
        ...
        IF (branch .NE. 0) THEN
          DO j=sny+1,0,-1
            DO i=snx+1,0,-1
              temp = salt(i, j, kc, bi, bj)
              temp0 = theta(i, j, kc, bi, bj)
              temp1 = salt(i, j, km, bi, bj)
              temp2 = theta(i, j, km, bi, bj)
              tempb = delpi*0.25d0*atm_rq*maskc(i, j, km, bi, bj)*
     +          tmpvarb(i, j)
              tmpvarb(i, j) = 0.D0
              wvelb(i, j, kc, bi, bj) = wvelb(i, j, kc, bi, bj) + (temp2
     +          *temp1+temp0*temp)*tempb
              tempb0 = wvel(i, j, kc, bi, bj)*tempb
              thetab(i, j, km, bi, bj) = thetab(i, j, km, bi, bj) + 
     +          temp1*tempb0
              saltb(i, j, km, bi, bj) = saltb(i, j, km, bi, bj) + temp2*
     +          tempb0
              thetab(i, j, kc, bi, bj) = thetab(i, j, kc, bi, bj) + temp
     +          *tempb0
              saltb(i, j, kc, bi, bj) = saltb(i, j, kc, bi, bj) + temp0*
     +          tempb0
            ENDDO
          ENDDO
        END IF
...
C  Differentiation of apply_forcing_s in reverse (adjoint) mode:
C   gradient     of useful results: surfaceforcings ssurfcor gs_arr
C   with respect to varying inputs: surfaceforcings ssurfcor gs_arr
C
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
CBOP
C     !ROUTINE: APPLY_FORCING_S
C     !INTERFACE:
      SUBROUTINE APPLY_FORCING_S_B(gs_arr, gs_arrb, imin, imax, jmin, 
     +                             jmax, k, bi, bj, mytime, myiter, 
     +                             mythid)
      IMPLICIT NONE
C     == Global data ==
...
C     myThid    :: my Thread Id number
      ...
      REAL*8 gs_arrb(1-olx:snx+olx, 1-oly:sny+oly)
      INTEGER imin, imax, jmin, jmax
      INTEGER k, bi, bj
      REAL*8 mytime
      INTEGER myiter
...
C
      IF (linfsconservetr) THEN
        ...
        DO j=sny+1,0,-1
          DO i=snx+1,0,-1
            CALL POPCONTROL1B(branch)
            IF (branch .NE. 0) ssurfcorb = ssurfcorb + recip_drf(k)*
     +          recip_hfacc(i, j, k, bi, bj)*gs_arrb(i, j)
          ENDDO
        ENDDO
      END IF
      CALL POPCONTROL2B(branch)
      IF (branch .EQ. 0) THEN
        DO j=sny+1,0,-1
          DO i=snx+1,0,-1
            surfaceforcingsb(i, j, bi, bj) = surfaceforcingsb(i, j, bi, 
     +        bj) + recip_drf(k)*recip_hfacc(i, j, k, bi, bj)*gs_arrb(i
     +        , j)
          ENDDO
        ENDDO
      ELSE IF (branch .EQ. 1) THEN
        DO j=sny+1,0,-1
          DO i=snx+1,0,-1
            CALL POPCONTROL1B(branch)
            IF (branch .NE. 0) surfaceforcingsb(i, j, bi, bj) = 
     +          surfaceforcingsb(i, j, bi, bj) + recip_drf(k)*
     +          recip_hfacc(i, j, k, bi, bj)*gs_arrb(i, j)
          ENDDO
        ENDDO
      END IF
