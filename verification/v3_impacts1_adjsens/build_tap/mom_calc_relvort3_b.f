...
C
C  Differentiation of mom_calc_relvort3 in reverse (adjoint) mode:
C   gradient     of useful results: vfld ufld vort3
...
C
C
      SUBROUTINE MOM_CALC_RELVORT3_B(bi, bj, k, ufld, ufldb, vfld, vfldb
...
C      (exch3 convention order):
          IF (MOD(myface, 2) .EQ. 1) THEN
            tempb = recip_raz(i, j, bi, bj)*recip_deepfacc(k)*vort3b(i, 
     +        j)
            vort3b(i, j) = 0.D0
            ufldb(i, j-1) = ufldb(i, j-1) + dxc(i, j-1, bi, bj)*tempb
            vfldb(i-1, j) = vfldb(i-1, j) - dyc(i-1, j, bi, bj)*tempb
            ufldb(i, j) = ufldb(i, j) - dxc(i, j, bi, bj)*tempb
          ELSE
            tempb = recip_raz(i, j, bi, bj)*recip_deepfacc(k)*vort3b(i, 
     +        j)
            vort3b(i, j) = 0.D0
     ...
     +        j)
            vort3b(i, j) = 0.D0
            ufldb(i, j-1) = ufldb(i, j-1) + dxc(i, j-1, bi, bj)*tempb
            vfldb(i, j) = vfldb(i, j) + dyc(i, j, bi, bj)*tempb
            ufldb(i, j) = ufldb(i, j) - dxc(i, j, bi, bj)*tempb
          ELSE
            i = 1
            j = sny + 1
     ...
     +        j)
        ...
        ELSE IF (branch .EQ. 2) THEN
          i = 1
          j = sny + 1
          tempb = recip_raz(i, j, bi, bj)*recip_deepfacc(k)*vort3b(i, j)
          vort3b(i, j) = 0.D0
          vfldb(i, j) = vfldb(i, j) + dyc(i, j, bi, bj)*tempb
          ufldb(i, j) = ufldb(i, j) - dxc(i, j, bi, bj)*tempb
          ufldb(i, j-1) = ufldb(i, j-1) + dxc(i, j-1, bi, bj)*tempb
