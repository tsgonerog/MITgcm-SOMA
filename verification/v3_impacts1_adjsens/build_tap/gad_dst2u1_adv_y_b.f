...
C
C  Differentiation of gad_dst2u1_adv_y in reverse (adjoint) mode:
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_DST2U1_ADV_Y_B(bi, bj, k, advectionscheme, calccfl
     +                              , deltatloc, vtrans, vtransb, vfld, 
     +                              vfldb, tracer, tracerb, vt, vtb, 
     +                              mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
     ...
     +        recip_deepfacc(k)
      ...
      DO j=sny+oly,1-oly+1,-1
        DO i=snx+olx,1-olx,-1
          vabs = abs0*(1.d0-ylimit*(1.d0-vcfl))
          tempb = tracer(i, j-1)*0.5d0*vtb(i, j)
          tracerb(i, j-1) = tracerb(i, j-1) + (vtrans(i, j)+vabs)*0.5d0*
     +      vtb(i, j)
          tempb0 = tracer(i, j)*0.5d0*vtb(i, j)
          tracerb(i, j) = tracerb(i, j) + (vtrans(i, j)-vabs)*0.5d0*vtb(
     +      i, j)
          vtb(i, j) = 0.D0
          vtransb(i, j) = vtransb(i, j) + tempb0 + tempb
          vabsb = tempb - tempb0
          abs0b = (1.d0-ylimit*(1.d0-vcfl))*vabsb
          vcflb = ylimit*abs0*vabsb
