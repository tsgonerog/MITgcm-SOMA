...
C
C  Differentiation of mom_vecinv in reverse (adjoint) mode:
...
C   ==================================================================
C
      SUBROUTINE MOM_VECINV_B(bi, bj, k, imin, imax, jmin, jmax, kapparu
     +                        , kapparv, fverukm, fverukmb, fvervkm, 
     +                        fvervkmb, fverukp, fverukpb, fvervkp, 
     +                        fvervkpb, gudiss, gudissb, gvdiss, gvdissb
...
C     i,j      :: Loop counters
      ...
      REAL*8 vrfb(1-olx:snx+olx, 1-oly:sny+oly)
      ...
      REAL*8 del2ub(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 del2v(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 del2vb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dstar(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dstarb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 zstar(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 zstarb(1-olx:snx+olx, 1-oly:sny+oly)
      ...
      REAL*8 strainbcb(1-olx:snx+olx, 1-oly:sny+oly)
      ...
      REAL*8 omega3b(1-olx:snx+olx, 1-oly:sny+oly)
      ...
      REAL*8 vort3bcb(1-olx:snx+olx, 1-oly:sny+oly)
...
C-     uses vort3BC & strainBC which account for no-slip / free-slip BC
     ...
     +                       visca4_d, hdiv, vort3bc, tension, strainbc
...
C--   Non-Hydrostatic (spherical) metric terms
     ...
     +      , bj)
      ...
      ELSE
        ...
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            vcfb(i, j) = vcfb(i, j) + gvb(i, j, k, bi, bj)
          ENDDO
        ENDDO
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
          ...
          ENDDO
        ENDDO
        CALL MOM_V_METRIC_NH_B(bi, bj, k, vfld, vfldb, wvel, wvelb, vcf
     +                         , vcfb, mythid)
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            ucfb(ii2, ii1) = 0.D0
        ...
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            ucfb(i, j) = ucfb(i, j) + gub(i, j, k, bi, bj)
          ENDDO
        ENDDO
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
          ...
          ENDDO
        ENDDO
        CALL MOM_U_METRIC_NH_B(bi, bj, k, ufld, ufldb, wvel, wvelb, ucf
     +                         , ucfb, mythid)
      END IF
     ...
     +                           mythid)
      ...
      DO j=jmax,jmin,-1
        ...
        ENDDO
      ENDDO
      CALL MOM_U_CORIOLIS_NH_B(bi, bj, k, wvel, wvelb, ucf, ucfb, mythid
     +                        )
 100  CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          ...
          ENDDO
        ENDDO
        CALL MOM_VI_V_GRAD_KE_B(bi, bj, k, ke, keb, vcf, vcfb, mythid)
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            ucfb(i, j) = ucfb(i, j) + gub(i, j, k, bi, bj)
          ENDDO
        ENDDO
        CALL MOM_VI_U_GRAD_KE_B(bi, bj, k, ke, keb, ucf, ucfb, mythid)
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
            ...
            ENDDO
          ENDDO
          CALL MOM_VI_V_VERTSHEAR_B(bi, bj, k, vvel, vvelb, wvel, wvelb
     +                              , vcf, vcfb, mythid)
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              ucfb(i, j) = ucfb(i, j) + gub(i, j, k, bi, bj)
            ENDDO
          ENDDO
          CALL MOM_VI_U_VERTSHEAR_B(bi, bj, k, uvel, uvelb, wvel, wvelb
     +                              , ucf, ucfb, mythid)
        END IF
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
        ...
        IF (branch .LT. 2) THEN
          IF (branch .EQ. 0) THEN
            DO ii1=1-oly,oly+sny
              ...
              ENDDO
            ENDDO
            CALL MOM_VI_V_CORIOLIS_B(bi, bj, k, selectvortscheme, 
     +                               usejamartmomadv, ufld, ufldb, vort3
     +                               , vort3b, hfacz, r_hfacz, vcf, vcfb
     +                               , mythid)
            DO ii1=1-oly,oly+sny
              DO ii2=1-olx,olx+snx
                omega3b(ii2, ii1) = 0.D0
              ENDDO
            ENDDO
          ELSE
            DO ii1=1-oly,oly+sny
            ...
            ENDDO
            CALL MOM_VI_V_CORIOLIS_B(bi, bj, k, selectvortscheme, 
     +                               usejamartmomadv, ufld, ufldb, 
     +                               omega3, omega3b, hfacz, r_hfacz, 
     +                               vcf, vcfb, mythid)
            DO ii1=1-oly,oly+sny
              DO ii2=1-olx,olx+snx
                vort3b(ii2, ii1) = 0.D0
        ...
        ELSE IF (branch .EQ. 2) THEN
          CALL MOM_VI_V_CORIOLIS_C4_B(bi, bj, k, selectvortscheme, 
     +                                highordervorticity, 
     +                                upwindvorticity, ufld, ufldb, 
     +                                vort3, vort3b, r_hfacz, vcf, vcfb
     +                                , mythid)
          DO ii1=1-oly,oly+sny
            DO ii2=1-olx,olx+snx
              omega3b(ii2, ii1) = 0.D0
     ...
     +                                vcfb, mythid)
        ...
        IF (branch .LT. 2) THEN
          IF (branch .EQ. 0) THEN
            CALL MOM_VI_U_CORIOLIS_B(bi, bj, k, selectvortscheme, 
     +                               usejamartmomadv, vfld, vfldb, vort3
     +                               , vort3b, hfacz, r_hfacz, ucf, ucfb
     +                               , mythid)
          ELSE
            CALL MOM_VI_U_CORIOLIS_B(bi, bj, k, selectvortscheme, 
     +                               usejamartmomadv, vfld, vfldb, 
     +                               omega3, omega3b, hfacz, r_hfacz, 
     +                               ucf, ucfb, mythid)
          END IF
        ELSE IF (branch .EQ. 2) THEN
          CALL MOM_VI_U_CORIOLIS_C4_B(bi, bj, k, selectvortscheme, 
     +                                highordervorticity, 
     +                                upwindvorticity, vfld, vfldb, 
     +                                vort3, vort3b, r_hfacz, ucf, ucfb
     +                                , mythid)
        ELSE
          CALL MOM_VI_U_CORIOLIS_C4_B(bi, bj, k, selectvortscheme, 
     +                                highordervorticity, 
     +                                upwindvorticity, vfld, vfldb, 
     +                                omega3, omega3b, r_hfacz, ucf, 
     +                                ucfb, mythid)
        END IF
      ELSE
        DO ii1=1-oly,oly+sny
      ...
      IF (branch .EQ. 0) THEN
        ...
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL MOM_VI_CORIOLIS_B(bi, bj, k, ufld, ufldb, vfld, vfldb, 
     +                           hfacz, r_hfacz, ucf, ucfb, vcf, vcfb, 
     +                           mythid)
        ELSE
          CALL MOM_VI_V_CORIOLIS_B(bi, bj, k, selectvortscheme, 
     +                             usejamartmomadv, ufld, ufldb, omega3
     +                             , omega3b, hfacz, r_hfacz, vcf, vcfb
     +                             , mythid)
          CALL MOM_VI_U_CORIOLIS_B(bi, bj, k, selectvortscheme, 
     +                             usejamartmomadv, vfld, vfldb, omega3
     +                             , omega3b, hfacz, r_hfacz, ucf, ucfb
     +                             , mythid)
        END IF
      ELSE
        DO j=jmax,jmin,-1
      ...
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) CALL MOM_CALC_ABSVORT3_B(bi, bj, k, vort3, 
     +                                            vort3b, omega3, 
     +                                            omega3b, mythid)
      CALL POPCONTROL2B(branch)
      IF (branch .EQ. 0) THEN
        ...
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            ...
            tempb = -(recip_hfacs(i, j, k, bi, bj)*recip_drf(k)*vfb(i, j
     +        ))
            vfb(i, j) = 0.D0
            cdragb(i, j) = cdragb(i, j) + vfld(i, j)*tempb
            vfldb(i, j) = vfldb(i, j) + cdrag(i, j)*tempb
          ENDDO
        ENDDO
     ...
     +                             cdragb, myiter, mythid)
      ...
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          ...
          ENDDO
        ENDDO
        CALL MOM_V_SIDEDRAG_B(bi, bj, k, vfld, vfldb, del2v, del2vb, 
     +                        h0facz, viscah_z, viscah_zb, visca4_z, 
     +                        visca4_zb, useharmonicvisc, 
     +                        usebiharmonicvisc, usevariablevisc, vf, 
     +                        vfb, mythid)
      ELSE
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
      ...
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            tempb = -(recip_drf(k)*rksign*recip_ras(i, j, bi, bj)*
     +        recip_hfacs(i, j, k, bi, bj)*recip_deepfac2c(k)*
     +        recip_rhofacc(k)*gvdissb(i, j))
        ...
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            vrfb(i, j) = vrfb(i, j) + ardvdrfac*fvervkpb(i, j)
            fvervkpb(i, j) = 0.D0
          ENDDO
        ENDDO
        CALL MOM_V_RVISCFLUX_B(bi, bj, arg1, vvel, vvelb, kapparv, vrf, 
     +                         vrfb, mythid)
      ELSE
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
      ...
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            ...
            tempb = -(recip_hfacw(i, j, k, bi, bj)*recip_drf(k)*vfb(i, j
     +        ))
            vfb(i, j) = 0.D0
            cdragb(i, j) = cdragb(i, j) + ufld(i, j)*tempb
            ufldb(i, j) = ufldb(i, j) + cdrag(i, j)*tempb
          ENDDO
        ENDDO
        CALL POPREAL8ARRAY(ke, 70**2)
     ...
     +                             cdragb, myiter, mythid)
      ...
      IF (branch .EQ. 0) THEN
        ...
        DO ii1=1-oly,oly+sny
          ...
          ENDDO
        ENDDO
        CALL MOM_U_SIDEDRAG_B(bi, bj, k, ufld, ufldb, del2u, del2ub, 
     +                        h0facz, viscah_z, viscah_zb, visca4_z, 
     +                        visca4_zb, useharmonicvisc, 
     +                        usebiharmonicvisc, usevariablevisc, vf, 
     +                        vfb, mythid)
      ...
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            tempb = -(recip_drf(k)*rksign*recip_raw(i, j, bi, bj)*
     +        recip_hfacw(i, j, k, bi, bj)*recip_deepfac2c(k)*
     +        recip_rhofacc(k)*gudissb(i, j))
            fverukpb(i, j) = fverukpb(i, j) + tempb
            fverukmb(i, j) = fverukmb(i, j) - tempb
          ENDDO
        ENDDO
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            vrfb(i, j) = vrfb(i, j) + ardudrfac*fverukpb(i, j)
            fverukpb(i, j) = 0.D0
          ENDDO
        ENDDO
        arg1 = k + 1
        CALL MOM_U_RVISCFLUX_B(bi, bj, arg1, uvel, uvelb, kapparu, vrf, 
     +                         vrfb, mythid)
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL MOM_HDISSIP_B(bi, bj, k, tension, tensionb, strain, strainb
     +                     , hfacz, viscah_z, viscah_zb, viscah_d, 
     +                     viscah_db, visca4_z, visca4_d, 
     +                     useharmonicvisc, usebiharmonicvisc, 
     +                     usevariablevisc, gudiss, gudissb, gvdiss, 
     +                     gvdissb, mythid)
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
        ...
        ENDDO
      ELSE
        CALL MOM_VI_HDISSIP_B(bi, bj, k, hdiv, hdivb, vort3, vort3b, 
     +                        dstar, dstarb, zstar, zstarb, hfacz, 
     +                        viscah_z, viscah_zb, viscah_d, viscah_db, 
     +                        visca4_z, visca4_zb, visca4_d, visca4_db, 
     +                        useharmonicvisc, usebiharmonicvisc, 
     +                        usevariablevisc, gudiss, gudissb, gvdiss, 
     +                        gvdissb, mythid)
        DO ii1=1-oly,oly+sny
      ...
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL MOM_CALC_RELVORT3_B(bi, bj, k, del2u, del2ub, del2v, del2vb
     +                           , hfacz, zstar, zstarb, mythid)
        CALL MOM_CALC_HDIV_B(bi, bj, k, 2, del2u, del2ub, del2v, del2vb
     +                       , dstar, dstarb, mythid)
        CALL MOM_VI_DEL2UV_B(bi, bj, k, hdiv, hdivb, vort3, vort3b, 
     +                       hfacz, del2u, del2ub, del2v, del2vb, mythid
     +                      )
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
     ...
     +                       visca4_db, hdiv, hdivb, vort3bc, vort3bcb, 
     +                       tension, tensionb, strainbc, strainbcb, 
      ...
      IF (branch .NE. 0) THEN
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            ...
            IF (branch .NE. 0) THEN
              strainb(i, j) = 0.D0
              strainbcb(i, j) = sidemaskfac*strainbcb(i, j)
            END IF
            strainb(i, j) = strainb(i, j) + strainbcb(i, j)
            strainbcb(i, j) = 0.D0
          ENDDO
        ENDDO
     ...
     +                          tension, tensionb, mythid)
      END IF
      CALL MOM_CALC_HDIV_B(bi, bj, k, 2, ufld, ufldb, vfld, vfldb, hdiv
     +                     , hdivb, mythid)
 110  DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx,-1
          CALL POPCONTROL1B(branch)
          IF (branch .NE. 0) THEN
            vort3b(i, j) = 0.D0
            vort3bcb(i, j) = sidemaskfac*vort3bcb(i, j)
          END IF
          vort3b(i, j) = vort3b(i, j) + vort3bcb(i, j)
          vort3bcb(i, j) = 0.D0
        ENDDO
      ENDDO
      CALL MOM_CALC_RELVORT3_B(bi, bj, k, ufld, ufldb, vfld, vfldb, 
     +                         hfacz, vort3, vort3b, mythid)
      CALL MOM_CALC_KE_B(bi, bj, k, selectkescheme, ufld, ufldb, vfld, 
     +                   vfldb, ke, keb, mythid)
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx,-1
