...
C
C  Differentiation of mom_hdissip in reverse (adjoint) mode:
C   gradient     of useful results: vdissip udissip viscah_s
C   with respect to varying inputs: vdissip udissip strain tension
C                viscah_s viscah_t
C
C
C
...
C
C
      SUBROUTINE MOM_HDISSIP_B(bi, bj, k, tension, tensionb, strain, 
     +                         strainb, hfacz, viscah_s, viscah_sb, 
     +                         viscah_t, viscah_tb, visca4_s, visca4_t, 
     +                         harmonic, biharmonic, 
     +                         usevariableviscosity, udissip, udissipb, 
     +                         vdissip, vdissipb, mythid)
      IMPLICIT NONE
C
...
C     == Routine arguments ==
      ...
      REAL*8 viscah_sb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 viscah_t(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 viscah_tb(1-olx:snx+olx, 1-oly:sny+oly)
...
C     - Laplacian and bi-harmonic terms
      ...
      ELSE
        ...
        IF (branch .EQ. 0) THEN
          ...
          DO ii1=1-oly,oly+sny
            DO ii2=1-olx,olx+snx
              viscah_tb(ii2, ii1) = 0.D0
            ENDDO
          ENDDO
          DO j=sny+oly-1,2-oly,-1
            DO i=snx+olx-1,2-olx,-1
              tempb4 = recip_dyc(i, j, bi, bj)**2*recip_dxg(i, j, bi, bj
     +          )*vdissipb(i, j)
              tempb1 = -(recip_dxg(i, j, bi, bj)**2*recip_dyc(i, j, bi, 
     +          bj)*vdissipb(i, j))
              vdissipb(i, j) = 0.D0
              tempb0 = dxf(i, j, bi, bj)**2*tempb1
              tempb = -(dxf(i, j-1, bi, bj)**2*tempb1)
              viscah_tb(i, j-1) = viscah_tb(i, j-1) + tension(i, j-1)*
     +          tempb
              tensionb(i, j-1) = tensionb(i, j-1) + viscah_t(i, j-1)*
     +          tempb
              viscah_tb(i, j) = viscah_tb(i, j) + tension(i, j)*tempb0
              tensionb(i, j) = tensionb(i, j) + viscah_t(i, j)*tempb0
              tempb3 = dyu(i+1, j, bi, bj)**2*tempb4
              tempb2 = -(dyu(i, j, bi, bj)**2*tempb4)
              viscah_sb(i, j) = viscah_sb(i, j) + strain(i, j)*tempb2
              strainb(i, j) = strainb(i, j) + viscah_s(i, j)*tempb2
              viscah_sb(i+1, j) = viscah_sb(i+1, j) + strain(i+1, j)*
     +          tempb3
              strainb(i+1, j) = strainb(i+1, j) + viscah_s(i+1, j)*
     +          tempb3
              tempb = recip_dyg(i, j, bi, bj)**2*recip_dxc(i, j, bi, bj)
     +          *udissipb(i, j)
              tempb2 = recip_dxc(i, j, bi, bj)**2*recip_dyg(i, j, bi, bj
     +          )*udissipb(i, j)
              udissipb(i, j) = 0.D0
              tempb3 = dxv(i, j+1, bi, bj)**2*tempb2
              tempb4 = -(dxv(i, j, bi, bj)**2*tempb2)
              viscah_sb(i, j) = viscah_sb(i, j) + strain(i, j)*tempb4
              strainb(i, j) = strainb(i, j) + viscah_s(i, j)*tempb4
              viscah_sb(i, j+1) = viscah_sb(i, j+1) + strain(i, j+1)*
     +          tempb3
              strainb(i, j+1) = strainb(i, j+1) + viscah_s(i, j+1)*
     +          tempb3
              tempb0 = dyf(i, j, bi, bj)**2*tempb
              tempb1 = -(dyf(i-1, j, bi, bj)**2*tempb)
              viscah_tb(i-1, j) = viscah_tb(i-1, j) + tension(i-1, j)*
     +          tempb1
              tensionb(i-1, j) = tensionb(i-1, j) + viscah_t(i-1, j)*
     +          tempb1
              viscah_tb(i, j) = viscah_tb(i, j) + tension(i, j)*tempb0
              tensionb(i, j) = tensionb(i, j) + viscah_t(i, j)*tempb0
            ENDDO
