...
C
C  Differentiation of mom_v_metric_cylinder in reverse (adjoint) mode:
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_V_METRIC_CYLINDER_B(bi, bj, k, ufld, ufldb, vfld, 
...
C         radius = yC(i,j,bi,bj)
      ...
      DO j=sny+oly,1-oly+1,-1
        DO i=snx+olx-1,1-olx,-1
          ...
          ELSE
            radius = (yg(i, j, bi, bj)+yg(i+1, j, bi, bj))*0.5d0
            tempb = -(2*(ufld(i, j)+ufld(i+1, j)+ufld(i, j-1)+ufld(i+1, 
     +        j-1))*0.25**2*vmetrictermsb(i, j)/radius)
            vmetrictermsb(i, j) = 0.D0
