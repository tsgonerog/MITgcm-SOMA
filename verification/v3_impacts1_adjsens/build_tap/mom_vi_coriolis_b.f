...
C
C  Differentiation of mom_vi_coriolis in reverse (adjoint) mode:
C   gradient     of useful results: vfld ufld vcoriolisterm ucoriolisterm
C   with respect to varying inputs: vfld ufld
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_VI_CORIOLIS_B(bi, bj, k, ufld, ufldb, vfld, vfldb, 
     +                             hfacz, r_hfacz, ucoriolisterm, 
     +                             ucoriolistermb, vcoriolisterm, 
     +                             vcoriolistermb, mythid)
      IMPLICIT NONE
C     == Global variables ==
CBOP
C    !ROUTINE: SIZE.h
...
C   epsil          :: small number
      ...
      REAL*8 ubarxyb, vbarxyb
      REAL*8 ubarym, ubaryp, vbarxm, vbarxp
      REAL*8 ubarymb, ubarypb, vbarxmb, vbarxpb
      REAL*8 epsil
      INTRINSIC MAX
      REAL*8 y1
      REAL*8 y2
...
C- Simple average, no hFac :
      ...
      ELSE IF (selectcorischeme .EQ. 1) THEN
...
C     (formerly called: useJamartWetPoints)
     ...
     +        hfacs(i, j+1, k, bi, bj)+hfacs(i-1, j+1, k, bi, bj))
      ...
      ELSE IF (selectcorischeme .EQ. 2) THEN
C- hFac weighted average :
        DO j=1-oly,sny+oly-1
          i = snx + olx + 1
        ENDDO
        CALL PUSHCONTROL2B(2)
      ELSE IF (selectcorischeme .EQ. 3) THEN
      ...
      IF (selectcorischeme .EQ. 0) THEN
        DO j=sny+oly,2-oly,-1
          DO i=snx+olx-1,1-olx,-1
            ubarxyb = -(0.5*recip_dyc(i, j, bi, bj)*masks(i, j, k, bi, 
     +        bj)*(fcorig(i, j, bi, bj)+fcorig(i+1, j, bi, bj))*
     +        vcoriolistermb(i, j))
            vcoriolistermb(i, j) = 0.D0
            tempb = 0.25*ubarxyb
            ufldb(i, j) = ufldb(i, j) + dyg(i, j, bi, bj)*tempb
            ufldb(i, j-1) = ufldb(i, j-1) + dyg(i, j-1, bi, bj)*tempb
            ufldb(i+1, j) = ufldb(i+1, j) + dyg(i+1, j, bi, bj)*tempb
...
C     (formerly called: useJamartWetPoints)
        DO j=2-oly,sny+oly
          DO i=1-olx,snx+olx-1
            y2 = hfacw(i, j, k, bi, bj) + hfacw(i, j-1, k, bi, bj) + (
     ...
     +        hfacw(i+1, j-1, k, bi, bj)*tempb
      ...
      ELSE IF (selectcorischeme .EQ. 2) THEN
        DO j=sny+oly,2-oly,-1
          DO i=snx+olx-1,1-olx,-1
            ubarxyb = -(0.5*recip_dyc(i, j, bi, bj)*recip_hfacs(i, j, k
     +        , bi, bj)*(fcorig(i, j, bi, bj)+fcorig(i+1, j, bi, bj))*
     +        vcoriolistermb(i, j))
            vcoriolistermb(i, j) = 0.D0
            tempb = 0.25*ubarxyb
            ufldb(i, j) = ufldb(i, j) + dyg(i, j, bi, bj)*hfacw(i, j, k
     ...
     +        hfacw(i+1, j-1, k, bi, bj)*tempb
      ...
      ELSE IF (selectcorischeme .EQ. 3) THEN
        DO j=sny+oly,2-oly,-1
          DO i=snx+olx-1,1-olx,-1
            tempb = -(recip_dyc(i, j, bi, bj)*0.5d0*recip_hfacs(i, j, k
     +        , bi, bj)*vcoriolistermb(i, j))
            vcoriolistermb(i, j) = 0.D0
            ubarymb = fcorig(i, j, bi, bj)*tempb
            ubarypb = fcorig(i+1, j, bi, bj)*tempb
            ufldb(i+1, j) = ufldb(i+1, j) + dyg(i+1, j, bi, bj)*hfacw(i+
     +        1, j, k, bi, bj)*halfrl*ubarypb
            ufldb(i+1, j-1) = ufldb(i+1, j-1) + dyg(i+1, j-1, bi, bj)*
     +        hfacw(i+1, j-1, k, bi, bj)*halfrl*ubarypb
            ufldb(i, j) = ufldb(i, j) + dyg(i, j, bi, bj)*hfacw(i, j, k
     +        , bi, bj)*halfrl*ubarymb
            ufldb(i, j-1) = ufldb(i, j-1) + dyg(i, j-1, bi, bj)*hfacw(i
     +        , j-1, k, bi, bj)*halfrl*ubarymb
          ENDDO
        ENDDO
      ELSE
        STOP
        ...
        IF (branch .EQ. 0) THEN
          DO j=sny+oly-1,1-oly,-1
            DO i=snx+olx,2-olx,-1
              vbarxyb = 0.5*recip_dxc(i, j, bi, bj)*maskw(i, j, k, bi, 
     +          bj)*(fcorig(i, j, bi, bj)+fcorig(i, j+1, bi, bj))*
     +          ucoriolistermb(i, j)
              ucoriolistermb(i, j) = 0.D0
              tempb = 0.25*vbarxyb
              vfldb(i, j) = vfldb(i, j) + dxg(i, j, bi, bj)*tempb
              vfldb(i-1, j) = vfldb(i-1, j) + dxg(i-1, j, bi, bj)*tempb
              vfldb(i, j+1) = vfldb(i, j+1) + dxg(i, j+1, bi, bj)*tempb
     ...
     +          hfacs(i-1, j+1, k, bi, bj)*tempb
      ...
      ELSE IF (branch .EQ. 2) THEN
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx,2-olx,-1
            vbarxyb = 0.5*recip_dxc(i, j, bi, bj)*recip_hfacw(i, j, k, 
     +        bi, bj)*(fcorig(i, j, bi, bj)+fcorig(i, j+1, bi, bj))*
     +        ucoriolistermb(i, j)
            ucoriolistermb(i, j) = 0.D0
            tempb = 0.25*vbarxyb
     ...
     +        hfacs(i-1, j+1, k, bi, bj)*tempb
      ...
      ELSE
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx,2-olx,-1
            tempb = recip_dxc(i, j, bi, bj)*0.5d0*recip_hfacw(i, j, k, 
     +        bi, bj)*ucoriolistermb(i, j)
            ucoriolistermb(i, j) = 0.D0
            vbarxmb = fcorig(i, j, bi, bj)*tempb
            vbarxpb = fcorig(i, j+1, bi, bj)*tempb
            vfldb(i, j+1) = vfldb(i, j+1) + dxg(i, j+1, bi, bj)*hfacs(i
     +        , j+1, k, bi, bj)*halfrl*vbarxpb
            vfldb(i-1, j+1) = vfldb(i-1, j+1) + dxg(i-1, j+1, bi, bj)*
     +        hfacs(i-1, j+1, k, bi, bj)*halfrl*vbarxpb
            vfldb(i, j) = vfldb(i, j) + dxg(i, j, bi, bj)*hfacs(i, j, k
     +        , bi, bj)*halfrl*vbarxmb
            vfldb(i-1, j) = vfldb(i-1, j) + dxg(i-1, j, bi, bj)*hfacs(i-
     +        1, j, k, bi, bj)*halfrl*vbarxmb
          ENDDO
        ENDDO
      END IF
      END
