...
C
C  Differentiation of dynamics in reverse (adjoint) mode:
C   gradient     of useful results: etan uvel vvel wvel theta salt
C                gu gv gunm1 gvnm1 rhoinsitu totphihyd surfaceforcingu
C                surfaceforcingv /surf_fixed/[117600,156800[
C   with respect to varying inputs: etan uvel vvel wvel theta salt
C                gu gv gunm1 gvnm1 rhoinsitu totphihyd surfaceforcingu
C                surfaceforcingv /surf_fixed/[117600,156800[
C
C
...
C     !ROUTINE: DYNAMICS
C     !INTERFACE:
      SUBROUTINE DYNAMICS_B(mytime, myiter, mythid)
      IMPLICIT NONE
C     == Global variables ===
CBOP
C    !ROUTINE: SIZE.h
...
C                   are switched with k to be the appropriate index into fVerU,V
      ...
      REAL*8 fverub(1-olx:snx+olx, 1-oly:sny+oly, 2)
      REAL*8 fverv(1-olx:snx+olx, 1-oly:sny+oly, 2)
      REAL*8 fvervb(1-olx:snx+olx, 1-oly:sny+oly, 2)
      REAL*8 phihydf(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 phihydfb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 phihydc(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 phihydcb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dphihydx(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dphihydxb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dphihydy(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dphihydyb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 phisurfx(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 phisurfxb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 phisurfy(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 phisurfyb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gudissip(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gudissipb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gvdissip(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gvdissipb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 kapparu(1-olx:snx+olx, 1-oly:sny+oly, nr+1)
      REAL*8 kapparub(1-olx:snx+olx, 1-oly:sny+oly, nr+1)
      REAL*8 kapparv(1-olx:snx+olx, 1-oly:sny+oly, nr+1)
      REAL*8 kapparvb(1-olx:snx+olx, 1-oly:sny+oly, nr+1)
C
      INTEGER bi, bj
      INTEGER i, j
...
C     LOGICAL dPhiHydDiagIsOn
      ...
      EXTERNAL DIAGNOSTICS_SCALE_FILL
      EXTERNAL DIAGNOSTICS_FILL_RS
...
C
C
      DO bj=ad_from0,mybyhi(mythid)
        ad_from = mybxlo(mythid)
C
C
        DO bi=ad_from,mybxhi(mythid)
C
C
...
C
          DO k=1,nr+1
            DO j=1-oly,sny+oly
              DO i=1-olx,snx+olx
                CALL PUSHREAL8(kapparu(i, j, k))
                kapparu(i, j, k) = 0.d0
                CALL PUSHREAL8(kapparv(i, j, k))
                kapparv(i, j, k) = 0.d0
              ENDDO
            ENDDO
          ENDDO
C--     Calculate the total vertical viscosity
          IF (momviscosity) THEN
            CALL PUSHREAL8ARRAY(kapparv, 70**2*32)
            CALL PUSHREAL8ARRAY(kapparu, 70**2*32)
...
C--      Integrate hydrostatic balance for phiHyd with BC of phiHyd(z=0)=0
     ...
     +                        , phihydc, dphihydx, dphihydy, mytime, 
     +                        myiter, mythid)
C
...
C        and step forward storing the result in gU, gV, etc...
            IF (momstepping) THEN
              ...
              IF (.NOT.vectorinvariantmomentum) THEN
                CALL MOM_FLUXFORM(bi, bj, k, imin, imax, jmin, jmax, 
     +                            kapparu, kapparv, fveru(1-olx, 1-oly, 
     +                            kup), fverv(1-olx, 1-oly, kup), fveru(
     +                            1-olx, 1-oly, kdown), fverv(1-olx, 1-
     +                            oly, kdown), gudissip, gvdissip, 
     +                            mytime, myiter, mythid)
                CALL PUSHCONTROL1B(0)
              ELSE
                CALL MOM_VECINV(bi, bj, k, imin, imax, jmin, jmax, 
     +                          kapparu, kapparv, fveru(1-olx, 1-oly, 
     +                          kup), fverv(1-olx, 1-oly, kup), fveru(1-
     +                          olx, 1-oly, kdown), fverv(1-olx, 1-oly, 
     +                          kdown), gudissip, gvdissip, mytime, 
     +                          myiter, mythid)
                CALL PUSHCONTROL1B(1)
              END IF
C
C
     ...
     +                      , dphihydy, phisurfx, phisurfy, gudissip, 
...
C--     Implicit Vertical advection & viscosity
          IF (implicitviscosity) THEN
            ...
            DO ii1=1-olx,151900-olx
              CALL PUSHREAL8(gu(ii1, 1-oly, 1, bi, bj))
            ENDDO
            CALL IMPLDIFF(bi, bj, imin, imax, jmin, jmax, -1, kapparu, 
     +                    recip_hfacw(1-olx, 1-oly, 1, bi, bj), gu(1-olx
     +                    , 1-oly, 1, bi, bj), mythid)
            DO ii1=1-olx,151900-olx
              CALL PUSHREAL8(gv(ii1, 1-oly, 1, bi, bj))
            ENDDO
            CALL IMPLDIFF(bi, bj, imin, imax, jmin, jmax, -2, kapparv, 
     +                    recip_hfacs(1-olx, 1-oly, 1, bi, bj), gv(1-olx
     +                    , 1-oly, 1, bi, bj), mythid)
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHCONTROL1B(1)
          END IF
        ENDDO
        CALL PUSHINTEGER4(bi - 1)
        CALL PUSHINTEGER4(ad_from)
      ENDDO
      CALL PUSHINTEGER4(bj - 1)
      CALL PUSHINTEGER4(ad_from0)
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          gudissipb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          dphihydxb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          dphihydyb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          phihydcb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          phihydfb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          phisurfxb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          phisurfyb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          gvdissipb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1,2
        DO ii2=1-oly,oly+sny
          DO ii3=1-olx,olx+snx
            fverub(ii3, ii2, ii1) = 0.D0
          ENDDO
        ENDDO
      ENDDO
      DO ii1=1,2
        DO ii2=1-oly,oly+sny
          DO ii3=1-olx,olx+snx
            fvervb(ii3, ii2, ii1) = 0.D0
          ENDDO
        ENDDO
      ENDDO
      CALL POPINTEGER4(ad_from0)
      CALL POPINTEGER4(ad_to0)
      DO bj=ad_to0,ad_from0,-1
        CALL POPINTEGER4(ad_from)
        CALL POPINTEGER4(ad_to)
        DO bi=ad_to,ad_from,-1
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            DO ii1=151900-olx,1-olx,-1
              CALL POPREAL8(gv(ii1, 1-oly, 1, bi, bj))
            ENDDO
            DO ii1=1,nr+1
              DO ii2=1-oly,oly+sny
                DO ii3=1-olx,olx+snx
                  kapparvb(ii3, ii2, ii1) = 0.D0
                ENDDO
              ENDDO
            ENDDO
            CALL IMPLDIFF_B(bi, bj, imin, imax, jmin, jmax, -2, kapparv
     +                      , kapparvb, recip_hfacs(1-olx, 1-oly, 1, bi
     +                      , bj), gv(1-olx, 1-oly, 1, bi, bj), gvb(1-
     +                      olx, 1-oly, 1, bi, bj), mythid)
            DO ii1=151900-olx,1-olx,-1
              CALL POPREAL8(gu(ii1, 1-oly, 1, bi, bj))
            ENDDO
            CALL POPINTEGER4(mythid)
            DO ii1=1,nr+1
              DO ii2=1-oly,oly+sny
                DO ii3=1-olx,olx+snx
                  kapparub(ii3, ii2, ii1) = 0.D0
                ENDDO
              ENDDO
            ENDDO
            CALL IMPLDIFF_B(bi, bj, imin, imax, jmin, jmax, -1, kapparu
     +                      , kapparub, recip_hfacw(1-olx, 1-oly, 1, bi
     +                      , bj), gu(1-olx, 1-oly, 1, bi, bj), gub(1-
     +                      olx, 1-oly, 1, bi, bj), mythid)
          END IF
          DO k=nr,1,-1
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              CALL TIMESTEP_B(bi, bj, imin, imax, jmin, jmax, k, 
     +                        dphihydx, dphihydxb, dphihydy, dphihydyb, 
     +                        phisurfx, phisurfxb, phisurfy, phisurfyb, 
     +                        gudissip, gudissipb, gvdissip, gvdissipb, 
     +                        mytime, myiter, mythid)
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL MOM_FLUXFORM_B(bi, bj, k, imin, imax, jmin, jmax, 
     +                              kapparu, kapparv, fveru(1-olx, 1-oly
     +                              , kup), fverub(1-olx, 1-oly, kup), 
     +                              fverv(1-olx, 1-oly, kup), fvervb(1-
     +                              olx, 1-oly, kup), fveru(1-olx, 1-oly
     +                              , kdown), fverub(1-olx, 1-oly, kdown
     +                              ), fverv(1-olx, 1-oly, kdown), 
     +                              fvervb(1-olx, 1-oly, kdown), 
     +                              gudissip, gudissipb, gvdissip, 
     +                              gvdissipb, mytime, myiter, mythid)
              ELSE
                CALL MOM_VECINV_B(bi, bj, k, imin, imax, jmin, jmax, 
     +                            kapparu, kapparv, fveru(1-olx, 1-oly, 
     +                            kup), fverub(1-olx, 1-oly, kup), fverv
     +                            (1-olx, 1-oly, kup), fvervb(1-olx, 1-
     +                            oly, kup), fveru(1-olx, 1-oly, kdown)
     +                            , fverub(1-olx, 1-oly, kdown), fverv(1
     +                            -olx, 1-oly, kdown), fvervb(1-olx, 1-
     +                            oly, kdown), gudissip, gudissipb, 
     +                            gvdissip, gvdissipb, mytime, myiter, 
     +                            mythid)
              END IF
     ...
     +                         *nsy)
            CALL CALC_PHI_HYD_B(bi, bj, imin, imax, jmin, jmax, k, 
     +                          phihydf, phihydfb, phihydc, phihydcb, 
     +                          dphihydx, dphihydxb, dphihydy, dphihydyb
     +                          , mytime, myiter, mythid)
          ...
          CALL POPCONTROL1B(branch)
          IF (branch .NE. 0) THEN
            CALL POPREAL8ARRAY(kapparu, 70**2*32)
            CALL POPREAL8ARRAY(kapparv, 70**2*32)
          END IF
          DO k=nr+1,1,-1
            DO j=sny+oly,1-oly,-1
              DO i=snx+olx,1-olx,-1
                CALL POPREAL8(kapparv(i, j, k))
                CALL POPREAL8(kapparu(i, j, k))
              ENDDO
            ENDDO
          ENDDO
          CALL POPCONTROL1B(branch)
          IF (branch .NE. 0) CALL CALC_GRAD_PHI_SURF_B(bi, bj, imin, 
     +                                                 imax, jmin, jmax
     +                                                 , etan, etanb, 
     +                                                 phisurfx, 
     +                                                 phisurfxb, 
