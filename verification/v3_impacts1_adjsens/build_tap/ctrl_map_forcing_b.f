...
C
C  Differentiation of ctrl_map_forcing in reverse (adjoint) mode:
C   gradient     of useful results: xx_gentim2d fu fv qnet qsw
C                empmr saltflux sst sss pload
C   with respect to varying inputs: xx_gentim2d fu fv qnet qsw
C                empmr saltflux sst sss pload
C
C
C
...
C     !INTERFACE:
      SUBROUTINE CTRL_MAP_FORCING_B(mytime, myiter, mythid)
      IMPLICIT NONE
C
C     == Global variables ===
CBOP
...
C     == Local variables ==
      ...
      REAL*8 tmpueb(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      REAL*8 tmpvn(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      REAL*8 tmpvnb(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      REAL*8 tmpux(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      REAL*8 tmpuxb(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      REAL*8 tmpvy(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      REAL*8 tmpvyb(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
...
C
          ...
          DO j=1,sny
            DO i=1,snx
              DO iarr=1,maxctrltim2d
                ...
                IF (temp_genarr_fnam(1:5) .EQ. 'xx_fe') THEN
     ...
     +              xx_gentim2d(i, j, bi, bj, iarr)
                ...
                END IF
                IF (temp_genarr_fnam(1:5) .EQ. 'xx_fn') THEN
     ...
     +              xx_gentim2d(i, j, bi, bj, iarr)
      ...
      ENDDO
C
      CALL PUSHREAL8ARRAY(tmpue, 70**2)
      CALL EXCH_XY_RL(tmpue, mythid)
      CALL PUSHREAL8ARRAY(tmpvn, 70**2)
...
C
        ...
        DO bi=mybxlo(mythid),mybxhi(mythid)
          DO j=1,sny
            DO i=1,snx
              ...
              DO iarr=1,maxctrltim2d
                temp_genarr_fnam = xx_gentim2d_file(iarr)
                IF (temp_genarr_fnam(1:7) .EQ. 'xx_qnet') THEN
                  qnet(i, j, bi, bj) = qnet(i, j, bi, bj) + xx_gentim2d(
     +              i, j, bi, bj, iarr)
                  CALL PUSHCONTROL1B(0)
                ELSE
                  CALL PUSHCONTROL1B(1)
                END IF
                IF (temp_genarr_fnam(1:8) .EQ. 'xx_empmr') THEN
     ...
     +              xx_gentim2d(i, j, bi, bj, iarr)
                ...
                ELSE
                  CALL PUSHCONTROL1B(1)
                END IF
                IF (temp_genarr_fnam(1:6) .EQ. 'xx_qsw') THEN
                  qsw(i, j, bi, bj) = qsw(i, j, bi, bj) + xx_gentim2d(i
     +              , j, bi, bj, iarr)
                  CALL PUSHCONTROL1B(0)
                ELSE
                  CALL PUSHCONTROL1B(1)
                END IF
                IF (temp_genarr_fnam(1:6) .EQ. 'xx_sst') THEN
                  sst(i, j, bi, bj) = sst(i, j, bi, bj) + xx_gentim2d(i
     +              , j, bi, bj, iarr)
                  CALL PUSHCONTROL1B(0)
                ELSE
                  CALL PUSHCONTROL1B(1)
                END IF
                IF (temp_genarr_fnam(1:6) .EQ. 'xx_sss') THEN
                  sss(i, j, bi, bj) = sss(i, j, bi, bj) + xx_gentim2d(i
     +              , j, bi, bj, iarr)
                  CALL PUSHCONTROL1B(0)
                ELSE
                  CALL PUSHCONTROL1B(1)
                END IF
                IF (temp_genarr_fnam(1:8) .EQ. 'xx_pload') THEN
                  pload(i, j, bi, bj) = pload(i, j, bi, bj) + 
     +              xx_gentim2d(i, j, bi, bj, iarr)
                  CALL PUSHCONTROL1B(0)
                ELSE
                  CALL PUSHCONTROL1B(1)
                END IF
                IF (temp_genarr_fnam(1:11) .EQ. 'xx_saltflux') THEN
     ...
     +              xx_gentim2d(i, j, bi, bj, iarr)
                ...
                ELSE
                  CALL PUSHCONTROL1B(1)
                END IF
                IF (temp_genarr_fnam(1:5) .EQ. 'xx_fu') THEN
                  fu(i, j, bi, bj) = fu(i, j, bi, bj) + xx_gentim2d(i, j
     +              , bi, bj, iarr)
                  CALL PUSHCONTROL1B(0)
                ELSE
                  CALL PUSHCONTROL1B(1)
                END IF
                IF (temp_genarr_fnam(1:5) .EQ. 'xx_fv') THEN
                  fv(i, j, bi, bj) = fv(i, j, bi, bj) + xx_gentim2d(i, j
     +              , bi, bj, iarr)
                  CALL PUSHCONTROL1B(1)
                ELSE
                  CALL PUSHCONTROL1B(0)
        ...
        ENDDO
      ENDDO
      CALL EXCH_UV_XY_RS_B(fu, fub, fv, fvb, .true., mythid)
      CALL EXCH_XY_RS_B(saltflux, saltfluxb, mythid)
      CALL EXCH_XY_RS_B(pload, ploadb, mythid)
      CALL EXCH_XY_RS_B(sss, sssb, mythid)
      CALL EXCH_XY_RS_B(sst, sstb, mythid)
      CALL EXCH_XY_RS_B(qsw, qswb, mythid)
      CALL EXCH_XY_RS_B(empmr, empmrb, mythid)
      CALL EXCH_XY_RS_B(qnet, qnetb, mythid)
      DO ii1=1,nsy
        DO ii2=1,nsx
          DO ii3=1-oly,oly+sny
            DO ii4=1-olx,olx+snx
              tmpuxb(ii4, ii3, ii2, ii1) = 0.D0
            ENDDO
          ENDDO
        ENDDO
      ENDDO
      ...
      DO bj=mybyhi(mythid),mybylo(mythid),-1
        DO bi=mybxhi(mythid),mybxlo(mythid),-1
          DO j=sny,1,-1
            DO i=snx,1,-1
              DO iarr=maxctrltim2d,1,-1
                CALL POPCONTROL1B(branch)
                IF (branch .NE. 0) xx_gentim2db(i, j, bi, bj, iarr) = 
     +              xx_gentim2db(i, j, bi, bj, iarr) + fvb(i, j, bi, bj)
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) xx_gentim2db(i, j, bi, bj, iarr) = 
     +              xx_gentim2db(i, j, bi, bj, iarr) + fub(i, j, bi, bj)
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) xx_gentim2db(i, j, bi, bj, iarr) = 
     +              xx_gentim2db(i, j, bi, bj, iarr) + saltfluxb(i, j, 
     +              bi, bj)
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) xx_gentim2db(i, j, bi, bj, iarr) = 
     +              xx_gentim2db(i, j, bi, bj, iarr) + ploadb(i, j, bi, 
     +              bj)
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) xx_gentim2db(i, j, bi, bj, iarr) = 
     +              xx_gentim2db(i, j, bi, bj, iarr) + sssb(i, j, bi, bj
     +              )
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) xx_gentim2db(i, j, bi, bj, iarr) = 
     +              xx_gentim2db(i, j, bi, bj, iarr) + sstb(i, j, bi, bj
     +              )
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) xx_gentim2db(i, j, bi, bj, iarr) = 
     +              xx_gentim2db(i, j, bi, bj, iarr) + qswb(i, j, bi, bj
     +              )
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) xx_gentim2db(i, j, bi, bj, iarr) = 
     +              xx_gentim2db(i, j, bi, bj, iarr) + empmrb(i, j, bi, 
     +              bj)
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) xx_gentim2db(i, j, bi, bj, iarr) = 
     +              xx_gentim2db(i, j, bi, bj, iarr) + qnetb(i, j, bi, 
     +              bj)
              ENDDO
              tmpvyb(i, j, bi, bj) = tmpvyb(i, j, bi, bj) + fvb(i, j, bi
     +          , bj)
              tmpuxb(i, j, bi, bj) = tmpuxb(i, j, bi, bj) + fub(i, j, bi
     +          , bj)
            ENDDO
          ENDDO
        ENDDO
      ENDDO
      CALL ROTATE_UV2EN_RL_B(tmpux, tmpuxb, tmpvy, tmpvyb, tmpue, tmpueb
     +                       , tmpvn, tmpvnb, .false., .true., .true., 1
     +                       , mythid)
      CALL POPREAL8ARRAY(tmpvn, 70**2)
      CALL EXCH_XY_RL_B(tmpvn, tmpvnb, mythid)
      CALL POPREAL8ARRAY(tmpue, 70**2)
      CALL EXCH_XY_RL_B(tmpue, tmpueb, mythid)
      DO bj=mybyhi(mythid),mybylo(mythid),-1
        DO bi=mybxhi(mythid),mybxlo(mythid),-1
          DO j=sny,1,-1
            DO i=snx,1,-1
              DO iarr=maxctrltim2d,1,-1
                CALL POPCONTROL1B(branch)
                IF (branch .NE. 0) xx_gentim2db(i, j, bi, bj, iarr) = 
     +              xx_gentim2db(i, j, bi, bj, iarr) + tmpvnb(i, j, bi, 
     +              bj)
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) xx_gentim2db(i, j, bi, bj, iarr) = 
     +              xx_gentim2db(i, j, bi, bj, iarr) + tmpueb(i, j, bi, 
     +              bj)
              ENDDO
            ENDDO
          ENDDO
