...
C
C  Differentiation of gad_dst2u1_impl_r in reverse (adjoint) mode:
C   gradient     of useful results: c3d b3d rtrans a3d
C   with respect to varying inputs: c3d b3d rtrans a3d
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE GAD_DST2U1_IMPL_R_B(bi, bj, k, imin, imax, jmin, jmax, 
     +                               advectionscheme, deltatarg, rtrans
     +                               , rtransb, recip_hfac, a3d, a3db, 
     +                               b3d, b3db, c3d, c3db, mythid)
      IMPLICIT NONE
C
C     == Global variables ===
CBOP
...
C    &            *recip_deepFac2F(k)*recip_rhoFacF(k)
     ...
     +        recip_deepfac2c(k)*recip_rhofacc(k)*a3db(i, j, k))
            rcenterb = rcenterb + tempb
            rupwindb = rupwindb + tempb
            abs0b = (1.d0-rlimit)*rupwindb
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              rcenterb = rcenterb + abs0b
            ELSE
              rcenterb = rcenterb - abs0b
            END IF
