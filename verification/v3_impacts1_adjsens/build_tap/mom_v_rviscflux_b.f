...
C
C  Differentiation of mom_v_rviscflux in reverse (adjoint) mode:
C   gradient     of useful results: vfld rviscfluxv
C   with respect to varying inputs: vfld rviscfluxv
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_V_RVISCFLUX_B(bi, bj, k, vfld, vfldb, kapparv, 
     +                             rviscfluxv, rviscfluxvb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C  rViscFluxV           :: viscous fluxes
      ...
      REAL*8 rviscfluxvb(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j
      ...
      IF (k .LE. 1 .OR. k .GT. nr) THEN
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            rviscfluxvb(i, j) = 0.D0
          ENDDO
        ENDDO
      ELSE
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx-1,1-olx,-1
            tempb = -(masks(i, j, k, bi, bj)*masks(i, j, k-1, bi, bj)*
     +        kapparv(i, j, k)*deepfac2f(k)*rksign*ras(i, j, bi, bj)*
     +        rhofacf(k)*recip_drc(k)*rviscfluxvb(i, j))
            rviscfluxvb(i, j) = 0.D0
