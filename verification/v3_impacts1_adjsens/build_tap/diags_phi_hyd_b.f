...
C
C  Differentiation of diags_phi_hyd in reverse (adjoint) mode:
C   gradient     of useful results: etan totphihyd phi0surf phihydc
C   with respect to varying inputs: etan totphihyd phi0surf phihydc
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE DIAGS_PHI_HYD_B(k, bi, bj, imin, imax, jmin, jmax, 
     +                           phihydc, phihydcb, mytime, myiter, 
     +                           mythid)
      IMPLICIT NONE
C     == Global variables ==
CBOP
...
C                   at fixed r-position, cell center level location.
      ...
      DO j=jmax,jmin,-1
        DO i=imax,imin,-1
          phihydcb(i, j) = phihydcb(i, j) + totphihydb(i, j, k, bi, bj)
          etanb(i, j, bi, bj) = etanb(i, j, bi, bj) + bo_surf(i, j, bi, 
     +      bj)*totphihydb(i, j, k, bi, bj)
          phi0surfb(i, j, bi, bj) = phi0surfb(i, j, bi, bj) + totphihydb
     +      (i, j, k, bi, bj)
          totphihydb(i, j, k, bi, bj) = 0.D0
        ENDDO
      ENDDO
      END
