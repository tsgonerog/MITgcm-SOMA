...
C
C  Differentiation of integrate_for_w in reverse (adjoint) mode:
C   gradient     of useful results: vfld ufld wfld
C   with respect to varying inputs: vfld ufld wfld
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE INTEGRATE_FOR_W_B(bi, bj, k, ufld, ufldb, vfld, vfldb, 
     +                             mfld, rstardhdt, wfld, wfldb, mythid)
      IMPLICIT NONE
C     == GLobal variables ==
CBOP
C    !ROUTINE: SIZE.h
...
C     conv2d         :: horizontal transport convergence [m^3/s]
      ...
      REAL*8 conv2db(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 tempb
      INTEGER ii2
      INTEGER ii1
C
...
C-  o Rigid-Lid case: zero at lower and upper boundaries
        IF (k .EQ. 1) THEN
          DO j=sny,1,-1
            DO i=snx,1,-1
              wfldb(i, j, k, bi, bj) = 0.D0
            ENDDO
          ENDDO
          DO ii1=1-oly,oly+sny
            DO ii2=1-olx,olx+snx
              conv2db(ii2, ii1) = 0.D0
            ENDDO
          ENDDO
        ELSE IF (k .EQ. nr) THEN
          ...
          DO j=sny,1,-1
            DO i=snx,1,-1
              conv2db(i, j) = conv2db(i, j) + recip_ra(i, j, bi, bj)*
     +          maskc(i, j, k-1, bi, bj)*maskc(i, j, k, bi, bj)*
     +          recip_deepfac2f(k)*recip_rhofacf(k)*wfldb(i, j, k, bi, 
     +          bj)
              wfldb(i, j, k, bi, bj) = 0.D0
            ENDDO
          ENDDO
        ELSE
          ...
          DO j=sny,1,-1
            DO i=snx,1,-1
              tempb = maskc(i, j, k, bi, bj)*recip_deepfac2f(k)*
     +          recip_rhofacf(k)*maskc(i, j, k-1, bi, bj)*wfldb(i, j, k
     +          , bi, bj)
              wfldb(i, j, k, bi, bj) = 0.D0
              wfldb(i, j, k+1, bi, bj) = wfldb(i, j, k+1, bi, bj) + 
     +          deepfac2f(k+1)*rhofacf(k+1)*tempb
              conv2db(i, j) = conv2db(i, j) + recip_ra(i, j, bi, bj)*
     +          tempb
            ENDDO
          ENDDO
...
C      non zero at surface ; zero under-ground and at r_lower boundary
        ...
        DO j=sny,1,-1
          DO i=snx,1,-1
            conv2db(i, j) = conv2db(i, j) + recip_ra(i, j, bi, bj)*maskc
     +        (i, j, k, bi, bj)*recip_deepfac2f(k)*recip_rhofacf(k)*
     +        wfldb(i, j, k, bi, bj)
            wfldb(i, j, k, bi, bj) = 0.D0
          ENDDO
        ENDDO
      ELSE
        ...
        DO j=sny,1,-1
          DO i=snx,1,-1
            tempb = maskc(i, j, k, bi, bj)*recip_deepfac2f(k)*
     +        recip_rhofacf(k)*wfldb(i, j, k, bi, bj)
            wfldb(i, j, k, bi, bj) = 0.D0
            wfldb(i, j, k+1, bi, bj) = wfldb(i, j, k+1, bi, bj) + 
     +        deepfac2f(k+1)*rhofacf(k+1)*tempb
            conv2db(i, j) = conv2db(i, j) + recip_ra(i, j, bi, bj)*tempb
          ENDDO
        ENDDO
      END IF
      DO ii1=1-oly,oly+sny
      ...
      DO j=sny,1,-1
        DO i=snx,1,-1
          utransb(i, j) = utransb(i, j) + conv2db(i, j)
          utransb(i+1, j) = utransb(i+1, j) - conv2db(i, j)
          vtransb(i, j) = vtransb(i, j) + conv2db(i, j)
          vtransb(i, j+1) = vtransb(i, j+1) - conv2db(i, j)
          conv2db(i, j) = 0.D0
        ENDDO
      ENDDO
      DO j=sny+1,1,-1
        DO i=snx+1,1,-1
          vfldb(i, j, k, bi, bj) = vfldb(i, j, k, bi, bj) + dxg(i, j, bi
     +      , bj)*drf(k)*deepfacc(k)*rhofacc(k)*hfacs(i, j, k, bi, bj)*
     +      vtransb(i, j)
          vtransb(i, j) = 0.D0
          ufldb(i, j, k, bi, bj) = ufldb(i, j, k, bi, bj) + dyg(i, j, bi
     +      , bj)*drf(k)*deepfacc(k)*rhofacc(k)*hfacw(i, j, k, bi, bj)*
     +      utransb(i, j)
          utransb(i, j) = 0.D0
        ENDDO
      ENDDO
