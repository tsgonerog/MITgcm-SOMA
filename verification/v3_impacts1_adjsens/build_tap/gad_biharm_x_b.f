...
C
C  Differentiation of gad_biharm_x in reverse (adjoint) mode:
C   gradient     of useful results: del2t dfx
C   with respect to varying inputs: del2t dfx
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_BIHARM_X_B(bi, bj, k, xa, del2t, del2tb, diffk4, 
     +                          dfx, dfxb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
...
C  myThid               :: thread number
      ...
      REAL*8 del2tb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 diffk4
      INTEGER mythid
C
C !OUTPUT PARAMETERS: ==================================================
C  dfx                  :: zonal diffusive flux
      ...
      REAL*8 dfxb(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j
      ...
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx+1,-1
          tempb = xa(i, j)*sqcosfacu(j, bi, bj)*recip_dxc(i, j, bi, bj)*
     +      diffk4*recip_deepfacc(k)*dfxb(i, j)
          del2tb(i, j) = del2tb(i, j) + tempb
          del2tb(i-1, j) = del2tb(i-1, j) - tempb
        ENDDO
      ENDDO
      END
