...
C
C  Differentiation of adams_bashforth2 in reverse (adjoint) mode:
C   gradient     of useful results: gtrnm1 ab_gtr gtracer
C   with respect to varying inputs: gtrnm1 ab_gtr gtracer
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE ADAMS_BASHFORTH2_B(bi, bj, karg, ksize, gtracer, 
     +                              gtracerb, gtrnm1, gtrnm1b, ab_gtr, 
     +                              ab_gtrb, startab, myiter, mythid)
      IMPLICIT NONE
C     == Global variables ===
CBOP
C    !ROUTINE: SIZE.h
...
C     myThid  :: my Thread Id. number
      ...
      REAL*8 gtracerb(1-olx:snx+olx, 1-oly:sny+oly, ksize)
      REAL*8 gtrnm1(1-olx:snx+olx, 1-oly:sny+oly, ksize)
      REAL*8 gtrnm1b(1-olx:snx+olx, 1-oly:sny+oly, ksize)
      REAL*8 ab_gtr(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 ab_gtrb(1-olx:snx+olx, 1-oly:sny+oly)
      INTEGER startab
        ...
        DO k=ksize,1,-1
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              ab_gtrb(i, j) = ab_gtrb(i, j) + gtrnm1b(i, j, k)
              gtracerb(i, j, k) = gtracerb(i, j, k) + gtrnm1b(i, j, k) +
     +          abfac*ab_gtrb(i, j)
              gtrnm1b(i, j, k) = -(abfac*ab_gtrb(i, j))
              ab_gtrb(i, j) = 0.D0
            ENDDO
          ENDDO
        ENDDO
...
C-    Extrapolate forward in time the tendency, with AB weights:
        ...
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            ab_gtrb(i, j) = ab_gtrb(i, j) + gtracerb(i, j, k)
            gtracerb(i, j, k) = gtracerb(i, j, k) + gtrnm1b(i, j, k) + 
     +        abfac*ab_gtrb(i, j)
            gtrnm1b(i, j, k) = -(abfac*ab_gtrb(i, j))
            ab_gtrb(i, j) = 0.D0
