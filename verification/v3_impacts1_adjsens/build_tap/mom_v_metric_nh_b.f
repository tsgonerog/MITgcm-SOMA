...
C
C  Differentiation of mom_v_metric_nh in reverse (adjoint) mode:
C   gradient     of useful results: vfld vmetricterms wfld
C   with respect to varying inputs: vfld vmetricterms wfld
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_V_METRIC_NH_B(bi, bj, k, vfld, vfldb, wfld, wfldb, 
     +                             vmetricterms, vmetrictermsb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C  vMetricTerms         :: metric term
      ...
      REAL*8 vmetrictermsb(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j, kp1
      ...
      DO j=sny+oly,1-olx+1,-1
        DO i=snx+olx,1-olx,-1
          temp = rvel2wunit(kp1)*wvelbottomoverride
          tempb = recip_rsphere*0.25*recip_deepfacc(k)*gravitysign*
     +      vmetrictermsb(i, j)
          vmetrictermsb(i, j) = 0.D0
          vfldb(i, j) = vfldb(i, j) + (temp*(wfld(i, j-1, kp1, bi, bj)+
     +      wfld(i, j, kp1, bi, bj))+rvel2wunit(k)*(wfld(i, j-1, k, bi, 
     +      bj)+wfld(i, j, k, bi, bj)))*tempb
          tempb0 = vfld(i, j)*tempb
          wfldb(i, j-1, kp1, bi, bj) = wfldb(i, j-1, kp1, bi, bj) + temp
     +      *tempb0
