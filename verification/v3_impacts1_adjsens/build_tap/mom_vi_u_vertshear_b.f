...
C
C  Differentiation of mom_vi_u_vertshear in reverse (adjoint) mode:
C   gradient     of useful results: ufld ushearterm wfld
C   with respect to varying inputs: ufld ushearterm wfld
C
C
C
C
...
C
C
      SUBROUTINE MOM_VI_U_VERTSHEAR_B(bi, bj, k, ufld, ufldb, wfld, 
     +                                wfldb, ushearterm, usheartermb, 
     +                                mythid)
      IMPLICIT NONE
C     *==========================================================*
...
C     == Routine arguments ==
      ...
      REAL*8 wfldb(1-olx:snx+olx, 1-oly:sny+oly, nr, nsx, nsy)
      REAL*8 ushearterm(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 usheartermb(1-olx:snx+olx, 1-oly:sny+oly)
      INTEGER mythid
C
C     == Local variables ==
      INTEGER i, j, kp1, km1
      REAL*8 mask_kp1, mask_km1, wbarxm, wbarxp
      REAL*8 wbarxmb, wbarxpb
      REAL*8 uzm, uzp
      REAL*8 uzmb, uzpb
...
C       Transport at interface k : Area weighted average
            CALL PUSHREAL8(wbarxm)
            wbarxm = 0.5*(wfld(i, j, k, bi, bj)*ra(i, j, bi, bj)*maskc(i
     +        , j, km1, bi, bj)+wfld(i-1, j, k, bi, bj)*ra(i-1, j, bi, 
     +        bj)*maskc(i-1, j, km1, bi, bj))*mask_km1*deepfac2f(k)*
     +        rhofacf(k)*recip_raw(i, j, bi, bj)
C
C       Transport at interface k+1 (here wFld is already masked)
            CALL PUSHREAL8(wbarxp)
            wbarxp = 0.5*(wfld(i, j, kp1, bi, bj)*ra(i, j, bi, bj)+wfld(
     +        i-1, j, kp1, bi, bj)*ra(i-1, j, bi, bj))*mask_kp1*
     +        deepfac2f(kp1)*rhofacf(kp1)*recip_raw(i, j, bi, bj)
            CALL PUSHCONTROL1B(0)
          ELSE
C       Transport at interface k : simple average
            CALL PUSHREAL8(wbarxm)
            wbarxm = 0.5*(wfld(i, j, k, bi, bj)*maskc(i, j, km1, bi, bj)
     +        +wfld(i-1, j, k, bi, bj)*maskc(i-1, j, km1, bi, bj))*
     +        mask_km1*deepfac2f(k)*rhofacf(k)
C
C       Transport at interface k+1 (here wFld is already masked)
            CALL PUSHREAL8(wbarxp)
            wbarxp = 0.5*(wfld(i, j, kp1, bi, bj)+wfld(i-1, j, kp1, bi, 
     +        bj))*mask_kp1*deepfac2f(kp1)*rhofacf(kp1)
...
C2   &                  *maskW(I,J,K,bi,bj)
      ...
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,2-olx,-1
          ...
          IF (branch .EQ. 0) THEN
            uzm = (ufld(i, j, k, bi, bj)-mask_km1*ufld(i, j, km1, bi, bj
     +        ))*rksign
            uzp = (mask_kp1*ufld(i, j, kp1, bi, bj)-ufld(i, j, k, bi, bj
     +        ))*rksign
            tempb = -(recip_hfacw(i, j, k, bi, bj)*recip_drf(k)*0.5*
     +        recip_deepfac2c(k)*recip_rhofacc(k)*usheartermb(i, j))
            usheartermb(i, j) = 0.D0
            wbarxpb = uzp*tempb
            uzpb = wbarxp*tempb
            wbarxmb = uzm*tempb
            uzmb = wbarxm*tempb
     ...
     +        recip_deepfac2c(k)*recip_rhofacc(k)*usheartermb(i, j))
            usheartermb(i, j) = 0.D0
            wbarxpb = uzp*tempb
            uzpb = (wbarxp+abs0)*tempb
            wbarxmb = uzm*tempb
            uzmb = (wbarxm-abs1)*tempb
            abs0b = uzp*tempb
            abs1b = -(uzm*tempb)
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREAL8(abs1)
              wbarxmb = wbarxmb + abs1b
            ELSE
              CALL POPREAL8(abs1)
              wbarxmb = wbarxmb - abs1b
            END IF
            ...
            IF (branch .EQ. 0) THEN
              CALL POPREAL8(abs0)
              wbarxpb = wbarxpb - abs0b
            ELSE
              CALL POPREAL8(abs0)
              wbarxpb = wbarxpb + abs0b
            END IF
          END IF
          ufldb(i, j, kp1, bi, bj) = ufldb(i, j, kp1, bi, bj) + mask_kp1
     +      *rksign*uzpb
          ufldb(i, j, k, bi, bj) = ufldb(i, j, k, bi, bj) + rksign*uzmb 
     +      - rksign*uzpb
          ufldb(i, j, km1, bi, bj) = ufldb(i, j, km1, bi, bj) - mask_km1
     +      *rksign*uzmb
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL8(wbarxp)
            tempb = deepfac2f(kp1)*rhofacf(kp1)*mask_kp1*0.5*recip_raw(i
     +        , j, bi, bj)*wbarxpb
            wfldb(i, j, kp1, bi, bj) = wfldb(i, j, kp1, bi, bj) + ra(i, 
     +        j, bi, bj)*tempb
            wfldb(i-1, j, kp1, bi, bj) = wfldb(i-1, j, kp1, bi, bj) + ra
     +        (i-1, j, bi, bj)*tempb
            CALL POPREAL8(wbarxm)
            tempb = deepfac2f(k)*rhofacf(k)*mask_km1*0.5*recip_raw(i, j
     +        , bi, bj)*wbarxmb
            wfldb(i, j, k, bi, bj) = wfldb(i, j, k, bi, bj) + ra(i, j, 
     +        bi, bj)*maskc(i, j, km1, bi, bj)*tempb
            wfldb(i-1, j, k, bi, bj) = wfldb(i-1, j, k, bi, bj) + ra(i-1
     +        , j, bi, bj)*maskc(i-1, j, km1, bi, bj)*tempb
          ELSE
            CALL POPREAL8(wbarxp)
            tempb = mask_kp1*0.5*deepfac2f(kp1)*rhofacf(kp1)*wbarxpb
            wfldb(i, j, kp1, bi, bj) = wfldb(i, j, kp1, bi, bj) + tempb
            wfldb(i-1, j, kp1, bi, bj) = wfldb(i-1, j, kp1, bi, bj) + 
     +        tempb
            CALL POPREAL8(wbarxm)
            tempb = mask_km1*0.5*deepfac2f(k)*rhofacf(k)*wbarxmb
            wfldb(i, j, k, bi, bj) = wfldb(i, j, k, bi, bj) + maskc(i, j
     +        , km1, bi, bj)*tempb
            wfldb(i-1, j, k, bi, bj) = wfldb(i-1, j, k, bi, bj) + maskc(
     +        i-1, j, km1, bi, bj)*tempb
          END IF
        ENDDO
      ENDDO
      END
