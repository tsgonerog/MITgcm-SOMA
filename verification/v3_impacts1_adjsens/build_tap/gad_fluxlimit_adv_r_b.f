...
C
C  Differentiation of gad_fluxlimit_adv_r in reverse (adjoint) mode:
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_FLUXLIMIT_ADV_R_B(bi, bj, k, dtarg, rtrans, rtransb
     +                                 , wfld, wfldb, tracer, tracerb, 
     +                                 wt, wtb, mythid)
      IMPLICIT NONE
...
C
            ...
            END IF
            rjp = (tracer(i, j, kp1)-tracer(i, j, k))*maskc(i, j, kp1, 
     +        bi, bj)
            rj = tracer(i, j, k) - tracer(i, j, km1)
            rjm = (tracer(i, j, km1)-tracer(i, j, km2))*maskc(i, j, km2
     +        , bi, bj)
C
        ...
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            rj = tracer(i, j, k) - tracer(i, j, km1)
            tempb0 = maskc(i, j, km1, bi, bj)*wtb(i, j)
            wtb(i, j) = 0.D0
            rtransb(i, j) = rtransb(i, j) + (tracer(i, j, k)+tracer(i, j
     +        , km1))*0.5d0*tempb0
            tempb = rtrans(i, j)*0.5d0*tempb0
            tempb1 = abs1*rj*0.5d0*tempb0
            tempb2 = (onerl+wcfl*cr-cr)*0.5d0*tempb0
            abs1b = rj*tempb2
            rjb = abs1*tempb2
            wcflb = cr*tempb1
            crb = (wcfl-1.0)*tempb1
            ...
            tracerb(i, j, k) = tracerb(i, j, k) + rjb
            tracerb(i, j, km1) = tracerb(i, j, km1) - rjb
