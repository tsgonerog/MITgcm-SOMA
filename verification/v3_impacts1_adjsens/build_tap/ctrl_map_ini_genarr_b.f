...
C
C  Differentiation of ctrl_map_ini_genarr in reverse (adjoint) mode:
C   gradient     of useful results: etan theta salt
...
C     !INTERFACE:
      SUBROUTINE CTRL_MAP_INI_GENARR_B(mythid)
      IMPLICIT NONE
C
C     == global variables ==
CBOP
...
C
      ...
      DO iarr=1,maxctrlarr2d
        IF (xx_genarr2d_weight(iarr) .NE. ' ') THEN
          ...
          IF (temp_genarr_fnam(1:7) .EQ. 'xx_etan') THEN
...
C
      ...
      DO iarr=1,maxctrlarr3d
        IF (xx_genarr3d_weight(iarr) .NE. ' ') THEN
          IF (xx_genarr3d_file(iarr)(1:8) .EQ. 'xx_theta') igen_theta0
     +       = iarr
          IF (xx_genarr3d_file(iarr)(1:7) .EQ. 'xx_salt') THEN
            CALL PUSHCONTROL2B(0)
            igen_salt0 = iarr
          ELSE
            CALL PUSHCONTROL2B(1)
      ...
      ENDDO
C
      IF (igen_theta0 .GT. 0) THEN
        CALL PUSHREAL8ARRAY(wgenarr3d, (snx+2*olx)*(sny+2*oly)*nr*nsx*
     +                      nsy*maxctrlarr3d)
        CALL PUSHREAL8ARRAY(theta, 70**2*31)
        CALL CTRL_MAP_GENARR3D(theta, igen_theta0, mythid)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      END IF
      IF (igen_salt0 .GT. 0) THEN
        CALL PUSHREAL8ARRAY(wgenarr3d, (snx+2*olx)*(sny+2*oly)*nr*nsx*
     +                      nsy*maxctrlarr3d)
        CALL PUSHREAL8ARRAY(salt, 70**2*31)
        CALL CTRL_MAP_GENARR3D(salt, igen_salt0, mythid)
        CALL POPREAL8ARRAY(salt, 70**2*31)
        CALL POPREAL8ARRAY(wgenarr3d, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy
     +                     *maxctrlarr3d)
        DO ii1=1,maxctrlarr3d
          xx_genarr3d_dummyb(ii1) = 0.D0
        ENDDO
        CALL CTRL_MAP_GENARR3D_B(salt, saltb, igen_salt0, mythid)
      ELSE
        DO ii1=1,maxctrlarr3d
          xx_genarr3d_dummyb(ii1) = 0.D0
        ENDDO
      ...
      IF (branch .EQ. 0) THEN
        CALL POPREAL8ARRAY(theta, 70**2*31)
        CALL POPREAL8ARRAY(wgenarr3d, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy
     +                     *maxctrlarr3d)
        CALL CTRL_MAP_GENARR3D_B(theta, thetab, igen_theta0, mythid)
      END IF
      DO iarr=maxctrlarr3d,1,-1
     ...
     +                     maxctrlarr2d)
        CALL CTRL_MAP_GENARR2D_B(etan, etanb, igen_etan, mythid)
      ELSE
        DO ii1=1,maxctrlarr2d
          xx_genarr2d_dummyb(ii1) = 0.D0
        ENDDO
      END IF
      DO iarr=maxctrlarr2d,1,-1
        CALL POPCONTROL2B(branch)
      ENDDO
      END
