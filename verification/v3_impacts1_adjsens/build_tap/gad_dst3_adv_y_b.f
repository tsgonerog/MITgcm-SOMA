...
C
C  Differentiation of gad_dst3_adv_y in reverse (adjoint) mode:
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_DST3_ADV_Y_B(bi, bj, k, calccfl, deltatloc, vtrans
     +                            , vtransb, vfld, vfldb, masklocs, 
     +                            tracer, tracerb, vt, vtb, mythid)
      IMPLICIT NONE
C
C     == GLobal variables ==
CBOP
...
C  vCFL              :: Courant-Friedrich-Levy number
      ...
      REAL*8 rjmb, rjb, rjpb, vcflb, d0b, d1b
      INTRINSIC ABS
      REAL*8 x1
      REAL*8 x1b
      REAL*8 abs0
     ...
     +        recip_deepfacc(k)
      ...
      DO j=sny+oly-1,1-oly+2,-1
        DO i=snx+olx,1-olx,-1
          ...
          tempb = (tracer(i, j-1)+d0*rj+d1*rjm)*0.5*vtb(i, j)
          tempb0 = (vtrans(i, j)+abs0)*0.5*vtb(i, j)
          tempb1 = (tracer(i, j)-d0*rj-d1*rjp)*0.5*vtb(i, j)
          tempb2 = (vtrans(i, j)-abs1)*0.5*vtb(i, j)
          vtb(i, j) = 0.D0
          ...
          IF (branch .EQ. 0) THEN
            CALL POPREAL8(abs0)
            vtransb(i, j) = vtransb(i, j) - abs0b
          ELSE
            CALL POPREAL8(abs0)
            vtransb(i, j) = vtransb(i, j) + abs0b
          END IF
          vcflb = -(2*vcfl*onesixth*d1b) - ((1.-vcfl)*onesixth+(2.-vcfl)
     +      *onesixth)*d0b
