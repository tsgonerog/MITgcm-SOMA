...
C
C  Differentiation of mom_v_sidedrag in reverse (adjoint) mode:
C   gradient     of useful results: vfld vdragterms
C   with respect to varying inputs: vfld visca4_z del2v vdragterms
C                viscah_z
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_V_SIDEDRAG_B(bi, bj, k, vfld, vfldb, del2v, del2vb
     +                            , hfacz, viscah_z, viscah_zb, visca4_z
     +                            , visca4_zb, harmonic, biharmonic, 
     +                            usevariableviscosity, vdragterms, 
     +                            vdragtermsb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
...
C  vDragTerms           :: drag term
      ...
      REAL*8 vdragtermsb(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
...
C
        DO j=2-oly,sny+oly-1
          DO i=2-olx,snx+olx-1
            x1 = viscah + viscahgrid*ras(i, j, bi, bj)/deltatmom
     ...
     +      ) THEN
        ...
        DO j=sny+oly-1,2-oly,-1
          DO i=snx+olx-1,2-olx,-1
            ...
            tempb = -(recip_hfacs(i, j, k, bi, bj)*recip_ras(i, j, bi, 
     +        bj)*recip_drf(k)*2.*drf(k)*(hfaczclosedw*dyu(i, j, bi, bj)
     +        *recip_dxv(i, j, bi, bj)+hfaczclosede*dyu(i+1, j, bi, bj)*
      ...
      ELSE
        ...
        DO j=sny+oly-1,2-oly,-1
          DO i=snx+olx-1,2-olx,-1
            ...
            hfaczclosedw = hfacs(i, j, k, bi, bj) - hfacz(i, j)
            tempb = -(recip_hfacs(i, j, k, bi, bj)*recip_ras(i, j, bi, 
     +        bj)*recip_drf(k)*sidedragfactor*drf(k)*vdragtermsb(i, j))
            vdragtermsb(i, j) = 0.D0
            tempb0 = hfaczclosedw*dyu(i, j, bi, bj)*recip_dxv(i, j, bi, 
     +        bj)*tempb
     ...
     +        visca4_z(i, j)*tempb2
            ...
            viscah_zb(i, j) = viscah_zb(i, j) + vfld(i, j)*tempb1
          ENDDO
        ENDDO
      END IF
      END
