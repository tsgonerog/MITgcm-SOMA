...
C
C  Differentiation of calc_grad_phi_hyd in reverse (adjoint) mode:
...
C     !INTERFACE:
      SUBROUTINE CALC_GRAD_PHI_HYD_B(k, bi, bj, imin, imax, jmin, jmax, 
     +                               phihydc, phihydcb, alphrho, 
     +                               dphihydx, dphihydxb, dphihydy, 
     +                               dphihydyb, mytime, myiter, mythid)
      IMPLICIT NONE
C     == Global variables ==
CBOP
C    !ROUTINE: SIZE.h
...
C     i,j :: Loop counters
      ...
      REAL*8 varlocb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 tempb
      INTEGER*4 branch
      INTEGER ii2
      INTEGER ii1
...
C
      ...
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          varlocb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO j=jmax,jmin+1,-1
        DO i=imax,imin,-1
          tempb = recip_dyc(i, j, bi, bj)*recip_deepfacc(k)*
     +      recip_rhofacc(k)*dphihydyb(i, j)
          dphihydyb(i, j) = 0.D0
          varlocb(i, j) = varlocb(i, j) + tempb
          varlocb(i, j-1) = varlocb(i, j-1) - tempb
        ENDDO
      ENDDO
      DO j=jmax,jmin,-1
        DO i=imax,imin+1,-1
          tempb = recip_dxc(i, j, bi, bj)*recip_deepfacc(k)*
     +      recip_rhofacc(k)*dphihydxb(i, j)
          dphihydxb(i, j) = 0.D0
          varlocb(i, j) = varlocb(i, j) + tempb
          varlocb(i-1, j) = varlocb(i-1, j) - tempb
        ENDDO
      ENDDO
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx,-1
      ...
      IF (branch .NE. 0) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            phihydcb(i, j) = phihydcb(i, j) + varlocb(i, j)
            phi0surfb(i, j, bi, bj) = phi0surfb(i, j, bi, bj) + varlocb(
     +        i, j)
            varlocb(i, j) = 0.D0
          ENDDO
        ENDDO
