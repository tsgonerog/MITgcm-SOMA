...
c     == local variables ==
      ...
      Real*8     globfldyz( nSx,nPx,sNy,nSy,nPy,Nr )
      Real*8     globfld3d( sNx,nSx,nPx,sNy,nSy,nPy,Nr )
      Real*8     globmskyz( nSx,nPx,sNy,nSy,nPy,Nr,nobcs )
         ...
         do jp = 1,nPy
            do bj = jtlo,jthi
               do j = jmin,jmax
                  do ip = 1,nPx
                     do bi = itlo,ithi
                        globfldyz  (bi,ip,j,bj,jp,k) = 0.D0
                        globfldtmp2(bi,ip,j,bj,jp)   = 0.
                        globfldtmp3(bi,ip,j,bj,jp)   = 0.
                        do iobcs=1,nobcs
                           globmskyz(bi,ip,j,bj,jp,k,iobcs) = 0.D0
                        enddo
                     enddo
                  enddo
               enddo
            ...
            do bj = jtlo,jthi
               do j = jmin,jmax
                  do ip = 1,nPx
                     do bi = itlo,ithi
                        do i = imin,imax
                           globfld3d(i,bi,ip,j,bj,jp,k) = 0.D0
                        enddo
                     enddo
                  enddo
               enddo
...
cph)
                       ...
                       globfld3d(ii,bi,ip,jj,bj,jp,kk) = 0.D0
                    endif
cph(
...
c     -- end of irec loop --
      ...
      do irec = nrec_nl*nx+1,ncvarrecs(ivar)
c     And now back-calculate what iobcs should be.
