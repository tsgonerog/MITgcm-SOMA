...
C
C  Differentiation of gad_biharm_y in reverse (adjoint) mode:
C   gradient     of useful results: dfy
C   with respect to varying inputs: del2t dfy
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_BIHARM_Y_B(bi, bj, k, ya, del2t, del2tb, diffk4, 
     +                          dfy, dfyb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
...
C  i,j                  :: loop indices
      ...
      DO j=sny+oly,1-oly+1,-1
        DO i=snx+olx,1-olx,-1
          tempb = recip_dyc(i, j, bi, bj)*ya(i, j)*diffk4*recip_deepfacc
     +      (k)*dfyb(i, j)
