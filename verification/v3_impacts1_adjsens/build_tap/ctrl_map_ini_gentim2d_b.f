...
C
C  Differentiation of ctrl_map_ini_gentim2d in reverse (adjoint) mode:
C   gradient     of useful results: xx_gentim2d_dummy
C   with respect to varying inputs: xx_gentim2d_dummy
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE CTRL_MAP_INI_GENTIM2D_B(mythid)
      IMPLICIT NONE
C
C     == global variables ==
CBOP
...
C     temporary values in the do loop during reverse pass.
      CHARACTER*(max_len_fnam) fnamegenin(maxctrltim2d)
      CHARACTER*(max_len_fnam) fnamegenout(maxctrltim2d)
      CHARACTER*(max_len_fnam) fnamegentmp(maxctrltim2d)
      ...
      EXTERNAL ACTIVE_WRITE_XY
      EXTERNAL ACTIVE_WRITE_XY_B
      INTRINSIC SQRT
...
C
          CALL PUSHINTEGER4(ilgen)
          ilgen = ILNBLNK(xx_gentim2d_file(iarr))
          temp_genarr_fnama = xx_gentim2d_file(iarr)
C
          CALL CTRL_INIT_REC(temp_genarr_fnama(1:ilgen), 
     +                       xx_gentim2d_startdate1(iarr), 
     +                       xx_gentim2d_startdate2(iarr), 
     +                       xx_gentim2d_period(iarr), 1, 
     +                       xx_gentim2d_startdate(1, iarr), diffrec, 
...
C
          ...
          ENDDO
C
          WRITE(temp_genarr_fnamb, '(2a,i10.10)') ctrldir(1:ildir)//
     +          temp_genarr_fnama(1:ilgen), '.', optimcycle
          fnamegenin(iarr) = temp_genarr_fnamb
          WRITE(temp_genarr_fnamb, '(2a,i10.10)') ctrldir(1:ildir)//
     +          temp_genarr_fnama(1:ilgen), '.effective.', optimcycle
          fnamegenout(iarr) = temp_genarr_fnamb
          WRITE(temp_genarr_fnamb, '(2a,i10.10)') ctrldir(1:ildir)//
     +          temp_genarr_fnama(1:ilgen), '.tmp.', optimcycle
C
C--   docycle
C
          ...
          DO k2=1,maxctrlproc
            IF (xx_gentim2d_preproc(k2, iarr) .EQ. 'docycle') THEN
              IF (xx_gentim2d_preproc_i(k2, iarr) .NE. 0) THEN
                IF (diffrec .GT. xx_gentim2d_preproc_i(k2, iarr)) THEN
                  replicated_nrec = xx_gentim2d_preproc_i(k2, iarr)
                ELSE
                  replicated_nrec = diffrec
                END IF
                replicated_ntimes = INT(FLOAT(diffrec)/FLOAT(
     +            replicated_nrec))
                IF (replicated_ntimes*replicated_nrec .LT. diffrec) 
     +            replicated_ntimes = replicated_ntimes + 1
                IF (replicated_ntimes*replicated_nrec .GT. diffrec) THEN
...
C
          DO jrec=1,replicated_ntimes+1
            DO irec=1,replicated_nrec
              CALL PUSHINTEGER4(krec)
              krec = replicated_nrec*(jrec-1) + irec
              CALL PUSHINTEGER4(lrec)
...
C https://mitgcm.readthedocs.io/en/latest/ocean_state_est/ocean_state_est.html#ctrl-model-parameter-adjustment-capability
                CALL PUSHREAL8ARRAY(xx_gen, 70**2)
...
C Here, we write record kRec (from 1 to diffrec) to file xx_*.effective.$iter
                CALL PUSHREAL8(xx_gentim2d_dummy(iarr))
                CALL ACTIVE_WRITE_XY(fnamegenout(iarr), xx_gen, krec, 
     +                               optimcycle, mythid, 
     +                               xx_gentim2d_dummy(iarr))
                CALL PUSHCONTROL1B(1)
              ELSE
                CALL PUSHCONTROL1B(0)
              END IF
            ENDDO
            CALL PUSHINTEGER4(irec - 1)
          ENDDO
          CALL PUSHINTEGER4(jrec - 1)
C
C--   rmcycle
C
...
C For now, just assumes records from startrec to endrec are available in file
            ...
            ENDDO
            DO ii1=1-olx,66
              CALL PUSHREAL8(wgentim2d(ii1, 1-oly, 1, 1, iarr))
            ENDDO
...
C
            DO bj=mybylo(mythid),mybyhi(mythid)
              DO bi=mybxlo(mythid),mybxhi(mythid)
                DO j=1,sny
                  DO i=1,snx
                    IF (mask2d(i, j, bi, bj) .NE. 0. .AND. wgentim2d(i, 
     +                  j, bi, bj, iarr) .GT. 0.) THEN
C IF (doscaling) then
                      IF (doscaling) THEN
                        CALL PUSHREAL8(xx_gen(i, j, bi, bj))
                        xx_gen(i, j, bi, bj) = xx_gen(i, j, bi, bj)/SQRT
     +                    (wgentim2d(i, j, bi, bj, iarr))
                        CALL PUSHCONTROL2B(2)
                      ELSE
                        CALL PUSHCONTROL2B(1)
                      END IF
...
C
     ...
     +                           optimcycle, mythid, xx_gentim2d_dummy(
     +                           iarr))
C--   end iRec loop
          ENDDO
          CALL PUSHINTEGER4(irec - 1)
      ...
      DO iarr=maxctrltim2d,1,-1
        ...
        IF (branch .EQ. 0) THEN
          ...
          DO irec=ad_to1,1,-1
            ...
            CALL POPREAL8(xx_gentim2d_dummy(iarr))
            ...
            DO ii1=1,nsy
              ...
              ENDDO
            ENDDO
            CALL ACTIVE_WRITE_XY_B(fnamegenout(iarr), xx_gen, xx_genb, 
     +                             irec, optimcycle, mythid, 
     +                             xx_gentim2d_dummy(iarr), 
     +                             xx_gentim2d_dummyb(iarr))
            CALL POPREAL8(xx_gentim2d_dummy(iarr))
            CALL POPREAL8ARRAY(xx_gen, 70**2)
            CALL EXCH_XY_RL_B(xx_gen, xx_genb, mythid)
     ...
     +                  (wgentim2d(i, j, bi, bj, iarr))
            ...
            ENDDO
            DO ii1=66,1-olx,-1
              CALL POPREAL8(wgentim2d(ii1, 1-oly, 1, 1, iarr))
            ENDDO
            DO k2=maxctrlproc,1,-1
              CALL POPCONTROL1B(branch)
            ENDDO
            CALL ACTIVE_READ_XY_B(fnamegenout(iarr), xx_gen, xx_genb, 
     +                            irec, doglobalread, ladinit, 
     +                            optimcycle, mythid, xx_gentim2d_dummy(
     +                            iarr), xx_gentim2d_dummyb(iarr))
            CALL POPREAL8ARRAY(xx_gen, 70**2)
          ENDDO
            ...
            DO irec=ad_to,1,-1
              ...
              IF (branch .NE. 0) THEN
                ...
                DO ii1=1,nsy
                  ...
                  ENDDO
                ENDDO
                CALL ACTIVE_WRITE_XY_B(fnamegenout(iarr), xx_gen, 
     +                                 xx_genb, krec, optimcycle, mythid
     +                                 , xx_gentim2d_dummy(iarr), 
     +                                 xx_gentim2d_dummyb(iarr))
                CALL POPREAL8(xx_gentim2d_dummy(iarr))
                CALL ACTIVE_READ_XY_B(fnamegenin(iarr), xx_gen, xx_genb
     +                                , lrec, doglobalread, ladinit, 
     +                                optimcycle, mythid, 
     +                                xx_gentim2d_dummy(iarr), 
     +                                xx_gentim2d_dummyb(iarr))
                CALL POPREAL8ARRAY(xx_gen, 70**2)
              END IF
              CALL POPINTEGER4(lrec)
              CALL POPINTEGER4(krec)
            ENDDO
          ENDDO
          DO k2=maxctrlproc,1,-1
            CALL POPCONTROL2B(branch)
