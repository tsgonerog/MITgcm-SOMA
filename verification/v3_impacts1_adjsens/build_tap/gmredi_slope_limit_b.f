...
C
C  Differentiation of gmredi_slope_limit in reverse (adjoint) mode:
C   gradient     of useful results: taperfct slopex slopey dsigmadr
C                dsigmadx dsigmady slopesqr
C   with respect to varying inputs: taperfct slopex slopey dsigmadr
C                dsigmadx dsigmady slopesqr
C
C
C
...
C     !INTERFACE:
      SUBROUTINE GMREDI_SLOPE_LIMIT_B(slopex, slopexb, slopey, slopeyb, 
     +                                slopesqr, slopesqrb, taperfct, 
     +                                taperfctb, htranslay, baseslope, 
     +                                reciplambda, dsigmadr, dsigmadrb, 
     +                                dsigmadx, dsigmadxb, dsigmady, 
     +                                dsigmadyb, lrho, hmixlay, rdepth, 
     +                                depthz, klow, kpos, k, bi, bj, 
...
C     !LOCAL VARIABLES:
      ...
      REAL*8 drdsigmaltdb(1-olx:snx+olx, 1-oly:sny+oly)
...
C
      IF (kpos .EQ. 3) THEN
        ...
        maxslopesqr = gm_maxslope*gm_maxslope*wunit2rvel(k)*wunit2rvel(k
     +    )
        convslopeunit = rvel2wunit(k)
      ELSE
        gm_bigslope = gm_bigslope*z2runit(k)
        maxslopesqr = gm_maxslope*gm_maxslope*z2runit(k)*z2runit(k)
        ...
        DO j=1-oly+1,sny+oly-1
          DO i=1-olx+1,snx+olx-1
            IF (dsigmadr(i, j) .NE. zerorl) THEN
              IF (dsigmadr(i, j) .LE. gm_small_number) THEN
...
C
     ...
     +            , j) .LT. gm_slopesqcutoff) THEN
                IF (slopesqr(i, j) .GT. gm_bigslope*gm_bigslope) THEN
          ...
          DO j=sny+oly-1,1-oly+1,-1
            DO i=snx+olx-1,1-olx+1,-1
              ...
              IF (branch .LT. 2) THEN
                IF (branch .EQ. 0) taperfctb(i, j) = 0.D0
              ELSE
                IF (branch .NE. 2) slopesqrb(i, j) = 0.D0
                temp = maxslopesqr/slopesqr(i, j)
                IF (.NOT.temp .EQ. 0.0) slopesqrb(i, j) = slopesqrb(i, j
     +              ) - temp*taperfctb(i, j)/(slopesqr(i, j)*2.0*SQRT(
     +              temp))
                taperfctb(i, j) = 0.D0
              END IF
            ENDDO
          ...
          DO j=sny+oly-1,1-oly+1,-1
            DO i=snx+olx-1,1-olx+1,-1
              ...
              ELSE IF (branch .EQ. 1) THEN
                slopesqrb(i, j) = slopesqrb(i, j) - maxslopesqr*
     +            taperfctb(i, j)/slopesqr(i, j)**2
                taperfctb(i, j) = 0.D0
              END IF
            ENDDO
          ENDDO
...
C
          ...
          DO j=sny+oly-1,1-oly+1,-1
            DO i=snx+olx-1,1-olx+1,-1
              ...
              IF (branch .NE. 0) THEN
                IF (branch .EQ. 1) THEN
                  smodb = -((1.0-TANH((gm_scrit-smod)/gm_sd)**2)*op5*
     +              taperfctb(i, j)/gm_sd)
                  taperfctb(i, j) = 0.D0
                  CALL POPREAL8(smod)
                  IF (.NOT.slopesqr(i, j) .EQ. 0.0) slopesqrb(i, j) = 
     +                slopesqrb(i, j) + convslopeunit*smodb/(2.0*SQRT(
     +                slopesqr(i, j)))
                ELSE
                  taperfctb(i, j) = 0.D0
                END IF
...
C
              ...
              ELSE IF (slopesqr(i, j) .LT. gm_slopesqcutoff) THEN
                ...
                CALL PUSHREAL8(f1)
                f1 = op5*(onerl+TANH((gm_scrit-smod*convslopeunit)/gm_sd
     +            ))
          ...
          DO j=sny+oly-1,1-oly+1,-1
            DO i=snx+olx-1,1-olx+1,-1
              ...
              IF (branch .EQ. 0) THEN
                taperfctb(i, j) = 0.D0
              ELSE IF (branch .EQ. 1) THEN
                f1b = f2*taperfctb(i, j)
                f2b = f1*taperfctb(i, j)
                taperfctb(i, j) = 0.D0
                ...
                ELSE
                  rnondim = rdepth/(lrho(i, j)*smod)
                  CALL POPREAL8(f2)
                  rnondimb = fpi*COS(fpi*(rnondim-op5))*op5*f2b
                END IF
                temp = lrho(i, j)*smod
                smodb = -(lrho(i, j)*rdepth*rnondimb/temp**2) - 
     +            convslopeunit*(1.0-TANH((gm_scrit-convslopeunit*smod)/
     +            gm_sd)**2)*op5*f1b/gm_sd
                CALL POPREAL8(f1)
                CALL POPREAL8(smod)
                IF (.NOT.slopesqr(i, j) .EQ. 0.0) slopesqrb(i, j) = 
     +              slopesqrb(i, j) + smodb/(2.0*SQRT(slopesqr(i, j)))
              END IF
            ENDDO
          ENDDO
        ELSE IF (gm_taper_scheme .EQ. 'stableGmAdjTap') THEN
C
C
C
          STOP
C
C
        ELSE IF (gm_taper_scheme .NE. ' ') THEN
          STOP
        END IF
        DO j=sny+oly-1,1-oly+1,-1
          DO i=snx+olx-1,1-olx+1,-1
            ...
            slopexb(i, j) = slopexb(i, j) + 2*slopex(i, j)*slopesqrb(i, 
     +        j)
            slopeyb(i, j) = slopeyb(i, j) + 2*slopey(i, j)*slopesqrb(i, 
     +        j)
            slopesqrb(i, j) = 0.D0
          ENDDO
        ENDDO
        ...
        DO j=sny+oly-1,1-oly+1,-1
          DO i=snx+olx-1,1-olx+1,-1
            ...
            IF (branch .EQ. 0) THEN
              ...
              dsigmadyb(i, j) = dsigmadyb(i, j) + drdsigmaltd(i, j)*
     +          slopeyb(i, j)
              drdsigmaltdb(i, j) = drdsigmaltdb(i, j) + dsigmady(i, j)*
     +          slopeyb(i, j) + dsigmadx(i, j)*slopexb(i, j)
              slopeyb(i, j) = 0.D0
              CALL POPREAL8(slopex(i, j))
              dsigmadxb(i, j) = dsigmadxb(i, j) + drdsigmaltd(i, j)*
     +          slopexb(i, j)
              slopexb(i, j) = 0.D0
              dsigmadrb(i, j) = dsigmadrb(i, j) - drdsigmaltdb(i, j)/
     +          dsigmadr(i, j)**2
              drdsigmaltdb(i, j) = 0.D0
            ELSE
              IF (branch .EQ. 1) THEN
                CALL POPREAL8(slopey(i, j))
                slopeyb(i, j) = 0.D0
        ...
        DO j=sny+oly-1,1-oly+1,-1
          DO i=snx+olx-1,1-olx+1,-1
            CALL POPCONTROL2B(branch)
            IF (branch .NE. 0) THEN
              IF (branch .NE. 1) dsigmadrb(i, j) = 0.D0
            END IF
          ENDDO
        ENDDO
      END IF
