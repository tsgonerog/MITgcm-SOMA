...
C
C  Differentiation of ctrl_get_gen in reverse (adjoint) mode:
C   gradient     of useful results: xx_gen_dummy xx_gen0 xx_gen1
C                genfld
C   with respect to varying inputs: xx_gen_dummy xx_gen0 xx_gen1
C                genfld
C
C
C
...
C   ==================================================================
C
      SUBROUTINE CTRL_GET_GEN_B(xx_gen_file, xx_genstartdate, 
     +                          xx_genperiod, genmask, genfld, genfldb, 
     +                          xx_gen0, xx_gen0b, xx_gen1, xx_gen1b, 
     +                          xx_gen_dummy, xx_gen_dummyb, 
     +                          xx_gen_remo_intercept, xx_gen_remo_slope
     +                          , genweight, mytime, myiter, mythid)
      IMPLICIT NONE
C
C     == global variables ==
C
...
C     == routine arguments ==
      ...
      REAL*8 genfldb(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      REAL*8 xx_gen0(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      REAL*8 xx_gen0b(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      REAL*8 xx_gen1(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      REAL*8 xx_gen1b(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      REAL*8 xx_gen_dummy
      REAL*8 xx_gen_dummyb
...
C
      IF (gencount0 .LE. 2 .AND. (xx_gen_file(1:7) .EQ. xx_tauu_file 
     +    .OR. xx_gen_file(1:7) .EQ. xx_tauv_file) .AND. xx_genperiod 
     +    .NE. zerorl) THEN
        doctrlupdate = .false.
      ELSE
        doctrlupdate = .true.
      END IF
      IF (xx_gen_file(1:7) .EQ. xx_tauu_file .OR. xx_gen_file(1:7) .EQ. 
     +    xx_tauv_file) THEN
        gensign = -1.
      ELSE
        gensign = 1.
      ...
      IF (doctrlupdate) THEN
        DO bj=mybyhi(mythid),mybylo(mythid),-1
          DO bi=mybxhi(mythid),mybxlo(mythid),-1
            DO j=sny,1,-1
              DO i=snx,1,-1
                genfldb(i, j, bi, bj) = genmask(i, j, bi, bj)*genfldb(i
     +            , j, bi, bj)
                xx_gen0b(i, j, bi, bj) = xx_gen0b(i, j, bi, bj) + 
     +            gensign*genfac*genfldb(i, j, bi, bj)
                xx_gen1b(i, j, bi, bj) = xx_gen1b(i, j, bi, bj) + 
     +            gensign*(1.d0-genfac)*genfldb(i, j, bi, bj)
              ENDDO
            ENDDO
          ENDDO
        ENDDO
      ...
      IF (branch .EQ. 0) THEN
        CALL ACTIVE_READ_XY_B(fnamegen, xx_gen1, xx_gen1b, gencount1, 
     +                        doglobalread, ladinit, optimcycle, mythid
     +                        , xx_gen_dummy, xx_gen_dummyb)
        CALL CTRL_SWAPFFIELDS_B(xx_gen0, xx_gen0b, xx_gen1, xx_gen1b, 
     +                          mythid)
      END IF
      CALL POPCONTROL2B(branch)
      IF (branch .EQ. 0) THEN
        CALL ACTIVE_READ_XY_B(fnamegen, xx_gen0, xx_gen0b, gencount0, 
     +                        doglobalread, ladinit, optimcycle, mythid
     +                        , xx_gen_dummy, xx_gen_dummyb)
        DO ii1=1,nsy
          DO ii2=1,nsx
      ...
      ELSE IF (branch .NE. 1) THEN
        GOTO 100
      END IF
      CALL ACTIVE_READ_XY_B(fnamegen, xx_gen1, xx_gen1b, gencount0, 
     +                      doglobalread, ladinit, optimcycle, mythid, 
     +                      xx_gen_dummy, xx_gen_dummyb)
      DO ii1=1,nsy
        DO ii2=1,nsx
