...
C
C  Differentiation of mom_u_metric_cylinder in reverse (adjoint) mode:
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_U_METRIC_CYLINDER_B(bi, bj, k, ufld, ufldb, vfld, 
     +                                   vfldb, umetricterms, 
...
C         radius = yC(i,j,bi,bj)
      ...
      DO j=sny+oly-1,1-oly,-1
        DO i=snx+olx,1-olx+1,-1
          ...
          IF (branch .EQ. 0) THEN
            radius = (yg(i, j, bi, bj)+yg(i, j+1, bi, bj))*0.5d0
            ufldb(i, j) = ufldb(i, j) + (vfld(i, j)+vfld(i-1, j)+vfld(i
     +        , j+1)+vfld(i-1, j+1))*0.25*umetrictermsb(i, j)/radius
            tempb = ufld(i, j)*0.25*umetrictermsb(i, j)/radius
            umetrictermsb(i, j) = 0.D0
            vfldb(i, j) = vfldb(i, j) + tempb
            vfldb(i-1, j) = vfldb(i-1, j) + tempb
            vfldb(i, j+1) = vfldb(i, j+1) + tempb
            vfldb(i-1, j+1) = vfldb(i-1, j+1) + tempb
          ELSE
            umetrictermsb(i, j) = 0.D0
          END IF
        ENDDO
