...
C
C  Differentiation of mom_u_rviscflux in reverse (adjoint) mode:
C   gradient     of useful results: ufld rviscfluxu
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_U_RVISCFLUX_B(bi, bj, k, ufld, ufldb, kapparu, 
     +                             rviscfluxu, rviscfluxub, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C  rViscFluxU           :: viscous fluxes
      ...
      REAL*8 rviscfluxub(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j
...
C     - Vertical viscous flux
      IF (.NOT.(k .LE. 1 .OR. k .GT. nr)) THEN
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx-1,1-olx,-1
            tempb = -(maskw(i, j, k, bi, bj)*maskw(i, j, k-1, bi, bj)*
     +        kapparu(i, j, k)*deepfac2f(k)*rksign*raw(i, j, bi, bj)*
     +        rhofacf(k)*recip_drc(k)*rviscfluxub(i, j))
            rviscfluxub(i, j) = 0.D0
