...
C
C  Differentiation of pressure_for_eos in reverse (adjoint) mode:
C   gradient     of useful results: totphihyd locpres
C   with respect to varying inputs: totphihyd
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE PRESSURE_FOR_EOS_B(bi, bj, imin, imax, jmin, jmax, k, 
     +                              dpref, locpres, locpresb, mythid)
      IMPLICIT NONE
C     == Global variables ==
CBOP
C    !ROUTINE: SIZE.h
        ...
        IF (selectp_ineos_zc .EQ. 2) THEN
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              totphihydb(i, j, k, bi, bj) = totphihydb(i, j, k, bi, bj) 
     +          + rhoconst*locpresb(i, j)
              locpresb(i, j) = 0.D0
            ENDDO
          ENDDO
        END IF
