...
C
C  Differentiation of mom_u_yviscflux in reverse (adjoint) mode:
C   gradient     of useful results: ufld visca4_z del2u yviscfluxu
C                viscah_z
C   with respect to varying inputs: ufld visca4_z del2u yviscfluxu
C                viscah_z
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_U_YVISCFLUX_B(bi, bj, k, ufld, ufldb, del2u, del2ub
     +                             , hfacz, yviscfluxu, yviscfluxub, 
     +                             viscah_z, viscah_zb, visca4_z, 
     +                             visca4_zb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
...
C  yViscFluxU           :: viscous fluxes
      ...
      REAL*8 yviscfluxub(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j
     ...
     +      bi, bj)*yviscfluxub(i, j)
          yviscfluxub(i, j) = 0.D0
          visca4_zb(i, j) = visca4_zb(i, j) + (del2u(i, j)-del2u(i, j-1)
     +      )*tempb
          tempb0 = visca4_z(i, j)*tempb
          viscah_zb(i, j) = viscah_zb(i, j) - (ufld(i, j)-ufld(i, j-1))*
     +      tempb
          tempb1 = -(viscah_z(i, j)*tempb)
          ufldb(i, j) = ufldb(i, j) + tempb1
          ufldb(i, j-1) = ufldb(i, j-1) - tempb1
          del2ub(i, j) = del2ub(i, j) + tempb0
          del2ub(i, j-1) = del2ub(i, j-1) - tempb0
        ENDDO
      ENDDO
      END
