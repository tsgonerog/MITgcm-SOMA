...
c     == routine arguments ==
      ...
      Real*8 xx_gen_dummy
      Real*8 xx_gen_period
      Real*8 xx_gen_weight(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      logical dodimensionalcost
      Real*8 num_gen_anom(nSx,nSy)
      Real*8 objf_gen_anom(nSx,nSy)
      Real*8 xx_gen_mask2D(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      integer myThid
...
c     == local variables ==
      ...
      Real*8 tmpfld2d(1-OLx:sNx+OLx,1-OLy:sNy+OLy,   nSx,nSy)
...
c--   >>> Loop 1 to compute mean forcing:
      do bj = jtlo,jthi
        do bi = itlo,ithi
          num_gen_anom(bi,bj)  = 0.D0
          objf_gen_anom(bi,bj) = 0.D0
        enddo
      enddo
...
c--         Determine the weights to be used.
            ...
            do j = jmin,jmax
              do i = imin,imax
                if (xx_gen_mask2D(i,j,bi,bj) .ne. 0.D0) then
                ...
                ENDIF
                if ( xx_gen_weight(i,j,bi,bj) .ne. 0.D0 )
...
c     == routine arguments ==
      ...
      character*(MAX_LEN_FNAM) xx_gen_file
      Real*8 xx_gen_dummy
      Real*8 xx_gen_weight(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      logical dodimensionalcost
      Real*8 num_gen(nSx,nSy)
      Real*8 objf_gen(nSx,nSy)
      Real*8 xx_gen_mask(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      INTEGER myThid
...
c     == local variables ==
      ...
      logical doglobalread
      logical ladinit
      Real*8     tmpfld3d(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
...
c--   Loop over this thread tiles.
      do bj = jtlo,jthi
        do bi = itlo,ithi
          ...
          do k = 1,Nr
            do j = jmin,jmax
              do i = imin,imax
                if (xx_gen_mask(i,j,k,bi,bj) .ne. 0.D0) then
     ...
     &                 *tmpx*tmpx
                ...
                ENDIF
                if ( xx_gen_weight(i,j,k,bi,bj) .ne. 0.D0 )
     &            num_gen(bi,bj) = num_gen(bi,bj) + 1.D0
                endif
              enddo
            enddo
          enddo
