...
C
C  Differentiation of ctrl_map_genarr2d in reverse (adjoint) mode:
C   gradient     of useful results: fld
C   with respect to varying inputs: xx_genarr2d_dummy
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE CTRL_MAP_GENARR2D_B(fld, fldb, iarr, mythid)
      IMPLICIT NONE
C
C     == global variables ==
CBOP
...
C     !INPUT/OUTPUT PARAMETERS:
      ...
      REAL*8 fldb(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      INTEGER iarr
      INTEGER mythid
      EXTERNAL ILNBLNK
C
...
C     !LOCAL VARIABLES:
      ...
      REAL*8 xx_genb(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      ...
      EXTERNAL ACTIVE_READ_XY_B
      INTRINSIC SQRT
      INTRINSIC EXP
      EXTERNAL CTRL_BOUND_2D
      EXTERNAL WRITE_REC_3D_RL
...
C
      DO k2=1,maxctrlproc
        IF (xx_genarr2d_preproc(k2, iarr) .EQ. 'noscaling') doscaling = 
     +      .false.
...
C     Find ctrlDir (w/o trailing blanks) length
      ...
      WRITE(fnamegenin, '(2A,I10.10)') ctrldir(1:ildir)//fnamebase(1:
     +      ilgen), '.', optimcycle
      WRITE(fnamegenout, '(2A,I10.10)') ctrldir(1:ildir)//fnamebase(1:
     +      ilgen), '.effective.', optimcycle
C
      CALL READ_REC_3D_RL(xx_genarr2d_weight(iarr), ctrlprec, 1, 
     +                    wgenarr2d(1-olx, 1-oly, 1, 1, iarr), 1, 1, 
     +                    mythid)
C
      CALL ACTIVE_READ_XY(fnamegenin, xx_gen, 1, doglobalread, ladinit, 
     +                    optimcycle, mythid, xx_genarr2d_dummy(iarr))
C
...
C scale param adjustment
                IF (wgenarr2d(i, j, bi, bj, iarr) .GT. 0.) THEN
                  xx_gen(i, j, bi, bj) = xx_gen(i, j, bi, bj)/SQRT(
     +              wgenarr2d(i, j, bi, bj, iarr))
C dolog10ctrls
                  IF (dolog10ctrl) THEN
...
C            xx_gen(i,j,bi,bj) = EXP(LOG(10.0) * xx_gen(i,j,bi,bj))
                    CALL PUSHREAL8(xx_gen(i, j, bi, bj))
...
C or in case of log10ctrl, fld = 10^(xx_gen)
          IF (dolog10ctrl) THEN
            DO j=1,sny
              DO i=1,snx
                fld(i, j, bi, bj) = xx_gen(i, j, bi, bj)*mask2d(i, j, bi
     +            , bj)
              ENDDO
            ENDDO
            CALL PUSHCONTROL1B(1)
          ELSE
            DO j=1,sny
              DO i=1,snx
                fld(i, j, bi, bj) = fld(i, j, bi, bj) + xx_gen(i, j, bi
     +            , bj)*mask2d(i, j, bi, bj)
              ENDDO
            ENDDO
            CALL PUSHCONTROL1B(0)
        ...
        ENDDO
      ENDDO
C$FWD-OF DO-NOT-DIFF 
C$END-FWD-OF DO-NOT-DIFF 
C$BWD-OF DO-NOT-DIFF 
C$END-BWD-OF DO-NOT-DIFF 
      CALL EXCH_XY_RL_B(fld, fldb, mythid)
      DO ii1=1,nsy
        DO ii2=1,nsx
          DO ii3=1-oly,oly+sny
            DO ii4=1-olx,olx+snx
              xx_genb(ii4, ii3, ii2, ii1) = 0.D0
            ENDDO
          ENDDO
        ENDDO
      ENDDO
      DO bj=mybyhi(mythid),mybylo(mythid),-1
        DO bi=mybxhi(mythid),mybxlo(mythid),-1
          ...
          IF (branch .EQ. 0) THEN
            DO j=sny,1,-1
              DO i=snx,1,-1
                xx_genb(i, j, bi, bj) = xx_genb(i, j, bi, bj) + mask2d(i
     +            , j, bi, bj)*fldb(i, j, bi, bj)
              ENDDO
            ENDDO
          ELSE
            DO j=sny,1,-1
          ...
          IF (branch .EQ. 0) THEN
            DO j=sny,1,-1
              DO i=snx,1,-1
                ...
                IF (branch .EQ. 0) THEN
                  xx_genb(i, j, bi, bj) = 0.D0
                ELSE
                  IF (branch .NE. 1) THEN
                    CALL POPREAL8(xx_gen(i, j, bi, bj))
                    xx_genb(i, j, bi, bj) = ln10*EXP(ln10*xx_gen(i, j, 
     +                bi, bj))*xx_genb(i, j, bi, bj)
                  END IF
                  xx_genb(i, j, bi, bj) = xx_genb(i, j, bi, bj)/SQRT(
     +              wgenarr2d(i, j, bi, bj, iarr))
                END IF
              ENDDO
            ENDDO
        ...
        ENDDO
      ENDDO
      DO ii1=1,maxctrlarr2d
        xx_genarr2d_dummyb(ii1) = 0.D0
      ENDDO
      CALL ACTIVE_READ_XY_B(fnamegenin, xx_gen, xx_genb, 1, doglobalread
     +                      , ladinit, optimcycle, mythid, 
     +                      xx_genarr2d_dummy(iarr), xx_genarr2d_dummyb(
     +                      iarr))
      DO k2=maxctrlproc,1,-1
        CALL POPCONTROL1B(branch)
      ENDDO
      END

C  Differentiation of ctrl_map_genarr3d in reverse (adjoint) mode:
C   gradient     of useful results: xx_genarr3d_dummy fld
C   with respect to varying inputs: xx_genarr3d_dummy
C
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
C
CBOP
...
C     !INTERFACE:
      SUBROUTINE CTRL_MAP_GENARR3D_B(fld, fldb, iarr, mythid)
      IMPLICIT NONE
C
C     == global variables ==
CBOP
...
C     !INPUT/OUTPUT PARAMETERS:
      ...
      REAL*8 fldb(1-olx:snx+olx, 1-oly:sny+oly, nr, nsx, nsy)
      INTEGER iarr
      INTEGER mythid
      EXTERNAL ILNBLNK
C
...
C     !LOCAL VARIABLES:
      ...
      REAL*8 xx_genb(1-olx:snx+olx, 1-oly:sny+oly, nr, nsx, nsy)
      CHARACTER*(max_len_fnam) fnamegenin
      CHARACTER*(max_len_fnam) fnamegenout
      CHARACTER*(max_len_fnam) fnamebase
      INTEGER ilgen
...
C
      DO k2=1,maxctrlproc
        IF (xx_genarr3d_preproc(k2, iarr) .EQ. 'noscaling') doscaling = 
     +      .false.
...
C
     ...
     +                    wgenarr3d(1-olx, 1-oly, 1, 1, 1, iarr), 1, 1, 
     +                    mythid)
C
      CALL ACTIVE_READ_XYZ(fnamegenin, xx_gen, 1, doglobalread, ladinit
     +                     , optimcycle, mythid, xx_genarr3d_dummy(iarr)
     +                    )
C
C --- Get appropriate mask for ctrl variable
...
C             xx_gen(i,j,k,bi,bj) = EXP(LOG(10.0) * xx_gen(i,j,k,bi,bj))
                      CALL PUSHREAL8(xx_gen(i, j, k, bi, bj))
                      xx_gen(i, j, k, bi, bj) = EXP(ln10*xx_gen(i, j, k
     +                  , bi, bj))
                      CALL PUSHCONTROL2B(2)
                    ELSE
                      CALL PUSHCONTROL2B(1)
...
C     or in case of log10ctrl, fld = 10^(xx_gen)
          IF (dolog10ctrl) THEN
            DO k=1,nr
              DO j=1,sny
                DO i=1,snx
                  fld(i, j, k, bi, bj) = xx_gen(i, j, k, bi, bj)*mask3d(
     +              i, j, k, bi, bj)
                ENDDO
              ENDDO
            ENDDO
          ...
          ELSE
            DO k=1,nr
              DO j=1,sny
                DO i=1,snx
                  fld(i, j, k, bi, bj) = fld(i, j, k, bi, bj) + xx_gen(i
     +              , j, k, bi, bj)*mask3d(i, j, k, bi, bj)
                ENDDO
              ENDDO
            ENDDO
            CALL PUSHCONTROL1B(1)
...
C  xx_uvel and xx_vvel are read in.
     ...
     +    xx_genarr3d_file(iarr)(1:7) .NE. 'xx_vvel') THEN
        CALL PUSHCONTROL1B(1)
      ELSE
        CALL PUSHCONTROL1B(0)
      END IF
...
C$END-BWD-OF DO-NOT-DIFF 
      CALL POPCONTROL1B(branch)
      IF (branch .NE. 0) CALL EXCH_XYZ_RL_B(fld, fldb, mythid)
      DO ii1=1,nsy
        DO ii2=1,nsx
          DO ii3=1,nr
            DO ii4=1-oly,oly+sny
              DO ii5=1-olx,olx+snx
                xx_genb(ii5, ii4, ii3, ii2, ii1) = 0.D0
              ENDDO
            ENDDO
          ENDDO
        ENDDO
        ...
        DO bi=mybxhi(mythid),mybxlo(mythid),-1
          ...
          IF (branch .EQ. 0) THEN
            DO k=nr,1,-1
              DO j=sny,1,-1
                DO i=snx,1,-1
                  xx_genb(i, j, k, bi, bj) = xx_genb(i, j, k, bi, bj) + 
     +              mask3d(i, j, k, bi, bj)*fldb(i, j, k, bi, bj)
                  fldb(i, j, k, bi, bj) = 0.D0
                ENDDO
              ENDDO
            ENDDO
          ELSE
            ...
            DO k=nr,1,-1
              DO j=sny,1,-1
                DO i=snx,1,-1
                  CALL POPCONTROL2B(branch)
                  IF (branch .EQ. 0) THEN
                    xx_genb(i, j, k, bi, bj) = 0.D0
                  ELSE
                    IF (branch .NE. 1) THEN
                      CALL POPREAL8(xx_gen(i, j, k, bi, bj))
                      xx_genb(i, j, k, bi, bj) = ln10*EXP(ln10*xx_gen(i
     +                  , j, k, bi, bj))*xx_genb(i, j, k, bi, bj)
                    END IF
                    xx_genb(i, j, k, bi, bj) = xx_genb(i, j, k, bi, bj)/
     +                SQRT(wgenarr3d(i, j, k, bi, bj, iarr))
                  END IF
        ...
        ENDDO
      ENDDO
      CALL ACTIVE_READ_XYZ_B(fnamegenin, xx_gen, xx_genb, 1, 
     +                       doglobalread, ladinit, optimcycle, mythid, 
     +                       xx_genarr3d_dummy(iarr), xx_genarr3d_dummyb
     +                       (iarr))
      DO k2=maxctrlproc,1,-1
        CALL POPCONTROL1B(branch)
      ENDDO
