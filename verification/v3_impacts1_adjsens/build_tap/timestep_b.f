...
C
C  Differentiation of timestep in reverse (adjoint) mode:
C   gradient     of useful results: uvel vvel gu gv gunm1 gvnm1
C                /surface_forcing/[0,39200[ /surface_forcing/[39200,78400[
C                gudissip dphihydx dphihydy phisurfx phisurfy gvdissip
C   with respect to varying inputs: uvel vvel gu gv gunm1 gvnm1
C                /surface_forcing/[0,39200[ /surface_forcing/[39200,78400[
C                gudissip dphihydx dphihydy phisurfx phisurfy gvdissip
C
C
...
C     !ROUTINE: TIMESTEP
C     !INTERFACE:
      SUBROUTINE TIMESTEP_B(bi, bj, imin, imax, jmin, jmax, k, dphihydx
     +                      , dphihydxb, dphihydy, dphihydyb, phisurfx, 
     +                      phisurfxb, phisurfy, phisurfyb, gudissip, 
     +                      gudissipb, gvdissip, gvdissipb, mytime, 
     +                      myiter, mythid)
      IMPLICIT NONE
C     == Global variables ==
CBOP
...
C     gv_AB    :: tendency increment from Adams-Bashforth, v component
      ...
      REAL*8 guextb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gvext(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gvextb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gutmp(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gutmpb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gvtmp(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gvtmpb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gu_ab(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gu_abb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gv_ab(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gv_abb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gudpx(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gudpxb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gvdpy(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gvdpyb(1-olx:snx+olx, 1-oly:sny+oly)
...
C-- Explicit (remaining) part of the Hyd pressure gradient:
      ...
      ELSE IF (implicsurfpress .NE. onerl) THEN
      ...
      DO j=jmax,jmin,-1
        DO i=imax,imin,-1
          vvelb(i, j, k, bi, bj) = vvelb(i, j, k, bi, bj) + gvb(i, j, k
     +      , bi, bj)
          tempb = deltatmom*masks(i, j, k, bi, bj)*gvb(i, j, k, bi, bj)
          gvb(i, j, k, bi, bj) = 0.D0
          gvtmpb(i, j) = gvtmpb(i, j) + tempb
          gvdpyb(i, j) = gvdpyb(i, j) + tempb
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
      ...
      DO j=jmax,jmin,-1
        DO i=imax,imin,-1
          uvelb(i, j, k, bi, bj) = uvelb(i, j, k, bi, bj) + gub(i, j, k
     +      , bi, bj)
          tempb = deltatmom*maskw(i, j, k, bi, bj)*gub(i, j, k, bi, bj)
          gub(i, j, k, bi, bj) = 0.D0
          gutmpb(i, j) = gutmpb(i, j) + tempb
          gudpxb(i, j) = gudpxb(i, j) + tempb
        ENDDO
      ENDDO
      ...
      IF (branch .NE. 0) THEN
        IF (branch .EQ. 1) THEN
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              phisurfyb(i, j) = phisurfyb(i, j) - psfac*gvdpyb(i, j)
              gvdpyb(i, j) = 0.D0
              phisurfxb(i, j) = phisurfxb(i, j) - psfac*gudpxb(i, j)
              gudpxb(i, j) = 0.D0
            ENDDO
          ENDDO
        ELSE
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              dphihydyb(i, j) = dphihydyb(i, j) - phfac*gvdpyb(i, j)
              phisurfyb(i, j) = phisurfyb(i, j) - psfac*gvdpyb(i, j)
              gvdpyb(i, j) = 0.D0
              dphihydxb(i, j) = dphihydxb(i, j) - phfac*gudpxb(i, j)
              phisurfxb(i, j) = phisurfxb(i, j) - psfac*gudpxb(i, j)
              gudpxb(i, j) = 0.D0
            ENDDO
          ENDDO
      ...
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            gvdissipb(i, j) = gvdissipb(i, j) + gvtmpb(i, j)
            gudissipb(i, j) = gudissipb(i, j) + gutmpb(i, j)
          ENDDO
        ENDDO
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        ...
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            gvextb(i, j) = gvextb(i, j) + gvtmpb(i, j)
            guextb(i, j) = guextb(i, j) + gutmpb(i, j)
          ENDDO
        ENDDO
      ELSE
        DO ii1=1-oly,oly+sny
      ...
      DO j=jmax,jmin,-1
        DO i=imax,imin,-1
          gvb(i, j, k, bi, bj) = gvb(i, j, k, bi, bj) + gvtmpb(i, j)
          gvtmpb(i, j) = 0.D0
          gub(i, j, k, bi, bj) = gub(i, j, k, bi, bj) + gutmpb(i, j)
          gutmpb(i, j) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        ...
        ENDDO
      ENDDO
      CALL ADAMS_BASHFORTH2_B(bi, bj, k, nr, gv(1-olx, 1-oly, 1, bi, bj)
     +                        , gvb(1-olx, 1-oly, 1, bi, bj), gvnm1(1-
     +                        olx, 1-oly, 1, bi, bj), gvnm1b(1-olx, 1-
     +                        oly, 1, bi, bj), gv_ab, gv_abb, 
     +                        mom_startab, myiter, mythid)
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          gu_abb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      CALL ADAMS_BASHFORTH2_B(bi, bj, k, nr, gu(1-olx, 1-oly, 1, bi, bj)
     +                        , gub(1-olx, 1-oly, 1, bi, bj), gunm1(1-
     +                        olx, 1-oly, 1, bi, bj), gunm1b(1-olx, 1-
     +                        oly, 1, bi, bj), gu_ab, gu_abb, 
     +                        mom_startab, myiter, mythid)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            gvextb(i, j) = gvextb(i, j) + gvb(i, j, k, bi, bj)
            guextb(i, j) = guextb(i, j) + gub(i, j, k, bi, bj)
          ENDDO
        ENDDO
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            gvdissipb(i, j) = gvdissipb(i, j) + gvb(i, j, k, bi, bj)
            gudissipb(i, j) = gudissipb(i, j) + gub(i, j, k, bi, bj)
          ENDDO
        ENDDO
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            dphihydyb(i, j) = dphihydyb(i, j) - phfac*gvb(i, j, k, bi, 
     +        bj)
            dphihydxb(i, j) = dphihydxb(i, j) - phfac*gub(i, j, k, bi, 
     +        bj)
          ENDDO
        ENDDO
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL APPLY_FORCING_V_B(gvext, gvextb, imin, imax, jmin, jmax, k
     +                         , bi, bj, mytime, myiter, mythid)
        CALL APPLY_FORCING_U_B(guext, guextb, imin, imax, jmin, jmax, k
     +                         , bi, bj, mytime, myiter, mythid)
      END IF
      END
