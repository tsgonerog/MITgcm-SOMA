...
C
C  Differentiation of gad_dst3fl_adv_y in reverse (adjoint) mode:
C   gradient     of useful results: vfld tracer vt vtrans
C   with respect to varying inputs: vfld tracer vt vtrans
C
C
C
C
...
C
C
      SUBROUTINE GAD_DST3FL_ADV_Y_B(bi, bj, k, calccfl, deltatloc, 
     +                              vtrans, vtransb, vfld, vfldb, 
     +                              masklocs, tracer, tracerb, vt, vtb, 
     +                              mythid)
      IMPLICIT NONE
C
C     == GLobal variables ==
...
C     == Local variables ==
      ...
      REAL*8 rjmb, rjb, rjpb, vcflb, d0b, d1b, psipb, psimb, thetapb, 
     +       thetamb
      REAL*8 thetamax
...
C
          ...
          vcfl = vfld(i, j)
          IF (calccfl) THEN
            x1 = vfld(i, j)*deltatloc*recip_dyc(i, j, bi, bj)*
     +        recip_deepfacc(k)
            IF (x1 .GE. 0.) THEN
              vcfl = x1
...
C
          ...
          ELSE
            ...
            CALL PUSHCONTROL1B(1)
          END IF
          IF (x2 .GT. thetap*(1.d0-vcfl)/(vcfl+1.d-20)) THEN
            y1 = thetap*(1.d0-vcfl)/(vcfl+1.d-20)
            CALL PUSHCONTROL1B(0)
          ELSE
            y1 = x2
            CALL PUSHCONTROL1B(1)
          ...
          ELSE
            ...
            CALL PUSHCONTROL1B(1)
          END IF
          IF (x3 .GT. thetam*(1.d0-vcfl)/(vcfl+1.d-20)) THEN
            y2 = thetam*(1.d0-vcfl)/(vcfl+1.d-20)
            CALL PUSHCONTROL1B(0)
          ELSE
            y2 = x3
            CALL PUSHCONTROL1B(1)
      ...
      DO j=sny+oly-1,1-oly+2,-1
        DO i=snx+olx,1-olx,-1
          rj = (tracer(i, j)-tracer(i, j-1))*masklocs(i, j)
          tempb = (tracer(i, j-1)+psip*rj)*0.5*vtb(i, j)
          tempb0 = (vtrans(i, j)+abs2)*0.5*vtb(i, j)
          tempb1 = (tracer(i, j)-psim*rj)*0.5*vtb(i, j)
          tempb2 = (vtrans(i, j)-abs5)*0.5*vtb(i, j)
          vtb(i, j) = 0.D0
          tracerb(i, j) = tracerb(i, j) + tempb2
          psimb = -(rj*tempb2)
          rjb = psip*tempb0 - psim*tempb2
          vtransb(i, j) = vtransb(i, j) + tempb1 + tempb
          abs5b = -tempb1
          tracerb(i, j-1) = tracerb(i, j-1) + tempb0
          psipb = rj*tempb0
          abs2b = tempb
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL8(abs5)
            vtransb(i, j) = vtransb(i, j) + abs5b
          ELSE
            CALL POPREAL8(abs5)
            vtransb(i, j) = vtransb(i, j) - abs5b
          END IF
          ...
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            tempb = y2b/(vcfl+1.d-20)
            thetamb = (1.d0-vcfl)*tempb
            vcflb = -((thetam+thetam*(1.d0-vcfl)/(vcfl+1.d-20))*tempb)
            x3b = 0.D0
          ELSE
            x3b = y2b
            vcflb = 0.D0
          ...
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            tempb = y1b/(vcfl+1.d-20)
            thetapb = (1.d0-vcfl)*tempb
            vcflb = vcflb - (thetap+thetap*(1.d0-vcfl)/(vcfl+1.d-20))*
     +        tempb
            x2b = 0.D0
          ELSE
            x2b = y1b
          ...
          ELSE
            ...
            rjmb = 0.D0
          END IF
          vcflb = vcflb - 2*vcfl*onesixth*d1b - ((1.d0-vcfl)*onesixth+(
     +      2.d0-vcfl)*onesixth)*d0b
          ...
          IF (branch .NE. 0) THEN
            ...
            ELSE
              x1b = vcflb
            END IF
            vfldb(i, j) = vfldb(i, j) + deltatloc*recip_deepfacc(k)*
     +        recip_dyc(i, j, bi, bj)*x1b
            vcflb = 0.D0
          END IF
          CALL POPREAL8(vcfl)
          vfldb(i, j) = vfldb(i, j) + vcflb
          tempb = masklocs(i, j-1)*rjmb
          tracerb(i, j-1) = tracerb(i, j-1) + tempb
          tracerb(i, j-2) = tracerb(i, j-2) - tempb
          tempb = masklocs(i, j)*rjb
          tracerb(i, j) = tracerb(i, j) + tempb
          tracerb(i, j-1) = tracerb(i, j-1) - tempb
          tempb = masklocs(i, j+1)*rjpb
          tracerb(i, j+1) = tracerb(i, j+1) + tempb
          tracerb(i, j) = tracerb(i, j) - tempb
        ENDDO
      ENDDO
