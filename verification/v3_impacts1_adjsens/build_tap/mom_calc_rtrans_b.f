...
C
C  Differentiation of mom_calc_rtrans in reverse (adjoint) mode:
C   gradient     of useful results: wvel rtransu rtransv
C   with respect to varying inputs: wvel rtransu rtransv
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE MOM_CALC_RTRANS_B(k, bi, bj, rtransu, rtransub, rtransv
...
C
      ...
      ELSE
        DO j=sny+oly,1-oly+1,-1
          DO i=snx+olx,1-olx+1,-1
            tempb = deepfac2f(k)*0.5d0*rhofacf(k)*rtransvb(i, j)
            rtransvb(i, j) = 0.D0
            wvelb(i, j-1, k, bi, bj) = wvelb(i, j-1, k, bi, bj) + ra(i, 
     +        j-1, bi, bj)*tempb
            wvelb(i, j, k, bi, bj) = wvelb(i, j, k, bi, bj) + ra(i, j, 
     +        bi, bj)*tempb
            tempb = deepfac2f(k)*0.5d0*rhofacf(k)*rtransub(i, j)
            rtransub(i, j) = 0.D0
            wvelb(i-1, j, k, bi, bj) = wvelb(i-1, j, k, bi, bj) + ra(i-1
     +        , j, bi, bj)*tempb
            wvelb(i, j, k, bi, bj) = wvelb(i, j, k, bi, bj) + ra(i, j, 
     +        bi, bj)*tempb
          ENDDO
