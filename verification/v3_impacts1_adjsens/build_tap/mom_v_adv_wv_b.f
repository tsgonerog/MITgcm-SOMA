...
C
C  Differentiation of mom_v_adv_wv in reverse (adjoint) mode:
C   gradient     of useful results: vfld advectivefluxwv rtrans
C                wfld
C   with respect to varying inputs: vfld advectivefluxwv rtrans
C                wfld
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_V_ADV_WV_B(bi, bj, k, vfld, vfldb, wfld, wfldb, 
     +                          rtrans, rtransb, advectivefluxwv, 
     +                          advectivefluxwvb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C  advectiveFluxWV      :: advective flux
      ...
      REAL*8 advectivefluxwvb(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j
     ...
     +THEN
        DO j=sny+oly,1-oly+1,-1
          DO i=snx+olx,1-olx+1,-1
            rtransb(i, j) = rtransb(i, j) + vfld(i, j, k-1, bi, bj)*
     +        advectivefluxwvb(i, j)
            vfldb(i, j, k-1, bi, bj) = vfldb(i, j, k-1, bi, bj) + rtrans
     +        (i, j)*advectivefluxwvb(i, j)
            advectivefluxwvb(i, j) = 0.D0
          ENDDO
        ENDDO
...
C
        DO j=sny+oly,1-oly+1,-1
          DO i=snx+olx,1-olx+1,-1
            rtransb(i, j) = rtransb(i, j) + vfld(i, j, k, bi, bj)*
     +        advectivefluxwvb(i, j)
            vfldb(i, j, k, bi, bj) = vfldb(i, j, k, bi, bj) + rtrans(i, 
     +        j)*advectivefluxwvb(i, j)
            advectivefluxwvb(i, j) = 0.D0
          ENDDO
        ENDDO
      ELSE
C
        IF (select_rstar .EQ. 0 .AND. (.NOT.rigidlid)) THEN
          DO j=sny+oly,1-oly+1,-1
            DO i=snx+olx,1-olx+1,-1
              temp = maskc(i, j, k, bi, bj) - maskc(i, j, k-1, bi, bj)
              temp0 = ra(i, j, bi, bj)
              temp1 = maskc(i, j-1, k, bi, bj) - maskc(i, j-1, k-1, bi, 
     +          bj)
              temp2 = ra(i, j-1, bi, bj)
              tempb = deepfac2f(k)*0.25*rhofacf(k)*advectivefluxwvb(i, j
     +          )
              tempb0 = vfld(i, j, k, bi, bj)*tempb
              vfldb(i, j, k, bi, bj) = vfldb(i, j, k, bi, bj) + (temp*(
     +          temp0*wfld(i, j, k, bi, bj))+temp1*(temp2*wfld(i, j-1, k
     +          , bi, bj)))*tempb
              wfldb(i, j, k, bi, bj) = wfldb(i, j, k, bi, bj) + temp0*
     +          temp*tempb0
              wfldb(i, j-1, k, bi, bj) = wfldb(i, j-1, k, bi, bj) + 
     +          temp2*temp1*tempb0
            ENDDO
          ENDDO
        END IF
        DO j=sny+oly,1-oly+1,-1
          DO i=snx+olx,1-olx+1,-1
            rtransb(i, j) = rtransb(i, j) + (vfld(i, j, k, bi, bj)+vfld(
     +        i, j, k-1, bi, bj))*0.5d0*advectivefluxwvb(i, j)
            tempb = rtrans(i, j)*0.5d0*advectivefluxwvb(i, j)
            advectivefluxwvb(i, j) = 0.D0
            vfldb(i, j, k, bi, bj) = vfldb(i, j, k, bi, bj) + tempb
            vfldb(i, j, k-1, bi, bj) = vfldb(i, j, k-1, bi, bj) + tempb
          ENDDO
        ENDDO
      END IF
      END
