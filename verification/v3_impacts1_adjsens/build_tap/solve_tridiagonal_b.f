...
C
C  Differentiation of solve_tridiagonal in reverse (adjoint) mode:
C   gradient     of useful results: y3d
C   with respect to varying inputs: c3d b3d y3d a3d
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE SOLVE_TRIDIAGONAL_B(imin, imax, jmin, jmax, a3d, a3db, 
     +                               b3d, b3db, c3d, c3db, y3d, y3db, 
...
C     errCode :: > 0 if singular matrix
      ...
      REAL*8 y3db(1-olx:snx+olx, 1-oly:sny+oly, nr)
      INTEGER errcode
...
C     == Local variables ==
      ...
      REAL*8 y3d_m1b(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 c3d_prime(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 c3d_primeb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 y3d_prime(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 y3d_primeb(1-olx:snx+olx, 1-oly:sny+oly, nr)
CEOP
C
C
      REAL*8 tempb
          ...
          DO j=1-oly,sny+oly
            DO i=1-olx,snx+olx
              IF (b3d(i, j, 1) .NE. 0.d0) THEN
                ...
                CALL PUSHREAL8(c3d_prime(i, j, 1))
                c3d_prime(i, j, 1) = c3d(i, j, 1)*recvar
                CALL PUSHREAL8(y3d_prime(i, j, 1))
          ...
          DO j=1-oly,sny+oly
            DO i=1-olx,snx+olx
              ...
              IF (tmpvar .NE. 0.d0) THEN
                recvar = 1.d0/tmpvar
                CALL PUSHREAL8(c3d_prime(i, j, k))
                c3d_prime(i, j, k) = c3d(i, j, k)*recvar
                CALL PUSHREAL8(y3d_prime(i, j, k))
                y3d_prime(i, j, k) = (y3d_m1(i, j, k)-a3d(i, j, k)*
     +            y3d_prime(i, j, k-1))*recvar
                CALL PUSHCONTROL1B(1)
              ELSE
                CALL PUSHREAL8(c3d_prime(i, j, k))
                c3d_prime(i, j, k) = 0.d0
      ...
      DO k=nr,1,-1
        ...
        ELSE
          DO j=1-oly,sny+oly
            DO i=1-olx,snx+olx
              CALL PUSHREAL8(y3d(i, j, k))
              y3d(i, j, k) = y3d_prime(i, j, k) - c3d_prime(i, j, k)*y3d
     +          (i, j, k+1)
            ENDDO
          ENDDO
          CALL PUSHCONTROL1B(1)
        ...
        IF (branch .EQ. 0) THEN
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              CALL POPREAL8(y3d(i, j, k))
              y3d_primeb(i, j, k) = y3d_primeb(i, j, k) + y3db(i, j, k)
              y3db(i, j, k) = 0.D0
            ENDDO
          ENDDO
        ELSE
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              CALL POPREAL8(y3d(i, j, k))
              y3d_primeb(i, j, k) = y3d_primeb(i, j, k) + y3db(i, j, k)
              c3d_primeb(i, j, k) = c3d_primeb(i, j, k) - y3d(i, j, k+1)
     +          *y3db(i, j, k)
              y3db(i, j, k+1) = y3db(i, j, k+1) - c3d_prime(i, j, k)*
     +          y3db(i, j, k)
              y3db(i, j, k) = 0.D0
            ENDDO
          ENDDO
        ...
        IF (branch .EQ. 0) THEN
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREAL8(y3d_prime(i, j, 1))
                y3d_primeb(i, j, 1) = 0.D0
                CALL POPREAL8(c3d_prime(i, j, 1))
                c3d_primeb(i, j, 1) = 0.D0
              ELSE
                recvar = 1.d0/b3d(i, j, 1)
                CALL POPREAL8(y3d_prime(i, j, 1))
                y3d_m1b(i, j, 1) = y3d_m1b(i, j, 1) + recvar*y3d_primeb(
     +            i, j, 1)
                recvarb = y3d_m1(i, j, 1)*y3d_primeb(i, j, 1) + c3d(i, j
     +            , 1)*c3d_primeb(i, j, 1)
                y3d_primeb(i, j, 1) = 0.D0
                CALL POPREAL8(c3d_prime(i, j, 1))
                c3db(i, j, 1) = c3db(i, j, 1) + recvar*c3d_primeb(i, j, 
     +            1)
                c3d_primeb(i, j, 1) = 0.D0
                b3db(i, j, 1) = b3db(i, j, 1) - recvarb/b3d(i, j, 1)**2
              END IF
            ENDDO
          ENDDO
        ELSE
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREAL8(y3d_prime(i, j, k))
                y3d_primeb(i, j, k) = 0.D0
                CALL POPREAL8(c3d_prime(i, j, k))
                c3d_primeb(i, j, k) = 0.D0
                tmpvarb = 0.D0
              ELSE
                recvar = 1.d0/tmpvar
                CALL POPREAL8(y3d_prime(i, j, k))
                tempb = recvar*y3d_primeb(i, j, k)
                recvarb = (y3d_m1(i, j, k)-a3d(i, j, k)*y3d_prime(i, j, 
     +            k-1))*y3d_primeb(i, j, k) + c3d(i, j, k)*c3d_primeb(i
     +            , j, k)
                y3d_primeb(i, j, k) = 0.D0
                y3d_m1b(i, j, k) = y3d_m1b(i, j, k) + tempb
                a3db(i, j, k) = a3db(i, j, k) - y3d_prime(i, j, k-1)*
     +            tempb
                y3d_primeb(i, j, k-1) = y3d_primeb(i, j, k-1) - a3d(i, j
     +            , k)*tempb
                CALL POPREAL8(c3d_prime(i, j, k))
                c3db(i, j, k) = c3db(i, j, k) + recvar*c3d_primeb(i, j, 
     +            k)
                c3d_primeb(i, j, k) = 0.D0
                tmpvarb = -(recvarb/tmpvar**2)
              END IF
              CALL POPREAL8(tmpvar)
              b3db(i, j, k) = b3db(i, j, k) + tmpvarb
              a3db(i, j, k) = a3db(i, j, k) - c3d_prime(i, j, k-1)*
     +          tmpvarb
              c3d_primeb(i, j, k-1) = c3d_primeb(i, j, k-1) - a3d(i, j, 
     +          k)*tmpvarb
            ENDDO
          ENDDO
        END IF
      ...
      DO k=nr,1,-1
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            y3db(i, j, k) = y3db(i, j, k) + y3d_m1b(i, j, k)
            y3d_m1b(i, j, k) = 0.D0
          ENDDO
        ENDDO
      ENDDO
