...
C
C  Differentiation of forward_step in reverse (adjoint) mode:
C   gradient     of useful results: kwx kwy kwz kux kvy kuz kvz
C                gm_psix gm_psiy xx_gentim2d_dummy xx_gentim2d0
C                xx_gentim2d1 xx_gentim2d etan uvel vvel wvel theta
C                salt gu gv gunm1 gvnm1 gtnm1 gsnm1 etah rhoinsitu
C                totphihyd fu fv qnet qsw empmr saltflux sst sss
C                pload surfaceforcingu surfaceforcingv surfaceforcingt
C                surfaceforcings phi0surf detahdt pmepr
...
C     !INTERFACE:
      SUBROUTINE FORWARD_STEP_B(iloop, mytime, myiter, mythid)
      IMPLICIT NONE
C     == Global variables ==
CBOP
C    !ROUTINE: SIZE.h
...
C
     ...
     +, objf_t_misfit, objf_eflux
      COMMON /cost_objf_b/ objf_testb
C
      REAL*8 objf_atl(nsx, nsy)
      REAL*8 objf_test(nsx, nsy)
      REAL*8 objf_testb(nsx, nsy)
...
C     myIterBeg :: iteration number at beginning of time step
      ...
      EXTERNAL DIAGNOSTICS_SWITCH_ONOFF
...
C--   Switch on/off diagnostics for snap-shot output:
      ...
      CALL PUSHREAL8ARRAY(xx_gentim2d, (snx+2*olx)*(sny+2*oly)*nsx*nsy*
     +                    maxctrltim2d)
      CALL PUSHREAL8ARRAY(xx_gentim2d1, (snx+2*olx)*(sny+2*oly)*nsx*nsy*
     +                    maxctrltim2d)
      CALL PUSHREAL8ARRAY(xx_gentim2d0, (snx+2*olx)*(sny+2*oly)*nsx*nsy*
     +                    maxctrltim2d)
...
C
      ...
      CALL PUSHREAL8ARRAY(rhoinsitu, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy)
      CALL PUSHREAL8ARRAY(theta, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy)
      CALL PUSHREAL8ARRAY(gm_psiy, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy)
      CALL PUSHREAL8ARRAY(gm_psix, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy)
...
C--   Check if it has reached the end of simulation
      ...
      IF (usesigreg) modelend = modelend .OR. i_got_signal .GT. 0
C
C--   Do IO if needed.
C
C
...
C
      IF (usesigreg) THEN
        IF (modelend .AND. i_got_signal .GT. 0) STOP
      END IF
      CALL DO_FIELDS_BLOCKING_EXCHANGES_B(mythid)
      CALL POPREAL8ARRAY(theta, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy)
      CALL POPREAL8ARRAY(salt, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy)
      CALL TRACERS_CORRECTION_STEP_B(mytime, myiter, mythid)
      ...
      IF (branch .EQ. 0) THEN
        ...
        CALL THERMODYNAMICS_B(mytime, myiter, mythid)
        ...
        CALL POPREAL8ARRAY(theta, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy)
        CALL POPREAL8ARRAY(salt, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy)
        CALL DO_STAGGER_FIELDS_EXCHANGES_B(mytime, myiter, mythid)
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        ...
        CALL INTEGR_CONTINUITY_B(uvel, uvelb, vvel, vvelb, mytime, 
     +                           myiter, mythid)
      END IF
      CALL POPREAL8ARRAY(uvel, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy)
      CALL POPREAL8ARRAY(vvel, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy)
      ...
      CALL MOMENTUM_CORRECTION_STEP_B(mytime, myiter, mythid)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREAL8ARRAY(etan, (snx+2*olx)*(sny+2*oly)*nsx*nsy)
        CALL SOLVE_FOR_PRESSURE_B(mytime, myiter, mythid)
      END IF
      mytime = starttime + deltatclock*(iloop-1)
      myiter = niter0 + (iloop-1)
      CALL POPINTEGER4(mythid)
      ...
      CALL POPREAL8ARRAY(botdragu, (snx+2*olx)*(sny+2*oly)*nsx*nsy)
      CALL POPREAL8ARRAY(botdragv, (snx+2*olx)*(sny+2*oly)*nsx*nsy)
      CALL DYNAMICS_B(mytime, myiter, mythid)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREAL8ARRAY(uvel, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy)
        CALL POPREAL8ARRAY(vvel, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy)
      ...
      CALL POPREAL8ARRAY(theta, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy)
      CALL POPREAL8ARRAY(rhoinsitu, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy)
      CALL POPREAL8ARRAY(pmepr, (snx+2*olx)*(sny+2*oly)*nsx*nsy)
      CALL DO_OCEANIC_PHYS_B(mytime, myiter, mythid)
      CALL DO_ATMOSPHERIC_PHYS_B(mytime, myiter, mythid)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        ...
        CALL CTRL_MAP_FORCING_B(mytime, myiter, mythid)
      END IF
      CALL POPREAL8ARRAY(xx_gentim2d0, (snx+2*olx)*(sny+2*oly)*nsx*nsy*
     +                   maxctrltim2d)
      CALL POPREAL8ARRAY(xx_gentim2d1, (snx+2*olx)*(sny+2*oly)*nsx*nsy*
     +                   maxctrltim2d)
      CALL POPREAL8ARRAY(xx_gentim2d, (snx+2*olx)*(sny+2*oly)*nsx*nsy*
     +                   maxctrltim2d)
      CALL POPREAL8ARRAY(fu, (snx+2*olx)*(sny+2*oly)*nsx*nsy)
      CALL POPREAL8ARRAY(fv, (snx+2*olx)*(sny+2*oly)*nsx*nsy)
      CALL POPREAL8ARRAY(qnet, (snx+2*olx)*(sny+2*oly)*nsx*nsy)
      ...
      CALL LOAD_FIELDS_DRIVER_B(mytime, myiter, mythid)
      END
