...
C
C  Differentiation of calc_wsurf_tr in reverse (adjoint) mode:
C   gradient     of useful results: tsurfcor ssurfcor thetafld
C                saltfld wvelfld
C   with respect to varying inputs: thetafld saltfld wvelfld
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE CALC_WSURF_TR_B(thetafld, thetafldb, saltfld, saltfldb
     +                           , wvelfld, wvelfldb, mytime, myiter, 
     +                           mythid)
      IMPLICIT NONE
C     == Global variables
CBOP
...
C     wvelFld  :: vertical velocity field
      ...
      REAL*8 thetafldb(1-olx:snx+olx, 1-oly:sny+oly, nr, nsx, nsy)
      REAL*8 saltfld(1-olx:snx+olx, 1-oly:sny+oly, nr, nsx, nsy)
      REAL*8 saltfldb(1-olx:snx+olx, 1-oly:sny+oly, nr, nsx, nsy)
      REAL*8 wvelfld(1-olx:snx+olx, 1-oly:sny+oly, nr, nsx, nsy)
      REAL*8 wvelfldb(1-olx:snx+olx, 1-oly:sny+oly, nr, nsx, nsy)
C
C     !LOCAL VARIABLES:
C     Local variables
C     i,j,k,bi,bj  :: loop counter
      ...
      REAL*8 wt_meanb, ws_meanb
      REAL*8 wt_tile(nsx, nsy)
      REAL*8 wt_tileb(nsx, nsy)
      REAL*8 ws_tile(nsx, nsy)
      REAL*8 ws_tileb(nsx, nsy)
        ...
        DO bi=mybxlo(mythid),mybxhi(mythid)
          ...
          DO j=1,sny
            DO i=1,snx
              ks = ksurfc(i, j, bi, bj)
              IF (ks .LE. nr) THEN
                wt_tile(bi, bj) = wt_tile(bi, bj) + ra(i, j, bi, bj)*
     +            wvelfld(i, j, ks, bi, bj)*thetafld(i, j, ks, bi, bj)
                ws_tile(bi, bj) = ws_tile(bi, bj) + ra(i, j, bi, bj)*
     +            wvelfld(i, j, ks, bi, bj)*saltfld(i, j, ks, bi, bj)
...
C
      IF (globalarea .GT. 0.) THEN
        IF (mythid .EQ. 1) THEN
          ws_meanb = ssurfcorb/globalarea
          wt_meanb = tsurfcorb/globalarea
        ELSE
          wt_meanb = 0.D0
          ws_meanb = 0.D0
        END IF
      ...
      DO ii1=1,nsy
        DO ii2=1,nsx
          ws_tileb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      CALL GLOBAL_SUM_TILE_RL_B(ws_tile, ws_tileb, ws_mean, ws_meanb, 
     +                          mythid)
      DO ii1=1,nsy
        DO ii2=1,nsx
          wt_tileb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      CALL GLOBAL_SUM_TILE_RL_B(wt_tile, wt_tileb, wt_mean, wt_meanb, 
     +                          mythid)
        ...
        DO bi=mybxhi(mythid),mybxlo(mythid),-1
          DO j=sny,1,-1
            DO i=snx,1,-1
              ...
              IF (branch .NE. 0) THEN
                ks = ksurfc(i, j, bi, bj)
                tempb = ra(i, j, bi, bj)*ws_tileb(bi, bj)
                wvelfldb(i, j, ks, bi, bj) = wvelfldb(i, j, ks, bi, bj) 
     +            + saltfld(i, j, ks, bi, bj)*tempb
                saltfldb(i, j, ks, bi, bj) = saltfldb(i, j, ks, bi, bj) 
     +            + wvelfld(i, j, ks, bi, bj)*tempb
                tempb = ra(i, j, bi, bj)*wt_tileb(bi, bj)
                wvelfldb(i, j, ks, bi, bj) = wvelfldb(i, j, ks, bi, bj) 
     +            + thetafld(i, j, ks, bi, bj)*tempb
                thetafldb(i, j, ks, bi, bj) = thetafldb(i, j, ks, bi, bj
     +            ) + wvelfld(i, j, ks, bi, bj)*tempb
              END IF
            ENDDO
          ENDDO
