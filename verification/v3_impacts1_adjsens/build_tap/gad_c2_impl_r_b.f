...
C
C  Differentiation of gad_c2_impl_r in reverse (adjoint) mode:
...
C     !INTERFACE:
      SUBROUTINE GAD_C2_IMPL_R_B(bi, bj, k, imin, imax, jmin, jmax, 
     +                           deltatarg, rtrans, rtransb, recip_hfac
     +                           , a3d, a3db, b3d, b3db, c3d, c3db, 
     +                           mythid)
      IMPLICIT NONE
C
C     == Global variables ===
...
C--   process interior interface only:
      IF (k .GT. 1 .AND. k .LE. nr) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            rcenterb = deltatarg(k-1)*recip_drf(k-1)*recip_deepfac2c(k-1
     +        )*recip_hfac(i, j, k-1)*recip_rhofacc(k-1)*c3db(i, j, k-1)
     +        + deltatarg(k-1)*recip_drf(k-1)*recip_deepfac2c(k-1)*
     +        recip_hfac(i, j, k-1)*recip_rhofacc(k-1)*b3db(i, j, k-1) -
     +        deltatarg(k)*recip_drf(k)*recip_deepfac2c(k)*recip_hfac(i
     +        , j, k)*recip_rhofacc(k)*b3db(i, j, k) - deltatarg(k)*
     +        recip_drf(k)*recip_deepfac2c(k)*recip_hfac(i, j, k)*
     +        recip_rhofacc(k)*a3db(i, j, k)
