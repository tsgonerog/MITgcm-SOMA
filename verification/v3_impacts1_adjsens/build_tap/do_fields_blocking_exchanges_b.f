...
C
C  Differentiation of do_fields_blocking_exchanges in reverse (adjoint) mode:
C   gradient     of useful results: uvel vvel wvel theta salt totphihyd
C   with respect to varying inputs: uvel vvel wvel theta salt totphihyd
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE DO_FIELDS_BLOCKING_EXCHANGES_B(mythid)
      IMPLICIT NONE
C     == Global variables ===
CBOP
C    !ROUTINE: SIZE.h
...
C    & CALL EXCH_XY_RL (  etaN ,  myThid  )
        ...
        END IF
C
        IF (storephihyd4phys) CALL EXCH_XYZ_RL_B(totphihyd, totphihydb, 
     +                                           mythid)
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL EXCH_XYZ_RL_B(salt, saltb, mythid)
          CALL EXCH_XYZ_RL_B(theta, thetab, mythid)
        END IF
        CALL POPCONTROL2B(branch)
        IF (branch .EQ. 0) THEN
          CALL EXCH_XYZ_RL_B(wvel, wvelb, mythid)
        ELSE IF (branch .NE. 1) THEN
          GOTO 100
        END IF
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) CALL EXCH_UV_XYZ_RL_B(uvel, uvelb, vvel, 
     +                                           vvelb, .true., mythid)
      END IF
 100  CONTINUE
      END
