...
C
C  Differentiation of gmredi_rtransport in reverse (adjoint) mode:
C   gradient     of useful results: kwx kwy gm_psix gm_psiy df
C                tracer
C   with respect to varying inputs: kwx kwy gm_psix gm_psiy df
C                tracer
C
C
C
...
C     !INTERFACE:
      SUBROUTINE GMREDI_RTRANSPORT_B(tridentity, bi, bj, k, imin, imax, 
     +                               jmin, jmax, maskup, tracer, tracerb
     +                               , df, dfb, mythid)
      IMPLICIT NONE
C     == GLobal variables ==
CBOP
...
C     i, j    ::  Loop counters
      ...
      REAL*8 dtdxb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dtdy(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dtdyb(1-olx:snx+olx, 1-oly:sny+oly)
...
C-    Horizontal gradients interpolated to W points
        DO j=jmin,jmax
          DO i=imin,imax
            dtdx(i, j) = op5*(op5*recip_deepfacc(k)*(maskw(i+1, j, k, bi
     +        , bj)*recip_dxc(i+1, j, bi, bj)*(tracer(i+1, j, k)-tracer(
     +        i, j, k))+maskw(i, j, k, bi, bj)*recip_dxc(i, j, bi, bj)*(
     +        tracer(i, j, k)-tracer(i-1, j, k)))+op5*recip_deepfacc(k-1
     +        )*(maskw(i+1, j, k-1, bi, bj)*recip_dxc(i+1, j, bi, bj)*(
     +        tracer(i+1, j, k-1)-tracer(i, j, k-1))+maskw(i, j, k-1, bi
     +        , bj)*recip_dxc(i, j, bi, bj)*(tracer(i, j, k-1)-tracer(i-
     +        1, j, k-1))))
C
            dtdy(i, j) = op5*(op5*recip_deepfacc(k)*(masks(i, j+1, k, bi
     +        , bj)*recip_dyc(i, j+1, bi, bj)*(tracer(i, j+1, k)-tracer(
     +        i, j, k))+masks(i, j, k, bi, bj)*recip_dyc(i, j, bi, bj)*(
     +        tracer(i, j, k)-tracer(i, j-1, k)))+op5*recip_deepfacc(k-1
     +        )*(masks(i, j+1, k-1, bi, bj)*recip_dyc(i, j+1, bi, bj)*(
     +        tracer(i, j+1, k-1)-tracer(i, j, k-1))+masks(i, j, k-1, bi
     +        , bj)*recip_dyc(i, j, bi, bj)*(tracer(i, j, k-1)-tracer(i
     +        , j-1, k-1))))
          ENDDO
        ENDDO
C
        IF (gm_advform .AND. gm_advseparate .AND. (.NOT.gm_inmomasstress
     +      )) THEN
...
C      flipSign4LHCoord = -gravitySign
          DO j=jmin,jmax
            DO i=imin,imax
              rtrans(i, j) = -(gravitysign*(dyg(i+1, j, bi, bj)*gm_psix(
     +          i+1, j, k, bi, bj)-dyg(i, j, bi, bj)*gm_psix(i, j, k, bi
     +          , bj)+dxg(i, j+1, bi, bj)*gm_psiy(i, j+1, k, bi, bj)-dxg
     +          (i, j, bi, bj)*gm_psiy(i, j, k, bi, bj))*deepfacf(k))
            ENDDO
          ENDDO
          ...
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              tempb4 = maskinc(i, j, bi, bj)*op5*maskup(i, j)*dfb(i, j)
              rtransb(i, j) = rtransb(i, j) + (tracer(i, j, k)+tracer(i
     +          , j, k-1))*tempb4
              tempb3 = rtrans(i, j)*tempb4
              tracerb(i, j, k) = tracerb(i, j, k) + tempb3
              tracerb(i, j, k-1) = tracerb(i, j, k-1) + tempb3
            ENDDO
          ENDDO
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              tempb4 = -(gravitysign*deepfacf(k)*rtransb(i, j))
              rtransb(i, j) = 0.D0
              gm_psixb(i+1, j, k, bi, bj) = gm_psixb(i+1, j, k, bi, bj) 
     +          + dyg(i+1, j, bi, bj)*tempb4
              gm_psixb(i, j, k, bi, bj) = gm_psixb(i, j, k, bi, bj) - 
     +          dyg(i, j, bi, bj)*tempb4
              gm_psiyb(i, j+1, k, bi, bj) = gm_psiyb(i, j+1, k, bi, bj) 
     +          + dxg(i, j+1, bi, bj)*tempb4
              gm_psiyb(i, j, k, bi, bj) = gm_psiyb(i, j, k, bi, bj) - 
     +          dxg(i, j, bi, bj)*tempb4
            ENDDO
          ENDDO
        END IF
        DO ii1=1-oly,oly+sny
        ...
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            tempb4 = -(ra(i, j, bi, bj)*maskinc(i, j, bi, bj)*deepfac2f(
     +        k)*maskup(i, j)*dfb(i, j))
            kwxb(i, j, k, bi, bj) = kwxb(i, j, k, bi, bj) + dtdx(i, j)*
     +        tempb4
            dtdxb(i, j) = dtdxb(i, j) + kwx(i, j, k, bi, bj)*tempb4
            kwyb(i, j, k, bi, bj) = kwyb(i, j, k, bi, bj) + dtdy(i, j)*
     +        tempb4
            dtdyb(i, j) = dtdyb(i, j) + kwy(i, j, k, bi, bj)*tempb4
          ENDDO
        ENDDO
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            tempb4 = op5**2*recip_deepfacc(k)*dtdyb(i, j)
            tempb1 = op5**2*recip_deepfacc(k-1)*dtdyb(i, j)
            dtdyb(i, j) = 0.D0
            tempb0 = masks(i, j+1, k-1, bi, bj)*recip_dyc(i, j+1, bi, bj
     +        )*tempb1
            tempb = masks(i, j, k-1, bi, bj)*recip_dyc(i, j, bi, bj)*
     +        tempb1
            tracerb(i, j, k-1) = tracerb(i, j, k-1) + tempb - tempb0
            tracerb(i, j-1, k-1) = tracerb(i, j-1, k-1) - tempb
            tracerb(i, j+1, k-1) = tracerb(i, j+1, k-1) + tempb0
            tempb3 = masks(i, j+1, k, bi, bj)*recip_dyc(i, j+1, bi, bj)*
     +        tempb4
            tempb2 = masks(i, j, k, bi, bj)*recip_dyc(i, j, bi, bj)*
     +        tempb4
            tracerb(i, j-1, k) = tracerb(i, j-1, k) - tempb2
            tracerb(i, j+1, k) = tracerb(i, j+1, k) + tempb3
            tempb = op5**2*recip_deepfacc(k)*dtdxb(i, j)
            tempb0 = maskw(i+1, j, k, bi, bj)*recip_dxc(i+1, j, bi, bj)*
     +        tempb
            tempb1 = maskw(i, j, k, bi, bj)*recip_dxc(i, j, bi, bj)*
     +        tempb
            tracerb(i, j, k) = tracerb(i, j, k) + tempb2 + tempb1 - 
     +        tempb3 - tempb0
            tempb2 = op5**2*recip_deepfacc(k-1)*dtdxb(i, j)
            dtdxb(i, j) = 0.D0
            tempb3 = maskw(i+1, j, k-1, bi, bj)*recip_dxc(i+1, j, bi, bj
     +        )*tempb2
            tempb4 = maskw(i, j, k-1, bi, bj)*recip_dxc(i, j, bi, bj)*
     +        tempb2
            tracerb(i, j, k-1) = tracerb(i, j, k-1) + tempb4 - tempb3
            tracerb(i-1, j, k-1) = tracerb(i-1, j, k-1) - tempb4
            tracerb(i+1, j, k-1) = tracerb(i+1, j, k-1) + tempb3
            tracerb(i-1, j, k) = tracerb(i-1, j, k) - tempb1
            tracerb(i+1, j, k) = tracerb(i+1, j, k) + tempb0
          ENDDO
        ENDDO
      END IF
      END
