...
C
C  Differentiation of mom_v_botdrag_coeff in reverse (adjoint) mode:
C   gradient     of useful results: vfld ufld ke cdrag
C   with respect to varying inputs: vfld ufld ke cdrag
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_V_BOTDRAG_COEFF_B(bi, bj, k, inp_ke, ufld, ufldb, 
     +                                 vfld, vfldb, kapparv, ke, keb, 
...
C-    calculate locally velocity norm @ V.pt (local V & 4 U averaged)
        DO j=1-oly+1,sny+oly-1
          DO i=1-olx,snx+olx-1
            CALL PUSHREAL8(vsq)
            vsq = vfld(i, j)*vfld(i, j) + (ufld(i, j-1)*ufld(i, j-1)*
     +        hfacw(i, j-1, k, bi, bj)+ufld(i, j)*ufld(i, j)*hfacw(i, j
     +        , k, bi, bj)+(ufld(i+1, j-1)*ufld(i+1, j-1)*hfacw(i+1, j-1
     +        , k, bi, bj)+ufld(i+1, j)*ufld(i+1, j)*hfacw(i+1, j, k, bi
     +        , bj)))*recip_hfacs(i, j, k, bi, bj)*0.25d0
...
C-    same as above but using wet-point method to average 4 U
     ...
     +        hfacw(i+1, j-1, k, bi, bj)+hfacw(i+1, j, k, bi, bj))
            IF (vsq .GT. zerorl) THEN
              CALL PUSHREAL8(vsq)
              vsq = vfld(i, j)*vfld(i, j) + (ufld(i, j-1)*ufld(i, j-1)*
     +          hfacw(i, j-1, k, bi, bj)+ufld(i, j)*ufld(i, j)*hfacw(i, 
     +          j, k, bi, bj)+(ufld(i+1, j-1)*ufld(i+1, j-1)*hfacw(i+1, 
     +          j-1, k, bi, bj)+ufld(i+1, j)*ufld(i+1, j)*hfacw(i+1, j, 
     +          k, bi, bj)))/vsq
              CALL PUSHCONTROL1B(0)
            ELSE
      ...
      IF (k .EQ. kbottom) THEN
        DO j=sny+oly,1-oly+1,-1
          DO i=snx+olx,1-olx,-1
            cdragb(i, j) = masks(i, j, k, bi, bj)*cdragb(i, j)
          ENDDO
        ENDDO
      ELSE
        DO j=sny+oly,1-oly+1,-1
          DO i=snx+olx,1-olx,-1
            cdragb(i, j) = masks(i, j, k, bi, bj)*(oners-masks(i, j, 
        ...
        IF (branch .EQ. 0) THEN
          DO j=sny+oly-1,1-oly+1,-1
            DO i=snx+olx-1,1-olx,-1
              ...
              IF (branch .NE. 0) THEN
                IF (ke(i, j) + ke(i, j-1) .EQ. 0.0) THEN
                  tempb = 0.D0
                ELSE
                  tempb = bottomdragquadratic*dragfac*cdragb(i, j)/(2.0*
     +              SQRT(ke(i, j)+ke(i, j-1)))
                END IF
        ...
        ELSE
          DO j=sny+oly-1,1-oly+1,-1
            DO i=snx+olx-1,1-olx,-1
              ...
              IF (branch .EQ. 0) THEN
                vsqb = 0.D0
              ELSE IF (vsq .EQ. 0.0) THEN
                vsqb = 0.D0
              ELSE
                vsqb = bottomdragquadratic*dragfac*cdragb(i, j)/(2.0*
     +            SQRT(vsq))
              END IF
              CALL POPREAL8(vsq)
              vfldb(i, j) = vfldb(i, j) + 2*vfld(i, j)*vsqb
              tempb = recip_hfacs(i, j, k, bi, bj)*0.25d0*vsqb
              ufldb(i, j-1) = ufldb(i, j-1) + 2*ufld(i, j-1)*hfacw(i, j-
     +          1, k, bi, bj)*tempb
              ufldb(i, j) = ufldb(i, j) + 2*ufld(i, j)*hfacw(i, j, k, bi
     +          , bj)*tempb
              ufldb(i+1, j-1) = ufldb(i+1, j-1) + 2*ufld(i+1, j-1)*hfacw
        ...
        DO j=sny+oly-1,1-oly+1,-1
          DO i=snx+olx-1,1-olx,-1
            ...
            ELSE IF (vsq .EQ. 0.0) THEN
              vsqb = 0.D0
            ELSE
              vsqb = bottomdragquadratic*dragfac*cdragb(i, j)/(2.0*SQRT(
     +          vsq))
            END IF
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
