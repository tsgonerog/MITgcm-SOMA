...
C
C  Differentiation of mom_u_adv_wu in reverse (adjoint) mode:
C   gradient     of useful results: ufld advectivefluxwu rtrans
C                wfld
C   with respect to varying inputs: ufld advectivefluxwu rtrans
C                wfld
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_U_ADV_WU_B(bi, bj, k, ufld, ufldb, wfld, wfldb, 
     +                          rtrans, rtransb, advectivefluxwu, 
     +                          advectivefluxwub, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C  advectiveFluxWU      :: advective flux
      ...
      REAL*8 advectivefluxwub(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j
     ...
     +THEN
        DO j=sny+oly,1-oly+1,-1
          DO i=snx+olx,1-olx+1,-1
            rtransb(i, j) = rtransb(i, j) + ufld(i, j, k-1, bi, bj)*
     +        advectivefluxwub(i, j)
            ufldb(i, j, k-1, bi, bj) = ufldb(i, j, k-1, bi, bj) + rtrans
     +        (i, j)*advectivefluxwub(i, j)
            advectivefluxwub(i, j) = 0.D0
          ENDDO
        ENDDO
...
C
        DO j=sny+oly,1-oly+1,-1
          DO i=snx+olx,1-olx+1,-1
            rtransb(i, j) = rtransb(i, j) + ufld(i, j, k, bi, bj)*
     +        advectivefluxwub(i, j)
            ufldb(i, j, k, bi, bj) = ufldb(i, j, k, bi, bj) + rtrans(i, 
     +        j)*advectivefluxwub(i, j)
            advectivefluxwub(i, j) = 0.D0
          ENDDO
        ENDDO
      ELSE
        ...
        IF (select_rstar .EQ. 0 .AND. (.NOT.rigidlid)) THEN
          DO j=sny+oly,1-oly+1,-1
            DO i=snx+olx,1-olx+1,-1
              ...
              temp0 = ra(i, j, bi, bj)
              temp1 = maskc(i-1, j, k, bi, bj) - maskc(i-1, j, k-1, bi, 
     +          bj)
              temp2 = ra(i-1, j, bi, bj)
              tempb = deepfac2f(k)*0.25*rhofacf(k)*advectivefluxwub(i, j
     +          )
              tempb0 = ufld(i, j, k, bi, bj)*tempb
              ufldb(i, j, k, bi, bj) = ufldb(i, j, k, bi, bj) + (temp*(
     +          temp0*wfld(i, j, k, bi, bj))+temp1*(temp2*wfld(i-1, j, k
     +          , bi, bj)))*tempb
              wfldb(i, j, k, bi, bj) = wfldb(i, j, k, bi, bj) + temp0*
     +          temp*tempb0
              wfldb(i-1, j, k, bi, bj) = wfldb(i-1, j, k, bi, bj) + 
     +          temp2*temp1*tempb0
            ENDDO
          ENDDO
        END IF
        DO j=sny+oly,1-oly+1,-1
          DO i=snx+olx,1-olx+1,-1
            rtransb(i, j) = rtransb(i, j) + (ufld(i, j, k, bi, bj)+ufld(
     +        i, j, k-1, bi, bj))*0.5d0*advectivefluxwub(i, j)
            tempb = rtrans(i, j)*0.5d0*advectivefluxwub(i, j)
            advectivefluxwub(i, j) = 0.D0
            ufldb(i, j, k, bi, bj) = ufldb(i, j, k, bi, bj) + tempb
            ufldb(i, j, k-1, bi, bj) = ufldb(i, j, k-1, bi, bj) + tempb
          ENDDO
        ENDDO
      END IF
      END
