...
C
C  Differentiation of timestep_tracer in reverse (adjoint) mode:
C   gradient     of useful results: tracer gtracer
...
C     !INTERFACE:
      SUBROUTINE TIMESTEP_TRACER_B(bi, bj, deltatlev, tracer, tracerb, 
     +                             gtracer, gtracerb, mytime, myiter, 
     +                             mythid)
      IMPLICIT NONE
C     == Global variables ===
CBOP
      ...
      DO k=nr,1,-1
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            tracerb(i, j, k) = tracerb(i, j, k) + gtracerb(i, j, k)
            gtracerb(i, j, k) = deltatlev(k)*gtracerb(i, j, k)
          ENDDO
        ENDDO
      ENDDO
      END
