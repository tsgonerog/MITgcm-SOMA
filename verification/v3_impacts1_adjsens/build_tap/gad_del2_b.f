...
C
C  Differentiation of gad_del2 in reverse (adjoint) mode:
C   gradient     of useful results: del2 dtdx dtdy
C   with respect to varying inputs: dtdx dtdy
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_DEL2_B(bi, bj, k, dtdx, dtdxb, dtdy, dtdyb, del2, 
     +                      del2b, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
...
C  del2                 :: Laplacian of tracer
      ...
      REAL*8 del2b(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j
      ...
      DO j=sny+oly-1,1-oly,-1
        DO i=snx+olx-1,1-olx,-1
          tempb = recip_ra(i, j, bi, bj)*recip_deepfac2c(k)*recip_drf(k)
     +      *recip_hfacc(i, j, k, bi, bj)*del2b(i, j)
          del2b(i, j) = 0.D0
          dtdxb(i+1, j) = dtdxb(i+1, j) + tempb
          dtdxb(i, j) = dtdxb(i, j) - tempb
          dtdyb(i, j+1) = dtdyb(i, j+1) + tempb
          dtdyb(i, j) = dtdyb(i, j) - tempb
        ENDDO
      ENDDO
      END
