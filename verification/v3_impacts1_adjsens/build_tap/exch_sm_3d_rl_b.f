...
C
C  Differentiation of exch_sm_3d_rl in reverse (adjoint) mode:
...
C
C     !INTERFACE:
      SUBROUTINE EXCH_SM_3D_RL_B(phi, phib, withsigns, mynz, mythid)
      IMPLICIT NONE
C
CBOP
C    !ROUTINE: SIZE.h
...
C     == Argument list variables ==
      ...
      REAL*8 phib(1-olx:snx+olx, 1-oly:sny+oly, mynz, nsx, nsy)
      LOGICAL withsigns
      INTEGER mythid
C
C     !LOCAL VARIABLES:
            ...
            DO bi=mybxhi(mythid),mybxlo(mythid),-1
              DO k=mynz,1,-1
                ...
                IF (branch .EQ. 0) THEN
                  DO j=sny,1,-1
                    DO i=exchwidthx,1,-1
                      phib(1-i, j, k, bi, bj) = negone*phib(1-i, j, k, 
     +                  bi, bj)
                    ENDDO
                  ENDDO
                  DO j=exchwidthy,1,-1
                    DO i=snx,1,-1
                      phib(i, sny+j, k, bi, bj) = negone*phib(i, sny+j, 
     +                  k, bi, bj)
                    ENDDO
                  ENDDO
                ELSE
                  DO j=exchwidthy,1,-1
                    DO i=snx,1,-1
                      phib(i, 1-j, k, bi, bj) = negone*phib(i, 1-j, k, 
     +                  bi, bj)
                    ENDDO
                  ENDDO
                  DO j=sny,1,-1
                    DO i=exchwidthx,1,-1
                      phib(snx+i, j, k, bi, bj) = negone*phib(snx+i, j, 
     +                  k, bi, bj)
                    ENDDO
                  ENDDO
                END IF
