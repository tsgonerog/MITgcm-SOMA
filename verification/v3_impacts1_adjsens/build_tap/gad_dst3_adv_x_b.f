...
C
C  Differentiation of gad_dst3_adv_x in reverse (adjoint) mode:
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_DST3_ADV_X_B(bi, bj, k, calccfl, deltatloc, utrans
     +                            , utransb, ufld, ufldb, masklocw, 
     +                            tracer, tracerb, ut, utb, mythid)
      IMPLICIT NONE
C
C     == GLobal variables ==
CBOP
...
C  uCFL              :: Courant-Friedrich-Levy number
      ...
      REAL*8 rjmb, rjb, rjpb, ucflb, d0b, d1b
      INTRINSIC ABS
      REAL*8 x1
      REAL*8 x1b
      REAL*8 abs0
     ...
     +        recip_deepfacc(k)
      ...
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx-1,1-olx+2,-1
          ...
          tempb = (tracer(i-1, j)+d0*rj+d1*rjm)*0.5*utb(i, j)
          tempb0 = (utrans(i, j)+abs0)*0.5*utb(i, j)
          tempb1 = (tracer(i, j)-d0*rj-d1*rjp)*0.5*utb(i, j)
          tempb2 = (utrans(i, j)-abs1)*0.5*utb(i, j)
          utb(i, j) = 0.D0
          tracerb(i, j) = tracerb(i, j) + tempb2
          d0b = rj*tempb0 - rj*tempb2
          rjb = d0*tempb0 - d0*tempb2
          d1b = rjm*tempb0 - rjp*tempb2
          rjpb = -(d1*tempb2)
          ...
          IF (branch .EQ. 0) THEN
            CALL POPREAL8(abs0)
            utransb(i, j) = utransb(i, j) - abs0b
          ELSE
            CALL POPREAL8(abs0)
            utransb(i, j) = utransb(i, j) + abs0b
          END IF
          ucflb = -(2*ucfl*onesixth*d1b) - ((1.-ucfl)*onesixth+(2.-ucfl)
     +      *onesixth)*d0b
