...
C
C  Differentiation of mom_v_metric_sphere in reverse (adjoint) mode:
C   gradient     of useful results: vmetricterms ufld
C   with respect to varying inputs: vmetricterms ufld
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_V_METRIC_SPHERE_B(bi, bj, k, ufld, ufldb, 
     +                                 vmetricterms, vmetrictermsb, 
     +                                 mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
...
C  i,j                  :: loop indices
      ...
      DO j=sny+oly,1-olx+1,-1
        DO i=snx+olx-1,1-olx,-1
          tempb = -(2*(ufld(i, j)+ufld(i+1, j)+ufld(i, j-1)+ufld(i+1, j-
     +      1))*recip_rsphere*recip_deepfacc(k)*0.25**2*tanphiatv(i, j, 
     +      bi, bj)*vmetrictermsb(i, j))
          vmetrictermsb(i, j) = 0.D0
          ufldb(i, j) = ufldb(i, j) + tempb
          ufldb(i+1, j) = ufldb(i+1, j) + tempb
          ufldb(i, j-1) = ufldb(i, j-1) + tempb
          ufldb(i+1, j-1) = ufldb(i+1, j-1) + tempb
        ENDDO
      ENDDO
      END
