...
C
C  Differentiation of gad_fluxlimit_impl_r in reverse (adjoint) mode:
C   gradient     of useful results: c3d tfld b3d rtrans a3d
C   with respect to varying inputs: c3d tfld b3d rtrans a3d
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE GAD_FLUXLIMIT_IMPL_R_B(bi, bj, k, imin, imax, jmin, 
     +                                  jmax, deltatarg, rtrans, rtransb
     +                                  , recip_hfac, tfld, tfldb, a3d, 
     +                                  a3db, b3d, b3db, c3d, c3db, 
     +                                  mythid)
      IMPLICIT NONE
...
C     myThid       :: thread number
      ...
      REAL*8 tfldb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 a3d(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 a3db(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 b3d(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 b3db(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 c3d(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 c3db(1-olx:snx+olx, 1-oly:sny+oly, nr)
      INTEGER mythid
C
C     == Local Variables ==
C     i, j         :: loop indices
...
C     rUpwind      :: upwind   contribution
      ...
      REAL*8 crb, rjmb, rjb, rjpb, w_cflb
      REAL*8 upwindfac(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 upwindfacb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 rcenter, rupwind
      REAL*8 rcenterb, rupwindb
      REAL*8 deltatcfl
      REAL*8 crmax
...
C--   Compute the upwind fraction:
        ...
        DO j=jmin,jmax
          DO i=imin,imax
            w_cfl = deltatcfl*rtrans(i, j)*recip_ra(i, j, bi, bj)*
     +        recip_drc(k)*recip_deepfac2f(k)*recip_rhofacf(k)
            rjp = (tfld(i, j, kp1)-tfld(i, j, k))*maskc(i, j, kp1, bi, 
     +        bj)
            rj = tfld(i, j, k) - tfld(i, j, km1)
            rjm = (tfld(i, j, km1)-tfld(i, j, km2))*maskc(i, j, km2, bi
     +        , bj)
C
...
C        calculate upwind fraction using Limiter Function:
            upwindfac(i, j) = 1.d0 - max1*(1.d0+abs3)
            IF (-1.d0 .LT. upwindfac(i, j)) THEN
              CALL PUSHCONTROL1B(0)
              upwindfac(i, j) = upwindfac(i, j)
            ELSE
              upwindfac(i, j) = -1.d0
              CALL PUSHCONTROL1B(1)
            END IF
...
C--    Add centered & upwind contributions
        ...
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            upwindfacb(ii2, ii1) = 0.D0
          ENDDO
        ENDDO
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            tempb = deltatarg(k-1)*recip_drf(k-1)*recip_deepfac2c(k-1)*
     +        recip_hfac(i, j, k-1)*recip_rhofacc(k-1)*c3db(i, j, k-1)
            rcenterb = tempb
            rupwindb = -tempb
            tempb = deltatarg(k-1)*recip_drf(k-1)*recip_deepfac2c(k-1)*
     +        recip_hfac(i, j, k-1)*recip_rhofacc(k-1)*b3db(i, j, k-1)
            rcenterb = rcenterb + tempb
            rupwindb = rupwindb + tempb
            tempb = -(recip_hfac(i, j, k)*deltatarg(k)*recip_drf(k)*
     +        recip_deepfac2c(k)*recip_rhofacc(k)*b3db(i, j, k))
            rcenterb = rcenterb + tempb
            rupwindb = rupwindb - tempb
            tempb = -(recip_hfac(i, j, k)*deltatarg(k)*recip_drf(k)*
     +        recip_deepfac2c(k)*recip_rhofacc(k)*a3db(i, j, k))
            rcenterb = rcenterb + tempb
            rupwindb = rupwindb + tempb
            abs1b = upwindfac(i, j)*rupwindb
            upwindfacb(i, j) = upwindfacb(i, j) + abs1*rupwindb
     ...
     +        bi, bj)*rcenterb
        ...
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            CALL POPCONTROL1B(branch)
            IF (branch .NE. 0) upwindfacb(i, j) = 0.D0
            max1b = -((abs3+1.d0)*upwindfacb(i, j))
            abs3b = -(max1*upwindfacb(i, j))
            upwindfacb(i, j) = 0.D0
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREAL8(abs3)
            ...
            END IF
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) crb = crb + 2.d0*x1b
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              rj = tfld(i, j, k) - tfld(i, j, km1)
              CALL POPREAL8(cr)
            ...
            ELSE
              ...
              rjpb = 0.D0
            END IF
            tempb = maskc(i, j, km2, bi, bj)*rjmb
            tfldb(i, j, km1) = tfldb(i, j, km1) + tempb
            tfldb(i, j, km2) = tfldb(i, j, km2) - tempb
            tfldb(i, j, k) = tfldb(i, j, k) + rjb
            tfldb(i, j, km1) = tfldb(i, j, km1) - rjb
            tempb = maskc(i, j, kp1, bi, bj)*rjpb
            tfldb(i, j, kp1) = tfldb(i, j, kp1) + tempb
            tfldb(i, j, k) = tfldb(i, j, k) - tempb
            rtransb(i, j) = rtransb(i, j) + deltatcfl*recip_drc(k)*
     +        recip_ra(i, j, bi, bj)*recip_deepfac2f(k)*recip_rhofacf(k)
     +        *w_cflb
          ENDDO
        ENDDO
      END IF
