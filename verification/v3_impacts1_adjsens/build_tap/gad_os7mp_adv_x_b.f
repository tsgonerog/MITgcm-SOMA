...
C
C  Differentiation of gad_os7mp_adv_x in reverse (adjoint) mode:
C   gradient     of useful results: q ufld utrans ut
C   with respect to varying inputs: q ufld utrans ut
C
C
C
C
...
C
C
      SUBROUTINE GAD_OS7MP_ADV_X_B(bi, bj, k, calccfl, deltatloc, utrans
     +                             , utransb, ufld, ufldb, masklocw, q, 
     +                             qb, ut, utb, mythid)
      IMPLICIT NONE
C
C     == GLobal variables ==
...
C     == Local variables ==
      ...
      REAL*8 ulocb, facb, delipb, delib, phib, rp1hb, rp1h_cflb
...
C
          ...
          cfl = uloc
          IF (calccfl) THEN
            x1 = uloc*deltatloc*recip_dxc(i, j, bi, bj)
            IF (x1 .GE. 0.) THEN
              cfl = x1
      ...
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx-3,1-olx+4,-1
          ...
          IF (branch .EQ. 0) THEN
            ...
            utransb(i, j) = utransb(i, j) + (qi+psi*delip)*utb(i, j)
            tempb = utrans(i, j)*utb(i, j)
            utb(i, j) = 0.D0
            qib = tempb
            psib = delip*tempb
            delipb = psi*tempb
          ...
          IF (branch .EQ. 0) THEN
            ...
            CALL POPREAL8(mski)
            CALL POPREAL8(mskip)
            qb(i-4, j) = qb(i-4, j) + qimmmb
            qb(i-3, j) = qb(i-3, j) + qimmb
            qb(i-2, j) = qb(i-2, j) + qimb
            CALL POPREAL8(qi)
            qb(i-1, j) = qb(i-1, j) + qib
            qb(i, j) = qb(i, j) + qipb
            qb(i+1, j) = qb(i+1, j) + qippb
          ELSE IF (branch .EQ. 1) THEN
            ...
            CALL POPREAL8(mski)
            CALL POPREAL8(mskip)
            qb(i+3, j) = qb(i+3, j) + qimmmb
            qb(i+2, j) = qb(i+2, j) + qimmb
            qb(i+1, j) = qb(i+1, j) + qimb
            CALL POPREAL8(qi)
            qb(i, j) = qb(i, j) + qib
            qb(i-1, j) = qb(i-1, j) + qipb
            qb(i-2, j) = qb(i-2, j) + qippb
          ELSE
            CALL POPREAL8(mskimmm)
            CALL POPREAL8(mskimm)
            CALL POPREAL8(mskim)
          ...
          ELSE
            ...
            ELSE
              x1b = cflb
            END IF
            ulocb = deltatloc*recip_dxc(i, j, bi, bj)*x1b
            cflb = 0.D0
          END IF
          CALL POPREAL8(cfl)
          ulocb = ulocb + cflb
          ufldb(i, j) = ufldb(i, j) + ulocb
        ENDDO
      ENDDO
      DO j=sny+oly,1-oly,-1
        utb(snx+olx, j) = 0.D0
        utb(snx+olx-1, j) = 0.D0
        utb(snx+olx-2, j) = 0.D0
        utb(4-olx, j) = 0.D0
        utb(3-olx, j) = 0.D0
        utb(2-olx, j) = 0.D0
        utb(1-olx, j) = 0.D0
