...
C
C  Differentiation of external_forcing_surf in reverse (adjoint) mode:
C   gradient     of useful results: theta salt fu fv qnet qsw empmr
C                saltflux sst sss pload surfaceforcingu surfaceforcingv
C                surfaceforcingt surfaceforcings phi0surf pmepr
C   with respect to varying inputs: theta salt fu fv qnet qsw empmr
C                saltflux sst sss pload surfaceforcingu surfaceforcingv
C                surfaceforcingt surfaceforcings phi0surf pmepr
C
C
...
C     !INTERFACE:
      SUBROUTINE EXTERNAL_FORCING_SURF_B(imin, imax, jmin, jmax, mytime
     +                                   , myiter, mythid)
      IMPLICIT NONE
C     === Global variables ===
CBOP
...
C
            ...
            END IF
          ELSE IF (convertfw2salt .EQ. -1.) THEN
C
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
C
...
C   (requires time varying dP(Nr) like dP(k-bottom) with NonLin FS).
      ...
      DO bj=mybyhi(mythid),mybylo(mythid),-1
        DO bi=mybxhi(mythid),mybxlo(mythid),-1
          ...
          IF (branch .EQ. 0) THEN
            DO j=jmax,jmin,-1
              DO i=imax,imin,-1
                ploadb(i, j, bi, bj) = ploadb(i, j, bi, bj) + 
     +            recip_rhoconst*phi0surfb(i, j, bi, bj)
                phi0surfb(i, j, bi, bj) = 0.D0
              ENDDO
            ENDDO
     ...
     +              salt_evprrn-salt(i, j, ks, bi, bj))*mass2runit*
     +              surfaceforcingsb(i, j, bi, bj)
                  saltb(i, j, ks, bi, bj) = saltb(i, j, ks, bi, bj) - 
     +              pmepr(i, j, bi, bj)*mass2runit*surfaceforcingsb(i, j
     +              , bi, bj)
                ENDDO
              ENDDO
            ELSE IF (branch .NE. 1) THEN
              DO j=jmax,jmin,-1
                DO i=imax,imin,-1
                  empmrb(i, j, bi, bj) = empmrb(i, j, bi, bj) + (salt(i
     +              , j, ks, bi, bj)-salt_evprrn)*mass2runit*
     +              surfaceforcingsb(i, j, bi, bj)
                  saltb(i, j, ks, bi, bj) = saltb(i, j, ks, bi, bj) + 
     +              empmr(i, j, bi, bj)*mass2runit*surfaceforcingsb(i, j
     +              , bi, bj)
                ENDDO
              ENDDO
              GOTO 100
            ...
            IF (branch .EQ. 0) THEN
              DO j=jmax,jmin,-1
                DO i=imax,imin,-1
                  pmeprb(i, j, bi, bj) = pmeprb(i, j, bi, bj) + (
     +              temp_evprrn-theta(i, j, ks, bi, bj))*mass2runit*
     +              surfaceforcingtb(i, j, bi, bj)
                  thetab(i, j, ks, bi, bj) = thetab(i, j, ks, bi, bj) - 
     +              pmepr(i, j, bi, bj)*mass2runit*surfaceforcingtb(i, j
     +              , bi, bj)
                ENDDO
              ENDDO
            END IF
          ...
          ELSE IF (branch .NE. 3) THEN
            IF (branch .EQ. 4) THEN
              DO j=jmax,jmin,-1
                DO i=imax,imin,-1
                  empmrb(i, j, bi, bj) = empmrb(i, j, bi, bj) + (
     +              convertfw2salt-salt_evprrn)*mass2runit*
     +              surfaceforcingsb(i, j, bi, bj)
                ENDDO
              ENDDO
            END IF
            ...
            IF (branch .EQ. 0) THEN
              DO j=jmax,jmin,-1
                DO i=imax,imin,-1
                  empmrb(i, j, bi, bj) = empmrb(i, j, bi, bj) + (tref(ks
     +              )-temp_evprrn)*mass2runit*surfaceforcingtb(i, j, bi
     +              , bj)
                ENDDO
              ENDDO
            END IF
 ...
 100      CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            DO j=jmax,jmin,-1
              DO i=imax,imin,-1
                empmrb(i, j, bi, bj) = empmrb(i, j, bi, bj) + (theta(i, 
     +            j, ks, bi, bj)-temp_evprrn)*mass2runit*
     +            surfaceforcingtb(i, j, bi, bj)
                thetab(i, j, ks, bi, bj) = thetab(i, j, ks, bi, bj) + 
     +            empmr(i, j, bi, bj)*mass2runit*surfaceforcingtb(i, j, 
     +            bi, bj)
              ENDDO
            ENDDO
          END IF
 110      DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              saltfluxb(i, j, bi, bj) = saltfluxb(i, j, bi, bj) - 
     +          mass2runit*surfaceforcingsb(i, j, bi, bj)
              tempb = -(recip_cp*mass2runit*surfaceforcingtb(i, j, bi, 
     +          bj))
              qnetb(i, j, bi, bj) = qnetb(i, j, bi, bj) + tempb
              qswb(i, j, bi, bj) = qswb(i, j, bi, bj) - tempb
              fvb(i, j, bi, bj) = fvb(i, j, bi, bj) + mass2runit*
     +          surfaceforcingvb(i, j, bi, bj)
              surfaceforcingvb(i, j, bi, bj) = 0.D0
              fub(i, j, bi, bj) = fub(i, j, bi, bj) + mass2runit*
     +          surfaceforcingub(i, j, bi, bj)
              surfaceforcingub(i, j, bi, bj) = 0.D0
            ENDDO
          ENDDO
        ENDDO
      ENDDO
      CALL POPCONTROL1B(branch)
      IF (branch .NE. 0) CALL FORCING_SURF_RELAX_B(imin, imax, jmin, 
     +                                             jmax, mytime, myiter
     +                                             , mythid)
      DO bj=mybyhi(mythid),mybylo(mythid),-1
        DO bi=mybxhi(mythid),mybxlo(mythid),-1
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              surfaceforcingsb(i, j, bi, bj) = 0.D0
              surfaceforcingtb(i, j, bi, bj) = 0.D0
            ENDDO
          ENDDO
        ENDDO
      ENDDO
