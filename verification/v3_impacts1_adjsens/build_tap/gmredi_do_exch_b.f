...
C
C  Differentiation of gmredi_do_exch in reverse (adjoint) mode:
C   gradient     of useful results: gm_psix gm_psiy
C   with respect to varying inputs: gm_psix gm_psiy
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE GMREDI_DO_EXCH_B(mytime, myiter, mythid)
      IMPLICIT NONE
C
C     == Global variables ==
CBOP
...
C     with GM-bolus 2-D mapping factor (GM_bolFac2d).
      IF (usecubedsphereexchange .AND. gm_advform .AND. (.NOT.
     +    gm_advseparate) .AND. usemultidimadvec) CALL EXCH_UV_XYZ_RL_B(
     +                                                           gm_psix
     +                                                                , 
     +                                                          gm_psixb
     +                                                                , 
