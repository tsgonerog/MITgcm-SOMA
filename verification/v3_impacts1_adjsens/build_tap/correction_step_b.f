...
C
C  Differentiation of correction_step in reverse (adjoint) mode:
C   gradient     of useful results: uvel vvel gu gv phisurfx phisurfy
C   with respect to varying inputs: uvel vvel gu gv phisurfx phisurfy
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE CORRECTION_STEP_B(bi, bj, imin, imax, jmin, jmax, 
     +                             phisurfx, phisurfxb, phisurfy, 
     +                             phisurfyb, mytime, myiter, mythid)
      IMPLICIT NONE
C     == Global variables ==
CBOP
C    !ROUTINE: SIZE.h
...
C     gV_dpy       :: implicit part of pressure gradient tendency
      ...
      REAL*8 gu_dpxb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gv_dpy(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gv_dpyb(1-olx:snx+olx, 1-oly:sny+oly)
...
C     Pressure gradient tendency (merid mom): Implicit part
      ...
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          gu_dpxb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          gv_dpyb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO k=nr,1,-1
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            tempb = masks(i, j, k, bi, bj)*vvelb(i, j, k, bi, bj)
            vvelb(i, j, k, bi, bj) = 0.D0
            gvb(i, j, k, bi, bj) = gvb(i, j, k, bi, bj) + tempb
            gv_dpyb(i, j) = gv_dpyb(i, j) + deltatmom*tempb
          ENDDO
        ENDDO
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            tempb = maskw(i, j, k, bi, bj)*uvelb(i, j, k, bi, bj)
            uvelb(i, j, k, bi, bj) = 0.D0
            gub(i, j, k, bi, bj) = gub(i, j, k, bi, bj) + tempb
            gu_dpxb(i, j) = gu_dpxb(i, j) + deltatmom*tempb
          ENDDO
        ENDDO
     ...
     +    (k)
        ...
        IF (branch .EQ. 0) THEN
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              phisurfyb(i, j) = phisurfyb(i, j) - psfac*masks(i, j, k, 
     +          bi, bj)*gv_dpyb(i, j)
              gv_dpyb(i, j) = 0.D0
            ENDDO
          ENDDO
     ...
     +          bi, bj)*gv_dpyb(i, j)
        ...
        IF (branch .EQ. 0) THEN
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              phisurfxb(i, j) = phisurfxb(i, j) - psfac*maskw(i, j, k, 
     +          bi, bj)*gu_dpxb(i, j)
              gu_dpxb(i, j) = 0.D0
            ENDDO
          ENDDO
