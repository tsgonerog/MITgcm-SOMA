...
C
C  Differentiation of convective_adjustment in reverse (adjoint) mode:
C   gradient     of useful results: theta salt
C   with respect to varying inputs: theta salt
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE CONVECTIVE_ADJUSTMENT_B(bi, bj, mytime, myiter, mythid)
      IMPLICIT NONE
C     == Global data ==
CBOP
C    !ROUTINE: SIZE.h
...
C--   Check to see if should convect now
      result1 = DIFFERENT_MULTIPLE(cadjfreq, mytime, deltatclock)
      IF (result1) THEN
C
...
C-          Density of k-1 layer (above W(k)) reference to k-1 T-level
          arg1 = k + deltak
          CALL FIND_RHO_2D(imin, imax, jmin, jmax, arg1, theta(1-olx, 1-
     +                     oly, k-1, bi, bj), salt(1-olx, 1-oly, k-1, bi
     +                     , bj), rhokm1, arg2, bi, bj, mythid)
C
C-          Density of k layer (below W(k)) reference to k-1 T-level.
          arg1 = k + deltak
          CALL FIND_RHO_2D(imin, imax, jmin, jmax, arg1, theta(1-olx, 1-
     +                     oly, k, bi, bj), salt(1-olx, 1-oly, k, bi, bj
     +                     ), rhok, k, bi, bj, mythid)
C
C-          Pre-calculate mixing weights for interface k
          CALL PUSHREAL8ARRAY(weightb, 70**2)
          CALL PUSHREAL8ARRAY(weighta, 70**2)
...
C--       End DO k=1,Nr
        ENDDO
        DO k=kbottom-MOD(kbottom-ktop, kdir),ktop,-kdir
          CALL POPREAL8ARRAY(salt, 70**2*31)
          CALL CONVECTIVELY_MIXTRACER_B(bi, bj, k, weighta, weightb, 
     +                                  salt, saltb, mythid)
          CALL POPREAL8ARRAY(theta, 70**2*31)
          CALL CONVECTIVELY_MIXTRACER_B(bi, bj, k, weighta, weightb, 
     +                                  theta, thetab, mythid)
          CALL POPREAL8ARRAY(weighta, 70**2)
          CALL POPREAL8ARRAY(weightb, 70**2)
        ENDDO
      END IF
      END
