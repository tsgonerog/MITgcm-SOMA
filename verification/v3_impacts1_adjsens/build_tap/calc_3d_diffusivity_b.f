...
C
C  Differentiation of calc_3d_diffusivity in reverse (adjoint) mode:
C   gradient     of useful results: /gm_wtensor/[2430400,3645600[
C                kappartr
C   with respect to varying inputs: /gm_wtensor/[2430400,3645600[
C                kappartr
C
C
C
...
C     !INTERFACE:
      SUBROUTINE CALC_3D_DIFFUSIVITY_B(bi, bj, imin, imax, jmin, jmax, 
     +                                 tridentity, trusegmredi, trusekpp
     +                                 , kappartr, kappartrb, mythid)
      IMPLICIT NONE
C     == GLobal variables ==
CBOP
C    !ROUTINE: SIZE.h
...
C     KappaRTr   :: Net diffusivity for this tracer (trIdentity)
      ...
      REAL*8 kappartrb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      INTEGER mythid
C
C     !LOCAL VARIABLES:
C     == Local variables ==
...
C
      IF (.NOT.trusekpp) THEN
        ...
        ELSE IF (tridentity .EQ. gad_salinity) THEN
...
C
C
      IF (trusegmredi) CALL GMREDI_CALC_DIFF_B(bi, bj, imin, imax, jmin
     +                                         , jmax, 0, nr, kappartr, 
     +                                         kappartrb, tridentity, 
     +                                         mythid)
