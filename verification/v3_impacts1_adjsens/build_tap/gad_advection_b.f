...
C
C  Differentiation of gad_advection in reverse (adjoint) mode:
C   gradient     of useful results: vfld ufld tracer gtracer wfld
C   with respect to varying inputs: vfld ufld tracer wfld
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_ADVECTION_B(implicitadvection, advectionscharg, 
     +                           vertadvecscharg, tridentity, deltatlev
     +                           , ufld, ufldb, vfld, vfldb, wfld, wfldb
     +                           , tracer, tracerb, gtracer, gtracerb, 
...
C       to one file per variable; else write one file per record)
      LOGICAL dumpadbyrec
C
      COMMON /autodiff_parm_l/ inadmode, inadexact, 
     +useapproxadvectioninadmode, usekppinadmode, usekppinfwdmode, 
     +usegmrediinadmode, usegmrediinfwdmode, useseaiceinadmode, 
     +useseaiceinfwdmode, useggl90inadmode, useggl90infwdmode, 
...
C       >= 1 (and useSEAICEinAdMode)    : omit pieces of seaice thermo adjoint
      ...
      COMMON /autodiff_parm_i/ dumpadvarexch, mon_advarexch, 
     +seaiceapproxlevinad
C
...
C     SIregFacInFw :: Factor for over shoots in FW
      ...
      COMMON /autodiff_parm_r/ viscfacinad, viscfacinfw, siregfacinad, 
     +siregfacinfw
C
...
C  myThid            :: thread number
      ...
      REAL*8 ufldb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 vfld(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 vfldb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      ...
      REAL*8 tracerb(1-olx:snx+olx, 1-oly:sny+oly, nr, nsx, nsy)
      INTEGER bi, bj
      REAL*8 mytime
      INTEGER myiter
      INTEGER mythid
...
C--   Get temporary terms used by tendency routines
        DO j=1-oly,sny+oly
          DO i=1-olx,snx+olx
            CALL PUSHREAL8(xa(i, j))
            xa(i, j) = dyg(i, j, bi, bj)*deepfacc(k)*drf(k)*hfacw(i, j, 
     +        k, bi, bj)
            CALL PUSHREAL8(ya(i, j))
            ya(i, j) = dxg(i, j, bi, bj)*deepfacc(k)*drf(k)*hfacs(i, j, 
     +        k, bi, bj)
          ENDDO
        ENDDO
...
C     anelastic: scaled by rhoFacC (~ mass transport)
        DO j=1-oly,sny+oly
          DO i=1-olx,snx+olx
            CALL PUSHREAL8(utrans(i, j))
            utrans(i, j) = ufld(i, j, k)*xa(i, j)*rhofacc(k)
            CALL PUSHREAL8(vtrans(i, j))
...
C--   Make local copy of tracer array and mask West & South
        DO j=1-oly,sny+oly
          DO i=1-olx,snx+olx
            CALL PUSHREAL8(localtij(i, j))
            localtij(i, j) = tracer(i, j, k, bi, bj)
            CALL PUSHREAL8(masklocw(i, j))
            masklocw(i, j) = maskw(i, j, k, bi, bj)
            CALL PUSHREAL8(masklocs(i, j))
...
C
        IF (usecubedsphereexchange) THEN
          withsigns = .false.
          CALL PUSHREAL8ARRAY(masklocs, 70**2)
          CALL PUSHREAL8ARRAY(masklocw, 70**2)
...
C-     Internal exchange for calculations in X
              IF (overlaponly) THEN
                CALL PUSHREAL8ARRAY(localtij, 70**2)
...
C
              IF (s_edge) THEN
                DO j=1-oly,0
                  ad_from = iminupd
                  DO i=ad_from,imaxupd
                    CALL PUSHREAL8(localtij(i, j))
                    localtij(i, j) = localtij(i, j) - deltatlev(k)*
     +                recip_rhofacc(k)*recip_hfacc(i, j, k, bi, bj)*
     +                recip_drf(k)*recip_ra(i, j, bi, bj)*
     +                recip_deepfac2c(k)*(af(i+1, j)-af(i, j)-tracer(i, 
     +                j, k, bi, bj)*(utrans(i+1, j)-utrans(i, j)))*
     +                maskinc(i, j, bi, bj)
                  ENDDO
...
C      do not only update the overlap
              ...
              ad_from1 = jminupd
              DO j=ad_from1,jmaxupd
                DO i=1-olx+1,snx+olx-1
                  CALL PUSHREAL8(localtij(i, j))
                  localtij(i, j) = localtij(i, j) - deltatlev(k)*
     +              recip_rhofacc(k)*recip_hfacc(i, j, k, bi, bj)*
     +              recip_drf(k)*recip_ra(i, j, bi, bj)*recip_deepfac2c(
     +              k)*(af(i+1, j)-af(i, j)-tracer(i, j, k, bi, bj)*(
     +              utrans(i+1, j)-utrans(i, j)))*maskinc(i, j, bi, bj)
                ENDDO
              ENDDO
...
C
     ...
     +                                , deltatlev(k), vtrans, vfld(1-olx
     +                                , 1-oly, k), localtij, af, mythid)
                CALL PUSHCONTROL3B(0)
              ELSE IF (advectionscheme .EQ. enum_flux_limit) THEN
                CALL GAD_FLUXLIMIT_ADV_Y(bi, bj, k, .true., deltatlev(k)
     +                                   , vtrans, vfld(1-olx, 1-oly, k)
     +                                   , masklocs, localtij, af, 
     +                                   mythid)
                CALL PUSHCONTROL3B(1)
              ELSE IF (advectionscheme .EQ. enum_dst3) THEN
                CALL GAD_DST3_ADV_Y(bi, bj, k, .true., deltatlev(k), 
     +                              vtrans, vfld(1-olx, 1-oly, k), 
     +                              masklocs, localtij, af, mythid)
                CALL PUSHCONTROL3B(2)
              ELSE IF (advectionscheme .EQ. enum_dst3_flux_limit) THEN
...
C
              IF (w_edge) THEN
                ad_from2 = jminupd
                DO j=ad_from2,jmaxupd
                  DO i=1-olx,0
                    CALL PUSHREAL8(localtij(i, j))
                    localtij(i, j) = localtij(i, j) - deltatlev(k)*
     +                recip_rhofacc(k)*recip_hfacc(i, j, k, bi, bj)*
     +                recip_drf(k)*recip_ra(i, j, bi, bj)*
     +                recip_deepfac2c(k)*(af(i, j+1)-af(i, j)-tracer(i, 
     +                j, k, bi, bj)*(vtrans(i, j+1)-vtrans(i, j)))*
     +                maskinc(i, j, bi, bj)
                  ENDDO
                ENDDO
                CALL PUSHINTEGER4(j - 1)
...
C      do not only update the overlap
              ...
              DO j=1-oly+1,sny+oly-1
                ad_from4 = iminupd
                DO i=ad_from4,imaxupd
                  CALL PUSHREAL8(localtij(i, j))
                  localtij(i, j) = localtij(i, j) - deltatlev(k)*
     +              recip_rhofacc(k)*recip_hfacc(i, j, k, bi, bj)*
     +              recip_drf(k)*recip_ra(i, j, bi, bj)*recip_deepfac2c(
     +              k)*(af(i, j+1)-af(i, j)-tracer(i, j, k, bi, bj)*(
     +              vtrans(i, j+1)-vtrans(i, j)))*maskinc(i, j, bi, bj)
                ENDDO
                CALL PUSHINTEGER4(i - 1)
...
C--   kDown  Cycles through 2,1 to point to w-layer below
          CALL PUSHINTEGER4(kup)
          kup = 1 + MOD(k + 1, 2)
          CALL PUSHINTEGER4(kdown)
          kdown = 1 + MOD(k, 2)
          CALL PUSHREAL8(kp1msk)
          kp1msk = 1.
          IF (k .EQ. nr) kp1msk = 0.
C
C
...
C- Interior interface :
            DO j=1-oly,sny+oly
              DO i=1-olx,snx+olx
                ...
                rtranskp(i, j) = kp1msk*rtrans(i, j)
                CALL PUSHREAL8(rtrans(i, j))
                rtrans(i, j) = wfld(i, j, k)*ra(i, j, bi, bj)*deepfac2f(
...
C--   Divergence of vertical fluxes
        ...
        DO i1=1,ad_count0
          IF (i1 .EQ. 1) THEN
            ...
            IF (branch .EQ. 0) THEN
              ...
              DO ii1=1-oly,oly+sny
                DO ii2=1-olx,olx+snx
                  rtranskpb(ii2, ii1) = 0.D0
                ENDDO
              ENDDO
              DO ii1=1-oly,oly+sny
                DO ii2=1-olx,olx+snx
                  localtijb(ii2, ii1) = 0.D0
                ENDDO
              ENDDO
              DO ii1=1,2
                DO ii2=1-oly,oly+sny
                  DO ii3=1-olx,olx+snx
                    fvertb(ii3, ii2, ii1) = 0.D0
                  ENDDO
                ENDDO
              ENDDO
              GOTO 110
          ...
          ELSE
            DO j=sny+oly,1-oly,-1
              DO i=snx+olx,1-olx,-1
                tempb0 = gtracerb(i, j, k)/deltatlev(k)
                gtracerb(i, j, k) = 0.D0
                localtijb(i, j) = localtijb(i, j) + tempb0
                tracerb(i, j, k, bi, bj) = tracerb(i, j, k, bi, bj) - 
     +            tempb0
                localt3db(i, j, k) = localt3db(i, j, k) + localtijb(i, j
     +            )
                tempb0 = -(recip_hfacc(i, j, k, bi, bj)*recip_rhofacc(k)
     +            *recip_drf(k)*deltatlev(k)*rksign*recip_deepfac2c(k)*
     +            recip_ra(i, j, bi, bj)*maskinc(i, j, bi, bj)*localtijb
     +            (i, j))
                localtijb(i, j) = 0.D0
                fvertb(i, j, kdown) = fvertb(i, j, kdown) + tempb0
                fvertb(i, j, kup) = fvertb(i, j, kup) - tempb0
                tracerb(i, j, k, bi, bj) = tracerb(i, j, k, bi, bj) - (
     +            rtranskp(i, j)-rtrans(i, j))*tempb0
                tempb = -(tracer(i, j, k, bi, bj)*tempb0)
                rtranskpb(i, j) = rtranskpb(i, j) + tempb
                rtransb(i, j) = rtransb(i, j) - tempb
              ENDDO
            ENDDO
            CALL POPCONTROL3B(branch)
            IF (branch .LT. 3) THEN
              IF (branch .EQ. 0) THEN
                CALL GAD_OS7MP_ADV_R_B(bi, bj, k, deltatlev(k), rtrans, 
     +                                 rtransb, wfld(1-olx, 1-oly, k), 
     +                                 wfldb(1-olx, 1-oly, k), localt3d
     +                                 , localt3db, fvert(1-olx, 1-oly, 
     +                                 kup), fvertb(1-olx, 1-oly, kup), 
     +                                 mythid)
              ELSE IF (branch .EQ. 1) THEN
                CALL GAD_DST3FL_ADV_R_B(bi, bj, k, deltatlev(k), rtrans
     +                                  , rtransb, wfld(1-olx, 1-oly, k)
     +                                  , wfldb(1-olx, 1-oly, k), 
     +                                  localt3d, localt3db, fvert(1-olx
     +                                  , 1-oly, kup), fvertb(1-olx, 1-
     +                                  oly, kup), mythid)
              ELSE
                CALL GAD_DST3_ADV_R_B(bi, bj, k, deltatlev(k), rtrans, 
     +                                rtransb, wfld(1-olx, 1-oly, k), 
     +                                wfldb(1-olx, 1-oly, k), localt3d, 
     +                                localt3db, fvert(1-olx, 1-oly, kup
     +                                ), fvertb(1-olx, 1-oly, kup), 
     +                                mythid)
              END IF
            ELSE IF (branch .EQ. 3) THEN
              CALL GAD_FLUXLIMIT_ADV_R_B(bi, bj, k, deltatlev(k), rtrans
     +                                   , rtransb, wfld(1-olx, 1-oly, k
     +                                   ), wfldb(1-olx, 1-oly, k), 
     +                                   localt3d, localt3db, fvert(1-
     +                                   olx, 1-oly, kup), fvertb(1-olx
     +                                   , 1-oly, kup), mythid)
            ELSE IF (branch .EQ. 4) THEN
              CALL GAD_DST2U1_ADV_R_B(bi, bj, k, advectionscheme, 
     +                                deltatlev(k), rtrans, rtransb, 
     +                                wfld(1-olx, 1-oly, k), wfldb(1-olx
     +                                , 1-oly, k), localt3d, localt3db, 
     +                                fvert(1-olx, 1-oly, kup), fvertb(1
     +                                -olx, 1-oly, kup), mythid)
            ELSE
              DO j=sny+oly,1-oly,-1
                DO i=snx+olx,1-olx,-1
                  fvertb(i, j, kup) = 0.D0
                  CALL POPREAL8(rtrans(i, j))
                  rtransb(i, j) = kp1msk*rtranskpb(i, j)
                  CALL POPREAL8(rtranskp(i, j))
                  rtranskpb(i, j) = 0.D0
                ENDDO
              ENDDO
              GOTO 100
     ...
     +          deepfac2f(k)*rhofacf(k)*maskc(i, j, k-1, bi, bj)*rtransb
     +          (i, j)
              rtransb(i, j) = kp1msk*rtranskpb(i, j)
              CALL POPREAL8(rtranskp(i, j))
              rtranskpb(i, j) = 0.D0
            ENDDO
          ENDDO
 100      CALL POPREAL8(kp1msk)
          CALL POPINTEGER4(kdown)
          CALL POPINTEGER4(kup)
 110      CALL POPINTEGER4(k)
        ENDDO
        GOTO 130
 ...
 140  CALL POPCONTROL2B(branch)
     ...
     +          tempb0
        ...
        ELSE
          ...
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              localtijb(i, j) = localtijb(i, j) + localt3db(i, j, k)
              localt3db(i, j, k) = 0.D0
            ENDDO
          ENDDO
        END IF
        CALL POPINTEGER4(ad_count)
        DO 180 i0=1,ad_count
          IF (i0 .EQ. 1) THEN
            ...
            IF (branch .LT. 2) THEN
              IF (branch .EQ. 0) THEN
                ...
                DO j=ad_to3,ad_from3,-1
                  DO i=snx+olx,snx+1,-1
                    CALL POPREAL8(localtij(i, j))
                    tempb0 = -(deltatlev(k)*recip_rhofacc(k)*recip_drf(k
     +                )*recip_deepfac2c(k)*recip_hfacc(i, j, k, bi, bj)*
     +                recip_ra(i, j, bi, bj)*maskinc(i, j, bi, bj)*
     +                localtijb(i, j))
                    afb(i, j+1) = afb(i, j+1) + tempb0
                    afb(i, j) = afb(i, j) - tempb0
                    tracerb(i, j, k, bi, bj) = tracerb(i, j, k, bi, bj) 
     +                - (vtrans(i, j+1)-vtrans(i, j))*tempb0
                    tempb = -(tracer(i, j, k, bi, bj)*tempb0)
                    vtransb(i, j+1) = vtransb(i, j+1) + tempb
                    vtransb(i, j) = vtransb(i, j) - tempb
                  ENDDO
                ENDDO
              END IF
              CALL POPCONTROL1B(branch)
     ...
     +              (vtrans(i, j+1)-vtrans(i, j))*tempb0
            ...
            CALL POPCONTROL2B(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREAL8ARRAY(localtij, 70**2)
              CALL FILL_CS_CORNER_TR_RL_B(1, .false., localtij, 
     +                                    localtijb, bi, bj, mythid)
            ...
            IF (branch .LT. 2) THEN
              IF (branch .EQ. 0) THEN
                CALL GAD_DST2U1_ADV_Y_B(bi, bj, k, advectionscheme, 
     +                                  .true., deltatlev(k), vtrans, 
     +                                  vtransb, vfld(1-olx, 1-oly, k), 
     +                                  vfldb(1-olx, 1-oly, k), localtij
     +                                  , localtijb, af, afb, mythid)
              ELSE
                CALL GAD_FLUXLIMIT_ADV_Y_B(bi, bj, k, .true., deltatlev(
     +                                     k), vtrans, vtransb, vfld(1-
     +                                     olx, 1-oly, k), vfldb(1-olx, 
     +                                     1-oly, k), masklocs, localtij
     +                                     , localtijb, af, afb, mythid)
              END IF
            ELSE IF (branch .EQ. 2) THEN
              CALL GAD_DST3_ADV_Y_B(bi, bj, k, .true., deltatlev(k), 
     +                              vtrans, vtransb, vfld(1-olx, 1-oly, 
     +                              k), vfldb(1-olx, 1-oly, k), masklocs
     +                              , localtij, localtijb, af, afb, 
     +                              mythid)
            ELSE IF (branch .EQ. 3) THEN
              CALL GAD_DST3FL_ADV_Y_B(bi, bj, k, .true., deltatlev(k), 
     +                                vtrans, vtransb, vfld(1-olx, 1-oly
     +                                , k), vfldb(1-olx, 1-oly, k), 
     +                                masklocs, localtij, localtijb, af
     +                                , afb, mythid)
            ELSE
              CALL GAD_OS7MP_ADV_Y_B(bi, bj, k, .true., deltatlev(k), 
     +                               vtrans, vtransb, vfld(1-olx, 1-oly
     +                               , k), vfldb(1-olx, 1-oly, k), 
     +                               masklocs, localtij, localtijb, af, 
     +                               afb, mythid)
          ...
          IF (branch .EQ. 0) THEN
            CALL POPREAL8ARRAY(localtij, 70**2)
            CALL FILL_CS_CORNER_TR_RL_B(2, .false., localtij, localtijb
     +                                  , bi, bj, mythid)
          END IF
 150      DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              afb(i, j) = 0.D0
            ENDDO
          ENDDO
     ...
     +              recip_ra(i, j, bi, bj)*maskinc(i, j, bi, bj)*
     +              localtijb(i, j))
                  afb(i+1, j) = afb(i+1, j) + tempb0
                  afb(i, j) = afb(i, j) - tempb0
                  tracerb(i, j, k, bi, bj) = tracerb(i, j, k, bi, bj) - 
     +              (utrans(i+1, j)-utrans(i, j))*tempb0
                  tempb = -(tracer(i, j, k, bi, bj)*tempb0)
                  utransb(i+1, j) = utransb(i+1, j) + tempb
                  utransb(i, j) = utransb(i, j) - tempb
                ENDDO
              ENDDO
            END IF
          ELSE
            IF (branch .NE. 2) THEN
              DO j=sny+oly,sny+1,-1
            ...
            IF (branch .EQ. 0) THEN
              DO j=0,1-oly,-1
                ...
                DO i=ad_to,ad_from,-1
                  CALL POPREAL8(localtij(i, j))
                  tempb = -(deltatlev(k)*recip_rhofacc(k)*recip_drf(k)*
     +              recip_deepfac2c(k)*recip_hfacc(i, j, k, bi, bj)*
     +              recip_ra(i, j, bi, bj)*maskinc(i, j, bi, bj)*
     +              localtijb(i, j))
                  afb(i+1, j) = afb(i+1, j) + tempb
                  afb(i, j) = afb(i, j) - tempb
                  tracerb(i, j, k, bi, bj) = tracerb(i, j, k, bi, bj) - 
     +              (utrans(i+1, j)-utrans(i, j))*tempb
                  tempb0 = -(tracer(i, j, k, bi, bj)*tempb)
                  utransb(i+1, j) = utransb(i+1, j) + tempb0
                  utransb(i, j) = utransb(i, j) - tempb0
                ENDDO
              ENDDO
            END IF
          END IF
     ...
     +                                  , bi, bj, mythid)
          ...
          IF (branch .LT. 2) THEN
            IF (branch .EQ. 0) THEN
              CALL GAD_DST2U1_ADV_X_B(bi, bj, k, advectionscheme, .true.
     +                                , deltatlev(k), utrans, utransb, 
     +                                ufld(1-olx, 1-oly, k), ufldb(1-olx
     +                                , 1-oly, k), localtij, localtijb, 
     +                                af, afb, mythid)
            ELSE
              CALL GAD_FLUXLIMIT_ADV_X_B(bi, bj, k, .true., deltatlev(k)
     +                                   , utrans, utransb, ufld(1-olx, 
     +                                   1-oly, k), ufldb(1-olx, 1-oly, 
     +                                   k), masklocw, localtij, 
     +                                   localtijb, af, afb, mythid)
            END IF
          ELSE IF (branch .EQ. 2) THEN
            CALL GAD_DST3_ADV_X_B(bi, bj, k, .true., deltatlev(k), 
     +                            utrans, utransb, ufld(1-olx, 1-oly, k)
     +                            , ufldb(1-olx, 1-oly, k), masklocw, 
     +                            localtij, localtijb, af, afb, mythid)
          ELSE IF (branch .EQ. 3) THEN
            CALL GAD_DST3FL_ADV_X_B(bi, bj, k, .true., deltatlev(k), 
     +                              utrans, utransb, ufld(1-olx, 1-oly, 
     +                              k), ufldb(1-olx, 1-oly, k), masklocw
     +                              , localtij, localtijb, af, afb, 
     +                              mythid)
          ELSE
            CALL GAD_OS7MP_ADV_X_B(bi, bj, k, .true., deltatlev(k), 
     +                             utrans, utransb, ufld(1-olx, 1-oly, k
     +                             ), ufldb(1-olx, 1-oly, k), masklocw, 
     +                             localtij, localtijb, af, afb, mythid)
          END IF
 160      CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL8ARRAY(localtij, 70**2)
            CALL FILL_CS_CORNER_TR_RL_B(1, .false., localtij, localtijb
     +                                  , bi, bj, mythid)
          END IF
 170      DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              afb(i, j) = 0.D0
            ENDDO
          ENDDO
 ...
 180    CONTINUE
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) THEN
          CALL POPREAL8ARRAY(masklocw, 70**2)
          CALL POPREAL8ARRAY(masklocs, 70**2)
        END IF
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            CALL POPREAL8(masklocs(i, j))
            CALL POPREAL8(masklocw(i, j))
            CALL POPREAL8(localtij(i, j))
            tracerb(i, j, k, bi, bj) = tracerb(i, j, k, bi, bj) + 
     +        localtijb(i, j)
            localtijb(i, j) = 0.D0
        ...
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            CALL POPREAL8(vtrans(i, j))
            vfldb(i, j, k) = vfldb(i, j, k) + ya(i, j)*rhofacc(k)*
     +        vtransb(i, j)
            vtransb(i, j) = 0.D0
            CALL POPREAL8(utrans(i, j))
            ufldb(i, j, k) = ufldb(i, j, k) + xa(i, j)*rhofacc(k)*
     +        utransb(i, j)
            utransb(i, j) = 0.D0
          ENDDO
        ENDDO
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            CALL POPREAL8(ya(i, j))
            CALL POPREAL8(xa(i, j))
          ENDDO
        ENDDO
        GOTO 140
      END IF
      CALL POPINTEGER4(k)
      GOTO 210
 190  CALL PUSHCONTROL2B(1)
      CALL PUSHINTEGER4(ad_count)
      STOP
 200  CALL PUSHCONTROL2B(2)
      CALL PUSHINTEGER4(ad_count)
      STOP
 210  CONTINUE
      END
