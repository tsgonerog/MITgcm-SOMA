...
C
C  Differentiation of gad_dst3_adv_r in reverse (adjoint) mode:
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_DST3_ADV_R_B(bi, bj, k, dtarg, rtrans, rtransb, 
     +                            wfld, wfldb, tracer, tracerb, wt, wtb
     +                            , mythid)
      IMPLICIT NONE
C
C     == GLobal variables ==
...
C  wCFL              :: Courant-Friedrich-Levy number
      ...
      REAL*8 rjmb, rjb, rjpb, cflb, d0b, d1b
      INTRINSIC MAX
      INTRINSIC MIN
      INTRINSIC ABS
      REAL*8 abs0
...
C
     ...
     +      , bj)
          tempb = (tracer(i, j, k)+d0*rj+d1*rjp)*0.5*wtb(i, j)
          tempb0 = (rtrans(i, j)+abs0)*0.5*wtb(i, j)
          tempb1 = (tracer(i, j, km1)-d0*rj-d1*rjm)*0.5*wtb(i, j)
          tempb2 = (rtrans(i, j)-abs1)*0.5*wtb(i, j)
          wtb(i, j) = 0.D0
          tracerb(i, j, km1) = tracerb(i, j, km1) + tempb2
          d0b = rj*tempb0 - rj*tempb2
          rjb = d0*tempb0 - d0*tempb2
          d1b = rjp*tempb0 - rjm*tempb2
          rjmb = -(d1*tempb2)
          ...
          ELSE
            ...
            rtransb(i, j) = rtransb(i, j) + abs0b
          END IF
          cflb = -(2*cfl*onesixth*d1b) - ((1.-cfl)*onesixth+(2.-cfl)*
     +      onesixth)*d0b
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL8(cfl)
            wlocb = dtarg*recip_drc(k)*cflb
          ELSE
            CALL POPREAL8(cfl)
            wlocb = -(dtarg*recip_drc(k)*cflb)
          END IF
