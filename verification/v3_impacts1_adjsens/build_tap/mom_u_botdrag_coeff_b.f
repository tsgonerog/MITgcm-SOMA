...
C
C  Differentiation of mom_u_botdrag_coeff in reverse (adjoint) mode:
C   gradient     of useful results: vfld ufld ke cdrag
C   with respect to varying inputs: vfld ufld ke
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_U_BOTDRAG_COEFF_B(bi, bj, k, inp_ke, ufld, ufldb, 
     +                                 vfld, vfldb, kapparu, ke, keb, 
     +                                 cdrag, cdragb, myiter, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
        ...
        IF (.NOT.inp_ke) THEN
          DO j=1-oly,sny+oly-1
            DO i=1-olx,snx+olx-1
              CALL PUSHREAL8(ke(i, j))
              ke(i, j) = 0.25*(ufld(i, j)*ufld(i, j)*hfacw(i, j, k, bi, 
     +          bj)+ufld(i+1, j)*ufld(i+1, j)*hfacw(i+1, j, k, bi, bj)+(
     +          vfld(i, j)*vfld(i, j)*hfacs(i, j, k, bi, bj)+vfld(i, j+1
     +          )*vfld(i, j+1)*hfacs(i, j+1, k, bi, bj)))*recip_hfacc(i
     +          , j, k, bi, bj)
            ENDDO
          ENDDO
          CALL PUSHCONTROL1B(1)
...
C-    average grid-cell-center KE to get velocity norm @ U.pt
      ...
      ELSE IF (selectbotdragquadr .EQ. 1) THEN
C-    calculate locally velocity norm @ U.pt (local U & 4 V averaged)
        DO j=1-oly,sny+oly-1
          DO i=1-olx+1,snx+olx-1
            CALL PUSHREAL8(usq)
            usq = ufld(i, j)*ufld(i, j) + (vfld(i-1, j)*vfld(i-1, j)*
     +        hfacs(i-1, j, k, bi, bj)+vfld(i, j)*vfld(i, j)*hfacs(i, j
     +        , k, bi, bj)+(vfld(i-1, j+1)*vfld(i-1, j+1)*hfacs(i-1, j+1
     +        , k, bi, bj)+vfld(i, j+1)*vfld(i, j+1)*hfacs(i, j+1, k, bi
     +        , bj)))*recip_hfacw(i, j, k, bi, bj)*0.25d0
      ...
      ELSE IF (selectbotdragquadr .EQ. 2) THEN
C-    same as above but using wet-point method to average 4 V
     ...
     +        hfacs(i-1, j+1, k, bi, bj)+hfacs(i, j+1, k, bi, bj))
            IF (usq .GT. zerorl) THEN
              CALL PUSHREAL8(usq)
              usq = ufld(i, j)*ufld(i, j) + (vfld(i-1, j)*vfld(i-1, j)*
     +          hfacs(i-1, j, k, bi, bj)+vfld(i, j)*vfld(i, j)*hfacs(i, 
     +          j, k, bi, bj)+(vfld(i-1, j+1)*vfld(i-1, j+1)*hfacs(i-1, 
     +          j+1, k, bi, bj)+vfld(i, j+1)*vfld(i, j+1)*hfacs(i, j+1, 
     +          k, bi, bj)))/usq
              CALL PUSHCONTROL1B(0)
            ELSE
      ...
      ELSE IF (selectbotdragquadr .NE. -1) THEN
        STOP
      ELSE
      ...
      IF (k .EQ. kbottom) THEN
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx+1,-1
            cdragb(i, j) = maskw(i, j, k, bi, bj)*cdragb(i, j)
          ENDDO
        ENDDO
      ELSE
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx+1,-1
            cdragb(i, j) = maskw(i, j, k, bi, bj)*(oners-maskw(i, j, 
     +        kdown, bi, bj))*cdragb(i, j)
          ENDDO
        ENDDO
      END IF
        ...
        IF (branch .EQ. 0) THEN
          DO j=sny+oly-1,1-oly,-1
            DO i=snx+olx-1,1-olx+1,-1
              ...
              IF (branch .NE. 0) THEN
                IF (ke(i, j) + ke(i-1, j) .EQ. 0.0) THEN
                  tempb = 0.D0
                ELSE
                  tempb = bottomdragquadratic*dragfac*cdragb(i, j)/(2.0*
     +              SQRT(ke(i, j)+ke(i-1, j)))
                END IF
          ...
          IF (branch .NE. 0) THEN
            DO j=sny+oly-1,1-oly,-1
              DO i=snx+olx-1,1-olx,-1
                CALL POPREAL8(ke(i, j))
                tempb = recip_hfacc(i, j, k, bi, bj)*0.25*keb(i, j)
                keb(i, j) = 0.D0
                ufldb(i, j) = ufldb(i, j) + 2*ufld(i, j)*hfacw(i, j, k, 
     +            bi, bj)*tempb
                ufldb(i+1, j) = ufldb(i+1, j) + 2*ufld(i+1, j)*hfacw(i+1
     +            , j, k, bi, bj)*tempb
                vfldb(i, j) = vfldb(i, j) + 2*vfld(i, j)*hfacs(i, j, k, 
     +            bi, bj)*tempb
                vfldb(i, j+1) = vfldb(i, j+1) + 2*vfld(i, j+1)*hfacs(i, 
     +            j+1, k, bi, bj)*tempb
              ENDDO
            ENDDO
          END IF
        ELSE
          DO j=sny+oly-1,1-oly,-1
            DO i=snx+olx-1,1-olx+1,-1
              ...
              IF (branch .EQ. 0) THEN
                usqb = 0.D0
              ELSE IF (usq .EQ. 0.0) THEN
                usqb = 0.D0
              ELSE
                usqb = bottomdragquadratic*dragfac*cdragb(i, j)/(2.0*
     +            SQRT(usq))
              END IF
              CALL POPREAL8(usq)
              ufldb(i, j) = ufldb(i, j) + 2*ufld(i, j)*usqb
              tempb = recip_hfacw(i, j, k, bi, bj)*0.25d0*usqb
              vfldb(i-1, j) = vfldb(i-1, j) + 2*vfld(i-1, j)*hfacs(i-1, 
     +          j, k, bi, bj)*tempb
              vfldb(i, j) = vfldb(i, j) + 2*vfld(i, j)*hfacs(i, j, k, bi
     +          , bj)*tempb
              vfldb(i-1, j+1) = vfldb(i-1, j+1) + 2*vfld(i-1, j+1)*hfacs
     +          (i-1, j+1, k, bi, bj)*tempb
              vfldb(i, j+1) = vfldb(i, j+1) + 2*vfld(i, j+1)*hfacs(i, j+
     +          1, k, bi, bj)*tempb
            ENDDO
          ENDDO
        END IF
        ...
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx-1,1-olx+1,-1
            ...
            ELSE IF (usq .EQ. 0.0) THEN
              usqb = 0.D0
            ELSE
              usqb = bottomdragquadratic*dragfac*cdragb(i, j)/(2.0*SQRT(
     +          usq))
            END IF
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
