...
C
C  Differentiation of mom_calc_ke in reverse (adjoint) mode:
C   gradient     of useful results: vfld ufld ke
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_CALC_KE_B(bi, bj, k, kescheme, ufld, ufldb, vfld, 
     +                         vfldb, ke, keb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
      ...
      IF (kescheme .EQ. -1) THEN
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx-1,1-olx,-1
            tempb = 2*(ufld(i, j)+ufld(i+1, j))*0.125*keb(i, j)
            tempb0 = 2*(vfld(i, j)+vfld(i, j+1))*0.125*keb(i, j)
            keb(i, j) = 0.D0
            vfldb(i, j) = vfldb(i, j) + tempb0
            vfldb(i, j+1) = vfldb(i, j+1) + tempb0
            ufldb(i, j) = ufldb(i, j) + tempb
            ufldb(i+1, j) = ufldb(i+1, j) + tempb
          ENDDO
        ENDDO
...
C
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx-1,1-olx,-1
            tempb0 = 0.25*keb(i, j)
            keb(i, j) = 0.D0
            ufldb(i, j) = ufldb(i, j) + 2*ufld(i, j)*tempb0
            ufldb(i+1, j) = ufldb(i+1, j) + 2*ufld(i+1, j)*tempb0
            vfldb(i, j) = vfldb(i, j) + 2*vfld(i, j)*tempb0
            vfldb(i, j+1) = vfldb(i, j+1) + 2*vfld(i, j+1)*tempb0
          ENDDO
        ENDDO
...
C
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx-1,1-olx,-1
            tempb0 = recip_ra(i, j, bi, bj)*0.25*keb(i, j)
            keb(i, j) = 0.D0
            ufldb(i, j) = ufldb(i, j) + 2*ufld(i, j)*raw(i, j, bi, bj)*
     +        tempb0
            ufldb(i+1, j) = ufldb(i+1, j) + 2*ufld(i+1, j)*raw(i+1, j, 
     +        bi, bj)*tempb0
            vfldb(i, j) = vfldb(i, j) + 2*vfld(i, j)*ras(i, j, bi, bj)*
     +        tempb0
            vfldb(i, j+1) = vfldb(i, j+1) + 2*vfld(i, j+1)*ras(i, j+1, 
     +        bi, bj)*tempb0
          ENDDO
        ENDDO
...
C
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx-1,1-olx,-1
            tempb0 = recip_hfacc(i, j, k, bi, bj)*0.25*keb(i, j)
            keb(i, j) = 0.D0
            ufldb(i, j) = ufldb(i, j) + 2*ufld(i, j)*hfacw(i, j, k, bi, 
     +        bj)*tempb0
            ufldb(i+1, j) = ufldb(i+1, j) + 2*ufld(i+1, j)*hfacw(i+1, j
     +        , k, bi, bj)*tempb0
            vfldb(i, j) = vfldb(i, j) + 2*vfld(i, j)*hfacs(i, j, k, bi, 
     +        bj)*tempb0
            vfldb(i, j+1) = vfldb(i, j+1) + 2*vfld(i, j+1)*hfacs(i, j+1
     +        , k, bi, bj)*tempb0
          ENDDO
        ENDDO
...
C
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx-1,1-olx,-1
            tempb0 = recip_hfacc(i, j, k, bi, bj)*recip_ra(i, j, bi, bj)
     +        *0.25*keb(i, j)
            keb(i, j) = 0.D0
            ufldb(i, j) = ufldb(i, j) + 2*ufld(i, j)*raw(i, j, bi, bj)*
     +        hfacw(i, j, k, bi, bj)*tempb0
            ufldb(i+1, j) = ufldb(i+1, j) + 2*ufld(i+1, j)*raw(i+1, j, 
     +        bi, bj)*hfacw(i+1, j, k, bi, bj)*tempb0
            vfldb(i, j) = vfldb(i, j) + 2*vfld(i, j)*ras(i, j, bi, bj)*
     +        hfacs(i, j, k, bi, bj)*tempb0
            vfldb(i, j+1) = vfldb(i, j+1) + 2*vfld(i, j+1)*ras(i, j+1, 
     +        bi, bj)*hfacs(i, j+1, k, bi, bj)*tempb0
          ENDDO
        ENDDO
      ELSE
C
