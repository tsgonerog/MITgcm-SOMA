...
C
C  Differentiation of mom_vi_hdissip in reverse (adjoint) mode:
C   gradient     of useful results: vdissip visca4_z udissip vort3
C                viscah_z
C   with respect to varying inputs: visca4_d vdissip zstar visca4_z
C                udissip hdiv dstar viscah_d vort3 viscah_z
C
C
C
C
...
C
C
      SUBROUTINE MOM_VI_HDISSIP_B(bi, bj, k, hdiv, hdivb, vort3, vort3b
     +                            , dstar, dstarb, zstar, zstarb, hfacz
     +                            , viscah_z, viscah_zb, viscah_d, 
     +                            viscah_db, visca4_z, visca4_zb, 
     +                            visca4_d, visca4_db, harmonic, 
     +                            biharmonic, usevariableviscosity, 
     +                            udissip, udissipb, vdissip, vdissipb, 
     +                            mythid)
      IMPLICIT NONE
C
...
C     == Routine arguments ==
      ...
      REAL*8 udissipb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 vdissip(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 vdissipb(1-olx:snx+olx, 1-oly:sny+oly)
      INTEGER mythid
C
C     == Local variables ==
      INTEGER i, j
      REAL*8 zip, zij, zpj, dim, dij, dmj, ud2, vd2
      REAL*8 zipb, zijb, zpjb, dimb, dijb, dmjb, ud2b, vd2b
      REAL*8 ud4(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 ud4b(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 vd4(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 vd4b(1-olx:snx+olx, 1-oly:sny+oly)
...
C
        ...
        DO j=sny+oly-1,2-oly,-1
          DO i=snx+olx-1,2-olx,-1
            vd4b(i, j) = vd4b(i, j) + vdissipb(i, j)
            ud4b(i, j) = ud4b(i, j) + udissipb(i, j)
            vd4b(i, j) = -(recip_deepfacc(k)*masks(i, j, k, bi, bj)*vd4b
     +        (i, j))
            ud4b(i, j) = -(recip_deepfacc(k)*maskw(i, j, k, bi, bj)*ud4b
     +        (i, j))
          ENDDO
        ENDDO
        ...
        IF (branch .EQ. 0) THEN
          ...
          DO j=sny+oly-1,2-oly,-1
            DO i=snx+olx-1,2-olx,-1
              tempb0 = visca4z*cosfacv(j, bi, bj)*recip_hfacs(i, j, k, 
     +          bi, bj)*recip_dxg(i, j, bi, bj)*vd4b(i, j)
              tempb = visca4d*recip_dyc(i, j, bi, bj)*vd4b(i, j)
              vd4b(i, j) = 0.D0
              dijb = tempb
              dimb = -tempb
              zpjb = tempb0
              tempb = -(visca4z*recip_dyg(i, j, bi, bj)*recip_hfacw(i, j
     +          , k, bi, bj)*ud4b(i, j))
              zijb = -tempb0 - tempb
              tempb0 = visca4d*cosfacu(j, bi, bj)*recip_dxc(i, j, bi, bj
     +          )*ud4b(i, j)
              ud4b(i, j) = 0.D0
              zipb = tempb
              dijb = dijb + tempb0
              dmjb = -tempb0
              zstarb(i+1, j) = zstarb(i+1, j) + hfacz(i+1, j)*zpjb
              zstarb(i, j) = zstarb(i, j) + hfacz(i, j)*zijb
              zstarb(i, j+1) = zstarb(i, j+1) + hfacz(i, j+1)*zipb
              dstarb(i-1, j) = dstarb(i-1, j) + dmjb
              dstarb(i, j) = dstarb(i, j) + dijb
              dstarb(i, j-1) = dstarb(i, j-1) + dimb
            ENDDO
          ENDDO
          DO ii1=1-oly,oly+sny
            DO ii2=1-olx,olx+snx
        ...
        ELSE
          ...
          DO j=sny+oly-1,2-oly,-1
            DO i=snx+olx-1,2-olx,-1
              tempb0 = cosfacv(j, bi, bj)*recip_hfacs(i, j, k, bi, bj)*
     +          recip_dxg(i, j, bi, bj)*vd4b(i, j)
              tempb = recip_dyc(i, j, bi, bj)*vd4b(i, j)
              vd4b(i, j) = 0.D0
              dijb = tempb
              dimb = -tempb
              zpjb = tempb0
              tempb = -(recip_dyg(i, j, bi, bj)*recip_hfacw(i, j, k, bi
     +          , bj)*ud4b(i, j))
              zijb = -tempb0 - tempb
              tempb0 = cosfacu(j, bi, bj)*recip_dxc(i, j, bi, bj)*ud4b(i
     +          , j)
              ud4b(i, j) = 0.D0
              zipb = tempb
              dijb = dijb + tempb0
              dmjb = -tempb0
              visca4_zb(i+1, j) = visca4_zb(i+1, j) + zpj*zpjb
              zpjb = visca4_z(i+1, j)*zpjb
              visca4_zb(i, j+1) = visca4_zb(i, j+1) + zip*zipb
              zipb = visca4_z(i, j+1)*zipb
              visca4_zb(i, j) = visca4_zb(i, j) + zij*zijb
              zijb = visca4_z(i, j)*zijb
              visca4_db(i-1, j) = visca4_db(i-1, j) + dmj*dmjb
              dmjb = visca4_d(i-1, j)*dmjb
              visca4_db(i, j-1) = visca4_db(i, j-1) + dim*dimb
              dimb = visca4_d(i, j-1)*dimb
              visca4_db(i, j) = visca4_db(i, j) + dij*dijb
              dijb = visca4_d(i, j)*dijb
              CALL POPREAL8(zpj)
              zstarb(i+1, j) = zstarb(i+1, j) + hfacz(i+1, j)*zpjb
              CALL POPREAL8(zij)
              zstarb(i, j) = zstarb(i, j) + hfacz(i, j)*zijb
      ...
      IF (branch .EQ. 0) THEN
        ...
        DO j=sny+oly-1,2-oly,-1
          DO i=snx+olx-1,2-olx,-1
            vd2b = recip_deepfacc(k)*masks(i, j, k, bi, bj)*vdissipb(i, 
     +        j)
            vdissipb(i, j) = 0.D0
            ud2b = recip_deepfacc(k)*maskw(i, j, k, bi, bj)*udissipb(i, 
     +        j)
            udissipb(i, j) = 0.D0
            tempb0 = cosfacv(j, bi, bj)*recip_hfacs(i, j, k, bi, bj)*
     +        recip_dxg(i, j, bi, bj)*vd2b
            tempb = recip_dyc(i, j, bi, bj)*vd2b
            dijb = tempb
            dimb = -tempb
            zpjb = tempb0
            zijb = -tempb0
            tempb = cosfacu(j, bi, bj)*recip_dxc(i, j, bi, bj)*ud2b
            tempb0 = -(recip_dyg(i, j, bi, bj)*recip_hfacw(i, j, k, bi, 
     +        bj)*ud2b)
            zipb = tempb0
            zijb = zijb - tempb0
            dijb = dijb + tempb
            dmjb = -tempb
            tempb = hfacz(i+1, j)*zpjb
            vort3b(i+1, j) = vort3b(i+1, j) + viscah_z(i+1, j)*tempb
            viscah_zb(i+1, j) = viscah_zb(i+1, j) + vort3(i+1, j)*tempb
            tempb = hfacz(i, j+1)*zipb
            vort3b(i, j+1) = vort3b(i, j+1) + viscah_z(i, j+1)*tempb
            viscah_zb(i, j+1) = viscah_zb(i, j+1) + vort3(i, j+1)*tempb
            tempb = hfacz(i, j)*zijb
            vort3b(i, j) = vort3b(i, j) + viscah_z(i, j)*tempb
            viscah_zb(i, j) = viscah_zb(i, j) + vort3(i, j)*tempb
            hdivb(i-1, j) = hdivb(i-1, j) + viscah_d(i-1, j)*dmjb
            viscah_db(i-1, j) = viscah_db(i-1, j) + hdiv(i-1, j)*dmjb
            hdivb(i, j-1) = hdivb(i, j-1) + viscah_d(i, j-1)*dimb
            viscah_db(i, j-1) = viscah_db(i, j-1) + hdiv(i, j-1)*dimb
            hdivb(i, j) = hdivb(i, j) + viscah_d(i, j)*dijb
            viscah_db(i, j) = viscah_db(i, j) + hdiv(i, j)*dijb
          ENDDO
        ENDDO
     ...
     +        j)
            udissipb(i, j) = 0.D0
            tempb0 = viscahz*cosfacv(j, bi, bj)*recip_hfacs(i, j, k, bi
     +        , bj)*recip_dxg(i, j, bi, bj)*vd2b
            tempb = viscahd*recip_dyc(i, j, bi, bj)*vd2b
            dijb = tempb
            dimb = -tempb
            zpjb = tempb0
            tempb = -(viscahz*recip_dyg(i, j, bi, bj)*recip_hfacw(i, j, 
     +        k, bi, bj)*ud2b)
            zijb = -tempb0 - tempb
            tempb0 = viscahd*cosfacu(j, bi, bj)*recip_dxc(i, j, bi, bj)*
     +        ud2b
            zipb = tempb
            dijb = dijb + tempb0
            dmjb = -tempb0
            vort3b(i+1, j) = vort3b(i+1, j) + hfacz(i+1, j)*zpjb
            vort3b(i, j) = vort3b(i, j) + hfacz(i, j)*zijb
            vort3b(i, j+1) = vort3b(i, j+1) + hfacz(i, j+1)*zipb
            hdivb(i-1, j) = hdivb(i-1, j) + dmjb
            hdivb(i, j) = hdivb(i, j) + dijb
            hdivb(i, j-1) = hdivb(i, j-1) + dimb
          ENDDO
        ENDDO
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
