...
C
C  Differentiation of mom_v_xviscflux in reverse (adjoint) mode:
C   gradient     of useful results: vfld visca4_z del2v viscah_z
C                xviscfluxv
C   with respect to varying inputs: vfld visca4_z del2v viscah_z
C                xviscfluxv
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_V_XVISCFLUX_B(bi, bj, k, vfld, vfldb, del2v, del2vb
     +                             , hfacz, xviscfluxv, xviscfluxvb, 
...
C  xViscFluxU           :: viscous fluxes
      ...
      REAL*8 xviscfluxvb(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j
     ...
     +      bi, bj)*xviscfluxvb(i, j)
          xviscfluxvb(i, j) = 0.D0
          tempb0 = sqcosfacv(j, bi, bj)*tempb
          tempb2 = -(cosfacv(j, bi, bj)*tempb)
          viscah_zb(i, j) = viscah_zb(i, j) + (vfld(i, j)-vfld(i-1, j))*
     +      tempb2
          tempb3 = viscah_z(i, j)*tempb2
          vfldb(i, j) = vfldb(i, j) + tempb3
          vfldb(i-1, j) = vfldb(i-1, j) - tempb3
          visca4_zb(i, j) = visca4_zb(i, j) + (del2v(i, j)-del2v(i-1, j)
     +      )*tempb0
          tempb1 = visca4_z(i, j)*tempb0
          del2vb(i, j) = del2vb(i, j) + tempb1
          del2vb(i-1, j) = del2vb(i-1, j) - tempb1
        ENDDO
      ENDDO
      END
