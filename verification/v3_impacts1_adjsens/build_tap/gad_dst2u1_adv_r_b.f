...
C
C  Differentiation of gad_dst2u1_adv_r in reverse (adjoint) mode:
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_DST2U1_ADV_R_B(bi, bj, k, advectionscheme, 
     +                              deltatloc, rtrans, rtransb, wfld, 
     +                              wfldb, tracer, tracerb, wt, wtb, 
     +                              mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
...
C
            ...
            IF (wloc*deltatloc*recip_drc(k) .GE. 0.) THEN
              CALL PUSHREAL8(wcfl)
              wcfl = wloc*deltatloc*recip_drc(k)
        ...
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            wabs = abs0*rksign*(1.d0-rlimit*(1.d0-wcfl))
            tempb = maskc(i, j, km1, bi, bj)*wtb(i, j)
            wtb(i, j) = 0.D0
            tempb0 = tracer(i, j, km1)*0.5d0*tempb
            tracerb(i, j, km1) = tracerb(i, j, km1) + (rtrans(i, j)+wabs
     +        )*0.5d0*tempb
            tempb1 = tracer(i, j, k)*0.5d0*tempb
            tracerb(i, j, k) = tracerb(i, j, k) + (rtrans(i, j)-wabs)*
     +        0.5d0*tempb
            rtransb(i, j) = rtransb(i, j) + tempb1 + tempb0
            wabsb = tempb0 - tempb1
            abs0b = (1.d0-rlimit*(1.d0-wcfl))*rksign*wabsb
            wcflb = rlimit*abs0*rksign*wabsb
            ...
            IF (branch .EQ. 0) THEN
              CALL POPREAL8(wcfl)
              wlocb = deltatloc*recip_drc(k)*wcflb
            ELSE
              CALL POPREAL8(wcfl)
              wlocb = -(deltatloc*recip_drc(k)*wcflb)
            END IF
