...
C
C  Differentiation of mom_u_metric_nh in reverse (adjoint) mode:
C   gradient     of useful results: ufld umetricterms wfld
C   with respect to varying inputs: ufld umetricterms wfld
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_U_METRIC_NH_B(bi, bj, k, ufld, ufldb, wfld, wfldb, 
     +                             umetricterms, umetrictermsb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C  uMetricTerms         :: metric term
      ...
      REAL*8 umetrictermsb(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j, kp1
      ...
      DO j=sny+oly,1-olx,-1
        DO i=snx+olx,1-olx+1,-1
          temp = rvel2wunit(kp1)*wvelbottomoverride
          tempb = recip_rsphere*0.25*recip_deepfacc(k)*gravitysign*
     +      umetrictermsb(i, j)
          umetrictermsb(i, j) = 0.D0
          ufldb(i, j) = ufldb(i, j) + (temp*(wfld(i-1, j, kp1, bi, bj)+
     +      wfld(i, j, kp1, bi, bj))+rvel2wunit(k)*(wfld(i-1, j, k, bi, 
     +      bj)+wfld(i, j, k, bi, bj)))*tempb
          tempb0 = ufld(i, j)*tempb
          wfldb(i-1, j, kp1, bi, bj) = wfldb(i-1, j, kp1, bi, bj) + temp
     +      *tempb0
          wfldb(i, j, kp1, bi, bj) = wfldb(i, j, kp1, bi, bj) + temp*
     +      tempb0
          wfldb(i-1, j, k, bi, bj) = wfldb(i-1, j, k, bi, bj) + 
     +      rvel2wunit(k)*tempb0
          wfldb(i, j, k, bi, bj) = wfldb(i, j, k, bi, bj) + rvel2wunit(k
     +      )*tempb0
        ENDDO
      ENDDO
      END
