...
C
C  Differentiation of salt_integrate in reverse (adjoint) mode:
...
C                /gm_bolus/[1215200,2430400[ salt gsnm1 /surface_forcing/[117600,156800[
C                /surf_correc/[8,16[ kappark vfld ufld wfld
...
C     !INTERFACE:
      SUBROUTINE SALT_INTEGRATE_B(bi, bj, recip_hfac, ufld, ufldb, vfld
...
C     gs_AB      :: Adams-Bashforth salinity tendency increment
      ...
      REAL*8 gs_locb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 gsforc(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gsforcb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gs_ab(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gs_abb(1-olx:snx+olx, 1-oly:sny+oly)
...
C     compute S^n+1/2 (stored in gsNm) extrapolating S forward in time
        DO ii1=1-olx,66
          CALL PUSHREAL8(salt(ii1, 1-oly, 1, bi, bj))
        ENDDO
        CALL ADAMS_BASHFORTH2(bi, bj, 0, nr, salt(1-olx, 1-oly, 1, bi, 
     +                        bj), gsnm1(1-olx, 1-oly, 1, bi, bj), gs_ab
     +                        , saltstartab, iternb, mythid)
...
C
     ...
     +                         gad_salinity, usegmredi, usekpp, kappark
     +                         , mythid)
C
...
C disable this section of code.
     ...
     +                     saltvertadvscheme, gad_salinity, dttracerlev
     +                     , ufld, vfld, wfld, salt, gs_loc, bi, bj, 
...
C
     ...
     +                    utrans, vtrans, rtrans, rtranskp, diffkhs, 
     +                    diffk4s, kappark(1-olx, 1-oly, k), diffkr4s, 
     +                    salt(1-olx, 1-oly, 1, bi, bj), gsnm1(1-olx, 1-
     +                    oly, 1, bi, bj), dttracerlev, gad_salinity, 
     +                    saltadvscheme, saltvertadvscheme, 
     +                    calcadvection, saltimplvertadv, 
     +                    adamsbashforth_s, saltvertdiff4, usegmredi, 
     +                    usekpp, salt_staypositive, fzon, fmer, fver, 
     +                    gs_loc, mytime, myiter, mythid)
C
...
C
        IF (adamsbashforthgs) THEN
          DO ii1=1-olx,66
            CALL PUSHREAL8(gsnm1(ii1, 1-oly, 1, bi, bj))
          ENDDO
          CALL ADAMS_BASHFORTH2(bi, bj, k, nr, gs_loc, gsnm1(1-olx, 1-
     +                          oly, 1, bi, bj), gs_ab, saltstartab, 
     +                          iternb, mythid)
          CALL PUSHCONTROL1B(0)
        ELSE
          CALL PUSHCONTROL1B(1)
...
C-    Integrate forward in time, storing in gS_loc:  gS <= S + dt*gS
      CALL TIMESTEP_TRACER(bi, bj, dttracerlev, salt(1-olx, 1-oly, 1, bi
     +                     , bj), gs_loc, mytime, myiter, mythid)
C
...
C
      IF (adamsbashforth_s) THEN
        CALL CYCLE_AB_TRACER_B(bi, bj, gs_loc, gs_locb, salt(1-olx, 1-
     +                         oly, 1, bi, bj), saltb(1-olx, 1-oly, 1, 
     +                         bi, bj), gsnm1(1-olx, 1-oly, 1, bi, bj), 
     +                         gsnm1b(1-olx, 1-oly, 1, bi, bj), mytime, 
     +                         myiter, mythid)
      ELSE
        CALL CYCLE_TRACER_B(bi, bj, salt(1-olx, 1-oly, 1, bi, bj), saltb
     +                      (1-olx, 1-oly, 1, bi, bj), gs_loc, gs_locb, 
     +                      mytime, myiter, mythid)
      END IF
      CALL POPCONTROL2B(branch)
      IF (branch .EQ. 0) THEN
        CALL GAD_IMPLICIT_R_B(saltimplvertadv, saltvertadvscheme, 
     +                        gad_salinity, dttracerlev, kappark, 
     +                        kapparkb, recip_hfac, wfld, wfldb, salt(1-
     +                        olx, 1-oly, 1, bi, bj), saltb(1-olx, 1-oly
     +                        , 1, bi, bj), gs_loc, gs_locb, bi, bj, 
     +                        mytime, myiter, mythid)
      ELSE IF (branch .EQ. 1) THEN
        CALL IMPLDIFF_B(bi, bj, imin, imax, jmin, jmax, gad_salinity, 
     +                  kappark, kapparkb, recip_hfac, gs_loc, gs_locb, 
     +                  mythid)
      END IF
      CALL TIMESTEP_TRACER_B(bi, bj, dttracerlev, salt(1-olx, 1-oly, 1, 
     +                       bi, bj), saltb(1-olx, 1-oly, 1, bi, bj), 
     +                       gs_loc, gs_locb, mytime, myiter, mythid)
      ...
      DO k=1,nr,1
        ...
        IF (branch .EQ. 0) THEN
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              gsforcb(i, j) = gsforcb(i, j) + gs_locb(i, j, k)
            ENDDO
          ENDDO
        END IF
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          DO ii1=66,1-olx,-1
            CALL POPREAL8(gsnm1(ii1, 1-oly, 1, bi, bj))
          ENDDO
          CALL ADAMS_BASHFORTH2_B(bi, bj, k, nr, gs_loc, gs_locb, gsnm1(
     +                            1-olx, 1-oly, 1, bi, bj), gsnm1b(1-olx
     +                            , 1-oly, 1, bi, bj), gs_ab, gs_abb, 
     +                            saltstartab, iternb, mythid)
        END IF
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
     ...
     +                      , rtranskp, rtranskpb, diffkhs, diffk4s, 
     +                      kappark(1-olx, 1-oly, k), kapparkb(1-olx, 1-
     +                      oly, k), diffkr4s, salt(1-olx, 1-oly, 1, bi
     +                      , bj), saltb(1-olx, 1-oly, 1, bi, bj), gsnm1
     +                      (1-olx, 1-oly, 1, bi, bj), gsnm1b(1-olx, 1-
     +                      oly, 1, bi, bj), dttracerlev, gad_salinity, 
     +                      saltadvscheme, saltvertadvscheme, 
     +                      calcadvection, saltimplvertadv, 
     +                      adamsbashforth_s, saltvertdiff4, usegmredi, 
     +                      usekpp, salt_staypositive, fzon, fzonb, fmer
     +                      , fmerb, fver, fverb, gs_loc, gs_locb, 
     +                      mytime, myiter, mythid)
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) CALL APPLY_FORCING_S_B(gsforc, gsforcb, imin
     +                                            , imax, jmin, jmax, k
     +                                            , bi, bj, mytime, 
     +                                            myiter, mythid)
        DO j=sny+oly,1-oly,-1
     ...
     +                       ya, k, bi, bj, mythid)
      ...
      IF (branch .EQ. 0) THEN
        CALL POPINTEGER4(mythid)
        CALL GAD_ADVECTION_B(saltimplvertadv, saltadvscheme, 
     +                       saltvertadvscheme, gad_salinity, 
     +                       dttracerlev, ufld, ufldb, vfld, vfldb, wfld
     +                       , wfldb, salt, saltb, gs_loc, gs_locb, bi, 
     +                       bj, mytime, myiter, mythid)
      END IF
      CALL CALC_3D_DIFFUSIVITY_B(bi, bj, imin, imax, jmin, jmax, 
     +                           gad_salinity, usegmredi, usekpp, 
     +                           kappark, kapparkb, mythid)
      ...
      IF (branch .NE. 0) THEN
        DO ii1=66,1-olx,-1
          CALL POPREAL8(salt(ii1, 1-oly, 1, bi, bj))
        ENDDO
        CALL ADAMS_BASHFORTH2_B(bi, bj, 0, nr, salt(1-olx, 1-oly, 1, bi
     +                          , bj), saltb(1-olx, 1-oly, 1, bi, bj), 
     +                          gsnm1(1-olx, 1-oly, 1, bi, bj), gsnm1b(1
     +                          -olx, 1-oly, 1, bi, bj), gs_ab, gs_abb, 
     +                          saltstartab, iternb, mythid)
      END IF
      END
