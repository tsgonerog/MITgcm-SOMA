...
C
C  Differentiation of gad_calc_rhs in reverse (adjoint) mode:
C   gradient     of useful results: /gm_wtensor/[0,1215200[ /gm_wtensor/[1215200,2430400[
C                /gm_hortensor/[0,1215200[ /gm_hortensor/[1215200,2430400[
C                /gm_uvtensor/[0,1215200[ /gm_uvtensor/[1215200,2430400[
C                /gm_bolus/[0,1215200[ /gm_bolus/[1215200,2430400[
C                vfld ufld utrans kappar gtracer tracern rtrans
C                rtranskp1 vtrans fzon fmer tracab fvert wfld
C   with respect to varying inputs: /gm_wtensor/[0,1215200[ /gm_wtensor/[1215200,2430400[
C                /gm_hortensor/[0,1215200[ /gm_hortensor/[1215200,2430400[
C                /gm_uvtensor/[0,1215200[ /gm_uvtensor/[1215200,2430400[
C                /gm_bolus/[0,1215200[ /gm_bolus/[1215200,2430400[
C                vfld ufld utrans kappar gtracer tracern rtrans
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_CALC_RHS_B(bi, bj, imin, imax, jmin, jmax, k, km1, 
     +                          kup, kdown, xa, ya, maskup, ufld, ufldb
     +                          , vfld, vfldb, wfld, wfldb, utrans, 
     +                          utransb, vtrans, vtransb, rtrans, 
     +                          rtransb, rtranskp1, rtranskp1b, diffkh, 
     +                          diffk4, kappar, kapparb, diffkr4, 
     +                          tracern, tracernb, tracab, tracabb, 
     +                          deltatlev, tridentity, advectionscharg, 
     +                          vertadvecscharg, calcadvection, 
     +                          implicitadvection, applyab_ontracer, 
     +                          trusediffkr4, trusegmredi, trusekpp, 
     +                          trusesmolhack, fzon, fzonb, fmer, fmerb
     +                          , fvert, fvertb, gtracer, gtracerb, 
...
C myThid           :: thread number
      ...
      REAL*8 wfldb(1-olx:snx+olx, 1-oly:sny+oly)
      ...
      REAL*8 rtranskp1b(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 diffkh, diffk4
      REAL*8 kappar(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 kapparb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 diffkr4(nr)
      REAL*8 tracern(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 tracernb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 tracab(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 tracabb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 deltatlev(nr)
      INTEGER tridentity
...
C     combination is possible.
        IF (advectionscharg .EQ. enum_dst3_flux_limit) advectionscheme
     +     = enum_dst3
        IF (vertadvecscharg .EQ. enum_dst3_flux_limit) THEN
...
C-    Compute vertical advective flux in the interior using TracAB:
     ...
     +        vertadvecscheme .EQ. enum_dst2) THEN
          ...
          ELSE
            STOP
          END IF
        ELSE IF (vertadvecscheme .EQ. enum_centered_2nd) THEN
...
C-    GM/Redi flux in R
      ...
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          localtb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO j=sny+oly-1,1-oly,-1
        DO i=snx+olx-1,1-olx,-1
          tempb = -(recip_hfacc(i, j, k, bi, bj)*recip_drf(k)*
     +      recip_deepfac2c(k)*recip_ra(i, j, bi, bj)*recip_rhofacc(k)*
     +      gtracerb(i, j, k))
          tempb0 = maskinc(i, j, bi, bj)*tempb
          tempb1 = maskinc(i, j, bi, bj)*tempb
          fvertb(i, j, kdown) = fvertb(i, j, kdown) + rksign*tempb
          fvertb(i, j, kup) = fvertb(i, j, kup) - rksign*tempb
          tempb2 = -(maskinc(i, j, bi, bj)*tempb)
          localtb(i, j) = localtb(i, j) + (advfac*(utrans(i+1, j)-utrans
     +      (i, j))+advfac*(vtrans(i, j+1)-vtrans(i, j))+radvfac*(
     +      rtranskp1(i, j)-rtrans(i, j)))*tempb2
          tempb3 = localt(i, j)*tempb2
          utransb(i+1, j) = utransb(i+1, j) + advfac*tempb3
          utransb(i, j) = utransb(i, j) - advfac*tempb3
          vtransb(i, j+1) = vtransb(i, j+1) + advfac*tempb3
          vtransb(i, j) = vtransb(i, j) - advfac*tempb3
          rtranskp1b(i, j) = rtranskp1b(i, j) + radvfac*tempb3
          rtransb(i, j) = rtransb(i, j) - radvfac*tempb3
          fmerb(i, j+1) = fmerb(i, j+1) + tempb1
          fmerb(i, j) = fmerb(i, j) - tempb1
          fzonb(i+1, j) = fzonb(i+1, j) + tempb0
          fzonb(i, j) = fzonb(i, j) - tempb0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
      ...
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx,-1
          dfb(i, j) = dfb(i, j) + fvertb(i, j, kup)
        ENDDO
      ENDDO
      CALL POPCONTROL1B(branch)
      IF (branch .NE. 0) CALL GMREDI_RTRANSPORT_B(tridentity, bi, bj, k
     +                                            , imin, imax, jmin, 
     +                                            jmax, maskup, tracern
     +                                            , tracernb, df, dfb, 
     +                                            mythid)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) CALL GAD_BIHARM_R_B(bi, bj, k, maskup, diffkr4
     +                                       , tracern, tracernb, df, 
     +                                       dfb, mythid)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        ...
        ENDDO
      ELSE
        CALL GAD_DIFF_R_B(bi, bj, k, maskup, kappar, kapparb, tracern, 
     +                    tracernb, df, dfb, mythid)
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        DO ii1=1-oly,oly+sny
        ...
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            afb(i, j) = afb(i, j) + maskinc(i, j, bi, bj)*fvertb(i, j, 
     +        kup)
          ENDDO
        ENDDO
     ...
     +                                 rtransb, wfld, wfldb, tracern, 
     +                                 tracernb, af, afb, mythid)
              ELSE
     ...
     +                              tracernb, af, afb, mythid)
            ELSE
              CALL GAD_C4_ADV_R_B(bi, bj, k, rtrans, rtransb, tracern, 
     +                            tracernb, af, afb, mythid)
            END IF
          ELSE IF (branch .LT. 6) THEN
            IF (branch .EQ. 4) THEN
              CALL GAD_U3_ADV_R_B(bi, bj, k, rtrans, rtransb, tracern, 
     +                            tracernb, af, afb, mythid)
            ELSE
     ...
     +                                   , tracernb, af, afb, mythid)
          ...
          ELSE IF (branch .EQ. 6) THEN
            CALL GAD_DST2U1_ADV_R_B(bi, bj, k, vertadvecscheme, 
     +                              deltatlev(k), rtrans, rtransb, wfld
     +                              , wfldb, tracern, tracernb, af, afb
     +                              , mythid)
          ELSE
            CALL GAD_C2_ADV_R_B(bi, bj, k, rtrans, rtransb, tracern, 
     +                          tracernb, af, afb, mythid)
          END IF
        ELSE IF (branch .LT. 12) THEN
          IF (branch .LT. 10) THEN
            IF (branch .EQ. 8) THEN
              CALL GAD_OS7MP_ADV_R_B(bi, bj, k, deltatlev(k), rtrans, 
     +                               rtransb, wfld, wfldb, tracab, 
     +                               tracabb, af, afb, mythid)
            ELSE
              CALL GAD_DST3FL_ADV_R_B(bi, bj, k, deltatlev(k), rtrans, 
     +                                rtransb, wfld, wfldb, tracab, 
     +                                tracabb, af, afb, mythid)
          ...
          ELSE IF (branch .EQ. 10) THEN
            CALL GAD_DST3_ADV_R_B(bi, bj, k, deltatlev(k), rtrans, 
     +                            rtransb, wfld, wfldb, tracab, tracabb
     +                            , af, afb, mythid)
          ELSE
            CALL GAD_C4_ADV_R_B(bi, bj, k, rtrans, rtransb, tracab, 
     +                          tracabb, af, afb, mythid)
          END IF
        ELSE IF (branch .LT. 14) THEN
          IF (branch .EQ. 12) THEN
            CALL GAD_U3_ADV_R_B(bi, bj, k, rtrans, rtransb, tracab, 
     +                          tracabb, af, afb, mythid)
          ELSE
            CALL GAD_FLUXLIMIT_ADV_R_B(bi, bj, k, deltatlev(k), rtrans, 
     +                                 rtransb, wfld, wfldb, tracab, 
     +                                 tracabb, af, afb, mythid)
        ...
        ELSE IF (branch .EQ. 14) THEN
          CALL GAD_DST2U1_ADV_R_B(bi, bj, k, vertadvecscheme, deltatlev(
     +                            k), rtrans, rtransb, wfld, wfldb, 
     +                            tracab, tracabb, af, afb, mythid)
        ELSE
          CALL GAD_C2_ADV_R_B(bi, bj, k, rtrans, rtransb, tracab, 
     +                        tracabb, af, afb, mythid)
        END IF
      ELSE
        DO ii1=1-oly,oly+sny
      ...
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx,-1
          dfb(i, j) = dfb(i, j) + rhofacc(k)*fmerb(i, j)
        ENDDO
      ENDDO
      CALL POPCONTROL1B(branch)
      IF (branch .NE. 0) CALL GMREDI_YTRANSPORT_B(tridentity, bi, bj, k
     +                                            , imin, imax, jmin, 
     +                                            arg1, ya, maskup, 
     +                                            tracern, tracernb, df
     +                                            , dfb, mythid)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL GAD_BIHARM_Y_B(bi, bj, k, ya, df4, df4b, diffk4, df, dfb, 
     +                      mythid)
      ELSE
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
      ...
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL GAD_DIFF_Y_B(bi, bj, k, ya, diffkh, localt, localtb, df, 
     +                    dfb, mythid)
      ELSE
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
        ...
        IF (branch .LT. 4) THEN
          IF (branch .LT. 2) THEN
            IF (branch .EQ. 0) THEN
              DO ii1=1-oly,oly+sny
                DO ii2=1-olx,olx+snx
                  locabtb(ii2, ii1) = 0.D0
                ENDDO
              ENDDO
              CALL GAD_OS7MP_ADV_Y_B(bi, bj, k, .true., deltatlev(k), 
     +                               vtrans, vtransb, vfld, vfldb, 
     +                               masklocs, locabt, locabtb, af, afb
     +                               , mythid)
            ELSE
              DO ii1=1-oly,oly+sny
                DO ii2=1-olx,olx+snx
     ...
     +                            vtrans, vtransb, vfld, vfldb, masklocs
     +                            , locabt, locabtb, af, afb, mythid)
          ELSE
            CALL GAD_C4_ADV_Y_B(bi, bj, k, vtrans, vtransb, masklocs, 
     +                          locabt, locabtb, af, afb, mythid)
          END IF
        ELSE IF (branch .LT. 6) THEN
          IF (branch .EQ. 4) THEN
            CALL GAD_U3_ADV_Y_B(bi, bj, k, vtrans, vtransb, masklocs, 
     +                          locabt, locabtb, af, afb, mythid)
          ELSE
            DO ii1=1-oly,oly+sny
              DO ii2=1-olx,olx+snx
              ...
              ENDDO
            ENDDO
            CALL GAD_FLUXLIMIT_ADV_Y_B(bi, bj, k, .true., deltatlev(k), 
     +                                 vtrans, vtransb, vfld, vfldb, 
     +                                 masklocs, locabt, locabtb, af, 
     +                                 afb, mythid)
          END IF
        ELSE IF (branch .EQ. 6) THEN
          DO ii1=1-oly,oly+sny
            ...
            ENDDO
          ENDDO
          CALL GAD_DST2U1_ADV_Y_B(bi, bj, k, advectionscheme, .true., 
     +                            deltatlev(k), vtrans, vtransb, vfld, 
     +                            vfldb, locabt, locabtb, af, afb, 
     +                            mythid)
        ELSE
          CALL GAD_C2_ADV_Y_B(bi, bj, k, vtrans, vtransb, locabt, 
     +                        locabtb, af, afb, mythid)
        END IF
      ELSE
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
      ...
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx,-1
          dfb(i, j) = dfb(i, j) + rhofacc(k)*fzonb(i, j)
        ENDDO
      ENDDO
      CALL POPCONTROL1B(branch)
      IF (branch .NE. 0) THEN
        arg1 = imax + 1
        CALL GMREDI_XTRANSPORT_B(tridentity, bi, bj, k, imin, arg1, jmin
     +                           , jmax, xa, maskup, tracern, tracernb, 
     +                           df, dfb, mythid)
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) CALL GAD_BIHARM_X_B(bi, bj, k, xa, df4, df4b, 
     +                                       diffk4, df, dfb, mythid)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) CALL GAD_DIFF_X_B(bi, bj, k, xa, diffkh, localt
     +                                     , localtb, df, dfb, mythid)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            afb(i, j) = afb(i, j) + fzonb(i, j)
          ENDDO
        ENDDO
        CALL POPCONTROL3B(branch)
        IF (branch .LT. 4) THEN
     ...
     +                               masklocw, locabt, locabtb, af, afb
     +                               , mythid)
            ELSE
     ...
     +                            utrans, utransb, ufld, ufldb, masklocw
     +                            , locabt, locabtb, af, afb, mythid)
          ELSE
            CALL GAD_C4_ADV_X_B(bi, bj, k, utrans, utransb, masklocw, 
     +                          locabt, locabtb, af, afb, mythid)
          END IF
        ELSE IF (branch .LT. 6) THEN
          IF (branch .EQ. 4) THEN
            CALL GAD_U3_ADV_X_B(bi, bj, k, utrans, utransb, masklocw, 
     +                          locabt, locabtb, af, afb, mythid)
          ELSE
            CALL GAD_FLUXLIMIT_ADV_X_B(bi, bj, k, .true., deltatlev(k), 
     +                                 utrans, utransb, ufld, ufldb, 
     +                                 masklocw, locabt, locabtb, af, 
     +                                 afb, mythid)
          END IF
        ELSE IF (branch .EQ. 6) THEN
          CALL GAD_DST2U1_ADV_X_B(bi, bj, k, advectionscheme, .true., 
     +                            deltatlev(k), utrans, utransb, ufld, 
     +                            ufldb, locabt, locabtb, af, afb, 
     +                            mythid)
        ELSE
          CALL GAD_C2_ADV_X_B(bi, bj, k, utrans, utransb, locabt, 
     +                        locabtb, af, afb, mythid)
        END IF
      END IF
      DO j=sny+oly,1-oly,-1
      ...
      CALL POPCONTROL1B(branch)
      IF (branch .NE. 0) THEN
        CALL GAD_DEL2_B(bi, bj, k, fzon, fzonb, fmer, fmerb, df4, df4b, 
     +                  mythid)
        CALL GAD_GRAD_Y_B(bi, bj, k, ya, localt, localtb, fmer, fmerb, 
     +                    mythid)
        CALL GAD_GRAD_X_B(bi, bj, k, xa, localt, localtb, fzon, fzonb, 
     +                    mythid)
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            tracabb(i, j, k) = tracabb(i, j, k) + locabtb(i, j)
            locabtb(i, j) = 0.D0
            tracernb(i, j, k) = tracernb(i, j, k) + localtb(i, j)
            localtb(i, j) = 0.D0
          ENDDO
        ENDDO
      ELSE
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            tracernb(i, j, k) = tracernb(i, j, k) + locabtb(i, j) + 
     +        localtb(i, j)
            locabtb(i, j) = 0.D0
            localtb(i, j) = 0.D0
          ENDDO
