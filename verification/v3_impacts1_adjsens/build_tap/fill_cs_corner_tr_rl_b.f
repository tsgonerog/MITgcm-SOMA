...
C
C  Differentiation of fill_cs_corner_tr_rl in reverse (adjoint) mode:
C   gradient     of useful results: trfld
C   with respect to varying inputs: trfld
C
C
C
C
...
C
C     !INTERFACE:
      SUBROUTINE FILL_CS_CORNER_TR_RL_B(fill4dir, withsigns, trfld, 
     +                                  trfldb, bi, bj, mythid)
      IMPLICIT NONE
C
C     == Global variables ==
...
C     myThid    :: thread number
      ...
      REAL*8 trfldb(1-olx:snx+olx, 1-oly:sny+oly)
      INTEGER bi, bj
      INTEGER mythid
C
C     !LOCAL VARIABLES:
...
C                      |   x T(sNx+1,    0)
          ...
          IF (northeastcorner) THEN
            DO j=oly,1,-1
              DO i=olx,1,-1
                tmpb2 = trfldb(snx+i, sny+j)
                trfldb(snx+i, sny+j) = 0.D0
                trfldb(snx+j, sny+1-i) = trfldb(snx+j, sny+1-i) + negone
     +            *tmpb2
              ENDDO
            ENDDO
          END IF
          ...
          IF (branch .EQ. 0) THEN
            DO j=oly,1,-1
              DO i=olx,1,-1
                tmpb1 = trfldb(1-i, sny+j)
                trfldb(1-i, sny+j) = 0.D0
                trfldb(1-j, sny+1-i) = trfldb(1-j, sny+1-i) + negone*
     +            tmpb1
              ENDDO
            ENDDO
          END IF
          ...
          IF (branch .EQ. 0) THEN
            DO j=oly,1,-1
              DO i=olx,1,-1
                tmpb0 = trfldb(snx+i, 1-j)
                trfldb(snx+i, 1-j) = 0.D0
                trfldb(snx+j, i) = trfldb(snx+j, i) + negone*tmpb0
              ENDDO
            ENDDO
          END IF
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            DO j=oly,1,-1
              DO i=olx,1,-1
                tmpb = trfldb(1-i, 1-j)
                trfldb(1-i, 1-j) = 0.D0
                trfldb(1-j, i) = trfldb(1-j, i) + negone*tmpb
              ENDDO
            ENDDO
          END IF
...
C         x T(sNx,0) =====>x T(sNx+1,    0)
          ...
          IF (northeastcorner) THEN
            DO j=oly,1,-1
              DO i=olx,1,-1
                tmpb6 = trfldb(snx+i, sny+j)
                trfldb(snx+i, sny+j) = 0.D0
                trfldb(snx+1-j, sny+i) = trfldb(snx+1-j, sny+i) + negone
     +            *tmpb6
              ENDDO
            ENDDO
          END IF
          ...
          IF (branch .EQ. 0) THEN
            DO j=oly,1,-1
              DO i=olx,1,-1
                tmpb5 = trfldb(1-i, sny+j)
                trfldb(1-i, sny+j) = 0.D0
                trfldb(j, sny+i) = trfldb(j, sny+i) + negone*tmpb5
              ENDDO
            ENDDO
          END IF
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            DO j=oly,1,-1
              DO i=olx,1,-1
                tmpb4 = trfldb(snx+i, 1-j)
                trfldb(snx+i, 1-j) = 0.D0
                trfldb(snx+1-j, 1-i) = trfldb(snx+1-j, 1-i) + negone*
     +            tmpb4
              ENDDO
            ENDDO
          END IF
          ...
          IF (branch .EQ. 0) THEN
            DO j=oly,1,-1
              DO i=olx,1,-1
                tmpb3 = trfldb(1-i, 1-j)
                trfldb(1-i, 1-j) = 0.D0
                trfldb(j, 1-i) = trfldb(j, 1-i) + negone*tmpb3
              ENDDO
            ENDDO
          END IF
        ELSE
