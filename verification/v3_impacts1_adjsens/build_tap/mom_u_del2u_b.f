...
C
C  Differentiation of mom_u_del2u in reverse (adjoint) mode:
C   gradient     of useful results: ufld del2u
C   with respect to varying inputs: ufld
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_U_DEL2U_B(bi, bj, k, ufld, ufldb, hfacz, h0facz, 
     +                         del2u, del2ub, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
      ...
      IF (no_slip_sides) THEN
        DO j=sny+oly-1,1-oly+1,-1
          DO i=snx+olx-1,1-olx+1,-1
            ...
            ufldb(i, j) = ufldb(i, j) - recip_deepfac2c(k)*
     +        sidedragfactor*maskw(i, j, k, bi, bj)*recip_hfacw(i, j, k
     +        , bi, bj)*recip_raw(i, j, bi, bj)*(hfaczcloseds*dxv(i, j, 
     +        bi, bj)*recip_dyu(i, j, bi, bj)+hfaczclosedn*dxv(i, j+1, 
     +        bi, bj)*recip_dyu(i, j+1, bi, bj))*del2ub(i, j)
          ENDDO
        ENDDO
      END IF
      ...
      DO j=sny+oly-1,1-oly+1,-1
        DO i=snx+olx-1,1-olx+1,-1
          tempb = recip_raw(i, j, bi, bj)*recip_drf(k)*recip_deepfac2c(k
     +      )*recip_hfacw(i, j, k, bi, bj)*maskw(i, j, k, bi, bj)*del2ub
     +      (i, j)
          del2ub(i, j) = 0.D0
          fzonb(i, j) = fzonb(i, j) + tempb
          fzonb(i-1, j) = fzonb(i-1, j) - tempb
          fmerb(i, j+1) = fmerb(i, j+1) + tempb
          fmerb(i, j) = fmerb(i, j) - tempb
        ENDDO
      ENDDO
      DO j=sny+oly,1-oly+1,-1
        DO i=snx+olx-1,1-olx+1,-1
          tempb = drf(k)*hfacz(i, j)*dxv(i, j, bi, bj)*recip_dyu(i, j, 
     +      bi, bj)*fmerb(i, j)
          fmerb(i, j) = 0.D0
          ufldb(i, j) = ufldb(i, j) + tempb
          ufldb(i, j-1) = ufldb(i, j-1) - tempb
        ENDDO
      ENDDO
      DO j=sny+oly-1,1-oly+1,-1
        DO i=snx+olx-1,1-olx,-1
          tempb = hfacc(i, j, k, bi, bj)*dyf(i, j, bi, bj)*recip_dxf(i, 
     +      j, bi, bj)*drf(k)*sqcosfacu(j, bi, bj)*fzonb(i, j)
          fzonb(i, j) = 0.D0
