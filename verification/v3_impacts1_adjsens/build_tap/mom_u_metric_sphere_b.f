...
C
C  Differentiation of mom_u_metric_sphere in reverse (adjoint) mode:
C   gradient     of useful results: vfld ufld umetricterms
C   with respect to varying inputs: vfld ufld umetricterms
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_U_METRIC_SPHERE_B(bi, bj, k, ufld, ufldb, vfld, 
     +                                 vfldb, umetricterms, 
     +                                 umetrictermsb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C  i,j                  :: loop indices
      ...
      REAL*8 tempb
      REAL*8 tempb0
      DO j=sny+oly-1,1-olx,-1
        DO i=snx+olx,1-olx+1,-1
          tempb = recip_rsphere*0.25*recip_deepfacc(k)*tanphiatu(i, j, 
     +      bi, bj)*umetrictermsb(i, j)
          umetrictermsb(i, j) = 0.D0
          ufldb(i, j) = ufldb(i, j) + (vfld(i, j)+vfld(i-1, j)+vfld(i, j
     +      +1)+vfld(i-1, j+1))*tempb
