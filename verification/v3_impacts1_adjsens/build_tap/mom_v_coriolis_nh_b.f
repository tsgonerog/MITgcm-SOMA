...
C
C  Differentiation of mom_v_coriolis_nh in reverse (adjoint) mode:
C   gradient     of useful results: vcoriolisterm wfld
C   with respect to varying inputs: vcoriolisterm wfld
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_V_CORIOLIS_NH_B(bi, bj, k, wfld, wfldb, 
...
C  i,j                  :: loop indices
      ...
      DO j=sny+oly,2-oly,-1
        DO i=snx+olx,1-olx,-1
          tempb = -(gravitysign*0.5*vcoriolistermb(i, j))
          vcoriolistermb(i, j) = 0.D0
          tempb0 = fcoricos(i, j, bi, bj)*0.5*anglesinc(i, j, bi, bj)*
     +      tempb
          tempb1 = fcoricos(i, j-1, bi, bj)*0.5*anglesinc(i, j-1, bi, bj
     +      )*tempb
          wfldb(i, j-1, k, bi, bj) = wfldb(i, j-1, k, bi, bj) + 
     +      rvel2wunit(k)*tempb1
          wfldb(i, j-1, kp1, bi, bj) = wfldb(i, j-1, kp1, bi, bj) + 
     +      rvel2wunit(kp1)*wmsk*tempb1
          wfldb(i, j, k, bi, bj) = wfldb(i, j, k, bi, bj) + rvel2wunit(k
     +      )*tempb0
          wfldb(i, j, kp1, bi, bj) = wfldb(i, j, kp1, bi, bj) + 
     +      rvel2wunit(kp1)*wmsk*tempb0
        ENDDO
      ENDDO
      END
