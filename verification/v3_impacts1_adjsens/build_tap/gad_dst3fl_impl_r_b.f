...
C
C  Differentiation of gad_dst3fl_impl_r in reverse (adjoint) mode:
C   gradient     of useful results: e5d b5d d5d a5d rtrans c5d
C   with respect to varying inputs: e5d b5d d5d a5d rtrans c5d
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE GAD_DST3FL_IMPL_R_B(bi, bj, k, imin, imax, jmin, jmax, 
     +                               deltatarg, rtrans, rtransb, 
     +                               recip_hfac, tfld, a5d, a5db, b5d, 
     +                               b5db, c5d, c5db, d5d, d5db, e5d, 
     +                               e5db, mythid)
      IMPLICIT NONE
C
C     == Global variables ===
...
C     rC4km, rC4kp :: high order contributions
      ...
      REAL*8 wcflb, rcenterb, rupwindb
      REAL*8 lowfac(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 lowfacb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 highfac(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 highfacb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 rc4km, rc4kp
      REAL*8 rc4kmb, rc4kpb
      REAL*8 mskm, mskp, maskm2, maskp1
      REAL*8 rj, rjh, cl1, ch3, cm2, th1, th2
      REAL*8 cl1b, ch3b, cm2b
      REAL*8 deltatcfl
      INTRINSIC MAX
      INTRINSIC MIN
      INTRINSIC ABS
...
C       lowFac & highFac are set such as Psi*Rj = lowFac*Rj + highFac*Rjh
        DO j=jmin,jmax
          DO i=imin,imax
            ...
            END IF
            CALL PUSHREAL8(wcfl)
            wcfl = deltatcfl*abs0*recip_ra(i, j, bi, bj)*recip_drc(k)*
...
C
            ...
            IF (rtrans(i, j) .GT. 0.d0) THEN
              rjh = (tfld(i, j, k-1)-tfld(i, j, km2))*maskc(i, j, km2, 
     +          bi, bj)
            ELSE
              rjh = (tfld(i, j, kp1)-tfld(i, j, k))*maskc(i, j, kp1, bi
     +          , bj)
            END IF
...
C-          2nd case: cL1+theta*cH3 = min of the three = Psi
              ...
              ELSE IF (th2 .LT. th1 .AND. th2 .LE. rj) THEN
...
C--    Add centered & upwind contributions
        ...
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            lowfacb(ii2, ii1) = 0.D0
          ENDDO
        ENDDO
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            highfacb(ii2, ii1) = 0.D0
          ENDDO
        ENDDO
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            tempb = recip_hfac(i, j, k-1)*deltatarg(k-1)*recip_drf(k-1)*
     +        recip_deepfac2c(k-1)*recip_rhofacc(k-1)*d5db(i, j, k-1)
            rc4kpb = tempb - deltatarg(k-1)*recip_drf(k-1)*
     +        recip_deepfac2c(k-1)*recip_hfac(i, j, k-1)*recip_rhofacc(k
     +        -1)*e5db(i, j, k-1)
            rcenterb = tempb
            rupwindb = -tempb
            tempb = recip_hfac(i, j, k-1)*deltatarg(k-1)*recip_drf(k-1)*
     +        recip_deepfac2c(k-1)*recip_rhofacc(k-1)*c5db(i, j, k-1)
            rcenterb = rcenterb + tempb
            rupwindb = rupwindb + tempb
            rc4kmb = tempb
            tempb = -(recip_hfac(i, j, k)*deltatarg(k)*recip_drf(k)*
     +        recip_deepfac2c(k)*recip_rhofacc(k)*c5db(i, j, k))
            rc4kpb = rc4kpb + deltatarg(k)*recip_drf(k)*recip_deepfac2c(
     +        k)*recip_hfac(i, j, k)*recip_rhofacc(k)*d5db(i, j, k) + 
     +        tempb
            rcenterb = rcenterb + tempb
            rupwindb = rupwindb - tempb
            tempb = -(recip_hfac(i, j, k)*deltatarg(k)*recip_drf(k)*
     +        recip_deepfac2c(k)*recip_rhofacc(k)*b5db(i, j, k))
            rc4kmb = rc4kmb + tempb - deltatarg(k-1)*recip_drf(k-1)*
     +        recip_deepfac2c(k-1)*recip_hfac(i, j, k-1)*recip_rhofacc(k
     +        -1)*b5db(i, j, k-1) + deltatarg(k)*recip_drf(k)*
     +        recip_deepfac2c(k)*recip_hfac(i, j, k)*recip_rhofacc(k)*
     +        a5db(i, j, k)
            rcenterb = rcenterb + tempb
            rupwindb = rupwindb + tempb
            rcenter = 0.5d0*rtrans(i, j)*recip_ra(i, j, bi, bj)*rksign
            mskp = maskc(i, j, kp1, bi, bj)*maskp1
            highfacb(i, j) = highfacb(i, j) + (rcenter-abs3)*mskp*rc4kpb
            tempb = highfac(i, j)*mskp*rc4kpb
            rcenterb = rcenterb + tempb
            abs3b = -tempb
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREAL8(abs3)
              rcenterb = rcenterb + abs3b
            ELSE
              CALL POPREAL8(abs3)
              rcenterb = rcenterb - abs3b
            END IF
            mskm = maskc(i, j, km2, bi, bj)*maskm2
            highfacb(i, j) = highfacb(i, j) + (rcenter+abs2)*mskm*rc4kmb
            tempb = highfac(i, j)*mskm*rc4kmb
            rcenterb = rcenterb + tempb
            abs2b = tempb
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREAL8(abs2)
              rcenterb = rcenterb + abs2b
            ELSE
              CALL POPREAL8(abs2)
              rcenterb = rcenterb - abs2b
            END IF
            lowfacb(i, j) = lowfacb(i, j) - abs1*2.d0*rupwindb
            abs1b = (0.5d0-lowfac(i, j))*2.d0*rupwindb
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREAL8(abs1)
              rcenterb = rcenterb + abs1b
            ELSE
              CALL POPREAL8(abs1)
              rcenterb = rcenterb - abs1b
            END IF
            rtransb(i, j) = rtransb(i, j) + rksign*0.5d0*recip_ra(i, j, 
     +        bi, bj)*rcenterb
          ENDDO
        ENDDO
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            ...
            ELSE
              ...
              cm2b = 0.D0
            END IF
            tempb = cm2b/(wcfl+1.d-20)
            wcflb = (1.0-(wcfl+1.d0)/(wcfl+1.d-20))*tempb - 2*wcfl*
     +        onesixth*ch3b - ((1.d0-wcfl)*onesixth+(2.d0-wcfl)*onesixth
     +        )*cl1b
            CALL POPREAL8(wcfl)
            abs0b = deltatcfl*recip_rhofacf(k)*recip_ra(i, j, bi, bj)*
     +        recip_drc(k)*recip_deepfac2f(k)*wcflb
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              rtransb(i, j) = rtransb(i, j) + abs0b
            ELSE
              rtransb(i, j) = rtransb(i, j) - abs0b
            END IF
          ENDDO
