...
C
C  Differentiation of momentum_correction_step in reverse (adjoint) mode:
C   gradient     of useful results: etan uvel vvel gu gv
C   with respect to varying inputs: etan uvel vvel gu gv
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE MOMENTUM_CORRECTION_STEP_B(mytime, myiter, mythid)
      IMPLICIT NONE
C     == Global variables ===
CBOP
C    !ROUTINE: SIZE.h
...
C-      Update velocity fields:  V(n) = V** - dt * grad Eta
     ...
     +                           phisurfx, phisurfy, mytime, myiter, 
     +                           mythid)
...
C     Apply EXCH to horiz velocity before integrating continuity (-> wVel)
      IF (applyexchuv_early) CALL EXCH_UV_3D_RL_B(uvel, uvelb, vvel, 
     +                                            vvelb, .true., nr, 
     +                                            mythid)
      ...
      DO bj=mybyhi(mythid),mybylo(mythid),-1
        DO bi=mybxhi(mythid),mybxlo(mythid),-1
          ...
          IF (branch .EQ. 0) THEN
            ...
            CALL CORRECTION_STEP_B(bi, bj, imin, imax, jmin, jmax, 
     +                             phisurfx, phisurfxb, phisurfy, 
     +                             phisurfyb, mytime, myiter, mythid)
            CALL CALC_GRAD_PHI_SURF_B(bi, bj, imin, imax, jmin, jmax, 
     +                                etan, etanb, phisurfx, phisurfxb, 
     +                                phisurfy, phisurfyb, mythid)
