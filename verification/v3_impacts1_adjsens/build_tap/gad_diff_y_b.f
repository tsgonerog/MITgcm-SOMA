...
C
C  Differentiation of gad_diff_y in reverse (adjoint) mode:
C   gradient     of useful results: tracer dfy
C   with respect to varying inputs: tracer dfy
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_DIFF_Y_B(bi, bj, k, ya, diffkh, tracer, tracerb, 
     +                        dfy, dfyb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
...
C  dfx                  :: meridional diffusive flux
      ...
      REAL*8 dfyb(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j
      ...
      DO j=sny+oly,1-oly+1,-1
        DO i=snx+olx,1-olx,-1
          tempb = -(recip_dyc(i, j, bi, bj)*ya(i, j)*diffkh*
     +      recip_deepfacc(k)*dfyb(i, j))
          dfyb(i, j) = 0.D0
