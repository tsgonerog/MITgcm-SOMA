...
C     == Local variables ==
      ...
      INTEGER i,j
      INTEGER iarr
      Real*8     tmpUE(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8     tmpVN(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8     tmpUX(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8     tmpVY(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
CEOP
      ...
      DO bj = myByLo(myThid),myByHi(myThid)
       DO bi = myBxLo(myThid),myBxHi(myThid)
        DO j = 1-OLy,sNy+OLy
         DO i = 1-OLx,sNx+OLx
          tmpUE(i,j,bi,bj) = 0.D0
          tmpVN(i,j,bi,bj) = 0.D0
          tmpUX(i,j,bi,bj) = 0.D0
          tmpVY(i,j,bi,bj) = 0.D0
         ENDDO
        ENDDO
       ENDDO
      ENDDO
