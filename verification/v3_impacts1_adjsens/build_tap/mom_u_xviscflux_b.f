...
C
C  Differentiation of mom_u_xviscflux in reverse (adjoint) mode:
C   gradient     of useful results: visca4_d ufld del2u viscah_d
C                xviscfluxu
C   with respect to varying inputs: visca4_d ufld del2u viscah_d
C                xviscfluxu
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_U_XVISCFLUX_B(bi, bj, k, ufld, ufldb, del2u, del2ub
     +                             , xviscfluxu, xviscfluxub, viscah_d, 
     +                             viscah_db, visca4_d, visca4_db, 
     +                             mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
...
C  xViscFluxU           :: viscous fluxes
      ...
      REAL*8 xviscfluxub(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j
      ...
      DO j=sny+oly-1,1-oly,-1
        DO i=snx+olx-1,1-olx,-1
          tempb = dyf(i, j, bi, bj)*drf(k)*hfacc(i, j, k, bi, bj)*
     +      recip_dxf(i, j, bi, bj)*xviscfluxub(i, j)
          xviscfluxub(i, j) = 0.D0
          tempb0 = sqcosfacu(j, bi, bj)*tempb
          tempb2 = -(cosfacu(j, bi, bj)*tempb)
          viscah_db(i, j) = viscah_db(i, j) + (ufld(i+1, j)-ufld(i, j))*
     +      tempb2
          tempb3 = viscah_d(i, j)*tempb2
          ufldb(i+1, j) = ufldb(i+1, j) + tempb3
          ufldb(i, j) = ufldb(i, j) - tempb3
          visca4_db(i, j) = visca4_db(i, j) + (del2u(i+1, j)-del2u(i, j)
     +      )*tempb0
          tempb1 = visca4_d(i, j)*tempb0
          del2ub(i+1, j) = del2ub(i+1, j) + tempb1
          del2ub(i, j) = del2ub(i, j) - tempb1
        ENDDO
      ENDDO
      END
