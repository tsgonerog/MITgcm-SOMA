...
C
C  Differentiation of convectively_mixtracer in reverse (adjoint) mode:
C   gradient     of useful results: tracer
C   with respect to varying inputs: tracer
C
C
C
C
...
C !INTERFACE:
      SUBROUTINE CONVECTIVELY_MIXTRACER_B(bi, bj, k, weighta, weightb, 
     +                                    tracer, tracerb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C     delTrac  :: Difference between tracer in each layer
      ...
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx,-1
          deltracb = weighta(i, j)*tracerb(i, j, k-1, bi, bj) - weightb(
     +      i, j)*tracerb(i, j, k, bi, bj)
          tracerb(i, j, k, bi, bj) = tracerb(i, j, k, bi, bj) + deltracb
          tracerb(i, j, k-1, bi, bj) = tracerb(i, j, k-1, bi, bj) - 
     +      deltracb
        ENDDO
      ENDDO
      END
