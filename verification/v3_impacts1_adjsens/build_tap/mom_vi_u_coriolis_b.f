...
C
C  Differentiation of mom_vi_u_coriolis in reverse (adjoint) mode:
C   gradient     of useful results: vfld ucoriolisterm omega3
C   with respect to varying inputs: vfld ucoriolisterm omega3
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE MOM_VI_U_CORIOLIS_B(bi, bj, k, selectvortscheme, 
     +                               usejamartmomadv, vfld, vfldb, 
     +                               omega3, omega3b, hfacz, r_hfacz, 
     +                               ucoriolisterm, ucoriolistermb, 
     +                               mythid)
      IMPLICIT NONE
C
C     == Global variables ==
...
C     == Routine arguments ==
      ...
      REAL*8 ucoriolistermb(1-olx:snx+olx, 1-oly:sny+oly)
...
C     msgBuf :: Informational/error meesage buffer
      ...
      REAL*8 vbarxyb, vbarxmb, vbarxpb
      REAL*8 vort3u
      REAL*8 vort3ub
      REAL*8 vort3mj, vort3ij, vort3mp, vort3ip
      REAL*8 vort3mjb, vort3ijb, vort3mpb, vort3ipb
...
C
        DO j=1-oly,sny+oly-1
          DO i=2-olx,snx+olx
            CALL PUSHREAL8(vbarxy)
            vbarxy = 0.25*(vfld(i, j)*dxg(i, j, bi, bj)*hfacs(i, j, k, 
     +        bi, bj)+vfld(i-1, j)*dxg(i-1, j, bi, bj)*hfacs(i-1, j, k, 
     +        bi, bj)+(vfld(i, j+1)*dxg(i, j+1, bi, bj)*hfacs(i, j+1, k
     +        , bi, bj)+vfld(i-1, j+1)*dxg(i-1, j+1, bi, bj)*hfacs(i-1, 
     +        j+1, k, bi, bj)))
            IF (upwindvort3) THEN
              IF (vbarxy .GT. 0.) THEN
                CALL PUSHREAL8(vort3u)
...
C
        DO j=1-oly,sny+oly-1
          DO i=2-olx,snx+olx
            IF (epsil .LT. hfacz(i, j) + hfacz(i, j+1)) THEN
              CALL PUSHREAL8(max1)
              max1 = hfacz(i, j) + hfacz(i, j+1)
            ...
            END IF
            CALL PUSHREAL8(vbarxy)
            vbarxy = 0.5*(vfld(i, j)*dxg(i, j, bi, bj)*hfacz(i, j)+vfld(
     +        i-1, j)*dxg(i-1, j, bi, bj)*hfacz(i, j)+(vfld(i, j+1)*dxg(
     +        i, j+1, bi, bj)*hfacz(i, j+1)+vfld(i-1, j+1)*dxg(i-1, j+1
     +        , bi, bj)*hfacz(i, j+1)))/max1
            IF (upwindvort3) THEN
              IF (vbarxy .GT. 0.) THEN
                CALL PUSHREAL8(vort3u)
                vort3u = omega3(i, j)
...
C
        DO j=1-oly,sny+oly-1
          DO i=2-olx,snx+olx
            CALL PUSHREAL8(vbarxm)
            vbarxm = 0.5*(vfld(i, j)*dxg(i, j, bi, bj)*hfacs(i, j, k, bi
     +        , bj)+vfld(i-1, j)*dxg(i-1, j, bi, bj)*hfacs(i-1, j, k, bi
     +        , bj))
            CALL PUSHREAL8(vbarxp)
            vbarxp = 0.5*(vfld(i, j+1)*dxg(i, j+1, bi, bj)*hfacs(i, j+1
     +        , k, bi, bj)+vfld(i-1, j+1)*dxg(i-1, j+1, bi, bj)*hfacs(i-
     +        1, j+1, k, bi, bj))
            IF (upwindvort3) THEN
...
C
      IF (usejamartmomadv) THEN
        DO j=1-oly,sny+oly-1
          DO i=2-olx,snx+olx-1
            y1 = hfacs(i, j, k, bi, bj) + hfacs(i-1, j, k, bi, bj) + (
     +        hfacs(i, j+1, k, bi, bj)+hfacs(i-1, j+1, k, bi, bj))
        ...
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx-1,2-olx,-1
            ucoriolistermb(i, j) = hfacw(i, j, k, bi, bj)*4.d0*
     +        ucoriolistermb(i, j)/max2
        ...
        IF (branch .EQ. 0) THEN
          DO j=sny+oly-1,1-oly,-1
            DO i=snx+olx,2-olx,-1
              tempb = recip_dxc(i, j, bi, bj)*maskw(i, j, k, bi, bj)*
     +          ucoriolistermb(i, j)
              ucoriolistermb(i, j) = 0.D0
              vort3ub = vbarxy*tempb
              vbarxyb = vort3u*tempb
              CALL POPCONTROL2B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREAL8(vort3u)
                omega3b(i, j) = omega3b(i, j) + r_hfacz(i, j)*vort3ub
              ELSE IF (branch .EQ. 1) THEN
                CALL POPREAL8(vort3u)
                omega3b(i, j+1) = omega3b(i, j+1) + r_hfacz(i, j+1)*
     +            vort3ub
              ELSE
                CALL POPREAL8(vort3u)
                omega3b(i, j) = omega3b(i, j) + r_hfacz(i, j)*0.5*
     +            vort3ub
                omega3b(i, j+1) = omega3b(i, j+1) + r_hfacz(i, j+1)*0.5*
     +            vort3ub
              END IF
              CALL POPREAL8(vbarxy)
              tempb = 0.25*vbarxyb
              vfldb(i, j) = vfldb(i, j) + dxg(i, j, bi, bj)*hfacs(i, j, 
     +          k, bi, bj)*tempb
              vfldb(i-1, j) = vfldb(i-1, j) + dxg(i-1, j, bi, bj)*hfacs(
     +          i-1, j, k, bi, bj)*tempb
              vfldb(i, j+1) = vfldb(i, j+1) + dxg(i, j+1, bi, bj)*hfacs(
     +          i, j+1, k, bi, bj)*tempb
              vfldb(i-1, j+1) = vfldb(i-1, j+1) + dxg(i-1, j+1, bi, bj)*
     +          hfacs(i-1, j+1, k, bi, bj)*tempb
            ENDDO
          ENDDO
        ELSE
          DO j=sny+oly-1,1-oly,-1
     ...
     +          ucoriolistermb(i, j)
              ...
              IF (branch .EQ. 0) THEN
                CALL POPREAL8(vort3u)
                omega3b(i, j) = omega3b(i, j) + vort3ub
              ELSE IF (branch .EQ. 1) THEN
                CALL POPREAL8(vort3u)
                omega3b(i, j+1) = omega3b(i, j+1) + vort3ub
              ELSE
                CALL POPREAL8(vort3u)
                omega3b(i, j) = omega3b(i, j) + 0.5*vort3ub
                omega3b(i, j+1) = omega3b(i, j+1) + 0.5*vort3ub
              END IF
              CALL POPREAL8(vbarxy)
              tempb = 0.5*vbarxyb/max1
              vfldb(i, j) = vfldb(i, j) + hfacz(i, j)*dxg(i, j, bi, bj)*
     +          tempb
              vfldb(i-1, j) = vfldb(i-1, j) + hfacz(i, j)*dxg(i-1, j, bi
     +          , bj)*tempb
              vfldb(i, j+1) = vfldb(i, j+1) + hfacz(i, j+1)*dxg(i, j+1, 
     +          bi, bj)*tempb
              vfldb(i-1, j+1) = vfldb(i-1, j+1) + hfacz(i, j+1)*dxg(i-1
     +          , j+1, bi, bj)*tempb
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREAL8(max1)
      ...
      ELSE IF (branch .EQ. 2) THEN
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx,2-olx,-1
            vort3ub = recip_dxc(i, j, bi, bj)*maskw(i, j, k, bi, bj)*
     +        ucoriolistermb(i, j)
            ucoriolistermb(i, j) = 0.D0
            CALL POPCONTROL2B(branch)
            IF (branch .EQ. 0) THEN
              tempb = r_hfacz(i, j)*vort3ub
              vbarxmb = omega3(i, j)*tempb
              omega3b(i, j) = omega3b(i, j) + vbarxm*tempb
              vbarxpb = 0.D0
            ELSE IF (branch .EQ. 1) THEN
              tempb = r_hfacz(i, j+1)*vort3ub
              vbarxpb = omega3(i, j+1)*tempb
              omega3b(i, j+1) = omega3b(i, j+1) + vbarxp*tempb
              vbarxmb = 0.D0
            ELSE
              tempb = r_hfacz(i, j)*0.5d0*vort3ub
              tempb0 = r_hfacz(i, j+1)*0.5d0*vort3ub
              vbarxpb = omega3(i, j+1)*tempb0
              omega3b(i, j+1) = omega3b(i, j+1) + vbarxp*tempb0
              vbarxmb = omega3(i, j)*tempb
              omega3b(i, j) = omega3b(i, j) + vbarxm*tempb
            END IF
            CALL POPREAL8(vbarxp)
            vfldb(i, j+1) = vfldb(i, j+1) + dxg(i, j+1, bi, bj)*hfacs(i
     +        , j+1, k, bi, bj)*0.5*vbarxpb
            vfldb(i-1, j+1) = vfldb(i-1, j+1) + dxg(i-1, j+1, bi, bj)*
     +        hfacs(i-1, j+1, k, bi, bj)*0.5*vbarxpb
            CALL POPREAL8(vbarxm)
            vfldb(i, j) = vfldb(i, j) + dxg(i, j, bi, bj)*hfacs(i, j, k
     +        , bi, bj)*0.5*vbarxmb
            vfldb(i-1, j) = vfldb(i-1, j) + dxg(i-1, j, bi, bj)*hfacs(i-
     +        1, j, k, bi, bj)*0.5*vbarxmb
          ENDDO
        ENDDO
      ELSE
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx-1,2-olx,-1
            tempb0 = 0.25d0*recip_dxc(i, j, bi, bj)*maskw(i, j, k, bi, 
     +        bj)*ucoriolistermb(i, j)
            ucoriolistermb(i, j) = 0.D0
            vort3mjb = tempb0
            vort3ijb = tempb0
            vort3mpb = tempb0
            vort3ipb = tempb0
            tempb0 = onethird*dxg(i, j+1, bi, bj)*hfacs(i, j+1, k, bi, 
     +        bj)*vort3ipb
            tempb = vfld(i, j+1)*tempb0
            vfldb(i, j+1) = vfldb(i, j+1) + (r_hfacz(i, j+1)*omega3(i, j
     +        +1)+r_hfacz(i, j)*omega3(i, j)+r_hfacz(i+1, j+1)*omega3(i+
     +        1, j+1))*tempb0
     ...
     +        tempb
            tempb0 = onethird*dxg(i-1, j+1, bi, bj)*hfacs(i-1, j+1, k, 
     +        bi, bj)*vort3mpb
            tempb = vfld(i-1, j+1)*tempb0
            vfldb(i-1, j+1) = vfldb(i-1, j+1) + (r_hfacz(i, j+1)*omega3(
     +        i, j+1)+r_hfacz(i, j)*omega3(i, j)+r_hfacz(i-1, j+1)*
     +        omega3(i-1, j+1))*tempb0
            omega3b(i, j+1) = omega3b(i, j+1) + r_hfacz(i, j+1)*tempb
            omega3b(i, j) = omega3b(i, j) + r_hfacz(i, j)*tempb
            omega3b(i-1, j+1) = omega3b(i-1, j+1) + r_hfacz(i-1, j+1)*
     +        tempb
            tempb0 = onethird*dxg(i, j, bi, bj)*hfacs(i, j, k, bi, bj)*
     +        vort3ijb
            tempb = vfld(i, j)*tempb0
            vfldb(i, j) = vfldb(i, j) + (r_hfacz(i, j)*omega3(i, j)+
     +        r_hfacz(i, j+1)*omega3(i, j+1)+r_hfacz(i+1, j)*omega3(i+1
     +        , j))*tempb0
            omega3b(i, j) = omega3b(i, j) + r_hfacz(i, j)*tempb
            omega3b(i, j+1) = omega3b(i, j+1) + r_hfacz(i, j+1)*tempb
            omega3b(i+1, j) = omega3b(i+1, j) + r_hfacz(i+1, j)*tempb
            tempb0 = onethird*dxg(i-1, j, bi, bj)*hfacs(i-1, j, k, bi, 
     +        bj)*vort3mjb
            tempb = vfld(i-1, j)*tempb0
            vfldb(i-1, j) = vfldb(i-1, j) + (r_hfacz(i, j)*omega3(i, j)+
     +        r_hfacz(i, j+1)*omega3(i, j+1)+r_hfacz(i-1, j)*omega3(i-1
     +        , j))*tempb0
            omega3b(i, j) = omega3b(i, j) + r_hfacz(i, j)*tempb
            omega3b(i, j+1) = omega3b(i, j+1) + r_hfacz(i, j+1)*tempb
