...
C
C  Differentiation of gad_c4_adv_y in reverse (adjoint) mode:
C   gradient     of useful results: vt vtrans
C   with respect to varying inputs: tracer vt vtrans
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_C4_ADV_Y_B(bi, bj, k, vtrans, vtransb, masklocs, 
...
C  Rjjm,Rjjp            :: second differences at j-1,j
     ...
     +      onesixth*(rjjp+rjjm))*0.5d0*vtb(i, j)
          tempb = vtrans(i, j)*0.5d0*vtb(i, j)
          tempb0 = onesixth*0.5d0*(1.d0-masks(i, j-1, k, bi, bj)*masks(i
     +      , j+1, k, bi, bj))*vtb(i, j)
          vtb(i, j) = 0.D0
