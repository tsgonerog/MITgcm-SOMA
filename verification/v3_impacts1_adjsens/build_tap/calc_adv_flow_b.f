...
C
C  Differentiation of calc_adv_flow in reverse (adjoint) mode:
C   gradient     of useful results: vfld ufld utrans rtrans vtrans
C                rtranskp wfld
C   with respect to varying inputs: vfld ufld utrans rtrans vtrans
C                rtranskp wfld
C
C
C
...
C     !INTERFACE:
      SUBROUTINE CALC_ADV_FLOW_B(ufld, ufldb, vfld, vfldb, wfld, wfldb, 
     +                           rtrans, rtransb, utrans, utransb, 
     +                           vtrans, vtransb, rtranskp, rtranskpb, 
     +                           maskup, xa, ya, k, bi, bj, mythid)
      IMPLICIT NONE
C     == GLobal variables ==
CBOP
C    !ROUTINE: SIZE.h
...
C       anelastic: rTrans is scaled by rhoFacF (~ mass transport)
        DO j=1-oly,sny+oly
          DO i=1-olx,snx+olx
            CALL PUSHREAL8(maskup(i, j))
            maskup(i, j) = maskc(i, j, k-1, bi, bj)*maskc(i, j, k, bi, 
     +        bj)
          ENDDO
        ENDDO
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            wfldb(i, j, k) = wfldb(i, j, k) + maskup(i, j)*ra(i, j, bi, 
     +        bj)*deepfac2f(k)*rhofacf(k)*rtransb(i, j)
            rtransb(i, j) = 0.D0
            CALL POPREAL8(maskup(i, j))
          ENDDO
        ENDDO
      END IF
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx,-1
          vfldb(i, j, k) = vfldb(i, j, k) + ya(i, j)*rhofacc(k)*vtransb(
     +      i, j)
          vtransb(i, j) = 0.D0
          ufldb(i, j, k) = ufldb(i, j, k) + xa(i, j)*rhofacc(k)*utransb(
     +      i, j)
          utransb(i, j) = 0.D0
        ENDDO
      ENDDO
      ...
      IF (branch .EQ. 0) THEN
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            wfldb(i, j, k+1) = wfldb(i, j, k+1) + ra(i, j, bi, bj)*
     +        deepfac2f(k+1)*rhofacf(k+1)*maskc(i, j, k, bi, bj)*maskc(i
     +        , j, k+1, bi, bj)*rtranskpb(i, j)
            rtranskpb(i, j) = 0.D0
          ENDDO
        ENDDO
      ELSE
