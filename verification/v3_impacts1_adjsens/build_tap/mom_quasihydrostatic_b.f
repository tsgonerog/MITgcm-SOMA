...
C
C  Differentiation of mom_quasihydrostatic in reverse (adjoint) mode:
C   gradient     of useful results: effectivebuoy vfld ufld
C   with respect to varying inputs: effectivebuoy vfld ufld
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_QUASIHYDROSTATIC_B(bi, bj, k, ufld, ufldb, vfld, 
     +                                  vfldb, effectivebuoy, 
     +                                  effectivebuoyb, mytime, myiter, 
     +                                  mythid)
      IMPLICIT NONE
C
C--   == Global data ==
...
C  effectiveBuoy :: Density (z-coord) / specific volume (p-coord) anomaly
      ...
      REAL*8 effectivebuoyb(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i, j          :: loop indices
...
C  scalingFactor :: scaling factor (from acceleration to density)
      ...
      REAL*8 gwinbuoyb(1-olx:snx+olx, 1-oly:sny+oly)
...
C
        ...
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            gwinbuoyb(ii2, ii1) = 0.D0
          ENDDO
        ENDDO
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            gwinbuoyb(i, j) = gwinbuoyb(i, j) + scalingfactor*
     +        effectivebuoyb(i, j)
          ENDDO
        ENDDO
        ...
        IF (branch .NE. 0) THEN
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              tempb1 = halfrl*recip_rsphere*recip_deepfacc(k)*gwinbuoyb(
     +          i, j)
              ufldb(i, j, k, bi, bj) = ufldb(i, j, k, bi, bj) + 2*ufld(i
     +          , j, k, bi, bj)*tempb1
              ufldb(i+1, j, k, bi, bj) = ufldb(i+1, j, k, bi, bj) + 2*
     +          ufld(i+1, j, k, bi, bj)*tempb1
              vfldb(i, j, k, bi, bj) = vfldb(i, j, k, bi, bj) + 2*vfld(i
     +          , j, k, bi, bj)*tempb1
              vfldb(i, j+1, k, bi, bj) = vfldb(i, j+1, k, bi, bj) + 2*
     +          vfld(i, j+1, k, bi, bj)*tempb1
            ENDDO
          ENDDO
        END IF
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              tempb = fcoricos(i, j, bi, bj)*gwinbuoyb(i, j)
              gwinbuoyb(i, j) = 0.D0
              tempb0 = anglecosc(i, j, bi, bj)*halfrl*tempb
              tempb1 = -(anglesinc(i, j, bi, bj)*halfrl*tempb)
              vfldb(i, j, k, bi, bj) = vfldb(i, j, k, bi, bj) + tempb1
              vfldb(i, j+1, k, bi, bj) = vfldb(i, j+1, k, bi, bj) + 
     +          tempb1
              ufldb(i, j, k, bi, bj) = ufldb(i, j, k, bi, bj) + tempb0
              ufldb(i+1, j, k, bi, bj) = ufldb(i+1, j, k, bi, bj) + 
     +          tempb0
            ENDDO
          ENDDO
        END IF
