...
C
C  Differentiation of solve_for_pressure in reverse (adjoint) mode:
C   gradient     of useful results: etan uvel vvel gu gv etah empmr
C   with respect to varying inputs: etan uvel vvel gu gv etah empmr
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE SOLVE_FOR_PRESSURE_B(mytime, myiter, mythid)
      IMPLICIT NONE
C     == Global variables
CBOP
C    !ROUTINE: SIZE.h
...
C     !LOCAL VARIABLES:
      ...
      REAL*8 sumempb, tileempb(nsx, nsy)
...
C--   Save previous solution & Initialise Vector solution and source term :
      ...
      DO bj=mybylo(mythid),mybyhi(mythid)
        DO bi=mybxlo(mythid),mybxhi(mythid)
          DO j=1-oly,sny+oly
            DO i=1-olx,snx+olx
              cg2d_x(i, j, bi, bj) = bo_surf(i, j, bi, bj)*etan(i, j, bi
     +          , bj)
              cg2d_b(i, j, bi, bj) = 0.d0
            ENDDO
          ENDDO
          IF (userealfreshwaterflux .AND. fluidiswater) THEN
            CALL PUSHREAL8(tmpfac)
            tmpfac = freesurffac*mass2runit*implicdiv2dflow
            DO j=1,sny
              DO i=1,snx
                cg2d_b(i, j, bi, bj) = tmpfac*ra(i, j, bi, bj)*empmr(i, 
     +            j, bi, bj)/deltatmom*maskinc(i, j, bi, bj)
              ENDDO
            ENDDO
          ...
          IF (putpmeinxvector) THEN
            ...
            DO j=1,sny
              DO i=1,snx
                tileemp(bi, bj) = tileemp(bi, bj) + ra(i, j, bi, bj)*
     +            empmr(i, j, bi, bj)*maskinc(i, j, bi, bj)
              ENDDO
            ENDDO
            CALL PUSHCONTROL1B(1)
          ELSE
        ...
        DO bi=mybxlo(mythid),mybxhi(mythid)
          IF (putpmeinxvector) THEN
            ...
            IF (globalarea .GT. 0.) THEN
              tmpfac = freesurffac*deltatfreesurf*mass2runit*sumemp/
     +          globalarea
              CALL PUSHCONTROL1B(1)
            ELSE
              CALL PUSHCONTROL1B(0)
            ...
            DO j=1,sny
              DO i=1,snx
                cg2d_x(i, j, bi, bj) = cg2d_x(i, j, bi, bj) - tmpfac*
     +            bo_surf(i, j, bi, bj)
              ENDDO
            ENDDO
            CALL PUSHCONTROL1B(1)
...
C--   Add source term arising from w=d/dt (p_s)
     ...
     +            freesurffac*ra(i, j, bi, bj)*deepfac2f(ks)/deltatmom/
     +            deltatfreesurf*etah(i, j, bi, bj)
              ENDDO
            ENDDO
            CALL PUSHCONTROL1B(0)
          ELSE
...
C--   Call the default CG solver
        CALL PUSHINTEGER4(nitermin)
        CALL PUSHINTEGER4(numiters)
        CALL PUSHREAL8ARRAY(cg2d_x, 70**2)
        ...
        DO bi=mybxhi(mythid),mybxlo(mythid),-1
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              cg2d_xb(i, j, bi, bj) = cg2d_xb(i, j, bi, bj) + recip_bo(i
     +          , j, bi, bj)*etanb(i, j, bi, bj)
              etanb(i, j, bi, bj) = 0.D0
            ENDDO
          ENDDO
        ENDDO
      ENDDO
      CALL EXCH_XY_RL_B(cg2d_x, cg2d_xb, mythid)
      ...
      IF (branch .EQ. 0) THEN
        DO ii1=1,nsy
          DO ii2=1,nsx
            DO ii3=1-oly,oly+sny
              DO ii4=1-olx,olx+snx
                cg2d_bb(ii4, ii3, ii2, ii1) = 0.D0
              ENDDO
            ENDDO
          ENDDO
        ENDDO
      ELSE
        CALL POPREAL8ARRAY(cg2d_x, 70**2)
        CALL POPINTEGER4(numiters)
        CALL POPINTEGER4(nitermin)
        DO ii1=1,nsy
          DO ii2=1,nsx
            DO ii3=1-oly,oly+sny
              DO ii4=1-olx,olx+snx
          ...
          ENDDO
        ENDDO
        CALL CG2D_B0(cg2d_b, cg2d_bb, cg2d_x, cg2d_xb, firstresidual, 
     +               minresidualsq, lastresidual, numiters, nitermin, 
     +               mythid)
      END IF
      DO bj=mybyhi(mythid),mybylo(mythid),-1
        DO bi=mybxhi(mythid),mybxlo(mythid),-1
          ...
          IF (branch .EQ. 0) THEN
            DO j=sny,1,-1
              DO i=snx,1,-1
                ks = ksurfc(i, j, bi, bj)
                etahb(i, j, bi, bj) = etahb(i, j, bi, bj) - ra(i, j, bi
     +            , bj)*freesurffac*deepfac2f(ks)*cg2d_bb(i, j, bi, bj)/
     +            (deltatmom*deltatfreesurf)
          ...
          ELSE
            DO j=sny,1,-1
              DO i=snx,1,-1
                ks = ksurfc(i, j, bi, bj)
                etanb(i, j, bi, bj) = etanb(i, j, bi, bj) - ra(i, j, bi
     +            , bj)*freesurffac*deepfac2f(ks)*cg2d_bb(i, j, bi, bj)/
     +            (deltatmom*deltatfreesurf)
              ENDDO
            ENDDO
      ...
      DO bj=mybyhi(mythid),mybylo(mythid),-1
        DO bi=mybxhi(mythid),mybxlo(mythid),-1
          DO k=1,nr,1
            CALL CALC_DIV_GHAT_B(bi, bj, k, cg2d_b, cg2d_bb, cg3d_b, 
     +                           mythid)
          ENDDO
          CALL POPCONTROL1B(branch)
          IF (branch .NE. 0) THEN
            ...
            DO j=sny,1,-1
              DO i=snx,1,-1
                tmpfacb = tmpfacb - bo_surf(i, j, bi, bj)*cg2d_xb(i, j, 
     +            bi, bj)
              ENDDO
            ENDDO
            CALL POPCONTROL1B(branch)
            IF (branch .NE. 0) sumempb = sumempb + freesurffac*
     +          deltatfreesurf*mass2runit*tmpfacb/globalarea
            CALL POPREAL8(tmpfac)
          END IF
        ENDDO
      ENDDO
      CALL POPCONTROL1B(branch)
      ...
      ELSE
        DO ii1=1,nsy
          ...
          ENDDO
        ENDDO
        CALL GLOBAL_SUM_TILE_RL_B(tileemp, tileempb, sumemp, sumempb, 
     +                            mythid)
      END IF
      DO bj=mybyhi(mythid),mybylo(mythid),-1
        DO bi=mybxhi(mythid),mybxlo(mythid),-1
          ...
          IF (branch .NE. 0) THEN
            DO j=sny,1,-1
              DO i=snx,1,-1
                empmrb(i, j, bi, bj) = empmrb(i, j, bi, bj) + ra(i, j, 
     +            bi, bj)*maskinc(i, j, bi, bj)*tileempb(bi, bj)
              ENDDO
            ENDDO
            tileempb(bi, bj) = 0.D0
          END IF
          ...
          IF (branch .EQ. 0) THEN
            DO j=sny,1,-1
              DO i=snx,1,-1
                empmrb(i, j, bi, bj) = empmrb(i, j, bi, bj) + tmpfac*ra(
     +            i, j, bi, bj)*maskinc(i, j, bi, bj)*cg2d_bb(i, j, bi, 
     +            bj)/deltatmom
                cg2d_bb(i, j, bi, bj) = 0.D0
              ENDDO
            ENDDO
            CALL POPREAL8(tmpfac)
          END IF
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              cg2d_bb(i, j, bi, bj) = 0.D0
              etanb(i, j, bi, bj) = etanb(i, j, bi, bj) + bo_surf(i, j, 
     +          bi, bj)*cg2d_xb(i, j, bi, bj)
              cg2d_xb(i, j, bi, bj) = 0.D0
            ENDDO
          ENDDO
        ENDDO
      ENDDO
