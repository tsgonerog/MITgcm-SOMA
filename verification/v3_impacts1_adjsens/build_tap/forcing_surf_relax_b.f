...
C
C  Differentiation of forcing_surf_relax in reverse (adjoint) mode:
C   gradient     of useful results: theta salt sst sss surfaceforcingt
C                surfaceforcings
C   with respect to varying inputs: theta salt sst sss surfaceforcingt
C                surfaceforcings
C
C
C
...
C     !INTERFACE:
      SUBROUTINE FORCING_SURF_RELAX_B(imin, imax, jmin, jmax, mytime, 
     +                                myiter, mythid)
      IMPLICIT NONE
C     === Global variables ===
CBOP
...
C
      ...
      DO bj=mybyhi(mythid),mybylo(mythid),-1
        DO bi=mybxhi(mythid),mybxlo(mythid),-1
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              tempb = -(lambdasaltclimrelax(i, j, bi, bj)*drf(ks)*hfacc(
     +          i, j, ks, bi, bj)*surfaceforcingsb(i, j, bi, bj))
              surfaceforcingsb(i, j, bi, bj) = 0.D0
              saltb(i, j, ks, bi, bj) = saltb(i, j, ks, bi, bj) + tempb
              sssb(i, j, bi, bj) = sssb(i, j, bi, bj) - tempb
              tempb = -(lambdathetaclimrelax(i, j, bi, bj)*drf(ks)*hfacc
     +          (i, j, ks, bi, bj)*surfaceforcingtb(i, j, bi, bj))
              surfaceforcingtb(i, j, bi, bj) = 0.D0
              thetab(i, j, ks, bi, bj) = thetab(i, j, ks, bi, bj) + 
     +          tempb
              sstb(i, j, bi, bj) = sstb(i, j, bi, bj) - tempb
            ENDDO
          ENDDO
        ENDDO
      ENDDO
