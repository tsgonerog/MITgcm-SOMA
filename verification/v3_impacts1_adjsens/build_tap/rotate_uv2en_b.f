...
C
C  Differentiation of rotate_uv2en_rl in reverse (adjoint) mode:
C   gradient     of useful results: ufldx vfldy
C   with respect to varying inputs: uflde vfldn
C
C
C
C
...
C--   o ROTATE_UV2EN_RS
C
      SUBROUTINE ROTATE_UV2EN_RL_B(ufldx, ufldxb, vfldy, vfldyb, uflde, 
     +                             ufldeb, vfldn, vfldnb, xy2en, 
...
C
      ...
      REAL*8 ufldxb(1-olx:snx+olx, 1-oly:sny+oly, ksize, nsx, nsy)
      REAL*8 vfldy(1-olx:snx+olx, 1-oly:sny+oly, ksize, nsx, nsy)
      REAL*8 vfldyb(1-olx:snx+olx, 1-oly:sny+oly, ksize, nsx, nsy)
      REAL*8 uflde(1-olx:snx+olx, 1-oly:sny+oly, ksize, nsx, nsy)
      REAL*8 ufldeb(1-olx:snx+olx, 1-oly:sny+oly, ksize, nsx, nsy)
      REAL*8 vfldn(1-olx:snx+olx, 1-oly:sny+oly, ksize, nsx, nsy)
      REAL*8 vfldnb(1-olx:snx+olx, 1-oly:sny+oly, ksize, nsx, nsy)
C
      INTEGER mythid
C
C     == local variables ==
C
      ...
      REAL*8 tmpub(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 tmpv(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 tmpvb(1-olx:snx+olx, 1-oly:sny+oly)
...
C     == end of interface ==
C
      IF (ksize .NE. 1 .AND. ksize .NE. nr .AND. applymask) THEN
        STOP
      ELSE
C
...
C 2b) stay at A grid velocity points (i.e. at the C grid cell center)
            ...
            DO ii3=1,ksize
              DO ii4=1-oly,oly+sny
                DO ii5=1-olx,olx+snx
                  ufldeb(ii5, ii4, ii3, ii2, ii1) = 0.D0
                ENDDO
              ENDDO
            ENDDO
          ENDDO
        ...
        DO ii1=1,nsy
          DO ii2=1,nsx
            DO ii3=1,ksize
              DO ii4=1-oly,oly+sny
                DO ii5=1-olx,olx+snx
                  vfldnb(ii5, ii4, ii3, ii2, ii1) = 0.D0
                ENDDO
              ENDDO
            ENDDO
          ENDDO
              ...
              IF (branch .EQ. 0) THEN
                DO j=sny+oly,1-oly,-1
                  DO i=snx+olx,1-olx,-1
                    tmpub(i, j) = tmpub(i, j) + anglesinc(i, j, bi, bj)*
     +                vfldnb(i, j, k, bi, bj) + anglecosc(i, j, bi, bj)*
     +                ufldeb(i, j, k, bi, bj)
                    tmpvb(i, j) = tmpvb(i, j) + anglecosc(i, j, bi, bj)*
     +                vfldnb(i, j, k, bi, bj) - anglesinc(i, j, bi, bj)*
     +                ufldeb(i, j, k, bi, bj)
                    vfldnb(i, j, k, bi, bj) = 0.D0
                    ufldeb(i, j, k, bi, bj) = 0.D0
                  ENDDO
                ENDDO
                ...
                IF (branch .EQ. 0) THEN
                  DO j=sny+oly,1-oly,-1
                    DO i=snx+olx,1-olx,-1
                      CALL POPCONTROL1B(branch)
                      IF (branch .NE. 0) THEN
                        tmpvb(i, j) = maskc(i, j, kk, bi, bj)*tmpvb(i, j
     +                    )
                        tmpub(i, j) = maskc(i, j, kk, bi, bj)*tmpub(i, j
     +                    )
                      END IF
                      vfldyb(i, j, k, bi, bj) = vfldyb(i, j, k, bi, bj) 
     +                  + tmpvb(i, j)
                      tmpvb(i, j) = 0.D0
                      ufldxb(i, j, k, bi, bj) = ufldxb(i, j, k, bi, bj) 
     +                  + tmpub(i, j)
                      tmpub(i, j) = 0.D0
                    ENDDO
                  ENDDO
     ...
     +                    )
                      END IF
                      vfldyb(i, j+1, k, bi, bj) = vfldyb(i, j+1, k, bi, 
     +                  bj) + 0.5d0*tmpvb(i, j)
                      vfldyb(i, j, k, bi, bj) = vfldyb(i, j, k, bi, bj) 
     +                  + 0.5d0*tmpvb(i, j)
                      tmpvb(i, j) = 0.D0
                      ufldxb(i+1, j, k, bi, bj) = ufldxb(i+1, j, k, bi, 
     +                  bj) + 0.5d0*tmpub(i, j)
                      ufldxb(i, j, k, bi, bj) = ufldxb(i, j, k, bi, bj) 
     +                  + 0.5d0*tmpub(i, j)
                      tmpub(i, j) = 0.D0
                    ENDDO
                    tmpvb(snx+olx, j) = 0.D0
                    tmpub(snx+olx, j) = 0.D0
                  ENDDO
                  DO i=snx+olx,1-olx,-1
                    tmpvb(i, sny+oly) = 0.D0
                    tmpub(i, sny+oly) = 0.D0
                  ENDDO
                END IF
              ELSE
                IF (branch .EQ. 1) THEN
                  DO j=sny+oly,1-oly+1,-1
                    DO i=snx+olx,1-olx+1,-1
                      CALL POPCONTROL1B(branch)
                      IF (branch .NE. 0) THEN
                        vfldyb(i, j, k, bi, bj) = masks(i, j, kk, bi, bj
     +                    )*vfldyb(i, j, k, bi, bj)
                        ufldxb(i, j, k, bi, bj) = maskw(i, j, kk, bi, bj
     +                    )*ufldxb(i, j, k, bi, bj)
                      END IF
                      tmpvb(i, j-1) = tmpvb(i, j-1) + 0.5d0*vfldyb(i, j
     +                  , k, bi, bj)
                      tmpvb(i, j) = tmpvb(i, j) + 0.5d0*vfldyb(i, j, k, 
     +                  bi, bj)
                      vfldyb(i, j, k, bi, bj) = 0.D0
                      tmpub(i-1, j) = tmpub(i-1, j) + 0.5d0*ufldxb(i, j
     +                  , k, bi, bj)
                      tmpub(i, j) = tmpub(i, j) + 0.5d0*ufldxb(i, j, k, 
     +                  bi, bj)
                      ufldxb(i, j, k, bi, bj) = 0.D0
                    ENDDO
                    vfldyb(1, j, k, bi, bj) = 0.D0
                    ufldxb(1, j, k, bi, bj) = 0.D0
                  ENDDO
                  DO i=snx+olx,1-olx,-1
                    vfldyb(i, 1, k, bi, bj) = 0.D0
                    ufldxb(i, 1, k, bi, bj) = 0.D0
                  ENDDO
                ELSE
                  DO j=sny+oly,1-oly,-1
                    DO i=snx+olx,1-olx,-1
                      CALL POPCONTROL1B(branch)
                      IF (branch .NE. 0) THEN
                        vfldyb(i, j, k, bi, bj) = maskc(i, j, kk, bi, bj
     +                    )*vfldyb(i, j, k, bi, bj)
                        ufldxb(i, j, k, bi, bj) = maskc(i, j, kk, bi, bj
     +                    )*ufldxb(i, j, k, bi, bj)
                      END IF
                      tmpvb(i, j) = tmpvb(i, j) + vfldyb(i, j, k, bi, bj
     +                  )
                      vfldyb(i, j, k, bi, bj) = 0.D0
                      tmpub(i, j) = tmpub(i, j) + ufldxb(i, j, k, bi, bj
     +                  )
                      ufldxb(i, j, k, bi, bj) = 0.D0
                    ENDDO
                  ENDDO
                ...
                DO j=sny+oly,1-oly,-1
                  DO i=snx+olx,1-olx,-1
                    vfldnb(i, j, k, bi, bj) = vfldnb(i, j, k, bi, bj) + 
     +                anglecosc(i, j, bi, bj)*tmpvb(i, j) + anglesinc(i
     +                , j, bi, bj)*tmpub(i, j)
                    ufldeb(i, j, k, bi, bj) = ufldeb(i, j, k, bi, bj) + 
     +                anglecosc(i, j, bi, bj)*tmpub(i, j) - anglesinc(i
     +                , j, bi, bj)*tmpvb(i, j)
                    tmpvb(i, j) = 0.D0
                    tmpub(i, j) = 0.D0
                  ENDDO
