...
C
C  Differentiation of calc_grad_phi_fv in reverse (adjoint) mode:
C   gradient     of useful results: phi0surf dphihydx dphihydy
C                phihydf phihydu
C   with respect to varying inputs: phi0surf dphihydx dphihydy
C                phihydf phihydu
C
C
C
...
C     !INTERFACE:
      SUBROUTINE CALC_GRAD_PHI_FV_B(k, bi, bj, imin, imax, jmin, jmax, 
     +                              phihydf, phihydfb, phihydu, phihydub
     +                              , pkappaf, pkappau, dphihydx, 
     +                              dphihydxb, dphihydy, dphihydyb, 
...
C     i,j :: Loop counters
      ...
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx,-1
          dphihydyb(i, j) = masks(i, j, k, bi, bj)*dphihydyb(i, j)
          dphihydxb(i, j) = maskw(i, j, k, bi, bj)*dphihydxb(i, j)
        ENDDO
      ENDDO
      DO j=jmax,jmin+1,-1
        DO i=imax,imin,-1
          dphihydyb(i, j) = recip_dyc(i, j, bi, bj)*dphihydyb(i, j)
          dpk_djm = pkappaf(i, j-1) - pkappau(i, j)
          dpk_djp = pkappaf(i, j) - pkappau(i, j-1)
          phi0surfb(i, j, bi, bj) = phi0surfb(i, j, bi, bj) + dphihydyb(
     +      i, j)
          phi0surfb(i, j-1, bi, bj) = phi0surfb(i, j-1, bi, bj) - 
     +      dphihydyb(i, j)
          tempb = dphihydyb(i, j)/(dpk_djp+dpk_djm)
          dphihydyb(i, j) = 0.D0
          phihydub(i, j) = phihydub(i, j) + dpk_djp*tempb
          phihydfb(i, j-1) = phihydfb(i, j-1) - dpk_djp*tempb
          phihydfb(i, j) = phihydfb(i, j) + dpk_djm*tempb
          phihydub(i, j-1) = phihydub(i, j-1) - dpk_djm*tempb
        ENDDO
      ENDDO
      DO j=jmax,jmin,-1
        DO i=imax,imin+1,-1
          dphihydxb(i, j) = recip_dxc(i, j, bi, bj)*dphihydxb(i, j)
          dpk_dim = pkappaf(i-1, j) - pkappau(i, j)
          dpk_dip = pkappaf(i, j) - pkappau(i-1, j)
          phi0surfb(i, j, bi, bj) = phi0surfb(i, j, bi, bj) + dphihydxb(
     +      i, j)
          phi0surfb(i-1, j, bi, bj) = phi0surfb(i-1, j, bi, bj) - 
     +      dphihydxb(i, j)
          tempb = dphihydxb(i, j)/(dpk_dip+dpk_dim)
          dphihydxb(i, j) = 0.D0
          phihydub(i, j) = phihydub(i, j) + dpk_dip*tempb
          phihydfb(i-1, j) = phihydfb(i-1, j) - dpk_dip*tempb
          phihydfb(i, j) = phihydfb(i, j) + dpk_dim*tempb
          phihydub(i-1, j) = phihydub(i-1, j) - dpk_dim*tempb
        ENDDO
      ENDDO
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx,-1
