...
C
C  Differentiation of mom_vi_v_coriolis_c4 in reverse (adjoint) mode:
C   gradient     of useful results: ufld vcoriolisterm
...
C     !INTERFACE:
      SUBROUTINE MOM_VI_V_CORIOLIS_C4_B(bi, bj, k, selectvortscheme, 
     +                                  highordervorticity, 
     +                                  upwindvorticity, ufld, ufldb, 
     +                                  omega3, omega3b, r_hfacz, 
...
C     msgBuf :: Informational/error meesage buffer
      ...
      REAL*8 vort3rb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 ubarxy, vort3v, rjp, rjm, rj
      REAL*8 ubarxyb, vort3vb, rjpb, rjmb, rjb
      REAL*8 ubarym, ubaryp
      REAL*8 ubarymb, ubarypb
C
      LOGICAL northwestcorner, northeastcorner, southwestcorner, 
     +        southeastcorner
      INTEGER myface
     ...
     +        vcoriolistermb(i, j))
            ...
            IF (branch .LT. 2) THEN
              IF (branch .EQ. 0) THEN
                ...
                vort3rb(i, j) = vort3rb(i, j) + vort3vb
              ELSE
                CALL POPREAL8(vort3v)
                vort3rb(i+1, j) = vort3rb(i+1, j) + vort3vb
              END IF
            ELSE IF (branch .EQ. 2) THEN
              CALL POPREAL8(vort3v)
              tempb = 0.5*vort3vb
              rjpb = -(onetwelve*tempb)
              vort3rb(i+1, j) = vort3rb(i+1, j) + tempb - rjpb
              rjmb = onetwelve*tempb
              vort3rb(i, j) = vort3rb(i, j) + tempb + rjmb
              vort3rb(i-1, j) = vort3rb(i-1, j) - rjmb
              vort3rb(i+2, j) = vort3rb(i+2, j) + rjpb
            ELSE
              CALL POPREAL8(vort3v)
              vort3rb(i, j) = vort3rb(i, j) + 0.5*vort3vb
              vort3rb(i+1, j) = vort3rb(i+1, j) + 0.5*vort3vb
            END IF
...
C         Rjm = vort3r( i ,j) +oneSixth*( vort3r(i+1,j)-vort3r(i-1,j) )
     ...
     +        vcoriolistermb(i, j))
            ...
            IF (branch .LT. 2) THEN
              IF (branch .EQ. 0) THEN
                ubarymb = vort3r(i, j)*vort3vb
                vort3rb(i, j) = vort3rb(i, j) + ubarym*vort3vb
                ubarypb = 0.D0
              ELSE
                ubarypb = vort3r(i+1, j)*vort3vb
                vort3rb(i+1, j) = vort3rb(i+1, j) + ubaryp*vort3vb
                ubarymb = 0.D0
              END IF
            ELSE IF (branch .EQ. 2) THEN
              tempb = 0.5*vort3vb
              ...
              ubarypb = rjp*tempb
              rjpb = ubaryp*tempb
              vort3rb(i, j) = vort3rb(i, j) + rjmb
              rjb = onesixth*rjpb - onesixth*rjmb
              rjmb = onesixth*rjmb
              vort3rb(i+1, j) = vort3rb(i+1, j) + rjpb
              rjpb = -(onesixth*rjpb)
              vort3rb(i+1, j) = vort3rb(i+1, j) + rjb - rjpb
              vort3rb(i, j) = vort3rb(i, j) + rjmb - rjb
              CALL POPREAL8(rjm)
              vort3rb(i-1, j) = vort3rb(i-1, j) - rjmb
              CALL POPREAL8(rjp)
              vort3rb(i+2, j) = vort3rb(i+2, j) + rjpb
            ELSE
              tempb = 0.5*vort3vb
              ubarymb = vort3r(i, j)*tempb
              vort3rb(i, j) = vort3rb(i, j) + ubarym*tempb
              ubarypb = vort3r(i+1, j)*tempb
              vort3rb(i+1, j) = vort3rb(i+1, j) + ubaryp*tempb
            END IF
...
C
      ...
      IF (branch .EQ. 0) THEN
        i = snx + 1
        j = sny + 1
        vort3rb(i, j-1) = vort3rb(i, j-1) + 0.5d0*vort3rb(i+1, j)
        vort3rb(i+1, j) = 0.5d0*vort3rb(i+1, j)
 ...
 100  DO ii1=1-oly,oly+sny
      ...
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx,-1
          omega3b(i, j) = omega3b(i, j) + r_hfacz(i, j)*vort3rb(i, j)
          vort3rb(i, j) = 0.D0
        ENDDO
      ENDDO
      END
