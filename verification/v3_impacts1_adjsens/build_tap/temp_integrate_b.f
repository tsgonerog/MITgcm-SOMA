...
C
C  Differentiation of temp_integrate in reverse (adjoint) mode:
C   gradient     of useful results: /gm_wtensor/[0,1215200[ /gm_wtensor/[1215200,2430400[
C                /gm_wtensor/[2430400,3645600[ /gm_hortensor/[0,1215200[
C                /gm_hortensor/[1215200,2430400[ /gm_uvtensor/[0,1215200[
C                /gm_uvtensor/[1215200,2430400[ /gm_bolus/[0,1215200[
C                /gm_bolus/[1215200,2430400[ wvel theta salt gtnm1
C                /ffields_qsw/[0,39200[ /surface_forcing/[78400,117600[
C                /surf_correc/[0,8[ kappark vfld ufld wfld
...
C     !INTERFACE:
      SUBROUTINE TEMP_INTEGRATE_B(bi, bj, recip_hfac, ufld, ufldb, vfld
     +                            , vfldb, wfld, wfldb, kappark, 
     +                            kapparkb, mytime, myiter, mythid)
      IMPLICIT NONE
C     == GLobal variables ==
CBOP
C    !ROUTINE: SIZE.h
...
C                      0= not available in pickup
     ...
     +mom_startab, nhydstartab, qhydstartab, dphinhstatus
      INTEGER nchecklev
      INTEGER tempstartab
      INTEGER saltstartab
      INTEGER mom_startab
...
C     myThid     :: my Thread Id. number
      ...
      REAL*8 kapparkb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 mytime
      INTEGER myiter
      INTEGER mythid
...
C   useVariableK :: T when vertical diffusion is not constant
      ...
      REAL*8 fverb(1-olx:snx+olx, 1-oly:sny+oly, 2)
      REAL*8 gt_loc(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 gt_locb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 gtforc(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gtforcb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gt_ab(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gt_abb(1-olx:snx+olx, 1-oly:sny+oly)
...
C     compute T^n+1/2 (stored in gtNm) extrapolating T forward in time
        DO ii1=1-olx,66
          CALL PUSHREAL8(theta(ii1, 1-oly, 1, bi, bj))
        ENDDO
        CALL ADAMS_BASHFORTH2(bi, bj, 0, nr, theta(1-olx, 1-oly, 1, bi, 
     +                        bj), gtnm1(1-olx, 1-oly, 1, bi, bj), gt_ab
     +                        , tempstartab, iternb, mythid)
...
C
     ...
     +                         gad_temperature, usegmredi, usekpp, 
     +                         kappark, mythid)
C
...
C disable this section of code.
     ...
     +                     tempvertadvscheme, gad_temperature, 
     +                     dttracerlev, ufld, vfld, wfld, theta, gt_loc
...
C-    Start vertical index (k) loop (Nr:1)
      ...
      DO k=nr,1,-1
        ...
        kdown = 1 + MOD(k, 2)
C
        CALL PUSHREAL8ARRAY(ya, 70**2)
        CALL PUSHREAL8ARRAY(xa, 70**2)
        CALL PUSHREAL8ARRAY(maskup, 70**2)
        CALL PUSHREAL8ARRAY(rtranskp, 70**2)
        CALL PUSHREAL8ARRAY(vtrans, 70**2)
        CALL PUSHREAL8ARRAY(utrans, 70**2)
        CALL PUSHREAL8ARRAY(rtrans, 70**2)
        CALL CALC_ADV_FLOW(ufld, vfld, wfld, rtrans, utrans, vtrans, 
     +                     rtranskp, maskup, xa, ya, k, bi, bj, mythid)
C
...
C
     ...
     +                    kdown, xa, ya, maskup, ufld(1-olx, 1-oly, k), 
     +                    vfld(1-olx, 1-oly, k), wfld(1-olx, 1-oly, k), 
     +                    utrans, vtrans, rtrans, rtranskp, diffkht, 
     +                    diffk4t, kappark(1-olx, 1-oly, k), diffkr4t, 
     +                    theta(1-olx, 1-oly, 1, bi, bj), gtnm1(1-olx, 1
     +                    -oly, 1, bi, bj), dttracerlev, gad_temperature
     +                    , tempadvscheme, tempvertadvscheme, 
     +                    calcadvection, tempimplvertadv, 
     +                    adamsbashforth_t, tempvertdiff4, usegmredi, 
     +                    usekpp, temp_staypositive, fzon, fmer, fver, 
     +                    gt_loc, mytime, myiter, mythid)
C
...
C
        IF (adamsbashforthgt) THEN
          DO ii1=1-olx,66
            CALL PUSHREAL8(gtnm1(ii1, 1-oly, 1, bi, bj))
          ENDDO
          CALL ADAMS_BASHFORTH2(bi, bj, k, nr, gt_loc, gtnm1(1-olx, 1-
     +                          oly, 1, bi, bj), gt_ab, tempstartab, 
     +                          iternb, mythid)
          CALL PUSHCONTROL1B(0)
        ELSE
          CALL PUSHCONTROL1B(1)
...
C-    Integrate forward in time, storing in gT_loc:  gT <= T + dt*gT
      CALL TIMESTEP_TRACER(bi, bj, dttracerlev, theta(1-olx, 1-oly, 1, 
     +                     bi, bj), gt_loc, mytime, myiter, mythid)
C
C--   Implicit vertical advection & diffusion
C
      IF (tempimplvertadv .OR. implicitdiffusion) THEN
        CALL PUSHCONTROL2B(0)
      ELSE IF (implicitdiffusion) THEN
...
C
      IF (adamsbashforth_t) THEN
        CALL CYCLE_AB_TRACER_B(bi, bj, gt_loc, gt_locb, theta(1-olx, 1-
     +                         oly, 1, bi, bj), thetab(1-olx, 1-oly, 1, 
     +                         bi, bj), gtnm1(1-olx, 1-oly, 1, bi, bj), 
     +                         gtnm1b(1-olx, 1-oly, 1, bi, bj), mytime, 
     +                         myiter, mythid)
      ELSE
        CALL CYCLE_TRACER_B(bi, bj, theta(1-olx, 1-oly, 1, bi, bj), 
     +                      thetab(1-olx, 1-oly, 1, bi, bj), gt_loc, 
     +                      gt_locb, mytime, myiter, mythid)
      END IF
      CALL POPCONTROL2B(branch)
      IF (branch .EQ. 0) THEN
        CALL GAD_IMPLICIT_R_B(tempimplvertadv, tempvertadvscheme, 
     +                        gad_temperature, dttracerlev, kappark, 
     +                        kapparkb, recip_hfac, wfld, wfldb, theta(1
     +                        -olx, 1-oly, 1, bi, bj), thetab(1-olx, 1-
     +                        oly, 1, bi, bj), gt_loc, gt_locb, bi, bj, 
     +                        mytime, myiter, mythid)
      ELSE IF (branch .EQ. 1) THEN
        CALL IMPLDIFF_B(bi, bj, imin, imax, jmin, jmax, gad_temperature
     +                  , kappark, kapparkb, recip_hfac, gt_loc, gt_locb
     +                  , mythid)
      END IF
      CALL TIMESTEP_TRACER_B(bi, bj, dttracerlev, theta(1-olx, 1-oly, 1
     +                       , bi, bj), thetab(1-olx, 1-oly, 1, bi, bj)
     +                       , gt_loc, gt_locb, mytime, myiter, mythid)
      ...
      DO k=1,nr,1
        ...
        IF (branch .EQ. 0) THEN
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              gtforcb(i, j) = gtforcb(i, j) + gt_locb(i, j, k)
            ENDDO
          ENDDO
        END IF
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          DO ii1=66,1-olx,-1
            CALL POPREAL8(gtnm1(ii1, 1-oly, 1, bi, bj))
          ENDDO
          CALL ADAMS_BASHFORTH2_B(bi, bj, k, nr, gt_loc, gt_locb, gtnm1(
     +                            1-olx, 1-oly, 1, bi, bj), gtnm1b(1-olx
     +                            , 1-oly, 1, bi, bj), gt_ab, gt_abb, 
     +                            tempstartab, iternb, mythid)
        END IF
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
        ...
        CALL GAD_CALC_RHS_B(bi, bj, imin, imax, jmin, jmax, k, km1, kup
     +                      , kdown, xa, ya, maskup, ufld(1-olx, 1-oly, 
     +                      k), ufldb(1-olx, 1-oly, k), vfld(1-olx, 1-
     +                      oly, k), vfldb(1-olx, 1-oly, k), wfld(1-olx
     +                      , 1-oly, k), wfldb(1-olx, 1-oly, k), utrans
     +                      , utransb, vtrans, vtransb, rtrans, rtransb
     +                      , rtranskp, rtranskpb, diffkht, diffk4t, 
     +                      kappark(1-olx, 1-oly, k), kapparkb(1-olx, 1-
     +                      oly, k), diffkr4t, theta(1-olx, 1-oly, 1, bi
     +                      , bj), thetab(1-olx, 1-oly, 1, bi, bj), 
     +                      gtnm1(1-olx, 1-oly, 1, bi, bj), gtnm1b(1-olx
     +                      , 1-oly, 1, bi, bj), dttracerlev, 
     +                      gad_temperature, tempadvscheme, 
     +                      tempvertadvscheme, calcadvection, 
     +                      tempimplvertadv, adamsbashforth_t, 
     +                      tempvertdiff4, usegmredi, usekpp, 
     +                      temp_staypositive, fzon, fzonb, fmer, fmerb
     +                      , fver, fverb, gt_loc, gt_locb, mytime, 
     +                      myiter, mythid)
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) CALL APPLY_FORCING_T_B(gtforc, gtforcb, imin
     +                                            , imax, jmin, jmax, k
     +                                            , bi, bj, mytime, 
     +                                            myiter, mythid)
        DO j=sny+oly,1-oly,-1
          ...
          ENDDO
        ENDDO
        CALL POPREAL8ARRAY(rtrans, 70**2)
        CALL POPREAL8ARRAY(utrans, 70**2)
        CALL POPREAL8ARRAY(vtrans, 70**2)
        CALL POPREAL8ARRAY(rtranskp, 70**2)
        CALL POPREAL8ARRAY(maskup, 70**2)
        CALL POPREAL8ARRAY(xa, 70**2)
        CALL POPREAL8ARRAY(ya, 70**2)
        CALL CALC_ADV_FLOW_B(ufld, ufldb, vfld, vfldb, wfld, wfldb, 
     +                       rtrans, rtransb, utrans, utransb, vtrans, 
     +                       vtransb, rtranskp, rtranskpb, maskup, xa, 
     +                       ya, k, bi, bj, mythid)
      ...
      IF (branch .EQ. 0) THEN
        CALL POPINTEGER4(mythid)
        CALL GAD_ADVECTION_B(tempimplvertadv, tempadvscheme, 
     +                       tempvertadvscheme, gad_temperature, 
     +                       dttracerlev, ufld, ufldb, vfld, vfldb, wfld
     +                       , wfldb, theta, thetab, gt_loc, gt_locb, bi
     +                       , bj, mytime, myiter, mythid)
      END IF
      CALL CALC_3D_DIFFUSIVITY_B(bi, bj, imin, imax, jmin, jmax, 
     +                           gad_temperature, usegmredi, usekpp, 
     +                           kappark, kapparkb, mythid)
      DO k=nr,1,-1
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            kapparkb(i, j, k) = 0.D0
      ...
      IF (branch .NE. 0) THEN
        DO ii1=66,1-olx,-1
          CALL POPREAL8(theta(ii1, 1-oly, 1, bi, bj))
        ENDDO
        CALL ADAMS_BASHFORTH2_B(bi, bj, 0, nr, theta(1-olx, 1-oly, 1, bi
     +                          , bj), thetab(1-olx, 1-oly, 1, bi, bj), 
     +                          gtnm1(1-olx, 1-oly, 1, bi, bj), gtnm1b(1
     +                          -olx, 1-oly, 1, bi, bj), gt_ab, gt_abb, 
     +                          tempstartab, iternb, mythid)
      END IF
      END
