...
C
C  Differentiation of mom_v_adv_uv in reverse (adjoint) mode:
C   gradient     of useful results: vfld advectfluxuv
C   with respect to varying inputs: vfld utrans advectfluxuv
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_V_ADV_UV_B(bi, bj, k, utrans, utransb, vfld, vfldb
     +                          , advectfluxuv, advectfluxuvb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C  AdvectFluxUV         :: advective flux
      ...
      REAL*8 advectfluxuvb(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j
      ...
      DO j=sny+oly,1-oly+1,-1
        DO i=snx+olx,1-olx+1,-1
          tempb = (vfld(i, j)+vfld(i-1, j))*0.25*advectfluxuvb(i, j)
          tempb0 = (utrans(i, j)+utrans(i, j-1))*0.25*advectfluxuvb(i, j
     +      )
          advectfluxuvb(i, j) = 0.D0
          vfldb(i, j) = vfldb(i, j) + tempb0
          vfldb(i-1, j) = vfldb(i-1, j) + tempb0
          utransb(i, j) = utransb(i, j) + tempb
          utransb(i, j-1) = utransb(i, j-1) + tempb
        ENDDO
      ENDDO
      END
