...
C
C  Differentiation of ctrl_map_gentim2d in reverse (adjoint) mode:
C   gradient     of useful results: xx_gentim2d_dummy xx_gentim2d0
C                xx_gentim2d1 xx_gentim2d
C   with respect to varying inputs: xx_gentim2d_dummy xx_gentim2d0
C                xx_gentim2d1 xx_gentim2d
C
C
C
...
C     !INTERFACE:
      SUBROUTINE CTRL_MAP_GENTIM2D_B(mytime, myiter, mythid)
      IMPLICIT NONE
C
C     === Global variables ===
CBOP
...
C     == Local variables ==
      ...
      REAL*8 xx_gentim2d_locb(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      REAL*8 mask2d(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      CHARACTER*(max_len_mbuf) msgbuf
      REAL*8 locsumtile(nsx, nsy), locsumglob
      REAL*8 locsumtileb(nsx, nsy), locsumglobb
...
CIF (xx_gentim2d_weight(iarr).NE.' ') THEN
C
...
C
          ...
          ENDDO
C
          CALL PUSHREAL8ARRAY(mask2d, 70**2)
          CALL CTRL_GET_MASK2D(xx_gentim2d_file(iarr), mask2d, mythid)
          CALL CTRL_GET_GEN(xx_gentim2d_file(iarr), 
     +                      xx_gentim2d_startdate(1, iarr), 
     +                      xx_gentim2d_period(iarr), mask2d, 
     +                      xx_gentim2d_loc, xx_gentim2d0(1-olx, 1-oly, 
     +                      1, 1, iarr), xx_gentim2d1(1-olx, 1-oly, 1, 1
     +                      , iarr), xx_gentim2d_dummy(iarr), zerorl, 
     +                      zerorl, wgentim2d(1-olx, 1-oly, 1, 1, iarr)
          ...
          IF (xx_gentim2d_cumsum(iarr)) THEN
            DO bj=mybylo(mythid),mybyhi(mythid)
              DO bi=mybxlo(mythid),mybxhi(mythid)
                DO j=1,sny
                  DO i=1,snx
                    xx_gentim2d(i, j, bi, bj, iarr) = xx_gentim2d(i, j, 
     +                bi, bj, iarr) + xx_gentim2d_loc(i, j, bi, bj)
                  ENDDO
                ENDDO
              ENDDO
            ENDDO
          ...
          ELSE
            DO bj=mybylo(mythid),mybyhi(mythid)
              DO bi=mybxlo(mythid),mybxhi(mythid)
                DO j=1,sny
                  DO i=1,snx
                    xx_gentim2d(i, j, bi, bj, iarr) = xx_gentim2d_loc(i
     +                , j, bi, bj)
                  ENDDO
                ENDDO
              ENDDO
          ...
          END IF
CIF (xx_gentim2d_glosum(iarr)) THEN
C
          IF (xx_gentim2d_glosum(iarr)) THEN
C
            DO bj=mybylo(mythid),mybyhi(mythid)
              DO bi=mybxlo(mythid),mybxhi(mythid)
                CALL PUSHREAL8(locsumtile(bi, bj))
...
C
            ...
            DO bj=mybylo(mythid),mybyhi(mythid)
              DO bi=mybxlo(mythid),mybxhi(mythid)
                DO j=1-oly,sny+oly
                  DO i=1-olx,snx+olx
                    xx_gentim2d(i, j, bi, bj, iarr) = locsumglob*maskc(i
     +                , j, 1, bi, bj)
                  ENDDO
                ENDDO
              ENDDO
            ENDDO
CIF (xx_gentim2d_file(iarr).EQ.'xx_gen_precip') THEN
C
            IF (xx_gentim2d_file(iarr) .EQ. 'xx_gen_precip') THEN
              CALL PUSHCONTROL2B(0)
C
              WRITE(msgbuf, '(A,I6,A,1PE21.14)') ' iter=', myiter, 
     +              ' ; genprecipGloH= ', locsumglob*rhoconstfresh*
     +              recip_rhoconst*deltatclock
C
            ELSE
        ...
        DO ii2=1,nsx
          DO ii3=1-oly,oly+sny
            DO ii4=1-olx,olx+snx
              xx_gentim2d_locb(ii4, ii3, ii2, ii1) = 0.D0
            ENDDO
          ENDDO
        ENDDO
      ENDDO
      DO ii1=1,nsy
        DO ii2=1,nsx
          locsumtileb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO 100 iarr=maxctrltim2d,1,-1
            ...
            DO bi=mybxhi(mythid),mybxlo(mythid),-1
              DO j=sny+oly,1-oly,-1
                DO i=snx+olx,1-olx,-1
                  locsumglobb = locsumglobb + maskc(i, j, 1, bi, bj)*
     +              xx_gentim2db(i, j, bi, bj, iarr)
                  xx_gentim2db(i, j, bi, bj, iarr) = 0.D0
                ENDDO
              ENDDO
            ENDDO
          ENDDO
          locsumglobb = locsumglobb/globalarea
          CALL GLOBAL_SUM_TILE_RL_B(locsumtile, locsumtileb, locsumglob
     +                              , locsumglobb, mythid)
          DO bj=mybyhi(mythid),mybylo(mythid),-1
            DO bi=mybxhi(mythid),mybxlo(mythid),-1
              DO j=sny,1,-1
                DO i=snx,1,-1
                  xx_gentim2db(i, j, bi, bj, iarr) = xx_gentim2db(i, j, 
     +              bi, bj, iarr) + maskc(i, j, 1, bi, bj)*ra(i, j, bi, 
     +              bj)*maskinc(i, j, bi, bj)*locsumtileb(bi, bj)
                ENDDO
              ENDDO
              CALL POPREAL8(locsumtile(bi, bj))
              locsumtileb(bi, bj) = 0.D0
            ENDDO
          ENDDO
        ...
        IF (branch .EQ. 0) THEN
          DO bj=mybyhi(mythid),mybylo(mythid),-1
            DO bi=mybxhi(mythid),mybxlo(mythid),-1
              DO j=sny,1,-1
                DO i=snx,1,-1
                  xx_gentim2d_locb(i, j, bi, bj) = xx_gentim2d_locb(i, j
     +              , bi, bj) + xx_gentim2db(i, j, bi, bj, iarr)
                ENDDO
              ENDDO
            ENDDO
          ENDDO
     ...
     +              , bi, bj) + xx_gentim2db(i, j, bi, bj, iarr)
          ...
          ENDDO
        END IF
        CALL CTRL_GET_GEN_B(xx_gentim2d_file(iarr), 
     +                      xx_gentim2d_startdate(1, iarr), 
     +                      xx_gentim2d_period(iarr), mask2d, 
     +                      xx_gentim2d_loc, xx_gentim2d_locb, 
     +                      xx_gentim2d0(1-olx, 1-oly, 1, 1, iarr), 
     +                      xx_gentim2d0b(1-olx, 1-oly, 1, 1, iarr), 
     +                      xx_gentim2d1(1-olx, 1-oly, 1, 1, iarr), 
     +                      xx_gentim2d1b(1-olx, 1-oly, 1, 1, iarr), 
     +                      xx_gentim2d_dummy(iarr), xx_gentim2d_dummyb(
     +                      iarr), zerorl, zerorl, wgentim2d(1-olx, 1-
     +                      oly, 1, 1, iarr), mytime, myiter, mythid)
        CALL POPREAL8ARRAY(mask2d, 70**2)
        DO bj=mybyhi(mythid),mybylo(mythid),-1
          DO bi=mybxhi(mythid),mybxlo(mythid),-1
            DO j=sny+oly,1-oly,-1
              DO i=snx+olx,1-olx,-1
                xx_gentim2d_locb(i, j, bi, bj) = 0.D0
              ENDDO
            ENDDO
          ENDDO
        ENDDO
