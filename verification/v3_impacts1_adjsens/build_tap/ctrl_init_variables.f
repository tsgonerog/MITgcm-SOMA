...
C-    Needs to be called after few {PKG}_init_varia (e.g., PTRACERS_INIT_VARIA)
      ...
      DO bj = myByLo(myThid),myByHi(myThid)
        DO bi = myBxLo(myThid),myBxHi(myThid)
          DO k=1,maxCtrlTim2D
            objf_gentim2d(bi,bj,k)     = 0.D0
            num_gentim2d(bi,bj,k)      = 0.D0
          ENDDO
        ENDDO
      ENDDO
      DO bj = myByLo(myThid),myByHi(myThid)
        DO bi = myBxLo(myThid),myBxHi(myThid)
          DO k=1,maxCtrlArr2D
            objf_genarr2d(bi,bj,k)    = 0.D0
            num_genarr2d(bi,bj,k)    = 0.D0
          ENDDO
        ENDDO
      ENDDO
      DO bj = myByLo(myThid),myByHi(myThid)
        DO bi = myBxLo(myThid),myBxHi(myThid)
          DO k=1,maxCtrlArr3D
            objf_genarr3d(bi,bj,k)    = 0.D0
            num_genarr3d(bi,bj,k)    = 0.D0
          ENDDO
        ENDDO
      ENDDO
