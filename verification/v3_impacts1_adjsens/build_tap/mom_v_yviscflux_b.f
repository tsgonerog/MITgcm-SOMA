...
C
C  Differentiation of mom_v_yviscflux in reverse (adjoint) mode:
C   gradient     of useful results: vfld del2v yviscfluxv
C   with respect to varying inputs: visca4_d vfld del2v viscah_d
C                yviscfluxv
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_V_YVISCFLUX_B(bi, bj, k, vfld, vfldb, del2v, del2vb
     +                             , yviscfluxv, yviscfluxvb, viscah_d, 
...
C  yViscFluxV           :: viscous fluxes
      ...
      REAL*8 yviscfluxvb(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j
      ...
      DO j=sny+oly-1,1-oly,-1
        DO i=snx+olx-1,1-olx,-1
          tempb = dxf(i, j, bi, bj)*drf(k)*hfacc(i, j, k, bi, bj)*
     +      recip_dyf(i, j, bi, bj)*yviscfluxvb(i, j)
          yviscfluxvb(i, j) = 0.D0
          visca4_db(i, j) = visca4_db(i, j) + (del2v(i, j+1)-del2v(i, j)
     +      )*tempb
          tempb0 = visca4_d(i, j)*tempb
          viscah_db(i, j) = viscah_db(i, j) - (vfld(i, j+1)-vfld(i, j))*
     +      tempb
          tempb1 = -(viscah_d(i, j)*tempb)
          vfldb(i, j+1) = vfldb(i, j+1) + tempb1
          vfldb(i, j) = vfldb(i, j) - tempb1
          del2vb(i, j+1) = del2vb(i, j+1) + tempb0
          del2vb(i, j) = del2vb(i, j) - tempb0
        ENDDO
      ENDDO
      END
