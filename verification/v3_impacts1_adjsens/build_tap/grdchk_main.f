...
C     !LOCAL VARIABLES:
      ...
      Real*8     ftlxxmemo
      Real*8     localEps
      Real*8     grdchk_epsfac
      Real*8 tmpplot1(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 tmpplot2(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 tmpplot3(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
CEOP
...
C--   forward run with unperturbed control vector
      myTime = startTime
      myIter = nIter0
      ...
      DO bj = myByLo(myThid), myByHi(myThid)
        DO bi = myBxLo(myThid), myBxHi(myThid)
          DO k = 1, Nr
            DO j = jMin, jMax
              DO i = iMin, iMax
                tmpplot1(i,j,k,bi,bj) = 0.D0
                tmpplot2(i,j,k,bi,bj) = 0.D0
                tmpplot3(i,j,k,bi,bj) = 0.D0
              ENDDO
            ENDDO
          ENDDO
        ENDDO
