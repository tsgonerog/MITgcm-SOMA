...
C
C  Differentiation of gad_fluxlimit_adv_x in reverse (adjoint) mode:
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_FLUXLIMIT_ADV_X_B(bi, bj, k, calccfl, deltatloc, 
     +                                 utrans, utransb, ufld, ufldb, 
     +                                 masklocw, tracer, tracerb, ut, 
     +                                 utb, mythid)
      IMPLICIT NONE
...
C
      ...
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx-1,1-olx+2,-1
          rj = (tracer(i, j)-tracer(i-1, j))*masklocw(i, j)
          utransb(i, j) = utransb(i, j) + (tracer(i, j)+tracer(i-1, j))*
     +      0.5d0*utb(i, j)
          tempb = utrans(i, j)*0.5d0*utb(i, j)
          tempb0 = -(abs1*rj*0.5d0*utb(i, j))
          tempb1 = -((onerl+ucfl*cr-cr)*0.5d0*utb(i, j))
          utb(i, j) = 0.D0
          abs1b = rj*tempb1
          rjb = abs1*tempb1
          ucflb = cr*tempb0
          ...
          END IF
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) crb = crb + 2.d0*x2b
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL8(cr)
            rjb = rjb - cr*crb/rj**2
