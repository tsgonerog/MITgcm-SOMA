...
C
C  Differentiation of mom_u_adv_vu in reverse (adjoint) mode:
C   gradient     of useful results: ufld vtrans advectfluxvu
C   with respect to varying inputs: ufld vtrans
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_U_ADV_VU_B(bi, bj, k, vtrans, vtransb, ufld, ufldb
     +                          , advectfluxvu, advectfluxvub, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C  AdvectFluxVU         :: advective flux
      ...
      REAL*8 advectfluxvub(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j
      ...
      DO j=sny+oly,1-oly+1,-1
        DO i=snx+olx,1-olx+1,-1
          tempb = (ufld(i, j)+ufld(i, j-1))*0.25*advectfluxvub(i, j)
          tempb0 = (vtrans(i, j)+vtrans(i-1, j))*0.25*advectfluxvub(i, j
     +      )
          advectfluxvub(i, j) = 0.D0
          ufldb(i, j) = ufldb(i, j) + tempb0
          ufldb(i, j-1) = ufldb(i, j-1) + tempb0
          vtransb(i, j) = vtransb(i, j) + tempb
          vtransb(i-1, j) = vtransb(i-1, j) + tempb
        ENDDO
      ENDDO
      END
