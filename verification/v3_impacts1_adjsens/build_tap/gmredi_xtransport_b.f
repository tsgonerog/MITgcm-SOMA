...
C
C  Differentiation of gmredi_xtransport in reverse (adjoint) mode:
C   gradient     of useful results: kux kuz gm_psix df tracer
C   with respect to varying inputs: kux kuz gm_psix df tracer
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE GMREDI_XTRANSPORT_B(tridentity, bi, bj, k, imin, imax, 
     +                               jmin, jmax, xa, maskfk, tracer, 
     +                               tracerb, df, dfb, mythid)
      IMPLICIT NONE
C     == GLobal variables ==
CBOP
C    !ROUTINE: SIZE.h
...
C     i, j       :: Loop counters
      ...
      REAL*8 dtdzb(1-olx:snx+olx, 1-oly:sny+oly)
...
C           AD-code to ensure exactly zero contribution to Tracer AD var.
          ...
          DO j=jmin,jmax
            DO i=imin,imax
              dtdz(i, j) = op5*(op5*recip_drc(k)*(maskfk(i-1, j)*(tracer
     +          (i-1, j, km1)-tracer(i-1, j, k))+maskfk(i, j)*(tracer(i
     +          , j, km1)-tracer(i, j, k)))+op5*recip_drc(kp1)*(maskc(i-
     +          1, j, k, bi, bj)*maskc(i-1, j, kp1, bi, bj)*maskp1*(
     +          tracer(i-1, j, k)-tracer(i-1, j, kp1))+maskc(i, j, k, bi
     +          , bj)*maskc(i, j, kp1, bi, bj)*maskp1*(tracer(i, j, k)-
     +          tracer(i, j, kp1))))
            ENDDO
          ENDDO
...
C
     ...
     +      )) THEN
          ...
          DO j=jmin,jmax
            DO i=imin,imax
              utrans(i, j) = -(gravitysign*dyg(i, j, bi, bj)*(gm_psix(i
     +          , j, kp1, bi, bj)*deepfacf(kp1)*maskp1-gm_psix(i, j, k, 
     +          bi, bj)*deepfacf(k))*maskw(i, j, k, bi, bj))
            ENDDO
          ENDDO
          ...
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              utransb(i, j) = utransb(i, j) + (tracer(i, j, k)+tracer(i-
     +          1, j, k))*op5*dfb(i, j)
              tempb4 = utrans(i, j)*op5*dfb(i, j)
              tracerb(i, j, k) = tracerb(i, j, k) + tempb4
              tracerb(i-1, j, k) = tracerb(i-1, j, k) + tempb4
            ENDDO
          ENDDO
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              tempb4 = -(gravitysign*dyg(i, j, bi, bj)*maskw(i, j, k, bi
     +          , bj)*utransb(i, j))
              utransb(i, j) = 0.D0
              gm_psixb(i, j, kp1, bi, bj) = gm_psixb(i, j, kp1, bi, bj) 
     +          + deepfacf(kp1)*maskp1*tempb4
     ...
     +          deepfacf(k)*tempb4
        ...
        IF (branch .EQ. 0) THEN
          ...
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              tempb4 = -(xa(i, j)*dfb(i, j))
              kuzb(i, j, k, bi, bj) = kuzb(i, j, k, bi, bj) + dtdz(i, j)
     +          *tempb4
              dtdzb(i, j) = dtdzb(i, j) + kuz(i, j, k, bi, bj)*tempb4
            ENDDO
          ENDDO
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              tempb0 = op5**2*recip_drc(k)*dtdzb(i, j)
              tempb2 = op5**2*recip_drc(kp1)*dtdzb(i, j)
              dtdzb(i, j) = 0.D0
              tempb3 = maskc(i-1, j, k, bi, bj)*maskp1*maskc(i-1, j, kp1
     +          , bi, bj)*tempb2
              tempb4 = maskc(i, j, kp1, bi, bj)*maskc(i, j, k, bi, bj)*
     +          maskp1*tempb2
              tracerb(i, j, k) = tracerb(i, j, k) + tempb4
              tracerb(i, j, kp1) = tracerb(i, j, kp1) - tempb4
              tracerb(i-1, j, k) = tracerb(i-1, j, k) + tempb3
              tracerb(i-1, j, kp1) = tracerb(i-1, j, kp1) - tempb3
              tempb = maskfk(i-1, j)*tempb0
              tempb1 = maskfk(i, j)*tempb0
              tracerb(i, j, km1) = tracerb(i, j, km1) + tempb1
              tracerb(i, j, k) = tracerb(i, j, k) - tempb1
              tracerb(i-1, j, km1) = tracerb(i-1, j, km1) + tempb
              tracerb(i-1, j, k) = tracerb(i-1, j, k) - tempb
            ENDDO
          ENDDO
        END IF
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            tempb = -(xa(i, j)*recip_deepfacc(k)*recip_dxc(i, j, bi, bj)
     +        *dfb(i, j))
            kuxb(i, j, k, bi, bj) = kuxb(i, j, k, bi, bj) + (tracer(i, j
     +        , k)-tracer(i-1, j, k))*tempb
            tempb0 = kux(i, j, k, bi, bj)*tempb
            tracerb(i, j, k) = tracerb(i, j, k) + tempb0
