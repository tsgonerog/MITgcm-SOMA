...
C
C  Differentiation of mom_v_coriolis in reverse (adjoint) mode:
C   gradient     of useful results: vcoriolisterm
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_V_CORIOLIS_B(bi, bj, k, ufld, ufldb, vcoriolisterm
...
C         "Numerical Boundary Layers and Spurious Residual Flows"
        DO j=1-oly+1,sny+oly
          DO i=1-olx,snx+olx-1
            y1 = maskw(i, j, k, bi, bj) + maskw(i+1, j, k, bi, bj) + 
     +        maskw(i, j-1, k, bi, bj) + maskw(i+1, j-1, k, bi, bj)
        ...
        DO j=sny+oly,1-oly+1,-1
          DO i=snx+olx-1,1-olx,-1
            vcoriolistermb(i, j) = 4.d0*vcoriolistermb(i, j)/max1
      ...
      IF (branch .EQ. 0) THEN
        ...
        DO j=sny+oly,1-oly+1,-1
          DO i=snx+olx-1,1-olx,-1
            tempb = -(fcori(i, j, bi, bj)*0.5**2*vcoriolistermb(i, j))
            tempb0 = -(fcori(i, j-1, bi, bj)*0.5**2*vcoriolistermb(i, j)
     +        )
            vcoriolistermb(i, j) = 0.D0
            ufldb(i, j-1) = ufldb(i, j-1) + tempb0
            ufldb(i+1, j-1) = ufldb(i+1, j-1) + tempb0
      ...
      ELSE
        ...
        DO j=sny+oly,1-oly+1,-1
          DO i=snx+olx-1,1-olx,-1
            tempb0 = -((fcori(i, j, bi, bj)+fcori(i, j-1, bi, bj))*0.25*
     +        0.5*vcoriolistermb(i, j))
            vcoriolistermb(i, j) = 0.D0
            ufldb(i, j) = ufldb(i, j) + tempb0
            ufldb(i+1, j) = ufldb(i+1, j) + tempb0
            ufldb(i, j-1) = ufldb(i, j-1) + tempb0
            ufldb(i+1, j-1) = ufldb(i+1, j-1) + tempb0
          ENDDO
        ENDDO
