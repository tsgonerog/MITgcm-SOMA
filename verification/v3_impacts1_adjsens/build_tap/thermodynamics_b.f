...
C
C  Differentiation of thermodynamics in reverse (adjoint) mode:
C   gradient     of useful results: kwx kwy kwz kux kvy kuz kvz
C                gm_psix gm_psiy uvel vvel wvel theta salt gtnm1
C                gsnm1 qsw surfaceforcingt surfaceforcings
...
C     !INTERFACE:
      SUBROUTINE THERMODYNAMICS_B(mytime, myiter, mythid)
      IMPLICIT NONE
C     == Global variables ===
CBOP
C    !ROUTINE: SIZE.h
...
C     i, j, k        :: loop indices
      ...
      EXTERNAL MON_CALC_ADVCFL_TILE
      EXTERNAL DEBUG_CALL
      EXTERNAL MON_CALC_ADVCFL_GLOB
...
C
          DO k=1,nr
            DO j=1-oly,sny+oly
              DO i=1-olx,snx+olx
                CALL PUSHREAL8(recip_hfacnew(i, j, k))
...
C-    just do a local copy:
          DO k=1,nr
            DO j=1-oly,sny+oly
              DO i=1-olx,snx+olx
                CALL PUSHREAL8(ufld(i, j, k))
                ufld(i, j, k) = uvel(i, j, k, bi, bj)
                CALL PUSHREAL8(vfld(i, j, k))
                vfld(i, j, k) = vvel(i, j, k, bi, bj)
                CALL PUSHREAL8(wfld(i, j, k))
...
C-    add Bolus velocity to Eulerian-mean velocity:
          IF (usegmredi) THEN
            CALL PUSHREAL8ARRAY(wfld, 70**2*31)
            CALL PUSHREAL8ARRAY(vfld, 70**2*31)
            CALL PUSHREAL8ARRAY(ufld, 70**2*31)
            CALL GMREDI_RESIDUAL_FLOW(ufld, vfld, wfld, bi, bj, myiter, 
     +                                mythid)
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHCONTROL1B(1)
...
C
     ...
     +                          nsy)
            ...
            CALL PUSHREAL8ARRAY(kappark, 70**2*31)
            CALL TEMP_INTEGRATE(bi, bj, recip_hfacnew, ufld, vfld, wfld
     +                          , kappark, mytime, myiter, mythid)
            CALL PUSHCONTROL1B(0)
          ELSE
            CALL PUSHCONTROL1B(1)
          END IF
C
     ...
     +                          , kappark, mytime, myiter, mythid)
      ...
      DO bj=ad_to0,ad_from0,-1
        ...
        DO bi=ad_to,ad_from,-1
          ...
          IF (branch .EQ. 0) THEN
            CALL POPREAL8ARRAY(kappark, 70**2*31)
     ...
     +                        )
            CALL SALT_INTEGRATE_B(bi, bj, recip_hfacnew, ufld, ufldb, 
     +                            vfld, vfldb, wfld, wfldb, kappark, 
     ...
     +                        )
            CALL TEMP_INTEGRATE_B(bi, bj, recip_hfacnew, ufld, ufldb, 
     +                            vfld, vfldb, wfld, wfldb, kappark, 
     +                            kapparkb, mytime, myiter, mythid)
          END IF
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL8ARRAY(ufld, 70**2*31)
            CALL POPREAL8ARRAY(vfld, 70**2*31)
            CALL POPREAL8ARRAY(wfld, 70**2*31)
            CALL GMREDI_RESIDUAL_FLOW_B(ufld, ufldb, vfld, vfldb, wfld, 
     +                                  wfldb, bi, bj, myiter, mythid)
          END IF
          DO k=nr,1,-1
            DO j=sny+oly,1-oly,-1
              DO i=snx+olx,1-olx,-1
                CALL POPREAL8(wfld(i, j, k))
                wvelb(i, j, k, bi, bj) = wvelb(i, j, k, bi, bj) + wfldb(
     +            i, j, k)
                wfldb(i, j, k) = 0.D0
                CALL POPREAL8(vfld(i, j, k))
                vvelb(i, j, k, bi, bj) = vvelb(i, j, k, bi, bj) + vfldb(
     +            i, j, k)
                vfldb(i, j, k) = 0.D0
                CALL POPREAL8(ufld(i, j, k))
                uvelb(i, j, k, bi, bj) = uvelb(i, j, k, bi, bj) + ufldb(
     +            i, j, k)
                ufldb(i, j, k) = 0.D0
              ENDDO
            ENDDO
          ...
          DO k=nr,1,-1
            DO j=sny+oly,1-oly,-1
              DO i=snx+olx,1-olx,-1
                CALL POPREAL8(recip_hfacnew(i, j, k))
              ENDDO
            ENDDO
          ENDDO
          DO k=nr,1,-1
      ...
      ENDDO
      CALL POPCONTROL1B(branch)
      IF (branch .NE. 0) CALL CALC_WSURF_TR_B(theta, thetab, salt, saltb
     +                                        , wvel, wvelb, mytime, 
     +                                        myiter, mythid)
      END
