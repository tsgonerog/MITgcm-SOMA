...
C
C  Differentiation of mom_vi_u_coriolis_c4 in reverse (adjoint) mode:
...
C     !INTERFACE:
      SUBROUTINE MOM_VI_U_CORIOLIS_C4_B(bi, bj, k, selectvortscheme, 
     +                                  highordervorticity, 
...
C     msgBuf :: Informational/error meesage buffer
      ...
      REAL*8 vbarxyb, vort3ub, rjpb, rjmb, rjb
      REAL*8 vbarxm, vbarxp
      REAL*8 vbarxmb, vbarxpb
C
     ...
     +        ucoriolistermb(i, j)
            ...
            IF (branch .LT. 2) THEN
              IF (branch .EQ. 0) THEN
                ...
                vort3rb(i, j) = vort3rb(i, j) + vort3ub
              ELSE
                CALL POPREAL8(vort3u)
                vort3rb(i, j+1) = vort3rb(i, j+1) + vort3ub
              END IF
            ELSE IF (branch .EQ. 2) THEN
              CALL POPREAL8(vort3u)
              tempb = 0.5*vort3ub
              rjpb = -(onetwelve*tempb)
              vort3rb(i, j+1) = vort3rb(i, j+1) + tempb - rjpb
              rjmb = onetwelve*tempb
              vort3rb(i, j) = vort3rb(i, j) + tempb + rjmb
              vort3rb(i, j-1) = vort3rb(i, j-1) - rjmb
              vort3rb(i, j+2) = vort3rb(i, j+2) + rjpb
            ELSE
              CALL POPREAL8(vort3u)
              vort3rb(i, j) = vort3rb(i, j) + 0.5*vort3ub
              vort3rb(i, j+1) = vort3rb(i, j+1) + 0.5*vort3ub
            END IF
...
C         Rjm = vort3r(i, j ) +oneSixth*( vort3r(i,j+1)-vort3r(i,j-1) )
     ...
     +        ucoriolistermb(i, j)
            ...
            IF (branch .LT. 2) THEN
              IF (branch .EQ. 0) THEN
                vbarxmb = vort3r(i, j)*vort3ub
                vort3rb(i, j) = vort3rb(i, j) + vbarxm*vort3ub
                vbarxpb = 0.D0
              ELSE
                vbarxpb = vort3r(i, j+1)*vort3ub
                vort3rb(i, j+1) = vort3rb(i, j+1) + vbarxp*vort3ub
                vbarxmb = 0.D0
              END IF
            ELSE IF (branch .EQ. 2) THEN
              tempb = 0.5*vort3ub
              ...
              rjmb = onesixth*rjmb
              vort3rb(i, j+1) = vort3rb(i, j+1) + rjpb
              rjpb = -(onesixth*rjpb)
              vort3rb(i, j+1) = vort3rb(i, j+1) + rjb - rjpb
            ...
            ELSE
              tempb = 0.5*vort3ub
              vbarxmb = vort3r(i, j)*tempb
              vort3rb(i, j) = vort3rb(i, j) + vbarxm*tempb
              vbarxpb = vort3r(i, j+1)*tempb
              vort3rb(i, j+1) = vort3rb(i, j+1) + vbarxp*tempb
            END IF
...
C
      ...
      IF (branch .EQ. 0) THEN
        i = snx + 1
        j = sny + 1
        vort3rb(i-1, j) = vort3rb(i-1, j) + 0.5d0*vort3rb(i, j+1)
        vort3rb(i, j+1) = 0.5d0*vort3rb(i, j+1)
