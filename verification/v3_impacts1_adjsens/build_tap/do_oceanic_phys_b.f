...
C
C  Differentiation of do_oceanic_phys in reverse (adjoint) mode:
C   gradient     of useful results: kwx kwy kwz kux kvy kuz kvz
C                gm_psix gm_psiy theta salt rhoinsitu totphihyd
C                fu fv qnet qsw empmr saltflux sst sss pload surfaceforcingu
C                surfaceforcingv surfaceforcingt surfaceforcings
C                phi0surf pmepr
...
C     !INTERFACE:
      SUBROUTINE DO_OCEANIC_PHYS_B(mytime, myiter, mythid)
      IMPLICIT NONE
C     == Global variables ===
CBOP
C    !ROUTINE: SIZE.h
...
C     kSrf          :: surface index
      ...
      REAL*8 rhokp1b(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 rhokm1(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 rhokm1b(1-olx:snx+olx, 1-oly:sny+oly)
      ...
      EXTERNAL DIAGS_OCEANIC_SURF_FLUX
...
C
      ...
      IF (usediagnostics .AND. fluidiswater) THEN
        IF (DIAGNOSTICS_IS_ON('MXLDEPTH', mythid)) dodiagsrho = 
     +      dodiagsrho + 1
        IF (DIAGNOSTICS_IS_ON('DRHODR  ', mythid)) dodiagsrho = 
     +      dodiagsrho + 2
        IF (DIAGNOSTICS_IS_ON('WdRHO_P ', mythid)) dodiagsrho = 
     +      dodiagsrho + 4
        IF (DIAGNOSTICS_IS_ON('WdRHOdP ', mythid)) THEN
...
C This is currently used by GMRedi, IVDC, MXL-depth  and Diagnostics
                CALL PUSHREAL8(sigmax(i, j, k))
                sigmax(i, j, k) = 0.d0
                CALL PUSHREAL8(sigmay(i, j, k))
                sigmay(i, j, k) = 0.d0
                CALL PUSHREAL8(sigmar(i, j, k))
                sigmar(i, j, k) = 0.d0
              ENDDO
            ENDDO
          ENDDO
...
Cph although some of these are re-initialised later.
          ...
          IF (fluidiswater) THEN
            IF (.NOT.(usedown_slope .OR. usebbl)) THEN
              DO k=1,nr
                CALL FIND_RHO_2D(imin, imax, jmin, jmax, k, theta(1-olx
     +                           , 1-oly, k, bi, bj), salt(1-olx, 1-oly
     +                           , k, bi, bj), rhoinsitu(1-olx, 1-oly, k
     +                           , bi, bj), k, bi, bj, mythid)
              ENDDO
...
C-    horizontally uniform (tRef) reference state
              ...
              DO j=1-oly,sny+oly
                DO i=1-olx,snx+olx
                  rhoinsitu(i, j, k, bi, bj) = (theta(i, j, k, bi, bj)*(
     +              salt(i, j, k, bi, bj)*atm_rq+onerl)-thetaref)*maskc(
     +              i, j, k, bi, bj)
                ENDDO
              ENDDO
            ENDDO
...
C         slope terms (e.g. GM/Redi tensor or IVDC diffusivity)
            IF (((((((calcgmredi .OR. (k .GT. 1 .AND. calcconvect)) .OR.
     +          usepp81) .OR. usekl10) .OR. usemy82) .OR. useggl90) .OR.
              ...
              IF (k .GT. 1) THEN
                IF (usingzcoords) THEN
                  DO j=jmin,jmax
                    ...
                    ENDDO
                  ENDDO
                  CALL FIND_RHO_2D(imin, imax, jmin, jmax, k, theta(1-
     +                             olx, 1-oly, k-1, bi, bj), salt(1-olx
     +                             , 1-oly, k-1, bi, bj), rhokm1, arg1, 
     +                             bi, bj, mythid)
                  CALL PUSHCONTROL2B(0)
                ELSE
                  arg1 = k - 1
                  CALL FIND_RHO_2D(imin, imax, jmin, jmax, arg1, theta(1
     +                             -olx, 1-oly, k, bi, bj), salt(1-olx, 
     +                             1-oly, k, bi, bj), rhokp1, k, bi, bj
     +                             , mythid)
                  DO j=jmin,jmax
                    DO i=imin,imax
              ...
              ELSE
                CALL PUSHCONTROL2B(2)
              END IF
              CALL PUSHREAL8ARRAY(sigmar, 70**2*31)
              CALL PUSHREAL8ARRAY(sigmay, 70**2*31)
              CALL PUSHREAL8ARRAY(sigmax, 70**2*31)
              CALL GRAD_SIGMA(bi, bj, imin, imax, jmin, jmax, k, 
     +                        rhoinsitu(1-olx, 1-oly, k, bi, bj), rhokm1
     +                        , rhokp1, sigmax, sigmay, sigmar, mythid)
C
C
              CALL PUSHCONTROL1B(0)
            ELSE
...
C--     Calculate iso-neutral slopes for the GM/Redi parameterisation
          IF (calcgmredi) THEN
            ...
            CALL PUSHINTEGER4(bj)
            CALL PUSHINTEGER4(bi)
            CALL GMREDI_CALC_TENSOR(imin, imax, jmin, jmax, sigmax, 
     +                              sigmay, sigmar, bi, bj, mytime, 
     +                              myiter, mythid)
            CALL PUSHCONTROL1B(0)
          ELSE
...
C
C
      IF (calcgmredi) CALL GMREDI_DO_EXCH_B(mytime, myiter, mythid)
      DO ii1=1,nr
        DO ii2=1-oly,oly+sny
          DO ii3=1-olx,olx+snx
            sigmarb(ii3, ii2, ii1) = 0.D0
          ENDDO
        ENDDO
      ENDDO
      DO ii1=1,nr
      ...
      DO ii1=1,nr
        DO ii2=1-oly,oly+sny
          DO ii3=1-olx,olx+snx
            sigmayb(ii3, ii2, ii1) = 0.D0
          ENDDO
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          rhokp1b(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          rhokm1b(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      ...
      DO bj=ad_to0,ad_from0,-1
        ...
        DO bi=ad_to,ad_from,-1
          ...
          IF (branch .EQ. 0) THEN
            ...
            CALL GMREDI_CALC_TENSOR_B(imin, imax, jmin, jmax, sigmax, 
     +                                sigmaxb, sigmay, sigmayb, sigmar, 
     +                                sigmarb, bi, bj, mytime, myiter, 
     +                                mythid)
          ELSE
            CALL GMREDI_CALC_TENSOR_DUMMY_B(imin, imax, jmin, jmax, 
     ...
     +                                      bj, mytime, myiter, mythid)
          ...
          DO k=1,nr,1
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREAL8ARRAY(sigmax, 70**2*31)
              CALL POPREAL8ARRAY(sigmay, 70**2*31)
              CALL POPREAL8ARRAY(sigmar, 70**2*31)
              CALL GRAD_SIGMA_B(bi, bj, imin, imax, jmin, jmax, k, 
     +                          rhoinsitu(1-olx, 1-oly, k, bi, bj), 
     +                          rhoinsitub(1-olx, 1-oly, k, bi, bj), 
     +                          rhokm1, rhokm1b, rhokp1, rhokp1b, sigmax
     +                          , sigmaxb, sigmay, sigmayb, sigmar, 
     +                          sigmarb, mythid)
              CALL POPCONTROL2B(branch)
              IF (branch .EQ. 0) THEN
                CALL FIND_RHO_2D_B(imin, imax, jmin, jmax, k, theta(1-
     +                             olx, 1-oly, k-1, bi, bj), thetab(1-
     +                             olx, 1-oly, k-1, bi, bj), salt(1-olx
     +                             , 1-oly, k-1, bi, bj), saltb(1-olx, 1
     +                             -oly, k-1, bi, bj), rhokm1, rhokm1b, 
     +                             arg1, bi, bj, mythid)
                DO j=jmax,jmin,-1
                  DO i=imax,imin,-1
                    rhoinsitub(i, j, k, bi, bj) = rhoinsitub(i, j, k, bi
     +                , bj) + rhokp1b(i, j)
                    rhokp1b(i, j) = 0.D0
                  ENDDO
                ENDDO
              ELSE IF (branch .EQ. 1) THEN
                DO j=jmax,jmin,-1
                  DO i=imax,imin,-1
                    rhoinsitub(i, j, k-1, bi, bj) = rhoinsitub(i, j, k-1
     +                , bi, bj) + rhokm1b(i, j)
                    rhokm1b(i, j) = 0.D0
                  ENDDO
                ENDDO
                arg1 = k - 1
                CALL FIND_RHO_2D_B(imin, imax, jmin, jmax, arg1, theta(1
     +                             -olx, 1-oly, k, bi, bj), thetab(1-olx
     +                             , 1-oly, k, bi, bj), salt(1-olx, 1-
     +                             oly, k, bi, bj), saltb(1-olx, 1-oly, 
     +                             k, bi, bj), rhokp1, rhokp1b, k, bi, 
     +                             bj, mythid)
              END IF
            END IF
          ENDDO
          ...
          IF (branch .EQ. 0) THEN
            DO k=nr,1,-1
              CALL FIND_RHO_2D_B(imin, imax, jmin, jmax, k, theta(1-olx
     +                           , 1-oly, k, bi, bj), thetab(1-olx, 1-
     +                           oly, k, bi, bj), salt(1-olx, 1-oly, k, 
     +                           bi, bj), saltb(1-olx, 1-oly, k, bi, bj)
     +                           , rhoinsitu(1-olx, 1-oly, k, bi, bj), 
     +                           rhoinsitub(1-olx, 1-oly, k, bi, bj), k
     +                           , bi, bj, mythid)
            ENDDO
            ...
            DO k=nr,1,-1
              DO j=sny+oly,1-oly,-1
                DO i=snx+olx,1-olx,-1
                  tempb = maskc(i, j, k, bi, bj)*rhoinsitub(i, j, k, bi
     +              , bj)
                  rhoinsitub(i, j, k, bi, bj) = 0.D0
                  thetab(i, j, k, bi, bj) = thetab(i, j, k, bi, bj) + (
     +              onerl+atm_rq*salt(i, j, k, bi, bj))*tempb
                  saltb(i, j, k, bi, bj) = saltb(i, j, k, bi, bj) + 
     +              atm_rq*theta(i, j, k, bi, bj)*tempb
                ENDDO
              ENDDO
            ENDDO
          END IF
          ...
          DO k=nr,1,-1
            DO j=sny+oly,1-oly,-1
              DO i=snx+olx,1-olx,-1
                CALL POPREAL8(sigmar(i, j, k))
                sigmarb(i, j, k) = 0.D0
                CALL POPREAL8(sigmay(i, j, k))
                sigmayb(i, j, k) = 0.D0
                CALL POPREAL8(sigmax(i, j, k))
                sigmaxb(i, j, k) = 0.D0
              ENDDO
            ENDDO
          ENDDO
        ENDDO
      ENDDO
      CALL EXTERNAL_FORCING_SURF_B(imin, imax, jmin, jmax, mytime, 
     +                             myiter, mythid)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREAL8ARRAY(theta, (snx+2*olx)*(sny+2*oly)*nr*nsx*nsy)
        CALL FREEZE_SURFACE_B(mytime, myiter, mythid)
      END IF
      CALL POPCONTROL1B(branch)
      END
