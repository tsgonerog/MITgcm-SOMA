...
C
C  Differentiation of mom_vi_v_coriolis in reverse (adjoint) mode:
C   gradient     of useful results: ufld vcoriolisterm omega3
C   with respect to varying inputs: ufld vcoriolisterm omega3
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE MOM_VI_V_CORIOLIS_B(bi, bj, k, selectvortscheme, 
     +                               usejamartmomadv, ufld, ufldb, 
     +                               omega3, omega3b, hfacz, r_hfacz, 
     +                               vcoriolisterm, vcoriolistermb, 
     +                               mythid)
      IMPLICIT NONE
C
C     == Global variables ==
...
C     == Routine arguments ==
      ...
      REAL*8 vcoriolistermb(1-olx:snx+olx, 1-oly:sny+oly)
...
C     msgBuf :: Informational/error meesage buffer
      ...
      REAL*8 ubarxyb, ubarymb, ubarypb
      REAL*8 vort3v
      REAL*8 vort3vb
      REAL*8 vort3im, vort3ij, vort3pm, vort3pj
      REAL*8 vort3imb, vort3ijb, vort3pmb, vort3pjb
...
C
        DO j=2-oly,sny+oly
          DO i=1-olx,snx+olx-1
            CALL PUSHREAL8(ubarxy)
            ubarxy = 0.25*(ufld(i, j)*dyg(i, j, bi, bj)*hfacw(i, j, k, 
     +        bi, bj)+ufld(i, j-1)*dyg(i, j-1, bi, bj)*hfacw(i, j-1, k, 
     +        bi, bj)+(ufld(i+1, j)*dyg(i+1, j, bi, bj)*hfacw(i+1, j, k
     +        , bi, bj)+ufld(i+1, j-1)*dyg(i+1, j-1, bi, bj)*hfacw(i+1, 
     +        j-1, k, bi, bj)))
            IF (upwindvort3) THEN
              IF (ubarxy .GT. 0.) THEN
                CALL PUSHREAL8(vort3v)
...
C
        DO j=2-oly,sny+oly
          DO i=1-olx,snx+olx-1
            IF (epsil .LT. hfacz(i, j) + hfacz(i+1, j)) THEN
              CALL PUSHREAL8(max1)
              max1 = hfacz(i, j) + hfacz(i+1, j)
            ...
            END IF
            CALL PUSHREAL8(ubarxy)
            ubarxy = 0.5*(ufld(i, j)*dyg(i, j, bi, bj)*hfacz(i, j)+ufld(
     +        i, j-1)*dyg(i, j-1, bi, bj)*hfacz(i, j)+(ufld(i+1, j)*dyg(
     +        i+1, j, bi, bj)*hfacz(i+1, j)+ufld(i+1, j-1)*dyg(i+1, j-1
     +        , bi, bj)*hfacz(i+1, j)))/max1
            IF (upwindvort3) THEN
              IF (ubarxy .GT. 0.) THEN
                CALL PUSHREAL8(vort3v)
                vort3v = omega3(i, j)
...
C
        DO j=2-oly,sny+oly
          DO i=1-olx,snx+olx-1
            CALL PUSHREAL8(ubarym)
            ubarym = 0.5*(ufld(i, j)*dyg(i, j, bi, bj)*hfacw(i, j, k, bi
     +        , bj)+ufld(i, j-1)*dyg(i, j-1, bi, bj)*hfacw(i, j-1, k, bi
     +        , bj))
            CALL PUSHREAL8(ubaryp)
            ubaryp = 0.5*(ufld(i+1, j)*dyg(i+1, j, bi, bj)*hfacw(i+1, j
     +        , k, bi, bj)+ufld(i+1, j-1)*dyg(i+1, j-1, bi, bj)*hfacw(i+
     +        1, j-1, k, bi, bj))
            IF (upwindvort3) THEN
...
C
      IF (usejamartmomadv) THEN
        DO j=2-oly,sny+oly-1
          DO i=1-olx,snx+olx-1
            y1 = hfacw(i, j, k, bi, bj) + hfacw(i, j-1, k, bi, bj) + (
     +        hfacw(i+1, j, k, bi, bj)+hfacw(i+1, j-1, k, bi, bj))
        ...
        DO j=sny+oly-1,2-oly,-1
          DO i=snx+olx-1,1-olx,-1
            vcoriolistermb(i, j) = hfacs(i, j, k, bi, bj)*4.d0*
     +        vcoriolistermb(i, j)/max2
      ...
      IF (branch .LT. 2) THEN
        IF (branch .EQ. 0) THEN
          DO j=sny+oly,2-oly,-1
            DO i=snx+olx-1,1-olx,-1
              tempb = -(recip_dyc(i, j, bi, bj)*masks(i, j, k, bi, bj)*
     +          vcoriolistermb(i, j))
              vcoriolistermb(i, j) = 0.D0
              vort3vb = ubarxy*tempb
              ubarxyb = vort3v*tempb
              CALL POPCONTROL2B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREAL8(vort3v)
                omega3b(i, j) = omega3b(i, j) + r_hfacz(i, j)*vort3vb
              ELSE IF (branch .EQ. 1) THEN
                CALL POPREAL8(vort3v)
                omega3b(i+1, j) = omega3b(i+1, j) + r_hfacz(i+1, j)*
     +            vort3vb
              ELSE
                CALL POPREAL8(vort3v)
                omega3b(i, j) = omega3b(i, j) + r_hfacz(i, j)*0.5*
     +            vort3vb
                omega3b(i+1, j) = omega3b(i+1, j) + r_hfacz(i+1, j)*0.5*
     +            vort3vb
              END IF
              CALL POPREAL8(ubarxy)
              tempb = 0.25*ubarxyb
              ufldb(i, j) = ufldb(i, j) + dyg(i, j, bi, bj)*hfacw(i, j, 
     +          k, bi, bj)*tempb
              ufldb(i, j-1) = ufldb(i, j-1) + dyg(i, j-1, bi, bj)*hfacw(
     +          i, j-1, k, bi, bj)*tempb
              ufldb(i+1, j) = ufldb(i+1, j) + dyg(i+1, j, bi, bj)*hfacw(
     +          i+1, j, k, bi, bj)*tempb
              ufldb(i+1, j-1) = ufldb(i+1, j-1) + dyg(i+1, j-1, bi, bj)*
     +          hfacw(i+1, j-1, k, bi, bj)*tempb
            ENDDO
          ENDDO
        ELSE
          DO j=sny+oly,2-oly,-1
     ...
     +          vcoriolistermb(i, j))
              ...
              IF (branch .EQ. 0) THEN
                CALL POPREAL8(vort3v)
                omega3b(i, j) = omega3b(i, j) + vort3vb
              ELSE IF (branch .EQ. 1) THEN
                CALL POPREAL8(vort3v)
                omega3b(i+1, j) = omega3b(i+1, j) + vort3vb
              ELSE
                CALL POPREAL8(vort3v)
                omega3b(i, j) = omega3b(i, j) + 0.5*vort3vb
                omega3b(i+1, j) = omega3b(i+1, j) + 0.5*vort3vb
              END IF
              CALL POPREAL8(ubarxy)
              tempb = 0.5*ubarxyb/max1
              ufldb(i, j) = ufldb(i, j) + hfacz(i, j)*dyg(i, j, bi, bj)*
     +          tempb
              ufldb(i, j-1) = ufldb(i, j-1) + hfacz(i, j)*dyg(i, j-1, bi
     +          , bj)*tempb
              ufldb(i+1, j) = ufldb(i+1, j) + hfacz(i+1, j)*dyg(i+1, j, 
     +          bi, bj)*tempb
              ufldb(i+1, j-1) = ufldb(i+1, j-1) + hfacz(i+1, j)*dyg(i+1
     +          , j-1, bi, bj)*tempb
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREAL8(max1)
      ...
      ELSE IF (branch .EQ. 2) THEN
        DO j=sny+oly,2-oly,-1
          DO i=snx+olx-1,1-olx,-1
            vort3vb = -(recip_dyc(i, j, bi, bj)*masks(i, j, k, bi, bj)*
     +        vcoriolistermb(i, j))
            vcoriolistermb(i, j) = 0.D0
            CALL POPCONTROL2B(branch)
            IF (branch .EQ. 0) THEN
              tempb = r_hfacz(i, j)*vort3vb
              ubarymb = omega3(i, j)*tempb
              omega3b(i, j) = omega3b(i, j) + ubarym*tempb
              ubarypb = 0.D0
            ELSE IF (branch .EQ. 1) THEN
              tempb = r_hfacz(i+1, j)*vort3vb
              ubarypb = omega3(i+1, j)*tempb
              omega3b(i+1, j) = omega3b(i+1, j) + ubaryp*tempb
              ubarymb = 0.D0
            ELSE
              tempb = r_hfacz(i, j)*0.5d0*vort3vb
              tempb0 = r_hfacz(i+1, j)*0.5d0*vort3vb
              ubarypb = omega3(i+1, j)*tempb0
              omega3b(i+1, j) = omega3b(i+1, j) + ubaryp*tempb0
              ubarymb = omega3(i, j)*tempb
              omega3b(i, j) = omega3b(i, j) + ubarym*tempb
            END IF
            CALL POPREAL8(ubaryp)
            ufldb(i+1, j) = ufldb(i+1, j) + dyg(i+1, j, bi, bj)*hfacw(i+
     +        1, j, k, bi, bj)*0.5*ubarypb
            ufldb(i+1, j-1) = ufldb(i+1, j-1) + dyg(i+1, j-1, bi, bj)*
     +        hfacw(i+1, j-1, k, bi, bj)*0.5*ubarypb
            CALL POPREAL8(ubarym)
            ufldb(i, j) = ufldb(i, j) + dyg(i, j, bi, bj)*hfacw(i, j, k
     +        , bi, bj)*0.5*ubarymb
            ufldb(i, j-1) = ufldb(i, j-1) + dyg(i, j-1, bi, bj)*hfacw(i
     +        , j-1, k, bi, bj)*0.5*ubarymb
          ENDDO
        ENDDO
      ELSE
        DO j=sny+oly-1,2-oly,-1
          DO i=snx+olx-1,1-olx,-1
            tempb0 = -(0.25d0*recip_dyc(i, j, bi, bj)*masks(i, j, k, bi
     +        , bj)*vcoriolistermb(i, j))
            vcoriolistermb(i, j) = 0.D0
            vort3imb = tempb0
            vort3ijb = tempb0
            vort3pmb = tempb0
            vort3pjb = tempb0
            tempb0 = onethird*dyg(i+1, j, bi, bj)*hfacw(i+1, j, k, bi, 
     +        bj)*vort3pjb
            tempb = ufld(i+1, j)*tempb0
            ufldb(i+1, j) = ufldb(i+1, j) + (r_hfacz(i+1, j)*omega3(i+1
     +        , j)+r_hfacz(i, j)*omega3(i, j)+r_hfacz(i+1, j+1)*omega3(i
     +        +1, j+1))*tempb0
            omega3b(i+1, j) = omega3b(i+1, j) + r_hfacz(i+1, j)*tempb
            omega3b(i, j) = omega3b(i, j) + r_hfacz(i, j)*tempb
            omega3b(i+1, j+1) = omega3b(i+1, j+1) + r_hfacz(i+1, j+1)*
     +        tempb
            tempb0 = onethird*dyg(i+1, j-1, bi, bj)*hfacw(i+1, j-1, k, 
     +        bi, bj)*vort3pmb
            tempb = ufld(i+1, j-1)*tempb0
            ufldb(i+1, j-1) = ufldb(i+1, j-1) + (r_hfacz(i+1, j)*omega3(
     +        i+1, j)+r_hfacz(i, j)*omega3(i, j)+r_hfacz(i+1, j-1)*
     +        omega3(i+1, j-1))*tempb0
            omega3b(i+1, j) = omega3b(i+1, j) + r_hfacz(i+1, j)*tempb
            omega3b(i, j) = omega3b(i, j) + r_hfacz(i, j)*tempb
            omega3b(i+1, j-1) = omega3b(i+1, j-1) + r_hfacz(i+1, j-1)*
     +        tempb
            tempb0 = onethird*dyg(i, j, bi, bj)*hfacw(i, j, k, bi, bj)*
     +        vort3ijb
            tempb = ufld(i, j)*tempb0
            ufldb(i, j) = ufldb(i, j) + (r_hfacz(i, j)*omega3(i, j)+
     +        r_hfacz(i+1, j)*omega3(i+1, j)+r_hfacz(i, j+1)*omega3(i, j
     +        +1))*tempb0
            omega3b(i, j) = omega3b(i, j) + r_hfacz(i, j)*tempb
            omega3b(i+1, j) = omega3b(i+1, j) + r_hfacz(i+1, j)*tempb
            omega3b(i, j+1) = omega3b(i, j+1) + r_hfacz(i, j+1)*tempb
            tempb0 = onethird*dyg(i, j-1, bi, bj)*hfacw(i, j-1, k, bi, 
     +        bj)*vort3imb
            tempb = ufld(i, j-1)*tempb0
            ufldb(i, j-1) = ufldb(i, j-1) + (r_hfacz(i, j)*omega3(i, j)+
     +        r_hfacz(i+1, j)*omega3(i+1, j)+r_hfacz(i, j-1)*omega3(i, j
     +        -1))*tempb0
            omega3b(i, j) = omega3b(i, j) + r_hfacz(i, j)*tempb
            omega3b(i+1, j) = omega3b(i+1, j) + r_hfacz(i+1, j)*tempb
