...
C
C  Differentiation of gad_diff_r in reverse (adjoint) mode:
C   gradient     of useful results: tracer kappar dfr
C   with respect to varying inputs: tracer kappar dfr
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_DIFF_R_B(bi, bj, k, maskup, kappar, kapparb, tracer
     +                        , tracerb, dfr, dfrb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C  dfr                  :: vertical diffusive flux
      ...
      REAL*8 dfrb(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
...
C
      ...
      ELSE
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            tempb = -(maskup(i, j)*deepfac2f(k)*rksign*ra(i, j, bi, bj)*
     +        rhofacf(k)*recip_drc(k)*dfrb(i, j))
            dfrb(i, j) = 0.D0
            kapparb(i, j) = kapparb(i, j) + (tracer(i, j, k)-tracer(i, j
     +        , km1))*tempb
            tempb0 = kappar(i, j)*tempb
            tracerb(i, j, k) = tracerb(i, j, k) + tempb0
            tracerb(i, j, km1) = tracerb(i, j, km1) - tempb0
          ENDDO
        ENDDO
      END IF
      END
