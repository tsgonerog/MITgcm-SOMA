...
C
C  Differentiation of mom_vi_v_vertshear in reverse (adjoint) mode:
C   gradient     of useful results: vfld vshearterm wfld
C   with respect to varying inputs: vfld vshearterm wfld
C
C
C
C
...
C
C
      SUBROUTINE MOM_VI_V_VERTSHEAR_B(bi, bj, k, vfld, vfldb, wfld, 
     +                                wfldb, vshearterm, vsheartermb, 
     +                                mythid)
      IMPLICIT NONE
C     *==========================================================*
...
C     == Routine arguments ==
      ...
      REAL*8 vsheartermb(1-olx:snx+olx, 1-oly:sny+oly)
      INTEGER mythid
C
C     == Local variables ==
      INTEGER i, j, kp1, km1
      REAL*8 mask_kp1, mask_km1, wbarym, wbaryp
      REAL*8 wbarymb, wbarypb
      REAL*8 vzm, vzp
      REAL*8 vzmb, vzpb
...
C       Transport at interface k : Area weighted average
            CALL PUSHREAL8(wbarym)
            wbarym = 0.5*(wfld(i, j, k, bi, bj)*ra(i, j, bi, bj)*maskc(i
     +        , j, km1, bi, bj)+wfld(i, j-1, k, bi, bj)*ra(i, j-1, bi, 
     +        bj)*maskc(i, j-1, km1, bi, bj))*mask_km1*deepfac2f(k)*
     +        rhofacf(k)*recip_ras(i, j, bi, bj)
C
C       Transport at interface k+1 (here wFld is already masked)
            CALL PUSHREAL8(wbaryp)
            wbaryp = 0.5*(wfld(i, j, kp1, bi, bj)*ra(i, j, bi, bj)+wfld(
     +        i, j-1, kp1, bi, bj)*ra(i, j-1, bi, bj))*mask_kp1*
     +        deepfac2f(kp1)*rhofacf(kp1)*recip_ras(i, j, bi, bj)
            CALL PUSHCONTROL1B(0)
          ELSE
C       Transport at interface k : simple average
            CALL PUSHREAL8(wbarym)
            wbarym = 0.5*(wfld(i, j, k, bi, bj)*maskc(i, j, km1, bi, bj)
     +        +wfld(i, j-1, k, bi, bj)*maskc(i, j-1, km1, bi, bj))*
     +        mask_km1*deepfac2f(k)*rhofacf(k)
C
C       Transport at interface k+1 (here wFld is already masked)
            CALL PUSHREAL8(wbaryp)
            wbaryp = 0.5*(wfld(i, j, kp1, bi, bj)+wfld(i, j-1, kp1, bi, 
...
C2   &                  *maskS(I,J,K,bi,bj)
      ...
      DO j=sny+oly,2-oly,-1
        DO i=snx+olx,1-olx,-1
          ...
          IF (branch .EQ. 0) THEN
            vzm = (vfld(i, j, k, bi, bj)-mask_km1*vfld(i, j, km1, bi, bj
     +        ))*rksign
            vzp = (mask_kp1*vfld(i, j, kp1, bi, bj)-vfld(i, j, k, bi, bj
     +        ))*rksign
            tempb = -(recip_hfacs(i, j, k, bi, bj)*recip_drf(k)*0.5*
     +        recip_deepfac2c(k)*recip_rhofacc(k)*vsheartermb(i, j))
            vsheartermb(i, j) = 0.D0
            wbarypb = vzp*tempb
            vzpb = wbaryp*tempb
            wbarymb = vzm*tempb
            vzmb = wbarym*tempb
     ...
     +        recip_deepfac2c(k)*recip_rhofacc(k)*vsheartermb(i, j))
            vsheartermb(i, j) = 0.D0
            wbarypb = vzp*tempb
            vzpb = (wbaryp+abs0)*tempb
            wbarymb = vzm*tempb
            vzmb = (wbarym-abs1)*tempb
            abs0b = vzp*tempb
            abs1b = -(vzm*tempb)
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              CALL POPREAL8(abs1)
              wbarymb = wbarymb + abs1b
            ELSE
              CALL POPREAL8(abs1)
              wbarymb = wbarymb - abs1b
            END IF
            ...
            IF (branch .EQ. 0) THEN
              CALL POPREAL8(abs0)
              wbarypb = wbarypb - abs0b
            ELSE
              CALL POPREAL8(abs0)
              wbarypb = wbarypb + abs0b
            END IF
          END IF
          vfldb(i, j, kp1, bi, bj) = vfldb(i, j, kp1, bi, bj) + mask_kp1
     +      *rksign*vzpb
          vfldb(i, j, k, bi, bj) = vfldb(i, j, k, bi, bj) + rksign*vzmb 
     +      - rksign*vzpb
          vfldb(i, j, km1, bi, bj) = vfldb(i, j, km1, bi, bj) - mask_km1
     +      *rksign*vzmb
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL8(wbaryp)
            tempb = deepfac2f(kp1)*rhofacf(kp1)*mask_kp1*0.5*recip_ras(i
     +        , j, bi, bj)*wbarypb
            wfldb(i, j, kp1, bi, bj) = wfldb(i, j, kp1, bi, bj) + ra(i, 
     +        j, bi, bj)*tempb
            wfldb(i, j-1, kp1, bi, bj) = wfldb(i, j-1, kp1, bi, bj) + ra
     +        (i, j-1, bi, bj)*tempb
            CALL POPREAL8(wbarym)
            tempb = deepfac2f(k)*rhofacf(k)*mask_km1*0.5*recip_ras(i, j
     +        , bi, bj)*wbarymb
            wfldb(i, j, k, bi, bj) = wfldb(i, j, k, bi, bj) + ra(i, j, 
     +        bi, bj)*maskc(i, j, km1, bi, bj)*tempb
            wfldb(i, j-1, k, bi, bj) = wfldb(i, j-1, k, bi, bj) + ra(i, 
     +        j-1, bi, bj)*maskc(i, j-1, km1, bi, bj)*tempb
          ELSE
            CALL POPREAL8(wbaryp)
            tempb = mask_kp1*0.5*deepfac2f(kp1)*rhofacf(kp1)*wbarypb
            wfldb(i, j, kp1, bi, bj) = wfldb(i, j, kp1, bi, bj) + tempb
            wfldb(i, j-1, kp1, bi, bj) = wfldb(i, j-1, kp1, bi, bj) + 
     +        tempb
            CALL POPREAL8(wbarym)
            tempb = mask_km1*0.5*deepfac2f(k)*rhofacf(k)*wbarymb
            wfldb(i, j, k, bi, bj) = wfldb(i, j, k, bi, bj) + maskc(i, j
     +        , km1, bi, bj)*tempb
            wfldb(i, j-1, k, bi, bj) = wfldb(i, j-1, k, bi, bj) + maskc(
     +        i, j-1, km1, bi, bj)*tempb
          END IF
        ENDDO
      ENDDO
      END
