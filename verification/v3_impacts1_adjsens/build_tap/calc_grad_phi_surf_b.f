...
C
C  Differentiation of calc_grad_phi_surf in reverse (adjoint) mode:
C   gradient     of useful results: phisurfx phisurfy etafld
C   with respect to varying inputs: phisurfx phisurfy etafld
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE CALC_GRAD_PHI_SURF_B(bi, bj, imin, imax, jmin, jmax, 
     +                                etafld, etafldb, phisurfx, 
     +                                phisurfxb, phisurfy, phisurfyb, 
     +                                mythid)
      IMPLICIT NONE
C     == Global variables ==
CBOP
...
C     myThid :: Instance number for this call of the routine.
      ...
      REAL*8 etafldb(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
...
C     i,j :: Loop counters
      ...
      DO j=jmax,jmin,-1
        DO i=imax,imin,-1
          tempb = recip_dyc(i, j, bi, bj)*phisurfyb(i, j)
          phisurfyb(i, j) = 0.D0
          etafldb(i, j, bi, bj) = etafldb(i, j, bi, bj) + bo_surf(i, j, 
     +      bi, bj)*tempb
          etafldb(i, j-1, bi, bj) = etafldb(i, j-1, bi, bj) - bo_surf(i
     +      , j-1, bi, bj)*tempb
        ENDDO
      ENDDO
      DO j=jmax,jmin,-1
        DO i=imax,imin,-1
          tempb = recip_dxc(i, j, bi, bj)*phisurfxb(i, j)
          phisurfxb(i, j) = 0.D0
          etafldb(i, j, bi, bj) = etafldb(i, j, bi, bj) + bo_surf(i, j, 
     +      bi, bj)*tempb
          etafldb(i-1, j, bi, bj) = etafldb(i-1, j, bi, bj) - bo_surf(i-
     +      1, j, bi, bj)*tempb
        ENDDO
      ENDDO
      END
