...
C
C  Differentiation of integr_continuity in reverse (adjoint) mode:
C   gradient     of useful results: etan wvel etah empmr detahdt
C                pmepr vfld ufld
C   with respect to varying inputs: etan wvel etah empmr detahdt
C                pmepr vfld ufld
C
C
C
...
C     !INTERFACE:
      SUBROUTINE INTEGR_CONTINUITY_B(ufld, ufldb, vfld, vfldb, mytime, 
     +                               myiter, mythid)
      IMPLICIT NONE
C     == Global variables
CBOP
...
C     myThid   :: my Thread Id. number
      ...
      REAL*8 ufldb(1-olx:snx+olx, 1-oly:sny+oly, nr, nsx, nsy)
      REAL*8 vfld(1-olx:snx+olx, 1-oly:sny+oly, nr, nsx, nsy)
      REAL*8 vfldb(1-olx:snx+olx, 1-oly:sny+oly, nr, nsx, nsy)
C
C     !LOCAL VARIABLES:
...
C     hDivFlow :: Div. Barotropic Flow [transport unit m3/s]
      ...
      REAL*8 hdivflowb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 facemp, facmass
      REAL*8 addmass(1)
      REAL*8 rstardhdt(1)
      EXTERNAL DUMMY_FOR_ETAN
      EXTERNAL DEBUG_CALL
      REAL*8 tempb
      INTEGER*4 branch
      INTEGER ii2
...
C
            CALL PUSHREAL8(facemp)
            facemp = 0.
            IF (fluidiswater .AND. userealfreshwaterflux) facemp = 
     +          mass2runit
C
...
C     anelastic: uTrans,vTrans are scaled by rhoFacC (~ mass transport).
     ...
     +              *deepfacc(k)*rhofacc(k)*drf(k)*hfacw(i, j, k, bi, bj
     +              )
                  vtrans(i, j) = vfld(i, j, k, bi, bj)*dxg(i, j, bi, bj)
     +              *deepfacc(k)*rhofacc(k)*drf(k)*hfacs(i, j, k, bi, bj
     +              )
                ENDDO
              ENDDO
C
C-    Integrate vertically the Horizontal Divergence
              DO j=1,sny
                DO i=1,snx
                  hdivflow(i, j) = hdivflow(i, j) + maskc(i, j, k, bi, 
     +              bj)*(utrans(i+1, j)-utrans(i, j)+vtrans(i, j+1)-
     +              vtrans(i, j))
                ENDDO
              ENDDO
            ENDDO
...
C       anelastic: always assumes that rhoFacF(1) = 1
            IF (myiter .EQ. niter0 .AND. myiter .NE. 0 .AND. 
...
C     and was not in old pickup-file ; try to use etaN & etaH instead.
              IF (usepickupbeforec54) THEN
                DO j=1,sny
                  DO i=1,snx
                    detahdt(i, j, bi, bj) = (etan(i, j, bi, bj)-etah(i, 
     +                j, bi, bj))/(implicdiv2dflow*deltatfreesurf)
                  ENDDO
                ENDDO
...
C
              DO j=1,sny
                DO i=1,snx
                  pmepr(i, j, bi, bj) = detahdt(i, j, bi, bj) + hdivflow
     +              (i, j)*recip_ra(i, j, bi, bj)*recip_deepfac2f(1)
                  pmepr(i, j, bi, bj) = pmepr(i, j, bi, bj)*runit2mass
                ENDDO
              ENDDO
              CALL PUSHCONTROL2B(0)
            ELSE IF (myiter .EQ. niter0) THEN
              DO j=1,sny
                DO i=1,snx
                  ks = ksurfc(i, j, bi, bj)
                  pmepr(i, j, bi, bj) = 0.d0
                  detahdt(i, j, bi, bj) = -(hdivflow(i, j)*recip_ra(i, j
     +              , bi, bj)*recip_deepfac2f(ks))
                ENDDO
              ENDDO
...
C       (e.g., if using KPP) => set over full index range
              ...
              DO j=1,sny
                DO i=1,snx
                  ks = ksurfc(i, j, bi, bj)
                  detahdt(i, j, bi, bj) = -(hdivflow(i, j)*recip_ra(i, j
     +              , bi, bj)*recip_deepfac2f(ks)) - facemp*empmr(i, j, 
     +              bi, bj)
                ENDDO
              ENDDO
...
C
            CALL PUSHREAL8ARRAY(wvel, 70**2*31)
            CALL INTEGRATE_FOR_W(bi, bj, k, ufld, vfld, addmass, 
     +                           rstardhdt, wvel, mythid)
C
            IF (k .EQ. nr .AND. myiter .NE. 0 .AND. usingpcoords .AND. 
     +          fluidiswater .AND. userealfreshwaterflux) THEN
C
              DO j=1,sny
                DO i=1,snx
...
C     In reverse, print adjoint variable for etaN
      CALL PUSHREAL8ARRAY(etan, 70**2)
      CALL EXCH_XY_RL(etan, mythid)
C
      IF (implicitintgravwave .OR. myiter .EQ. niter0) THEN
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
      END IF
...
C--   Update etaH (from etaN and dEtaHdt):
      IF (exactconserv) CALL UPDATE_ETAH_B(mytime, myiter, mythid)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) CALL EXCH_XYZ_RL_B(wvel, wvelb, mythid)
      CALL POPREAL8ARRAY(etan, 70**2)
      CALL EXCH_XY_RL_B(etan, etanb, mythid)
      ...
      DO bj=mybyhi(mythid),mybylo(mythid),-1
        DO 100 bi=mybxhi(mythid),mybxlo(mythid),-1
          DO k=1,nr,1
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              DO j=sny,1,-1
                DO i=snx,1,-1
                  pmeprb(i, j, bi, bj) = pmeprb(i, j, bi, bj) + 
     +              mass2runit*maskc(i, j, k, bi, bj)*wvelb(i, j, k, bi
     +              , bj)
                ENDDO
              ENDDO
            END IF
            CALL POPREAL8ARRAY(wvel, 70**2*31)
            CALL INTEGRATE_FOR_W_B(bi, bj, k, ufld, ufldb, vfld, vfldb, 
     +                             addmass, rstardhdt, wvel, wvelb, 
     +                             mythid)
          ENDDO
          ...
          IF (branch .NE. 0) THEN
            IF (branch .EQ. 1) THEN
              DO j=sny,1,-1
                DO i=snx,1,-1
                  CALL POPREAL8(etan(i, j, bi, bj))
                  etahb(i, j, bi, bj) = etahb(i, j, bi, bj) + etanb(i, j
     +              , bi, bj)
                  detahdtb(i, j, bi, bj) = detahdtb(i, j, bi, bj) + 
     +              implicdiv2dflow*deltatfreesurf*etanb(i, j, bi, bj)
                  etanb(i, j, bi, bj) = 0.D0
                ENDDO
              ENDDO
            ELSE
     ...
     +              , bi, bj)
          ...
          IF (branch .LT. 2) THEN
            IF (branch .EQ. 0) THEN
              DO j=sny,1,-1
                DO i=snx,1,-1
                  pmeprb(i, j, bi, bj) = runit2mass*pmeprb(i, j, bi, bj)
                  detahdtb(i, j, bi, bj) = detahdtb(i, j, bi, bj) + 
     +              pmeprb(i, j, bi, bj)
                  hdivflowb(i, j) = hdivflowb(i, j) + recip_deepfac2f(1)
     +              *recip_ra(i, j, bi, bj)*pmeprb(i, j, bi, bj)
                  pmeprb(i, j, bi, bj) = 0.D0
                ENDDO
              ENDDO
              CALL POPCONTROL1B(branch)
              IF (branch .NE. 0) THEN
                DO j=sny,1,-1
                  DO i=snx,1,-1
                    tempb = detahdtb(i, j, bi, bj)/(implicdiv2dflow*
     +                deltatfreesurf)
                    detahdtb(i, j, bi, bj) = 0.D0
                    etanb(i, j, bi, bj) = etanb(i, j, bi, bj) + tempb
                    etahb(i, j, bi, bj) = etahb(i, j, bi, bj) - tempb
                  ENDDO
                ENDDO
              END IF
            ELSE
              DO j=sny,1,-1
                DO i=snx,1,-1
                  ks = ksurfc(i, j, bi, bj)
                  hdivflowb(i, j) = hdivflowb(i, j) - recip_deepfac2f(ks
     +              )*recip_ra(i, j, bi, bj)*detahdtb(i, j, bi, bj)
                  detahdtb(i, j, bi, bj) = 0.D0
                  pmeprb(i, j, bi, bj) = 0.D0
                ENDDO
              ENDDO
          ...
          ELSE IF (branch .EQ. 2) THEN
            DO j=sny,1,-1
              DO i=snx,1,-1
                ...
                hdivflowb(i, j) = hdivflowb(i, j) - recip_deepfac2f(ks)*
     +            recip_ra(i, j, bi, bj)*detahdtb(i, j, bi, bj)
                empmrb(i, j, bi, bj) = empmrb(i, j, bi, bj) - facemp*
     +            detahdtb(i, j, bi, bj)
                detahdtb(i, j, bi, bj) = 0.D0
              ENDDO
            ENDDO
            DO j=sny+oly,1-oly,-1
              DO i=snx+olx,1-olx,-1
                empmrb(i, j, bi, bj) = empmrb(i, j, bi, bj) - pmeprb(i, 
     +            j, bi, bj)
                pmeprb(i, j, bi, bj) = 0.D0
              ENDDO
            ENDDO
          ...
          DO k=nr,1,-1
            DO j=sny,1,-1
              DO i=snx,1,-1
                tempb = maskc(i, j, k, bi, bj)*hdivflowb(i, j)
            ...
            DO j=sny+1,1,-1
              DO i=snx+1,1,-1
                vfldb(i, j, k, bi, bj) = vfldb(i, j, k, bi, bj) + dxg(i
     +            , j, bi, bj)*drf(k)*deepfacc(k)*rhofacc(k)*hfacs(i, j
     +            , k, bi, bj)*vtransb(i, j)
                vtransb(i, j) = 0.D0
                ufldb(i, j, k, bi, bj) = ufldb(i, j, k, bi, bj) + dyg(i
     +            , j, bi, bj)*drf(k)*deepfacc(k)*rhofacc(k)*hfacw(i, j
     +            , k, bi, bj)*utransb(i, j)
                utransb(i, j) = 0.D0
              ENDDO
            ENDDO
