...
C
C  Differentiation of do_stagger_fields_exchanges in reverse (adjoint) mode:
C   gradient     of useful results: uvel vvel wvel theta salt
C   with respect to varying inputs: uvel vvel wvel theta salt
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE DO_STAGGER_FIELDS_EXCHANGES_B(mytime, myiter, mythid)
      IMPLICIT NONE
C     == Global variables ===
CBOP
C    !ROUTINE: SIZE.h
...
C #ifdef ALLOW_NONHYDROSTATIC
        IF (implicitintgravwave) THEN
          CALL EXCH_SM_3D_RL_B(salt, saltb, .false., nr, mythid)
          CALL EXCH_SM_3D_RL_B(theta, thetab, .false., nr, mythid)
        END IF
        CALL POPCONTROL2B(branch)
        IF (branch .EQ. 0) THEN
          CALL EXCH_3D_RL_B(wvel, wvelb, nr, mythid)
        ELSE IF (branch .NE. 1) THEN
          GOTO 100
        END IF
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) CALL EXCH_UV_3D_RL_B(uvel, uvelb, vvel, vvelb
     +                                          , .true., nr, mythid)
      END IF
 100  CONTINUE
      END
