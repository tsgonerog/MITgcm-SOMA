...
C
C  Differentiation of gad_c4_adv_r in reverse (adjoint) mode:
C   gradient     of useful results: wt tracer rtrans
C   with respect to varying inputs: wt tracer rtrans
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_C4_ADV_R_B(bi, bj, k, rtrans, rtransb, tracer, 
     +                          tracerb, wt, wtb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C  maskP1               :: =1 for k<Nr, =0 for k>=Nr
      ...
      REAL*8 rjmb, rjb, rjpb, rjjmb, rjjpb
      ...
      ELSE
        ...
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            ...
            rjm = (tracer(i, j, km1)-tracer(i, j, km2))*maskc(i, j, km1
     +        , bi, bj)
            rjjm = rj - rjm
            rjp = (tracer(i, j, kp1)-tracer(i, j, k))*maskc(i, j, kp1, 
     +        bi, bj)
            rjjp = rjp - rj
            maskbound = maskpm*maskc(i, j, km2, bi, bj)*maskc(i, j, kp1
     +        , bi, bj)
            tempb = maskc(i, j, km1, bi, bj)*wtb(i, j)
            wtb(i, j) = 0.D0
            rtransb(i, j) = rtransb(i, j) + (0.5d0*(tracer(i, j, k)+
     +        tracer(i, j, km1))-onesixth*0.5d0*(rjjm+rjjp))*tempb
            tempb0 = rtrans(i, j)*tempb
            tempb2 = onesixth*0.5d0*(1.d0-maskbound)*tempb
            abs0b = (rjjm-rjjp)*tempb2
            tracerb(i, j, k) = tracerb(i, j, k) + 0.5d0*tempb0
            tracerb(i, j, km1) = tracerb(i, j, km1) + 0.5d0*tempb0
            tempb1 = -(onesixth*0.5d0*tempb0)
            rjjmb = abs0*tempb2 + tempb1
            rjjpb = tempb1 - abs0*tempb2
