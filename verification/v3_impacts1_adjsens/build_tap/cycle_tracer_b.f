...
C
C  Differentiation of cycle_tracer in reverse (adjoint) mode:
C   gradient     of useful results: tracer
C   with respect to varying inputs: tracer gtracer
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE CYCLE_TRACER_B(bi, bj, tracer, tracerb, gtracer, 
     +                          gtracerb, mytime, myiter, mythid)
      IMPLICIT NONE
C Common
CBOP
C    !ROUTINE: SIZE.h
      ...
      DO k=nr,1,-1
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            gtracerb(i, j, k) = gtracerb(i, j, k) + tracerb(i, j, k)
            tracerb(i, j, k) = 0.D0
          ENDDO
        ENDDO
      ENDDO
