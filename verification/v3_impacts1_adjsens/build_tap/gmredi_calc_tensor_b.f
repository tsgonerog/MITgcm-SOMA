...
C
C  Differentiation of gmredi_calc_tensor in reverse (adjoint) mode:
C   gradient     of useful results: kwx kwy kwz kux kvy kuz kvz
C                gm_psix gm_psiy sigmar sigmax sigmay
C   with respect to varying inputs: kwx kwy kwz kux kvy kuz kvz
C                gm_psix gm_psiy sigmar sigmax sigmay
C
C
C
...
C     !INTERFACE:
      SUBROUTINE GMREDI_CALC_TENSOR_B(imin, imax, jmin, jmax, sigmax, 
     +                                sigmaxb, sigmay, sigmayb, sigmar, 
     +                                sigmarb, bi, bj, mytime, myiter, 
     +                                mythid)
      IMPLICIT NONE
C
C     == Global variables ==
...
C     maskFk    :: 2-D mask for vertical interface k (between level k-1 & k)
      ...
      REAL*8 slopexb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 slopey(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 slopeyb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dsigmadx(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dsigmadxb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dsigmady(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dsigmadyb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dsigmadr(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dsigmadrb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 slopesqr(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 slopesqrb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 taperfct(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 taperfctb(1-olx:snx+olx, 1-oly:sny+oly)
...
C
      ...
      EXTERNAL GMREDI_DIAGNOSTICS_FILL
      REAL*8 y1
      REAL*8 y2
      REAL*8 y3
      REAL*8 abs0
        ...
        DO j=1-oly,sny+oly
          DO i=1-olx,snx+olx
            IF (fcori(i, j, bi, bj) .NE. 0.) THEN
              IF (fcori(i, j, bi, bj) .GE. 0.) THEN
                CALL PUSHCONTROL1B(0)
                abs0 = fcori(i, j, bi, bj)
              ELSE
                CALL PUSHCONTROL1B(0)
                abs0 = -fcori(i, j, bi, bj)
              END IF
              ldd97_lrhoc(i, j) = cspd/abs0
            ELSE
              CALL PUSHCONTROL1B(1)
              ldd97_lrhoc(i, j) = lrhosup
            END IF
            IF (ldd97_lrhoc(i, j) .GT. lrhosup) THEN
              y1 = lrhosup
            ELSE
              y1 = ldd97_lrhoc(i, j)
            END IF
            IF (lrhoinf .LT. y1) THEN
              CALL PUSHCONTROL1B(0)
              ldd97_lrhoc(i, j) = y1
            ELSE
              CALL PUSHCONTROL1B(1)
              ldd97_lrhoc(i, j) = lrhoinf
            END IF
          ENDDO
        ENDDO
          ...
          DO i=1-olx+1,snx+olx
            ...
            IF (fcoriloc .NE. zerorl) THEN
              ...
              ELSE
                ...
                abs1 = -fcoriloc
              END IF
              ldd97_lrhow(i, j) = cspd/abs1
            ELSE
              CALL PUSHCONTROL1B(1)
              ldd97_lrhow(i, j) = lrhosup
            END IF
            IF (ldd97_lrhow(i, j) .GT. lrhosup) THEN
              y2 = lrhosup
            ELSE
              y2 = ldd97_lrhow(i, j)
            END IF
            IF (lrhoinf .LT. y2) THEN
              CALL PUSHCONTROL1B(0)
              ldd97_lrhow(i, j) = y2
            ELSE
              CALL PUSHCONTROL1B(1)
              ldd97_lrhow(i, j) = lrhoinf
            END IF
          ENDDO
        ENDDO
          ...
          DO i=1-olx,snx+olx
            ...
            IF (fcoriloc .NE. zerorl) THEN
              ...
              ELSE
                ...
                abs2 = -fcoriloc
              END IF
              ldd97_lrhos(i, j) = cspd/abs2
            ELSE
              CALL PUSHCONTROL1B(1)
              ldd97_lrhos(i, j) = lrhosup
            END IF
            IF (ldd97_lrhos(i, j) .GT. lrhosup) THEN
              y3 = lrhosup
            ELSE
              y3 = ldd97_lrhos(i, j)
            END IF
            IF (lrhoinf .LT. y3) THEN
              CALL PUSHCONTROL1B(0)
              ldd97_lrhos(i, j) = y3
            ELSE
              CALL PUSHCONTROL1B(1)
              ldd97_lrhos(i, j) = lrhoinf
            END IF
          ENDDO
        ENDDO
        CALL PUSHCONTROL1B(1)
...
C
        DO j=1-oly,sny+oly
          DO i=1-olx,snx+olx
            CALL PUSHREAL8(slopex(i, j))
            slopex(i, j) = 0.d0
            CALL PUSHREAL8(slopey(i, j))
            slopey(i, j) = 0.d0
            CALL PUSHREAL8(dsigmadx(i, j))
            dsigmadx(i, j) = 0.d0
            CALL PUSHREAL8(dsigmady(i, j))
            dsigmady(i, j) = 0.d0
            dsigmadr(i, j) = 0.d0
            CALL PUSHREAL8(slopesqr(i, j))
            slopesqr(i, j) = 0.d0
            CALL PUSHREAL8(taperfct(i, j))
            taperfct(i, j) = 0.d0
            CALL PUSHREAL8(maskfk(i, j))
            maskfk(i, j) = maskc(i, j, k-1, bi, bj)*maskc(i, j, k, bi, 
     +        bj)
          ENDDO
        ENDDO
C
...
C      Gradient of Sigma at rVel points
     ...
     +        +sigmax(i+1, j, k)+sigmax(i, j, k))*maskfk(i, j)
            CALL PUSHREAL8(dsigmady(i, j))
            dsigmady(i, j) = op25*(sigmay(i, j+1, k-1)+sigmay(i, j, k-1)
     +        +sigmay(i, j+1, k)+sigmay(i, j, k))*maskfk(i, j)
...
C     Calculate slopes for use in tensor, taper and/or clip
        CALL PUSHREAL8ARRAY(dsigmadr, 70**2)
        CALL PUSHREAL8ARRAY(taperfct, 70**2)
        CALL PUSHREAL8ARRAY(slopesqr, 70**2)
        CALL PUSHREAL8ARRAY(slopey, 70**2)
        CALL PUSHREAL8ARRAY(slopex, 70**2)
        CALL GMREDI_SLOPE_LIMIT(slopex, slopey, slopesqr, taperfct, 
     +                          htranslay, baseslope, reciplambda, 
     +                          dsigmadr, dsigmadx, dsigmady, 
     +                          ldd97_lrhoc, locmixlayer, rdepth, rf, 
     +                          klowc(1-olx, 1-oly, bi, bj), 3, k, bi, 
...
C     Update components of Redi/GM tensor, second step: multiply by Diffusivity
      DO k=1,nr
        ...
        ELSE
          km1 = k - 1
        END IF
        CALL PUSHREAL8(isopyck)
        isopyck = gm_isopyck*(gm_isofac1d(km1)+gm_isofac1d(k))*op5
        bolus_k = gm_background_k*(gm_bolfac1d(km1)+gm_bolfac1d(k))*op5
        DO j=1-oly+1,sny+oly-1
          DO i=1-olx+1,snx+olx-1
            CALL PUSHREAL8(kgm_tmp)
            kgm_tmp = isopyck*gm_isofac2d(i, j, bi, bj) + gm_skewflx*
     +        bolus_k*gm_bolfac2d(i, j, bi, bj)
...
C     Gradient of Sigma at U points
     ...
     +        sigmar(i-1, j, kp1)+sigmar(i, j, kp1))*maskp1)*maskw(i, j
     +        , k, bi, bj)*gravitysign
          ENDDO
        ENDDO
C
...
C     Calculate slopes for use in tensor, taper and/or clip
     ...
     +                          htranslay, baseslope, reciplambda, 
     +                          dsigmadr, dsigmadx, dsigmady, 
     +                          ldd97_lrhow, locmixlayer, rdepth, rc, 
     +                          klow_w, 1, k, bi, bj, mytime, myiter, 
     +                          mythid)
C
C
C      IF ( GM_nonUnitDiag ) THEN
        DO j=1-oly+1,sny+oly-1
          DO i=1-olx+1,snx+olx-1
            kux(i, j, k, bi, bj) = gm_isopyck*gm_isofac1d(k)*op5*(
     +        gm_isofac2d(i-1, j, bi, bj)+gm_isofac2d(i, j, bi, bj))*
     +        taperfct(i, j)
C no GM_GEOM_VARIABLE here because this is the Redi part
          ENDDO
        ENDDO
        DO j=1-oly+1,sny+oly-1
          DO i=1-olx+1,snx+olx-1
            IF (kux(i, j, k, bi, bj) .LT. gm_kmin_horiz) THEN
...
C
     ...
     +        sigmax(i, j-1, k)+sigmax(i+1, j-1, k))*masks(i, j, k, bi, 
     +        bj)
            CALL PUSHREAL8(dsigmady(i, j))
            dsigmady(i, j) = sigmay(i, j, k)*masks(i, j, k, bi, bj)
            dsigmadr(i, j) = op25*(sigmar(i, j-1, k)+sigmar(i, j, k)+(
     +        sigmar(i, j-1, kp1)+sigmar(i, j, kp1))*maskp1)*masks(i, j
     +        , k, bi, bj)*gravitysign
          ENDDO
        ENDDO
C
...
C     Calculate slopes for use in tensor, taper and/or clip
     ...
     +                          htranslay, baseslope, reciplambda, 
     +                          dsigmadr, dsigmadx, dsigmady, 
     +                          ldd97_lrhos, locmixlayer, rdepth, rc, 
     +                          klow_s, 2, k, bi, bj, mytime, myiter, 
     +                          mythid)
C
C
C      IF ( GM_nonUnitDiag ) THEN
        DO j=1-oly+1,sny+oly-1
          DO i=1-olx+1,snx+olx-1
            kvy(i, j, k, bi, bj) = gm_isopyck*gm_isofac1d(k)*op5*(
     +        gm_isofac2d(i, j-1, bi, bj)+gm_isofac2d(i, j, bi, bj))*
     +        taperfct(i, j)
C no GM_GEOM_VARIABLE here because this is the Redi part
          ENDDO
        ENDDO
        DO j=1-oly+1,sny+oly-1
          DO i=1-olx+1,snx+olx-1
            IF (kvy(i, j, k, bi, bj) .LT. gm_kmin_horiz) THEN
...
C
      ...
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          taperfctb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          slopexb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          slopeyb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          dsigmadrb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          dsigmadxb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          dsigmadyb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO ii1=1-oly,oly+sny
        DO ii2=1-olx,olx+snx
          slopesqrb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO k=1,nr,1
        ...
        IF (branch .EQ. 0) THEN
          DO j=sny+oly-1,1-oly+1,-1
            DO i=snx+olx-1,1-olx+1,-1
              tempb = -(gravitysign*(gm_isofac1d(k)*(gm_isopyck*op5)*(
     +          gm_isofac2d(i, j-1, bi, bj)+gm_isofac2d(i, j, bi, bj))-
     +          gm_skewflx*gm_background_k*(gm_bolfac1d(k)*op5)*(
     +          gm_bolfac2d(i, j-1, bi, bj)+gm_bolfac2d(i, j, bi, bj)))*
     +          kvzb(i, j, k, bi, bj))
              kvzb(i, j, k, bi, bj) = 0.D0
              slopeyb(i, j) = slopeyb(i, j) + taperfct(i, j)*tempb
              taperfctb(i, j) = taperfctb(i, j) + slopey(i, j)*tempb
            ENDDO
          ENDDO
        END IF
        DO j=sny+oly-1,1-oly+1,-1
          DO i=snx+olx-1,1-olx+1,-1
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) kvyb(i, j, k, bi, bj) = 0.D0
          ENDDO
        ENDDO
        DO j=sny+oly-1,1-oly+1,-1
          DO i=snx+olx-1,1-olx+1,-1
            taperfctb(i, j) = taperfctb(i, j) + gm_isofac1d(k)*
     +        gm_isopyck*op5*(gm_isofac2d(i, j-1, bi, bj)+gm_isofac2d(i
     +        , j, bi, bj))*kvyb(i, j, k, bi, bj)
            kvyb(i, j, k, bi, bj) = 0.D0
          ENDDO
        ENDDO
        CALL POPREAL8ARRAY(slopex, 70**2)
        CALL POPREAL8ARRAY(slopey, 70**2)
        CALL POPREAL8ARRAY(slopesqr, 70**2)
        CALL POPREAL8ARRAY(taperfct, 70**2)
        CALL POPREAL8ARRAY(dsigmadr, 70**2)
        CALL GMREDI_SLOPE_LIMIT_B(slopex, slopexb, slopey, slopeyb, 
     +                            slopesqr, slopesqrb, taperfct, 
     +                            taperfctb, htranslay, baseslope, 
     +                            reciplambda, dsigmadr, dsigmadrb, 
     +                            dsigmadx, dsigmadxb, dsigmady, 
     +                            dsigmadyb, ldd97_lrhos, locmixlayer, 
     +                            rdepth, rc, klow_s, 2, k, bi, bj, 
     +                            mytime, myiter, mythid)
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREAL8(rdepth)
        ELSE
          CALL POPREAL8(rdepth)
        END IF
        DO j=sny+oly-1,1-oly+1,-1
          DO i=snx+olx-1,1-olx+1,-1
            tempb = masks(i, j, k, bi, bj)*op25*gravitysign*dsigmadrb(i
     +        , j)
            dsigmadrb(i, j) = 0.D0
            sigmarb(i, j-1, k) = sigmarb(i, j-1, k) + tempb
            sigmarb(i, j, k) = sigmarb(i, j, k) + tempb
            sigmarb(i, j-1, kp1) = sigmarb(i, j-1, kp1) + maskp1*tempb
            sigmarb(i, j, kp1) = sigmarb(i, j, kp1) + maskp1*tempb
            CALL POPREAL8(dsigmady(i, j))
            sigmayb(i, j, k) = sigmayb(i, j, k) + masks(i, j, k, bi, bj)
     +        *dsigmadyb(i, j)
            dsigmadyb(i, j) = 0.D0
            CALL POPREAL8(dsigmadx(i, j))
            tempb = op25*masks(i, j, k, bi, bj)*dsigmadxb(i, j)
            dsigmadxb(i, j) = 0.D0
            sigmaxb(i, j, k) = sigmaxb(i, j, k) + tempb
            sigmaxb(i+1, j, k) = sigmaxb(i+1, j, k) + tempb
            sigmaxb(i, j-1, k) = sigmaxb(i, j-1, k) + tempb
            sigmaxb(i+1, j-1, k) = sigmaxb(i+1, j-1, k) + tempb
          ENDDO
        ENDDO
        CALL POPREAL8(maskp1)
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
     ...
     +          kuzb(i, j, k, bi, bj))
              kuzb(i, j, k, bi, bj) = 0.D0
              slopexb(i, j) = slopexb(i, j) + taperfct(i, j)*tempb
              taperfctb(i, j) = taperfctb(i, j) + slopex(i, j)*tempb
            ENDDO
          ENDDO
        END IF
        DO j=sny+oly-1,1-oly+1,-1
          DO i=snx+olx-1,1-olx+1,-1
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) kuxb(i, j, k, bi, bj) = 0.D0
          ENDDO
        ENDDO
        DO j=sny+oly-1,1-oly+1,-1
          DO i=snx+olx-1,1-olx+1,-1
            taperfctb(i, j) = taperfctb(i, j) + gm_isofac1d(k)*
     +        gm_isopyck*op5*(gm_isofac2d(i-1, j, bi, bj)+gm_isofac2d(i
     +        , j, bi, bj))*kuxb(i, j, k, bi, bj)
            kuxb(i, j, k, bi, bj) = 0.D0
          ENDDO
        ENDDO
        CALL POPREAL8ARRAY(slopex, 70**2)
        CALL POPREAL8ARRAY(slopey, 70**2)
     ...
     +                            reciplambda, dsigmadr, dsigmadrb, 
     +                            dsigmadx, dsigmadxb, dsigmady, 
     +                            dsigmadyb, ldd97_lrhow, locmixlayer, 
     +                            rdepth, rc, klow_w, 1, k, bi, bj, 
     +                            mytime, myiter, mythid)
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREAL8(rdepth)
        ...
        DO j=sny+oly-1,1-oly+1,-1
          DO i=snx+olx-1,1-olx+1,-1
            tempb = maskw(i, j, k, bi, bj)*op25*gravitysign*dsigmadrb(i
     +        , j)
            dsigmadrb(i, j) = 0.D0
            sigmarb(i-1, j, k) = sigmarb(i-1, j, k) + tempb
            sigmarb(i, j, k) = sigmarb(i, j, k) + tempb
            sigmarb(i-1, j, kp1) = sigmarb(i-1, j, kp1) + maskp1*tempb
            sigmarb(i, j, kp1) = sigmarb(i, j, kp1) + maskp1*tempb
            CALL POPREAL8(dsigmady(i, j))
            tempb = op25*maskw(i, j, k, bi, bj)*dsigmadyb(i, j)
            dsigmadyb(i, j) = 0.D0
            sigmayb(i-1, j+1, k) = sigmayb(i-1, j+1, k) + tempb
            sigmayb(i, j+1, k) = sigmayb(i, j+1, k) + tempb
            sigmayb(i-1, j, k) = sigmayb(i-1, j, k) + tempb
            sigmayb(i, j, k) = sigmayb(i, j, k) + tempb
            CALL POPREAL8(dsigmadx(i, j))
            sigmaxb(i, j, k) = sigmaxb(i, j, k) + maskw(i, j, k, bi, bj)
     +        *dsigmadxb(i, j)
            dsigmadxb(i, j) = 0.D0
          ENDDO
        ENDDO
      ...
      IF (branch .EQ. 0) CALL GMREDI_CALC_PSI_B_B(bi, bj, imin, imax, 
     +                                            jmin, jmax, sigmax, 
     +                                            sigmaxb, sigmay, 
     +                                            sigmayb, sigmar, 
     +                                            sigmarb, ldd97_lrhow, 
     +                                            ldd97_lrhos, mythid)
      DO k=nr,1,-1
        DO j=sny+oly-1,1-oly+1,-1
          DO i=snx+olx-1,1-olx+1,-1
            kwzb(i, j, k, bi, bj) = isopyck*gm_isofac2d(i, j, bi, bj)*
     +        kwzb(i, j, k, bi, bj)
            kwyb(i, j, k, bi, bj) = kgm_tmp*kwyb(i, j, k, bi, bj)
            kwxb(i, j, k, bi, bj) = kgm_tmp*kwxb(i, j, k, bi, bj)
            CALL POPREAL8(kgm_tmp)
          ENDDO
        ENDDO
        CALL POPREAL8(isopyck)
      ENDDO
      DO k=2,nr,1
        DO j=sny+oly-1,1-oly+1,-1
          DO i=snx+olx-1,1-olx+1,-1
            slopesqrb(i, j) = slopesqrb(i, j) + taperfct(i, j)*kwzb(i, j
     +        , k, bi, bj)
            taperfctb(i, j) = taperfctb(i, j) + slopesqr(i, j)*kwzb(i, j
     +        , k, bi, bj) - slopey(i, j)*gravitysign*kwyb(i, j, k, bi, 
     +        bj) - slopex(i, j)*gravitysign*kwxb(i, j, k, bi, bj)
            kwzb(i, j, k, bi, bj) = 0.D0
            slopeyb(i, j) = slopeyb(i, j) - taperfct(i, j)*gravitysign*
     +        kwyb(i, j, k, bi, bj)
            kwyb(i, j, k, bi, bj) = 0.D0
            slopexb(i, j) = slopexb(i, j) - taperfct(i, j)*gravitysign*
     +        kwxb(i, j, k, bi, bj)
            kwxb(i, j, k, bi, bj) = 0.D0
          ENDDO
        ENDDO
        CALL POPREAL8ARRAY(slopex, 70**2)
        CALL POPREAL8ARRAY(slopey, 70**2)
     ...
     +                            reciplambda, dsigmadr, dsigmadrb, 
     +                            dsigmadx, dsigmadxb, dsigmady, 
     +                            dsigmadyb, ldd97_lrhoc, locmixlayer, 
     +                            rdepth, rf, klowc(1-olx, 1-oly, bi, bj
     +                            ), 3, k, bi, bj, mytime, myiter, 
     +                            mythid)
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL POPREAL8(rdepth)
        ...
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            sigmarb(i, j, k) = sigmarb(i, j, k) + gravitysign*dsigmadrb(
     +        i, j)
            dsigmadrb(i, j) = 0.D0
          ENDDO
        ENDDO
        DO j=sny+oly-1,1-oly+1,-1
          DO i=snx+olx-1,1-olx+1,-1
            CALL POPREAL8(dsigmady(i, j))
            tempb = op25*maskfk(i, j)*dsigmadyb(i, j)
            dsigmadyb(i, j) = 0.D0
            sigmayb(i, j+1, k-1) = sigmayb(i, j+1, k-1) + tempb
            sigmayb(i, j, k-1) = sigmayb(i, j, k-1) + tempb
            sigmayb(i, j+1, k) = sigmayb(i, j+1, k) + tempb
            sigmayb(i, j, k) = sigmayb(i, j, k) + tempb
            CALL POPREAL8(dsigmadx(i, j))
            tempb = op25*maskfk(i, j)*dsigmadxb(i, j)
            dsigmadxb(i, j) = 0.D0
            sigmaxb(i+1, j, k-1) = sigmaxb(i+1, j, k-1) + tempb
            sigmaxb(i, j, k-1) = sigmaxb(i, j, k-1) + tempb
            sigmaxb(i+1, j, k) = sigmaxb(i+1, j, k) + tempb
            sigmaxb(i, j, k) = sigmaxb(i, j, k) + tempb
          ENDDO
        ENDDO
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            CALL POPREAL8(maskfk(i, j))
            CALL POPREAL8(taperfct(i, j))
            taperfctb(i, j) = 0.D0
            CALL POPREAL8(slopesqr(i, j))
            slopesqrb(i, j) = 0.D0
            dsigmadrb(i, j) = 0.D0
            CALL POPREAL8(dsigmady(i, j))
            dsigmadyb(i, j) = 0.D0
            CALL POPREAL8(dsigmadx(i, j))
            dsigmadxb(i, j) = 0.D0
            CALL POPREAL8(slopey(i, j))
            slopeyb(i, j) = 0.D0
            CALL POPREAL8(slopex(i, j))
            slopexb(i, j) = 0.D0
          ENDDO
        ENDDO
      ENDDO
      ...
      DO k=nr,1,-1
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            gm_psiyb(i, j, k, bi, bj) = 0.D0
            gm_psixb(i, j, k, bi, bj) = 0.D0
          ENDDO
        ENDDO
      ENDDO
      CALL POPCONTROL1B(branch)
...
C  Differentiation of gmredi_calc_tensor_dummy in reverse (adjoint) mode:
C   gradient     of useful results: kwx kwy kwz kux kvy kuz kvz
C   with respect to varying inputs: kwx kwy kwz kux kvy kuz kvz
C
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
...
C     !INTERFACE:
      SUBROUTINE GMREDI_CALC_TENSOR_DUMMY_B(imin, imax, jmin, jmax, 
     +                                      sigmax, sigmay, sigmar, bi, 
     +                                      bj, mytime, myiter, mythid)
      IMPLICIT NONE
C
C     == Global variables ==
