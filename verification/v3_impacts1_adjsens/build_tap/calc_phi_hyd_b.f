...
C
C  Differentiation of calc_phi_hyd in reverse (adjoint) mode:
C   gradient     of useful results: etan uvel vvel theta salt rhoinsitu
C                totphihyd phi0surf dphihydx dphihydy phihydc phihydf
C   with respect to varying inputs: etan uvel vvel theta salt rhoinsitu
C                totphihyd phi0surf dphihydx dphihydy phihydc phihydf
C
C
C
...
C     !INTERFACE:
      SUBROUTINE CALC_PHI_HYD_B(bi, bj, imin, imax, jmin, jmax, k, 
...
C     pKappaC    :: (p/Po)^kappa at cell center k
      ...
      REAL*8 alpharhob(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 phihydu(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 phihydub(1-olx:snx+olx, 1-oly:sny+oly)
...
C--     Calculate density
     ...
     +                     , k, bi, bj), salt(1-olx, 1-oly, k, bi, bj), 
     +                     alpharho, k, bi, bj, mythid)
          CALL PUSHCONTROL1B(1)
        ELSE
          ad_from10 = jmin
          DO j=ad_from10,jmax
...
C
          ...
          DO j=ad_from22,jmax
            ...
            DO i=ad_from21,imax
              ...
              pkappaf(i, j) = ((r_low(i, j, bi, bj)+ahybsigmf(k)*
     +          fulldepth+bhybsigmf(k)*locdepth)/atm_po)**atm_kappa
              CALL PUSHREAL8(pkappac)
              pkappac = ((r_low(i, j, bi, bj)+ahybsigmc(k)*fulldepth+
     +          bhybsigmc(k)*locdepth)/atm_po)**atm_kappa
              pkappau(i, j) = ((r_low(i, j, bi, bj)+ahybsigmf(k+1)*
     +          fulldepth+bhybsigmf(k+1)*locdepth)/atm_po)**atm_kappa
            ENDDO
            CALL PUSHINTEGER4(i - 1)
        ...
        ELSE IF (integr_geopot .EQ. 0) THEN
...
C
      ...
      END IF
      CALL DIAGS_PHI_HYD_B(k, bi, bj, imin, imax, jmin, jmax, phihydc, 
     +                     phihydcb, mytime, myiter, mythid)
      CALL POPCONTROL2B(branch)
      IF (branch .EQ. 0) THEN
        CALL CALC_GRAD_PHI_HYD_B(k, bi, bj, imin, imax, jmin, jmax, 
     +                           phihydc, phihydcb, alpharho, dphihydx, 
     +                           dphihydxb, dphihydy, dphihydyb, mytime
     +                           , myiter, mythid)
      ELSE IF (branch .NE. 1) THEN
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            phihydub(ii2, ii1) = 0.D0
          ENDDO
        ENDDO
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            phihydub(i, j) = phihydub(i, j) + phihydfb(i, j)
            phihydfb(i, j) = 0.D0
          ENDDO
        ENDDO
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) CALL CALC_GRAD_PHI_FV_B(k, bi, bj, imin, imax
     +                                             , jmin, jmax, phihydf
     +                                             , phihydfb, phihydu, 
     +                                             phihydub, pkappaf, 
     +                                             pkappau, dphihydx, 
     ...
     +                                             myiter, mythid)
      ...
      DO ii1=1-oly,oly+sny
        ...
        ENDDO
      ENDDO
 100  CALL POPCONTROL4B(branch)
      IF (branch .LT. 5) THEN
        IF (branch .LT. 2) THEN
          IF (branch .EQ. 0) THEN
            DO ii1=1-oly,oly+sny
              DO ii2=1-olx,olx+snx
                alpharhob(ii2, ii1) = 0.D0
              ENDDO
            ENDDO
            ...
            DO j=ad_to2,ad_from2,-1
              ...
              DO i=ad_to1,ad_from1,-1
                alpharhob(i, j) = alpharhob(i, j) + gravity*
     +            recip_rhoconst*drf(k)*gravfacc(k)*phihydfb(i, j) + 
     +            gravfacc(k)*drf(k)*recip_rhoconst*halfrl*gravity*
     +            phihydcb(i, j)
                phihydfb(i, j) = phihydfb(i, j) + phihydcb(i, j)
                phihydcb(i, j) = 0.D0
              ENDDO
            ENDDO
          ELSE
            ...
            DO j=ad_to4,ad_from4,-1
              ...
              DO i=ad_to3,ad_from3,-1
                phihydcb(i, j) = phihydcb(i, j) + phihydfb(i, j)
                alpharhob(i, j) = alpharhob(i, j) + gravfacc(k)*drf(k)*
     +            recip_rhoconst*halfrl*gravity*phihydfb(i, j)
                phihydfb(i, j) = 0.D0
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  ddrloc = ro_surf(i, j, bi, bj) - rc(k)
                  alpharhob(i, j) = alpharhob(i, j) + ddrloc*gravity*
     +              gravfacc(k)*recip_rhoconst*phihydcb(i, j)
                  phihydcb(i, j) = 0.D0
                ELSE
                  phihydfb(i, j) = phihydfb(i, j) + phihydcb(i, j)
                  alpharhob(i, j) = alpharhob(i, j) + gravfacc(k)*drf(k)
     +              *recip_rhoconst*halfrl*gravity*phihydcb(i, j)
                  phihydcb(i, j) = 0.D0
                END IF
              ENDDO
            ENDDO
        ...
        ELSE IF (branch .EQ. 2) THEN
          ...
          DO j=ad_to6,ad_from6,-1
            ...
            DO i=ad_to5,ad_from5,-1
              phihydcb(i, j) = phihydcb(i, j) + phihydfb(i, j)
              alpharhob(i, j) = alpharhob(i, j) + recip_rhoconst*drlocp*
     +          gravity*phihydfb(i, j) + recip_rhoconst*drlocm*gravity*
     +          phihydcb(i, j)
              phihydfb(i, j) = phihydcb(i, j)
              phihydcb(i, j) = 0.D0
            ENDDO
          ENDDO
        ELSE IF (branch .EQ. 3) THEN
          ...
          DO j=ad_to8,ad_from8,-1
            ...
            DO i=ad_to7,ad_from7,-1
              phihydcb(i, j) = phihydcb(i, j) + phihydfb(i, j)
              alpharhob(i, j) = alpharhob(i, j) + recip_rhoconst*drlocp*
     +          gravity*phihydfb(i, j)
              phihydfb(i, j) = 0.D0
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                alpharhob(i, j) = alpharhob(i, j) + gravity*
     +            recip_rhoconst*(max1*rec_drm*drlocm+min1*rec_drp*
     +            drlocp)*phihydcb(i, j)
                phihydcb(i, j) = 0.D0
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
              ...
              ELSE
                phihydfb(i, j) = phihydfb(i, j) + phihydcb(i, j)
                alpharhob(i, j) = alpharhob(i, j) + recip_rhoconst*
     +            drlocm*gravity*phihydcb(i, j)
                phihydcb(i, j) = 0.D0
              END IF
            ENDDO
          ENDDO
        ELSE
          ...
          DO j=ad_to14,ad_from14,-1
            ...
            DO i=ad_to13,ad_from13,-1
              phihydcb(i, j) = phihydcb(i, j) + phihydfb(i, j)
              alpharhob(i, j) = alpharhob(i, j) + halfrl*drf(k)*phihydfb
     +          (i, j)
              phihydfb(i, j) = 0.D0
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                ddrloc = ro_surf(i, j, bi, bj) - rc(k)
                alpharhob(i, j) = alpharhob(i, j) + ddrloc*phihydcb(i, j
     +            )
                phihydcb(i, j) = 0.D0
              ELSE
                phihydfb(i, j) = phihydfb(i, j) + phihydcb(i, j)
                alpharhob(i, j) = alpharhob(i, j) + halfrl*drf(k)*
     +            phihydcb(i, j)
                phihydcb(i, j) = 0.D0
              END IF
            ENDDO
        ...
        END IF
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) CALL MOM_QUASIHYDROSTATIC_B(bi, bj, k, uvel, 
     +                                                 uvelb, vvel, 
     +                                                 vvelb, alpharho, 
     +                                                 alpharhob, mytime
     +                                                 , myiter, mythid)
        CALL POPCONTROL1B(branch)
        IF (branch .EQ. 0) THEN
          CALL FIND_RHO_2D_B(imin, imax, jmin, jmax, k, theta(1-olx, 1-
     +                       oly, k, bi, bj), thetab(1-olx, 1-oly, k, bi
     +                       , bj), salt(1-olx, 1-oly, k, bi, bj), saltb
     +                       (1-olx, 1-oly, k, bi, bj), alpharho, 
     +                       alpharhob, k, bi, bj, mythid)
        ELSE
          ...
          DO j=ad_to0,ad_from0,-1
            ...
            DO i=ad_to,ad_from,-1
              rhoinsitub(i, j, k, bi, bj) = rhoinsitub(i, j, k, bi, bj) 
     +          + alpharhob(i, j)
              alpharhob(i, j) = 0.D0
            ENDDO
          ENDDO
          ...
          IF (branch .EQ. 5) THEN
            ...
            DO j=ad_to16,ad_from16,-1
              ...
              DO i=ad_to15,ad_from15,-1
                phihydcb(i, j) = phihydcb(i, j) + phihydfb(i, j)
                alpharhob(i, j) = alpharhob(i, j) + drlocp*phihydfb(i, j
     +            )
                phihydfb(i, j) = 0.D0
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  alpharhob(i, j) = alpharhob(i, j) + (max2*rec_drm*
     +              drlocm+min2*rec_drp*drlocp)*phihydcb(i, j)
                  phihydcb(i, j) = 0.D0
                  CALL POPCONTROL1B(branch)
                  IF (branch .EQ. 0) THEN
                    CALL POPREAL8(min2)
                ...
                ELSE
                  phihydfb(i, j) = phihydfb(i, j) + phihydcb(i, j)
                  alpharhob(i, j) = alpharhob(i, j) + drlocm*phihydcb(i
     +              , j)
                  phihydcb(i, j) = 0.D0
                END IF
              ENDDO
          ...
          ELSE
            ...
            DO j=ad_to22,ad_from22,-1
              ...
              DO i=ad_to21,ad_from21,-1
                phihydfb(i, j) = phihydfb(i, j) + phihydub(i, j) + 
     +            phihydcb(i, j)
                alpharhob(i, j) = alpharhob(i, j) + atm_cp*(pkappaf(i, j
     +            )-pkappau(i, j))*phihydub(i, j) + atm_cp*(pkappaf(i, j
     +            )-pkappac)*phihydcb(i, j)
                phihydub(i, j) = 0.D0
                phihydcb(i, j) = 0.D0
                CALL POPREAL8(pkappac)
              ENDDO
            ENDDO
          END IF
        ELSE IF (branch .EQ. 7) THEN
          ...
          DO j=ad_to24,ad_from24,-1
            ...
            DO i=ad_to23,ad_from23,-1
              phihydcb(i, j) = phihydcb(i, j) + phihydfb(i, j)
              alpharhob(i, j) = alpharhob(i, j) + ddpip*phihydfb(i, j) +
     +          ddpim*phihydcb(i, j)
              phihydfb(i, j) = phihydcb(i, j)
              phihydcb(i, j) = 0.D0
            ENDDO
        ...
        ELSE IF (branch .EQ. 8) THEN
          ...
          DO j=ad_to26,ad_from26,-1
            ...
            DO i=ad_to25,ad_from25,-1
              ...
              IF (branch .EQ. 0) THEN
                ddrloc = ro_surf(i, j, bi, bj) - rc(k)
                alpharhob(i, j) = alpharhob(i, j) + ddrloc*2.d0*
     +            recip_drf(k)*ddpim*phihydcb(i, j)
                phihydcb(i, j) = 0.D0
              ELSE
                phihydfb(i, j) = phihydfb(i, j) + phihydcb(i, j)
                alpharhob(i, j) = alpharhob(i, j) + ddpim*phihydcb(i, j)
                phihydcb(i, j) = 0.D0
              END IF
            ENDDO
          ENDDO
        ELSE
          ...
          DO j=ad_to28,ad_from28,-1
            ...
            DO i=ad_to27,ad_from27,-1
              ...
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                alpharhob(i, j) = alpharhob(i, j) + (max3*rec_drm*ddpim+
     +            min3*rec_drp*ddpip)*phihydcb(i, j)
                phihydcb(i, j) = 0.D0
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  CALL POPREAL8(min3)
        ...
        IF (branch .EQ. 0) THEN
          ...
          DO j=ad_to18,ad_from18,-1
            ...
            DO i=ad_to17,ad_from17,-1
              tempb = maskc(i, j, k, bi, bj)*alpharhob(i, j)
              alpharhob(i, j) = 0.D0
              thetab(i, j, k, bi, bj) = thetab(i, j, k, bi, bj) + (onerl
     +          +atm_rq*salt(i, j, k, bi, bj))*tempb
              saltb(i, j, k, bi, bj) = saltb(i, j, k, bi, bj) + atm_rq*
     +          theta(i, j, k, bi, bj)*tempb
            ENDDO
          ENDDO
        ELSE
 ...
 110  CALL POPCONTROL1B(branch)
     ...
     +                                               myiter, mythid)
      ...
      DO j=ad_to12,ad_from12,-1
        ...
        DO i=ad_to11,ad_from11,-1
          tempb = -(recip_rhoconst*maskc(i, j, k, bi, bj)*alpharhob(i, j
     +      )/(onerl+locbuoy))
          locbuoyb = (1.0-locbuoy/(onerl+locbuoy))*tempb
          alpharhob(i, j) = recip_rhoconst*locbuoyb
          CALL POPREAL8(locbuoy)
        ENDDO
      ENDDO
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
     ...
     +        alpharhob(i, j)
        ...
        ENDDO
      ELSE
        CALL FIND_RHO_2D_B(imin, imax, jmin, jmax, k, theta(1-olx, 1-oly
     +                     , k, bi, bj), thetab(1-olx, 1-oly, k, bi, bj)
     +                     , salt(1-olx, 1-oly, k, bi, bj), saltb(1-olx
     +                     , 1-oly, k, bi, bj), alpharho, alpharhob, k, 
     +                     bi, bj, mythid)
      END IF
