...
C
C  Differentiation of mom_u_coriolis in reverse (adjoint) mode:
C   gradient     of useful results: ucoriolisterm
C   with respect to varying inputs: vfld
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_U_CORIOLIS_B(bi, bj, k, vfld, vfldb, ucoriolisterm
     +                            , ucoriolistermb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C         "Numerical Boundary Layers and Spurious Residual Flows"
        DO j=1-oly,sny+oly-1
          DO i=1-olx+1,snx+olx
            y1 = masks(i, j, k, bi, bj) + masks(i, j+1, k, bi, bj) + 
     +        masks(i-1, j, k, bi, bj) + masks(i-1, j+1, k, bi, bj)
        ...
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx,1-olx+1,-1
            ucoriolistermb(i, j) = 4.d0*ucoriolistermb(i, j)/max1
      ...
      IF (branch .EQ. 0) THEN
        ...
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx,1-olx+1,-1
            tempb = fcori(i, j, bi, bj)*0.5**2*ucoriolistermb(i, j)
            tempb0 = fcori(i-1, j, bi, bj)*0.5**2*ucoriolistermb(i, j)
            ucoriolistermb(i, j) = 0.D0
            vfldb(i-1, j) = vfldb(i-1, j) + tempb0
            vfldb(i-1, j+1) = vfldb(i-1, j+1) + tempb0
      ...
      ELSE
        ...
        DO j=sny+oly-1,1-oly,-1
          DO i=snx+olx,1-olx+1,-1
            tempb0 = (fcori(i, j, bi, bj)+fcori(i-1, j, bi, bj))*0.25*
     +        0.5*ucoriolistermb(i, j)
            ucoriolistermb(i, j) = 0.D0
