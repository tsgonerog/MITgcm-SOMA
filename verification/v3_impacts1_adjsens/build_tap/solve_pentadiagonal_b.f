...
C
C  Differentiation of solve_pentadiagonal in reverse (adjoint) mode:
C   gradient     of useful results: y5d
C   with respect to varying inputs: e5d b5d y5d d5d a5d c5d
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE SOLVE_PENTADIAGONAL_B(imin, imax, jmin, jmax, a5d, a5db
     +                                 , b5d, b5db, c5d, c5db, d5d, d5db
     +                                 , e5d, e5db, y5d, y5db, errcode, 
     +                                 bi, bj, mythid)
      IMPLICIT NONE
C     == Global data ==
CBOP
...
C     errCode :: > 0 if singular matrix
      ...
      REAL*8 y5db(1-olx:snx+olx, 1-oly:sny+oly, nr)
      INTEGER errcode
...
C     == Local variables ==
      ...
      REAL*8 tmpvarb, recvarb
      REAL*8 y5d_m1(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 y5d_m1b(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 c5d_prime(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 c5d_primeb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 d5d_prime(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 d5d_primeb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 e5d_prime(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 e5d_primeb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 y5d_prime(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 y5d_primeb(1-olx:snx+olx, 1-oly:sny+oly, nr)
CEOP
C
C
      INTEGER*4 branch
...
C-    just copy terms
          DO j=1-oly,sny+oly
            DO i=1-olx,snx+olx
              ...
              CALL PUSHREAL8(d5d_prime(i, j, k))
              d5d_prime(i, j, k) = d5d(i, j, k)
              CALL PUSHREAL8(e5d_prime(i, j, k))
              e5d_prime(i, j, k) = e5d(i, j, k)
              CALL PUSHREAL8(y5d_prime(i, j, k))
...
C-    subtract one term
          DO j=1-oly,sny+oly
            DO i=1-olx,snx+olx
              CALL PUSHREAL8(tmpvar)
              tmpvar = b5d(i, j, k)
              c5d_prime(i, j, k) = c5d(i, j, k) - tmpvar*d5d_prime(i, j
     +          , k-1)
              CALL PUSHREAL8(d5d_prime(i, j, k))
              d5d_prime(i, j, k) = d5d(i, j, k) - tmpvar*e5d_prime(i, j
     +          , k-1)
              CALL PUSHREAL8(e5d_prime(i, j, k))
              e5d_prime(i, j, k) = e5d(i, j, k)
              CALL PUSHREAL8(y5d_prime(i, j, k))
              y5d_prime(i, j, k) = y5d_m1(i, j, k) - tmpvar*y5d_prime(i
     +          , j, k-1)
            ENDDO
          ENDDO
...
C-    subtract two terms
          DO j=1-oly,sny+oly
            DO i=1-olx,snx+olx
              ...
              c5d_prime(i, j, k) = c5d(i, j, k) - tmpvar*d5d_prime(i, j
     +          , k-1) - a5d(i, j, k)*e5d_prime(i, j, k-2)
              CALL PUSHREAL8(d5d_prime(i, j, k))
              d5d_prime(i, j, k) = d5d(i, j, k) - tmpvar*e5d_prime(i, j
     +          , k-1)
              CALL PUSHREAL8(e5d_prime(i, j, k))
              ...
              CALL PUSHREAL8(y5d_prime(i, j, k))
              y5d_prime(i, j, k) = y5d_m1(i, j, k) - tmpvar*y5d_prime(i
     +          , j, k-1) - a5d(i, j, k)*y5d_prime(i, j, k-2)
            ENDDO
          ENDDO
...
C--   Backward sweep (starting from bottom)
      DO k=nr,1,-1
        ...
        ELSE IF (k .EQ. nr - 1) THEN
          DO j=1-oly,sny+oly
            DO i=1-olx,snx+olx
              CALL PUSHREAL8(y5d(i, j, k))
              y5d(i, j, k) = y5d_prime(i, j, k) - y5d(i, j, k+1)*
     +          d5d_prime(i, j, k)
            ENDDO
          ENDDO
          CALL PUSHCONTROL2B(1)
        ELSE
          DO j=1-oly,sny+oly
            DO i=1-olx,snx+olx
              CALL PUSHREAL8(y5d(i, j, k))
              y5d(i, j, k) = y5d_prime(i, j, k) - y5d(i, j, k+1)*
     +          d5d_prime(i, j, k) - y5d(i, j, k+2)*e5d_prime(i, j, k)
            ENDDO
          ENDDO
          CALL PUSHCONTROL2B(2)
        END IF
        ...
        IF (branch .EQ. 0) THEN
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              CALL POPREAL8(y5d(i, j, k))
              y5d_primeb(i, j, k) = y5d_primeb(i, j, k) + y5db(i, j, k)
              y5db(i, j, k) = 0.D0
            ENDDO
          ENDDO
        ELSE IF (branch .EQ. 1) THEN
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              CALL POPREAL8(y5d(i, j, k))
              y5d_primeb(i, j, k) = y5d_primeb(i, j, k) + y5db(i, j, k)
              y5db(i, j, k+1) = y5db(i, j, k+1) - d5d_prime(i, j, k)*
     +          y5db(i, j, k)
              d5d_primeb(i, j, k) = d5d_primeb(i, j, k) - y5d(i, j, k+1)
     +          *y5db(i, j, k)
              y5db(i, j, k) = 0.D0
            ENDDO
          ENDDO
     ...
     +          y5db(i, j, k)
              d5d_primeb(i, j, k) = d5d_primeb(i, j, k) - y5d(i, j, k+1)
     +          *y5db(i, j, k)
              y5db(i, j, k+2) = y5db(i, j, k+2) - e5d_prime(i, j, k)*
     +          y5db(i, j, k)
              e5d_primeb(i, j, k) = e5d_primeb(i, j, k) - y5d(i, j, k+2)
     +          *y5db(i, j, k)
              y5db(i, j, k) = 0.D0
            ENDDO
          ENDDO
      ...
      DO k=nr,1,-1
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            ...
            IF (branch .EQ. 0) THEN
              CALL POPREAL8(y5d_prime(i, j, k))
              y5d_primeb(i, j, k) = 0.D0
              CALL POPREAL8(e5d_prime(i, j, k))
              e5d_primeb(i, j, k) = 0.D0
              CALL POPREAL8(d5d_prime(i, j, k))
              d5d_primeb(i, j, k) = 0.D0
              tmpvarb = 0.D0
            ELSE
              tmpvar = c5d_prime(i, j, k)
              recvar = 1.d0/tmpvar
              ...
              CALL POPREAL8(e5d_prime(i, j, k))
              CALL POPREAL8(d5d_prime(i, j, k))
              recvarb = y5d_prime(i, j, k)*y5d_primeb(i, j, k) + 
     +          e5d_prime(i, j, k)*e5d_primeb(i, j, k) + d5d_prime(i, j
     +          , k)*d5d_primeb(i, j, k)
              y5d_primeb(i, j, k) = recvar*y5d_primeb(i, j, k)
              e5d_primeb(i, j, k) = recvar*e5d_primeb(i, j, k)
              d5d_primeb(i, j, k) = recvar*d5d_primeb(i, j, k)
              tmpvarb = -(recvarb/tmpvar**2)
            END IF
            CALL POPREAL8(tmpvar)
            c5d_primeb(i, j, k) = c5d_primeb(i, j, k) + tmpvarb
          ENDDO
        ENDDO
        CALL POPCONTROL2B(branch)
        IF (branch .EQ. 0) THEN
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              CALL POPREAL8(y5d_prime(i, j, k))
              y5d_m1b(i, j, k) = y5d_m1b(i, j, k) + y5d_primeb(i, j, k)
              tmpvarb = -(y5d_prime(i, j, k-1)*y5d_primeb(i, j, k)) - 
     +          e5d_prime(i, j, k-1)*d5d_primeb(i, j, k) - d5d_prime(i, 
     +          j, k-1)*c5d_primeb(i, j, k)
              y5d_primeb(i, j, k-1) = y5d_primeb(i, j, k-1) - tmpvar*
     +          y5d_primeb(i, j, k)
              a5db(i, j, k) = a5db(i, j, k) - y5d_prime(i, j, k-2)*
     +          y5d_primeb(i, j, k) - e5d_prime(i, j, k-2)*c5d_primeb(i
     +          , j, k) - d5d_prime(i, j, k-2)*tmpvarb
              y5d_primeb(i, j, k-2) = y5d_primeb(i, j, k-2) - a5d(i, j, 
     +          k)*y5d_primeb(i, j, k)
              y5d_primeb(i, j, k) = 0.D0
              CALL POPREAL8(e5d_prime(i, j, k))
              e5db(i, j, k) = e5db(i, j, k) + e5d_primeb(i, j, k)
              e5d_primeb(i, j, k) = 0.D0
              CALL POPREAL8(d5d_prime(i, j, k))
              d5db(i, j, k) = d5db(i, j, k) + d5d_primeb(i, j, k)
              e5d_primeb(i, j, k-1) = e5d_primeb(i, j, k-1) - tmpvar*
     +          d5d_primeb(i, j, k)
              d5d_primeb(i, j, k) = 0.D0
              c5db(i, j, k) = c5db(i, j, k) + c5d_primeb(i, j, k)
              d5d_primeb(i, j, k-1) = d5d_primeb(i, j, k-1) - tmpvar*
     +          c5d_primeb(i, j, k)
              e5d_primeb(i, j, k-2) = e5d_primeb(i, j, k-2) - a5d(i, j, 
     +          k)*c5d_primeb(i, j, k)
              c5d_primeb(i, j, k) = 0.D0
              CALL POPREAL8(tmpvar)
              b5db(i, j, k) = b5db(i, j, k) + tmpvarb
              d5d_primeb(i, j, k-2) = d5d_primeb(i, j, k-2) - a5d(i, j, 
     +          k)*tmpvarb
            ENDDO
          ENDDO
        ELSE IF (branch .EQ. 1) THEN
     ...
     +          c5d_primeb(i, j, k)
      ...
      DO k=nr,1,-1
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            y5db(i, j, k) = y5db(i, j, k) + y5d_m1b(i, j, k)
            y5d_m1b(i, j, k) = 0.D0
          ENDDO
        ENDDO
      ENDDO
