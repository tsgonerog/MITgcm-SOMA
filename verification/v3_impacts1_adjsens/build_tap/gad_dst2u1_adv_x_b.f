...
C
C  Differentiation of gad_dst2u1_adv_x in reverse (adjoint) mode:
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_DST2U1_ADV_X_B(bi, bj, k, advectionscheme, calccfl
     +                              , deltatloc, utrans, utransb, ufld, 
     +                              ufldb, tracer, tracerb, ut, utb, 
     +                              mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
     ...
     +        recip_deepfacc(k)
      ...
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx+1,-1
          uabs = abs0*(1.d0-xlimit*(1.d0-ucfl))
          tempb = tracer(i-1, j)*0.5d0*utb(i, j)
          tracerb(i-1, j) = tracerb(i-1, j) + (utrans(i, j)+uabs)*0.5d0*
     +      utb(i, j)
          tempb0 = tracer(i, j)*0.5d0*utb(i, j)
          tracerb(i, j) = tracerb(i, j) + (utrans(i, j)-uabs)*0.5d0*utb(
     +      i, j)
          utb(i, j) = 0.D0
          utransb(i, j) = utransb(i, j) + tempb0 + tempb
          uabsb = tempb - tempb0
          abs0b = (1.d0-xlimit*(1.d0-ucfl))*uabsb
          ucflb = xlimit*abs0*uabsb
