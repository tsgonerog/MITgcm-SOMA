...
C
C  Differentiation of gmredi_residual_flow in reverse (adjoint) mode:
C   gradient     of useful results: gm_psix gm_psiy vfld ufld wfld
C   with respect to varying inputs: gm_psix gm_psiy vfld ufld wfld
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE GMREDI_RESIDUAL_FLOW_B(ufld, ufldb, vfld, vfldb, wfld, 
        ...
        DO k=nr,1,-1
          DO j=sny+oly-1,1-oly,-1
            DO i=snx+olx-1,1-olx,-1
              delpsib = flipsign4lhcoord*recip_deepfacf(k)*recip_ra(i, j
     +          , bi, bj)*wfldb(i, j, k)
              gm_psixb(i+1, j, k, bi, bj) = gm_psixb(i+1, j, k, bi, bj) 
     +          + dyg(i+1, j, bi, bj)*delpsib
              gm_psixb(i, j, k, bi, bj) = gm_psixb(i, j, k, bi, bj) - 
     +          dyg(i, j, bi, bj)*delpsib
              gm_psiyb(i, j+1, k, bi, bj) = gm_psiyb(i, j+1, k, bi, bj) 
     +          + dxg(i, j+1, bi, bj)*delpsib
              gm_psiyb(i, j, k, bi, bj) = gm_psiyb(i, j, k, bi, bj) - 
     +          dxg(i, j, bi, bj)*delpsib
            ENDDO
          ENDDO
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              delpsib = flipsign4lhcoord*recip_drf(k)*recip_deepfacc(k)*
     +          recip_hfacs(i, j, k, bi, bj)*vfldb(i, j, k)
              gm_psiyb(i, j, kp1, bi, bj) = gm_psiyb(i, j, kp1, bi, bj) 
     +          + deepfacf(kp1)*maskp1*delpsib
              gm_psiyb(i, j, k, bi, bj) = gm_psiyb(i, j, k, bi, bj) - 
     +          deepfacf(k)*delpsib
            ENDDO
          ENDDO
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              delpsib = flipsign4lhcoord*recip_drf(k)*recip_deepfacc(k)*
     +          recip_hfacw(i, j, k, bi, bj)*ufldb(i, j, k)
