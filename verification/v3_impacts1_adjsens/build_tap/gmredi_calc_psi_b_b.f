...
C
C  Differentiation of gmredi_calc_psi_b in reverse (adjoint) mode:
C   gradient     of useful results: gm_psix gm_psiy sigmar sigmax
C                sigmay
C   with respect to varying inputs: gm_psix gm_psiy sigmar sigmax
C                sigmay
C
C
C
...
C     !INTERFACE:
      SUBROUTINE GMREDI_CALC_PSI_B_B(bi, bj, imin, imax, jmin, jmax, 
     +                               sigmax, sigmaxb, sigmay, sigmayb, 
     +                               sigmar, sigmarb, ldd97_lrhow, 
     +                               ldd97_lrhos, mythid)
      IMPLICIT NONE
C
C     == Global variables ==
...
C     !LOCAL VARIABLES:
      ...
      REAL*8 dsigmadrwb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dsigmadrs(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 dsigmadrsb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 taperx(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 taperxb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 tapery(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 taperyb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 tempb
      INTEGER*4 branch
      INTEGER ii2
      INTEGER ii1
...
C      Gradient of Sigma below U and V points
     ...
     +          maskw(i, j, km1, bi, bj)*maskw(i, j, k, bi, bj)
              dsigmadrw(i, j) = (sigmar(i-1, j, k)+sigmar(i, j, k))*
     +          halfsign*maskw(i, j, km1, bi, bj)*maskw(i, j, k, bi, bj)
            ENDDO
          ENDDO
     ...
     +          masks(i, j, km1, bi, bj)*masks(i, j, k, bi, bj)
              dsigmadrs(i, j) = (sigmar(i, j-1, k)+sigmar(i, j, k))*
     +          halfsign*masks(i, j, km1, bi, bj)*masks(i, j, k, bi, bj)
            ENDDO
          ENDDO
C
...
C      Calculate slopes , taper and/or clip
          CALL PUSHREAL8ARRAY(dsigmadrs, 70**2)
          CALL PUSHREAL8ARRAY(dsigmadrw, 70**2)
          CALL PUSHREAL8ARRAY(slopey, 70**2)
          CALL PUSHREAL8ARRAY(slopex, 70**2)
          CALL PUSHREAL8ARRAY(tapery, 70**2)
          CALL PUSHREAL8ARRAY(taperx, 70**2)
          CALL GMREDI_SLOPE_PSI(taperx, tapery, slopex, slopey, 
     +                          dsigmadrw, dsigmadrs, ldd97_lrhow, 
     +                          ldd97_lrhos, rdepth, k, bi, bj, mythid)
C
C
...
C   Note: since SlopeX,Y have been masked, no needs to mask again GM_PsiX,Y
        ...
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            dsigmadrsb(ii2, ii1) = 0.D0
          ENDDO
        ENDDO
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            dsigmadrwb(ii2, ii1) = 0.D0
          ENDDO
        ENDDO
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            taperxb(ii2, ii1) = 0.D0
          ENDDO
        ENDDO
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            taperyb(ii2, ii1) = 0.D0
          ENDDO
        ENDDO
        DO k=nr,2,-1
          km1 = k - 1
          half_k = gm_background_k*(gm_bolfac1d(km1)+gm_bolfac1d(k))*
     +      op25
          DO j=sny+oly,1-oly+1,-1
            DO i=snx+olx,1-olx,-1
              tempb = half_k*(gm_bolfac2d(i, j-1, bi, bj)+gm_bolfac2d(i
     +          , j, bi, bj))*gm_psiyb(i, j, k, bi, bj)
              gm_psiyb(i, j, k, bi, bj) = 0.D0
              slopeyb(i, j) = slopeyb(i, j) + tapery(i, j)*tempb
              taperyb(i, j) = taperyb(i, j) + slopey(i, j)*tempb
            ENDDO
          ENDDO
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx+1,-1
              tempb = half_k*(gm_bolfac2d(i-1, j, bi, bj)+gm_bolfac2d(i
     +          , j, bi, bj))*gm_psixb(i, j, k, bi, bj)
              gm_psixb(i, j, k, bi, bj) = 0.D0
              slopexb(i, j) = slopexb(i, j) + taperx(i, j)*tempb
              taperxb(i, j) = taperxb(i, j) + slopex(i, j)*tempb
            ENDDO
          ENDDO
          CALL POPREAL8ARRAY(taperx, 70**2)
          CALL POPREAL8ARRAY(tapery, 70**2)
          CALL POPREAL8ARRAY(slopex, 70**2)
          CALL POPREAL8ARRAY(slopey, 70**2)
          CALL POPREAL8ARRAY(dsigmadrw, 70**2)
          CALL POPREAL8ARRAY(dsigmadrs, 70**2)
          CALL GMREDI_SLOPE_PSI_B(taperx, taperxb, tapery, taperyb, 
     +                            slopex, slopexb, slopey, slopeyb, 
     +                            dsigmadrw, dsigmadrwb, dsigmadrs, 
     +                            dsigmadrsb, ldd97_lrhow, ldd97_lrhos, 
     +                            rdepth, k, bi, bj, mythid)
          ...
          DO j=sny+oly,1-oly+1,-1
            DO i=snx+olx,1-olx,-1
              tempb = halfsign*masks(i, j, km1, bi, bj)*masks(i, j, k, 
     +          bi, bj)*dsigmadrsb(i, j)
              dsigmadrsb(i, j) = 0.D0
              sigmarb(i, j-1, k) = sigmarb(i, j-1, k) + tempb
              sigmarb(i, j, k) = sigmarb(i, j, k) + tempb
              CALL POPREAL8(slopey(i, j))
              tempb = masks(i, j, k, bi, bj)*halfrl*masks(i, j, km1, bi
     +          , bj)*slopeyb(i, j)
              slopeyb(i, j) = 0.D0
              sigmayb(i, j, km1) = sigmayb(i, j, km1) + tempb
              sigmayb(i, j, k) = sigmayb(i, j, k) + tempb
            ENDDO
          ENDDO
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx+1,-1
              tempb = halfsign*maskw(i, j, km1, bi, bj)*maskw(i, j, k, 
     +          bi, bj)*dsigmadrwb(i, j)
              dsigmadrwb(i, j) = 0.D0
              sigmarb(i-1, j, k) = sigmarb(i-1, j, k) + tempb
              sigmarb(i, j, k) = sigmarb(i, j, k) + tempb
              CALL POPREAL8(slopex(i, j))
              tempb = maskw(i, j, k, bi, bj)*halfrl*maskw(i, j, km1, bi
     +          , bj)*slopexb(i, j)
              slopexb(i, j) = 0.D0
              sigmaxb(i, j, km1) = sigmaxb(i, j, km1) + tempb
