...
C
C  Differentiation of mom_v_del2v in reverse (adjoint) mode:
C   gradient     of useful results: vfld del2v
C   with respect to varying inputs: vfld del2v
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_V_DEL2V_B(bi, bj, k, vfld, vfldb, hfacz, h0facz, 
     +                         del2v, del2vb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
      ...
      IF (no_slip_sides) THEN
        DO j=sny+oly-1,1-oly+1,-1
          DO i=snx+olx-1,1-olx+1,-1
            ...
            vfldb(i, j) = vfldb(i, j) - recip_deepfac2c(k)*
     +        sidedragfactor*masks(i, j, k, bi, bj)*recip_hfacs(i, j, k
     +        , bi, bj)*recip_ras(i, j, bi, bj)*(hfaczclosedw*dyu(i, j, 
     +        bi, bj)*recip_dxv(i, j, bi, bj)+hfaczclosede*dyu(i+1, j, 
     +        bi, bj)*recip_dxv(i+1, j, bi, bj))*del2vb(i, j)
          ENDDO
        ENDDO
      END IF
      ...
      DO j=sny+oly-1,1-oly+1,-1
        DO i=snx+olx-1,1-olx+1,-1
          tempb = recip_ras(i, j, bi, bj)*recip_drf(k)*recip_deepfac2c(k
     +      )*recip_hfacs(i, j, k, bi, bj)*masks(i, j, k, bi, bj)*del2vb
     +      (i, j)
          del2vb(i, j) = 0.D0
      ...
      DO j=sny+oly-1,1-oly,-1
        DO i=snx+olx-1,1-olx+1,-1
          tempb = drf(k)*dxf(i, j, bi, bj)*hfacc(i, j, k, bi, bj)*
     +      recip_dyf(i, j, bi, bj)*fmerb(i, j)
          fmerb(i, j) = 0.D0
          vfldb(i, j+1) = vfldb(i, j+1) + tempb
