...
C
C  Differentiation of calc_div_ghat in reverse (adjoint) mode:
C   gradient     of useful results: uvel vvel gu gv cg2d_b
C   with respect to varying inputs: uvel vvel gu gv cg2d_b
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE CALC_DIV_GHAT_B(bi, bj, k, cg2d_b, cg2d_bb, cg3d_b, 
     +                           mythid)
      IMPLICIT NONE
C     == Global variables ==
CBOP
...
C     pf     :: Intermediate array for building RHS source term.
      ...
      REAL*8 pfb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 tempb
      INTEGER*4 branch
      INTEGER ii2
      INTEGER ii1
...
C
      ...
      DO j=sny,1,-1
        DO i=snx,1,-1
          pfb(i, j+1) = pfb(i, j+1) + cg2d_bb(i, j, bi, bj)
          pfb(i, j) = pfb(i, j) - cg2d_bb(i, j, bi, bj)
        ENDDO
      ENDDO
      ...
      IF (branch .EQ. 0) THEN
        DO j=sny+1,1,-1
          DO i=snx,1,-1
            tempb = ya(i, j)*pfb(i, j)/deltatmom
            pfb(i, j) = 0.D0
            gvb(i, j, k, bi, bj) = gvb(i, j, k, bi, bj) + 
     +        implicdiv2dflow*tempb
            vvelb(i, j, k, bi, bj) = vvelb(i, j, k, bi, bj) + (1.d0-
     +        implicdiv2dflow)*tempb
          ENDDO
        ENDDO
      ELSE IF (branch .EQ. 1) THEN
        DO j=sny+1,1,-1
          DO i=snx,1,-1
            gvb(i, j, k, bi, bj) = gvb(i, j, k, bi, bj) + 
     +        implicdiv2dflow*ya(i, j)*pfb(i, j)/deltatmom
            pfb(i, j) = 0.D0
          ENDDO
        ENDDO
      ELSE
        DO j=sny+1,1,-1
          DO i=snx,1,-1
            gvb(i, j, k, bi, bj) = gvb(i, j, k, bi, bj) + ya(i, j)*pfb(i
     +        , j)/deltatmom
            pfb(i, j) = 0.D0
          ENDDO
        ENDDO
      ...
      DO j=sny,1,-1
        DO i=snx,1,-1
          pfb(i+1, j) = pfb(i+1, j) + cg2d_bb(i, j, bi, bj)
          pfb(i, j) = pfb(i, j) - cg2d_bb(i, j, bi, bj)
        ENDDO
      ENDDO
      CALL POPCONTROL2B(branch)
      IF (branch .EQ. 0) THEN
        DO j=sny,1,-1
          DO i=snx+1,1,-1
            tempb = xa(i, j)*pfb(i, j)/deltatmom
            pfb(i, j) = 0.D0
            gub(i, j, k, bi, bj) = gub(i, j, k, bi, bj) + 
     +        implicdiv2dflow*tempb
            uvelb(i, j, k, bi, bj) = uvelb(i, j, k, bi, bj) + (1.d0-
     +        implicdiv2dflow)*tempb
          ENDDO
        ENDDO
      ELSE IF (branch .EQ. 1) THEN
        DO j=sny,1,-1
          DO i=snx+1,1,-1
            gub(i, j, k, bi, bj) = gub(i, j, k, bi, bj) + 
     +        implicdiv2dflow*xa(i, j)*pfb(i, j)/deltatmom
            pfb(i, j) = 0.D0
          ENDDO
        ENDDO
      ELSE
        DO j=sny,1,-1
          DO i=snx+1,1,-1
            gub(i, j, k, bi, bj) = gub(i, j, k, bi, bj) + xa(i, j)*pfb(i
     +        , j)/deltatmom
            pfb(i, j) = 0.D0
          ENDDO
        ENDDO
