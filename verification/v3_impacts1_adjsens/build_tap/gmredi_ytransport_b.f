...
C
C  Differentiation of gmredi_ytransport in reverse (adjoint) mode:
C   gradient     of useful results: kvy kvz gm_psiy df tracer
C   with respect to varying inputs: kvy kvz gm_psiy df tracer
C
C
C
C
...
C     !INTERFACE:
      SUBROUTINE GMREDI_YTRANSPORT_B(tridentity, bi, bj, k, imin, imax, 
     +                               jmin, jmax, ya, maskfk, tracer, 
...
C           AD-code to ensure exactly zero contribution to Tracer AD var.
          ...
          DO j=jmin,jmax
            DO i=imin,imax
              dtdz(i, j) = op5*(op5*recip_drc(k)*(maskfk(i, j-1)*(tracer
     +          (i, j-1, km1)-tracer(i, j-1, k))+maskfk(i, j)*(tracer(i
     +          , j, km1)-tracer(i, j, k)))+op5*recip_drc(kp1)*(maskc(i
     +          , j-1, k, bi, bj)*maskc(i, j-1, kp1, bi, bj)*maskp1*(
     +          tracer(i, j-1, k)-tracer(i, j-1, kp1))+maskc(i, j, k, bi
...
C
     ...
     +      )) THEN
          ...
          DO j=jmin,jmax
            DO i=imin,imax
              vtrans(i, j) = -(gravitysign*dxg(i, j, bi, bj)*(gm_psiy(i
     +          , j, kp1, bi, bj)*deepfacf(kp1)*maskp1-gm_psiy(i, j, k, 
     +          bi, bj)*deepfacf(k))*masks(i, j, k, bi, bj))
            ENDDO
          ENDDO
          ...
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              vtransb(i, j) = vtransb(i, j) + (tracer(i, j, k)+tracer(i
     +          , j-1, k))*op5*dfb(i, j)
              tempb4 = vtrans(i, j)*op5*dfb(i, j)
              tracerb(i, j, k) = tracerb(i, j, k) + tempb4
              tracerb(i, j-1, k) = tracerb(i, j-1, k) + tempb4
            ENDDO
          ENDDO
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              tempb4 = -(gravitysign*dxg(i, j, bi, bj)*masks(i, j, k, bi
     +          , bj)*vtransb(i, j))
              vtransb(i, j) = 0.D0
              gm_psiyb(i, j, kp1, bi, bj) = gm_psiyb(i, j, kp1, bi, bj) 
     ...
     +          deepfacf(k)*tempb4
        ...
        IF (branch .EQ. 0) THEN
          ...
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              tempb4 = -(ya(i, j)*dfb(i, j))
              kvzb(i, j, k, bi, bj) = kvzb(i, j, k, bi, bj) + dtdz(i, j)
     +          *tempb4
              dtdzb(i, j) = dtdzb(i, j) + kvz(i, j, k, bi, bj)*tempb4
            ENDDO
          ENDDO
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              ...
              dtdzb(i, j) = 0.D0
              tempb3 = maskc(i, j-1, k, bi, bj)*maskp1*maskc(i, j-1, kp1
     +          , bi, bj)*tempb2
     ...
     +          maskp1*tempb2
              ...
              tracerb(i, j-1, kp1) = tracerb(i, j-1, kp1) - tempb3
              tempb = maskfk(i, j-1)*tempb0
              tempb1 = maskfk(i, j)*tempb0
              tracerb(i, j, km1) = tracerb(i, j, km1) + tempb1
              tracerb(i, j, k) = tracerb(i, j, k) - tempb1
              tracerb(i, j-1, km1) = tracerb(i, j-1, km1) + tempb
              tracerb(i, j-1, k) = tracerb(i, j-1, k) - tempb
            ENDDO
          ENDDO
        END IF
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            tempb = -(ya(i, j)*recip_deepfacc(k)*recip_dyc(i, j, bi, bj)
     +        *dfb(i, j))
            kvyb(i, j, k, bi, bj) = kvyb(i, j, k, bi, bj) + (tracer(i, j
     +        , k)-tracer(i, j-1, k))*tempb
            tempb0 = kvy(i, j, k, bi, bj)*tempb
            tracerb(i, j, k) = tracerb(i, j, k) + tempb0
            tracerb(i, j-1, k) = tracerb(i, j-1, k) - tempb0
          ENDDO
        ENDDO
      END IF
      END
