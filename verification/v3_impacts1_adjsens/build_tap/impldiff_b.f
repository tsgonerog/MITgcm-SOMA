...
C
C  Differentiation of impldiff in reverse (adjoint) mode:
C   gradient     of useful results: kapparx gtracer
C   with respect to varying inputs: kapparx gtracer
C
C
C
C
...
C     !ROUTINE: IMPLDIFF
C     !INTERFACE:
      SUBROUTINE IMPLDIFF_B(bi, bj, imin, imax, jmin, jmax, tracerid, 
     +                      kapparx, kapparxb, recip_hfac, gtracer, 
     +                      gtracerb, mythid)
      IMPLICIT NONE
C     == Global data ==
CBOP
...
C     !LOCAL VARIABLES:
      ...
      REAL*8 loctrb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 a(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 ab(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 b(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 bb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 c(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 cb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 bet(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 betb(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 gam(1-olx:snx+olx, 1-oly:sny+oly, nr)
      REAL*8 gamb(1-olx:snx+olx, 1-oly:sny+oly, nr)
...
C--   Old aLower
     ...
     +        recip_deepfac2c(k)*recip_rhofacc(k)*kapparx(i, j, k)*
     +        recip_drc(k)*deepfac2f(k)*rhofacf(k))
            IF (recip_hfac(i, j, k-1) .EQ. 0.) THEN
              a(i, j, k) = 0.
...
C--   Old aUpper
     ...
     +        recip_deepfac2c(k)*recip_rhofacc(k)*kapparx(i, j, k+1)*
     +        recip_drc(k+1)*deepfac2f(k+1)*rhofacf(k+1))
            IF (recip_hfac(i, j, k+1) .EQ. 0.) THEN
              c(i, j, k) = 0.
              CALL PUSHCONTROL1B(1)
            ELSE
...
C
      ...
      DO k=2,nr
        ...
        DO j=ad_from12,jmax
          ad_from11 = imin
          DO i=ad_from11,imax
            CALL PUSHREAL8(loctr(i, j, k))
            loctr(i, j, k) = bet(i, j, k)*(gtracer(i, j, k)-a(i, j, k)*
     +        loctr(i, j, k-1))
          ENDDO
          CALL PUSHINTEGER4(i - 1)
...
CADJ loop = sequential
      DO k=nr-1,1,-1
        ...
        DO j=ad_from14,jmax
          ...
          DO i=ad_from13,imax
            CALL PUSHREAL8(loctr(i, j, k))
            loctr(i, j, k) = loctr(i, j, k) - gam(i, j, k+1)*loctr(i, j
     +        , k+1)
          ENDDO
          CALL PUSHINTEGER4(i - 1)
...
C--   Diagnostics of vertical diffusion flux (or vertical viscous flux):
      IF (usediagnostics .AND. tracerid .NE. 0) THEN
        IF (tracerid .LT. 1) THEN
          IF (tracerid .NE. -1) THEN
            IF (tracerid .NE. -2) STOP
          END IF
        END IF
      END IF
      DO ii1=1,nr
      ...
      DO k=nr,1,-1
        ...
        DO j=ad_to16,ad_from16,-1
          ...
          DO i=ad_to15,ad_from15,-1
            locupdateb = loctrb(i, j, k)
            loctrb(i, j, k) = gtracerb(i, j, k) + locupdateb
            gtracerb(i, j, k) = -locupdateb
          ENDDO
        ENDDO
      ENDDO
      DO ii1=1,nr
      ...
      DO k=1,nr-1,1
        ...
        DO j=ad_to14,ad_from14,-1
          ...
          DO i=ad_to13,ad_from13,-1
            CALL POPREAL8(loctr(i, j, k))
            gamb(i, j, k+1) = gamb(i, j, k+1) - loctr(i, j, k+1)*loctrb(
     +        i, j, k)
            loctrb(i, j, k+1) = loctrb(i, j, k+1) - gam(i, j, k+1)*
     +        loctrb(i, j, k)
          ENDDO
        ENDDO
      ENDDO
      ...
      DO k=nr,2,-1
        ...
        DO j=ad_to12,ad_from12,-1
          ...
          DO i=ad_to11,ad_from11,-1
            CALL POPREAL8(loctr(i, j, k))
            betb(i, j, k) = betb(i, j, k) + (gtracer(i, j, k)-a(i, j, k)
     +        *loctr(i, j, k-1))*loctrb(i, j, k)
            tempb = bet(i, j, k)*loctrb(i, j, k)
            loctrb(i, j, k) = 0.D0
            gtracerb(i, j, k) = gtracerb(i, j, k) + tempb
            ab(i, j, k) = ab(i, j, k) - loctr(i, j, k-1)*tempb
            loctrb(i, j, k-1) = loctrb(i, j, k-1) - a(i, j, k)*tempb
          ENDDO
        ENDDO
      ENDDO
      ...
      DO j=ad_to10,ad_from10,-1
        ...
        DO i=ad_to9,ad_from9,-1
          gtracerb(i, j, 1) = gtracerb(i, j, 1) + bet(i, j, 1)*loctrb(i
     +      , j, 1)
          betb(i, j, 1) = betb(i, j, 1) + gtracer(i, j, 1)*loctrb(i, j, 
     +      1)
          loctrb(i, j, 1) = 0.D0
        ENDDO
      ENDDO
        ...
        DO k=nr,2,-1
          ...
          DO j=ad_to8,ad_from8,-1
            ...
            DO i=ad_to7,ad_from7,-1
              CALL POPCONTROL1B(branch)
              IF (branch .NE. 0) THEN
                CALL POPREAL8(bet(i, j, k))
                temp = b(i, j, k) - a(i, j, k)*gam(i, j, k)
                tempb = -(betb(i, j, k)/temp**2)
                betb(i, j, k) = 0.D0
                bb(i, j, k) = bb(i, j, k) + tempb
                ab(i, j, k) = ab(i, j, k) - gam(i, j, k)*tempb
                gamb(i, j, k) = gamb(i, j, k) - a(i, j, k)*tempb
              END IF
              cb(i, j, k-1) = cb(i, j, k-1) + bet(i, j, k-1)*gamb(i, j, 
     +          k)
              betb(i, j, k-1) = betb(i, j, k-1) + c(i, j, k-1)*gamb(i, j
     +          , k)
              gamb(i, j, k) = 0.D0
            ENDDO
          ENDDO
      ...
      IF (branch .EQ. 0) THEN
        ...
        DO j=ad_to6,ad_from6,-1
          ...
          DO i=ad_to5,ad_from5,-1
            CALL POPCONTROL1B(branch)
            IF (branch .NE. 0) THEN
              bb(i, j, 1) = bb(i, j, 1) - betb(i, j, 1)/b(i, j, 1)**2
              betb(i, j, 1) = 0.D0
            END IF
          ENDDO
        ENDDO
      ...
      DO k=nr,1,-1
        ...
        DO j=ad_to4,ad_from4,-1
          ...
          DO i=ad_to3,ad_from3,-1
            cb(i, j, k) = cb(i, j, k) - bb(i, j, k)
            ab(i, j, k) = ab(i, j, k) - bb(i, j, k)
            bb(i, j, k) = 0.D0
          ENDDO
        ENDDO
      ENDDO
      ...
      DO k=nr-1,1,-1
        ...
        DO j=ad_to2,ad_from2,-1
          ...
          DO i=ad_to1,ad_from1,-1
            CALL POPCONTROL1B(branch)
            IF (branch .NE. 0) cb(i, j, k) = 0.D0
            kapparxb(i, j, k+1) = kapparxb(i, j, k+1) - deltatx(k)*
     +        recip_drf(k)*recip_hfac(i, j, k)*rhofacf(k+1)*
     +        recip_deepfac2c(k)*recip_rhofacc(k)*recip_drc(k+1)*
     +        deepfac2f(k+1)*cb(i, j, k)
            cb(i, j, k) = 0.D0
          ENDDO
        ENDDO
      ENDDO
      DO k=nr,2,-1
        ...
        DO j=ad_to0,ad_from0,-1
          ...
          DO i=ad_to,ad_from,-1
            CALL POPCONTROL1B(branch)
            IF (branch .NE. 0) ab(i, j, k) = 0.D0
            kapparxb(i, j, k) = kapparxb(i, j, k) - deltatx(k)*recip_drf
     +        (k)*recip_hfac(i, j, k)*rhofacf(k)*recip_deepfac2c(k)*
     +        recip_rhofacc(k)*recip_drc(k)*deepfac2f(k)*ab(i, j, k)
            ab(i, j, k) = 0.D0
          ENDDO
        ENDDO
      ENDDO
