...
C
C  Differentiation of mom_u_adv_uu in reverse (adjoint) mode:
C   gradient     of useful results: ufld utrans advectfluxuu
C   with respect to varying inputs: ufld utrans
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_U_ADV_UU_B(bi, bj, k, utrans, utransb, ufld, ufldb
     +                          , advectfluxuu, advectfluxuub, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C  AdvectFluxUU         :: advective flux
      ...
      REAL*8 advectfluxuub(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
      INTEGER i, j
      ...
      DO j=sny+oly-1,1-oly,-1
        DO i=snx+olx-1,1-olx,-1
          tempb = (ufld(i, j)+ufld(i+1, j))*0.25*advectfluxuub(i, j)
          tempb0 = (utrans(i, j)+utrans(i+1, j))*0.25*advectfluxuub(i, j
     +      )
          advectfluxuub(i, j) = 0.D0
