...
C
C  Differentiation of gad_fluxlimit_adv_y in reverse (adjoint) mode:
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_FLUXLIMIT_ADV_Y_B(bi, bj, k, calccfl, deltatloc, 
     +                                 vtrans, vtransb, vfld, vfldb, 
     +                                 masklocs, tracer, tracerb, vt, 
     +                                 vtb, mythid)
      IMPLICIT NONE
...
C
      ...
      DO j=sny+oly-1,1-oly+2,-1
        DO i=snx+olx,1-olx,-1
          rj = (tracer(i, j)-tracer(i, j-1))*masklocs(i, j)
          vtransb(i, j) = vtransb(i, j) + (tracer(i, j)+tracer(i, j-1))*
     +      0.5d0*vtb(i, j)
          tempb = vtrans(i, j)*0.5d0*vtb(i, j)
          tempb0 = -(abs1*rj*0.5d0*vtb(i, j))
          tempb1 = -((onerl+vcfl*cr-cr)*0.5d0*vtb(i, j))
          vtb(i, j) = 0.D0
          abs1b = rj*tempb1
          rjb = abs1*tempb1
          vcflb = cr*tempb0
