...
C
C  Differentiation of gmredi_slope_psi in reverse (adjoint) mode:
C   gradient     of useful results: slopex slopey dsigmadrs dsigmadrw
C                taperx tapery
C   with respect to varying inputs: slopex slopey dsigmadrs dsigmadrw
C                taperx tapery
C
C
C
...
C     !INTERFACE:
      SUBROUTINE GMREDI_SLOPE_PSI_B(taperx, taperxb, tapery, taperyb, 
     +                              slopex, slopexb, slopey, slopeyb, 
     +                              dsigmadrw, dsigmadrwb, dsigmadrs, 
     +                              dsigmadrsb, lrhow, lrhos, rdepth, k
     +                              , bi, bj, mythid)
      IMPLICIT NONE
C
C     == Global variables ==
...
C                                  stratified (dSigmaDr < 0) region :
        DO j=1-oly,sny+oly
          DO i=1-olx+1,snx+olx
            IF (dsigmadrw(i, j) .LE. gm_small_number) THEN
        ...
        IF (gm_taper_scheme .NE. 'stableGmAdjTap') THEN
          DO j=1-oly,sny+oly
            DO i=1-olx+1,snx+olx
              IF (slopex(i, j) .GE. 0.) THEN
                abs0 = slopex(i, j)
              ELSE
                abs0 = -slopex(i, j)
              END IF
              IF (abs0 .GE. slopecutoff) THEN
                CALL PUSHREAL8(slopex(i, j))
...
C
        DO j=1-oly+1,sny+oly
          DO i=1-olx,snx+olx
            IF (dsigmadrs(i, j) .LE. gm_small_number) THEN
        ...
        IF (gm_taper_scheme .NE. 'stableGmAdjTap') THEN
          DO j=1-oly+1,sny+oly
            DO i=1-olx,snx+olx
              IF (slopey(i, j) .GE. 0.) THEN
                abs1 = slopey(i, j)
              ELSE
                abs1 = -slopey(i, j)
              END IF
              IF (abs1 .GE. slopecutoff) THEN
                CALL PUSHREAL8(slopey(i, j))
          ...
          DO j=1-oly,sny+oly
            DO i=1-olx+1,snx+olx
              ...
              ELSE
                ...
                CALL PUSHCONTROL1B(1)
              END IF
              IF (smod .GT. loc_maxslope .AND. smod .LT. slopecutoff) 
     +        THEN
                CALL PUSHCONTROL1B(1)
              ELSE
                CALL PUSHCONTROL1B(0)
          ...
          DO j=sny+oly,1-oly+1,-1
            DO i=snx+olx,1-olx,-1
              ...
              IF (branch .EQ. 0) THEN
                smodb = 0.D0
              ELSE
                smodb = -(loc_maxslope*taperyb(i, j)/(gm_small_number+
     +            smod)**2)
                taperyb(i, j) = 0.D0
              END IF
              CALL POPCONTROL1B(branch)
          ...
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx+1,-1
              ...
              IF (branch .EQ. 0) THEN
                smodb = 0.D0
              ELSE
                smodb = -(loc_maxslope*taperxb(i, j)/(gm_small_number+
     +            smod)**2)
                taperxb(i, j) = 0.D0
              END IF
              CALL POPCONTROL1B(branch)
            ...
            ENDDO
          ENDDO
        ELSE IF (gm_taper_scheme .EQ. 'gkw91' .OR. gm_taper_scheme .EQ. 
     +      'ac02') THEN
C
C
     ...
     +        THEN
          ...
          DO j=sny+oly,1-oly+1,-1
            DO i=snx+olx,1-olx,-1
              CALL POPCONTROL1B(branch)
              IF (branch .NE. 0) THEN
                temp = gm_small_number + slopey(i, j)*slopey(i, j)
                slopeyb(i, j) = slopeyb(i, j) - 2*slopey(i, j)*
     +            maxslopesqr*taperyb(i, j)/temp**2
                taperyb(i, j) = 0.D0
              END IF
            ENDDO
          ENDDO
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx+1,-1
              CALL POPCONTROL1B(branch)
              IF (branch .NE. 0) THEN
                temp = gm_small_number + slopex(i, j)*slopex(i, j)
                slopexb(i, j) = slopexb(i, j) - 2*slopex(i, j)*
     +            maxslopesqr*taperxb(i, j)/temp**2
                taperxb(i, j) = 0.D0
              END IF
            ENDDO
          ENDDO
...
C-      Danabasoglu and McWilliams, J. Clim. 1995
          ...
          DO j=1-oly+1,sny+oly
            DO i=1-olx,snx+olx
              ...
              END IF
              CALL PUSHREAL8(smod)
              smod = abs3*rvel2wunit(k)
            ENDDO
          ENDDO
          DO j=sny+oly,1-oly+1,-1
            DO i=snx+olx,1-olx,-1
              smodb = -((1.0-TANH((gm_scrit-smod)/gm_sd)**2)*op5*taperyb
     +          (i, j)/gm_sd)
              taperyb(i, j) = 0.D0
              CALL POPREAL8(smod)
              abs3b = rvel2wunit(k)*smodb
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                slopeyb(i, j) = slopeyb(i, j) + abs3b
              ELSE
                slopeyb(i, j) = slopeyb(i, j) - abs3b
              END IF
            ENDDO
          ...
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx+1,-1
              smodb = -((1.0-TANH((gm_scrit-smod)/gm_sd)**2)*op5*taperxb
     +          (i, j)/gm_sd)
              taperxb(i, j) = 0.D0
              CALL POPREAL8(smod)
              abs2b = rvel2wunit(k)*smodb
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                slopexb(i, j) = slopexb(i, j) + abs2b
              ELSE
                slopexb(i, j) = slopexb(i, j) - abs2b
              END IF
            ENDDO
          ...
          DO j=1-oly,sny+oly
            DO i=1-olx+1,snx+olx
              ...
              IF (smod .LT. slopecutoff) THEN
                CALL PUSHREAL8(f1)
                f1 = op5*(onerl+TANH((gm_scrit-smod*rvel2wunit(k))/gm_sd
     +            ))
                IF (smod .NE. zerorl) THEN
                  CALL PUSHREAL8(rnondim)
          ...
          DO j=sny+oly,1-oly+1,-1
            DO i=snx+olx,1-olx,-1
              ...
              ELSE
                ...
                CALL POPCONTROL1B(branch)
                IF (branch .EQ. 0) THEN
                  CALL POPREAL8(rnondim)
                  temp = lrhos(i, j)*smod
                  smodb = -(lrhos(i, j)*rdepth*rnondimb/temp**2)
                ELSE
                  CALL POPREAL8(rnondim)
                  smodb = 0.D0
                END IF
                CALL POPREAL8(f1)
                smodb = smodb - rvel2wunit(k)*(1.0-TANH((gm_scrit-
     +            rvel2wunit(k)*smod)/gm_sd)**2)*op5*f1b/gm_sd
              END IF
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREAL8(smod)
            ...
            DO i=snx+olx,1-olx+1,-1
              ...
              ELSE
                ...
                IF (branch .EQ. 0) THEN
                  CALL POPREAL8(rnondim)
                  temp = lrhow(i, j)*smod
                  smodb = -(lrhow(i, j)*rdepth*rnondimb/temp**2)
                ELSE
                  CALL POPREAL8(rnondim)
                  smodb = 0.D0
                END IF
...
C
        ...
        DO j=sny+oly,1-oly+1,-1
          DO i=snx+olx,1-olx,-1
            taperyb(i, j) = 0.D0
            CALL POPREAL8(slopey(i, j))
            tempb = slopeyb(i, j)/dsigmadrs(i, j)
            slopeyb(i, j) = tempb
            dsigmadrsb(i, j) = dsigmadrsb(i, j) - slopey(i, j)*tempb/
     +        dsigmadrs(i, j)
          ENDDO
        ENDDO
        DO j=sny+oly,1-oly+1,-1
          DO i=snx+olx,1-olx,-1
            CALL POPCONTROL1B(branch)
            IF (branch .NE. 0) dsigmadrsb(i, j) = 0.D0
          ENDDO
        ENDDO
        CALL POPCONTROL1B(branch)
        IF (branch .NE. 0) THEN
        ...
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx+1,-1
            taperxb(i, j) = 0.D0
            CALL POPREAL8(slopex(i, j))
            tempb = slopexb(i, j)/dsigmadrw(i, j)
            slopexb(i, j) = tempb
            dsigmadrwb(i, j) = dsigmadrwb(i, j) - slopex(i, j)*tempb/
     +        dsigmadrw(i, j)
          ENDDO
        ENDDO
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx+1,-1
            CALL POPCONTROL1B(branch)
            IF (branch .NE. 0) dsigmadrwb(i, j) = 0.D0
          ENDDO
        ENDDO
      END IF
      END
