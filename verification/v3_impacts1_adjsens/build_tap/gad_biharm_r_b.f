...
C
C  Differentiation of gad_biharm_r in reverse (adjoint) mode:
C   gradient     of useful results: d4f tracer
C   with respect to varying inputs: d4f tracer
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_BIHARM_R_B(bi, bj, k, maskup, diffkr4, tracer, 
     +                          tracerb, d4f, d4fb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C  dfy                  :: vertical diffusive flux
      ...
      REAL*8 d4fb(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
...
C  gradR                :: vertical 1rst derivative of tracer (*rkSign)
      INTEGER i, j, n
      INTEGER kl, km, kp
      REAL*8 del2t(1-olx:snx+olx, 1-oly:sny+oly, 2)
      REAL*8 del2tb(1-olx:snx+olx, 1-oly:sny+oly, 2)
      REAL*8 gradr(1-olx:snx+olx, 1-oly:sny+oly, 3)
      REAL*8 gradrb(1-olx:snx+olx, 1-oly:sny+oly, 3)
      REAL*8 tmpfac
      REAL*8 tempb
      INTEGER*4 branch
      INTEGER ii3
...
C     Calculate vertical gradient @ interface k-1, k & k+1 (n=1:3)
        DO n=1,3
          CALL PUSHINTEGER4(km)
          km = k + n - 3
          CALL PUSHINTEGER4(kl)
          kl = k + n - 2
        ...
        DO ii1=1,2
          DO ii2=1-oly,oly+sny
            DO ii3=1-olx,olx+snx
              del2tb(ii3, ii2, ii1) = 0.D0
            ENDDO
          ENDDO
        ENDDO
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            tempb = ra(i, j, bi, bj)*diffkr4(k)*tmpfac*maskup(i, j)*d4fb
     +        (i, j)
            del2tb(i, j, 2) = del2tb(i, j, 2) + tempb
            del2tb(i, j, 1) = del2tb(i, j, 1) - tempb
          ENDDO
        ENDDO
        ...
        DO n=2,1,-1
          DO j=sny+oly,1-oly,-1
            DO i=snx+olx,1-olx,-1
              tempb = recip_hfacc(i, j, kl, bi, bj)*del2tb(i, j, n)
              del2tb(i, j, n) = 0.D0
              gradrb(i, j, n+1) = gradrb(i, j, n+1) + tempb
              gradrb(i, j, n) = gradrb(i, j, n) - tempb
            ENDDO
          ENDDO
          CALL POPINTEGER4(kl)
        ENDDO
        DO n=3,1,-1
          ...
          IF (branch .EQ. 0) THEN
            DO j=sny+oly,1-oly,-1
              DO i=snx+olx,1-olx,-1
                tempb = maskc(i, j, km, bi, bj)*tmpfac*maskc(i, j, kl, 
     +            bi, bj)*gradrb(i, j, n)
                gradrb(i, j, n) = 0.D0
                tracerb(i, j, kl) = tracerb(i, j, kl) + tempb
                tracerb(i, j, km) = tracerb(i, j, km) - tempb
              ENDDO
            ENDDO
            CALL POPREAL8(tmpfac)
          ELSE
