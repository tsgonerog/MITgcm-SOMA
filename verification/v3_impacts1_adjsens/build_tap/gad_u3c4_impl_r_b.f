...
C
C  Differentiation of gad_u3c4_impl_r in reverse (adjoint) mode:
...
C     !INTERFACE:
      SUBROUTINE GAD_U3C4_IMPL_R_B(bi, bj, k, imin, imax, jmin, jmax, 
     +                             advectionscheme, deltatarg, rtrans, 
     +                             rtransb, recip_hfac, a5d, a5db, b5d, 
     +                             b5db, c5d, c5db, d5d, d5db, e5d, e5db
     +                             , mythid)
      IMPLICIT NONE
C
C     == Global variables ===
...
C--   process interior interface only:
      IF (k .GT. 1 .AND. k .LE. nr) THEN
        ...
        flagc4 = advectionscheme .EQ. enum_centered_4th .AND. k .GT. 2 
     +    .AND. k .LT. nr
C
...
C           rUpwind= (2.D0*rHigh - wCFL)*ABS(rCenter)
     ...
     +        a5db(i, j, k)
            ...
            IF (branch .EQ. 0) THEN
              mskm = maskc(i, j, km2, bi, bj)*maskm2
              mskp = maskc(i, j, kp1, bi, bj)*maskp1
              rcenterb = rcenterb + onesixth*mskp*rc4kpb + onesixth*mskm
     +          *rc4kmb
            ELSE IF (branch .EQ. 1) THEN
              ...
              mskp = maskc(i, j, kp1, bi, bj)*maskp1
              tempb = mskp*rc4kpb
              rhighb = (rcenter-abs3)*tempb
              rcenterb = rcenterb + rhigh*tempb
              abs3b = -(rhigh*tempb)
              ...
              mskm = maskc(i, j, km2, bi, bj)*maskm2
              tempb = mskm*rc4kmb
              rhighb = rhighb + (rcenter+abs2)*tempb
              rcenterb = rcenterb + rhigh*tempb
              abs2b = rhigh*tempb
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
              ...
              ELSE
                ...
                rcenterb = rcenterb - abs2b
              END IF
              rhighb = rhighb + abs1*2.d0*rupwindb
              abs1b = rhigh*2.d0*rupwindb
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                CALL POPREAL8(abs1)
     ...
     +          recip_drc(k)*recip_deepfac2f(k)*wcflb
            ...
            ELSE
              mskp = maskc(i, j, kp1, bi, bj)*maskp1
              tempb = onesixth*mskp*rc4kpb
              rcenterb = rcenterb + tempb
              abs6b = -tempb
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                rcenterb = rcenterb + abs6b
              ELSE
                rcenterb = rcenterb - abs6b
              END IF
              mskm = maskc(i, j, km2, bi, bj)*maskm2
              tempb = onesixth*mskm*rc4kmb
              rcenterb = rcenterb + tempb
              abs5b = tempb
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                rcenterb = rcenterb + abs5b
              ELSE
                rcenterb = rcenterb - abs5b
              END IF
              abs4b = onesixth*2.d0*rupwindb
              CALL POPCONTROL1B(branch)
              IF (branch .EQ. 0) THEN
                rcenterb = rcenterb + abs4b
              ELSE
                rcenterb = rcenterb - abs4b
              END IF
            END IF
