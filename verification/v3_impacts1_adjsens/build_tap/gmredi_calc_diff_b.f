...
C
C  Differentiation of gmredi_calc_diff in reverse (adjoint) mode:
C   gradient     of useful results: kwz kapparx
C   with respect to varying inputs: kwz kapparx
C
C
C
C
...
C
C
      SUBROUTINE GMREDI_CALC_DIFF_B(bi, bj, imin, imax, jmin, jmax, karg
     +                              , ksize, kapparx, kapparxb, 
     +                              traceridentity, mythid)
      IMPLICIT NONE
C
C     == GLobal variables ==
CBOP
...
C
      ...
      REAL*8 kapparxb(1-olx:snx+olx, 1-oly:sny+oly, ksize)
        ...
        DO k=min1,1,-1
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              ...
              IF (branch .EQ. 0) THEN
                kwzb(i, j, k, bi, bj) = kwzb(i, j, k, bi, bj) + maskinc(
     +            i, j, bi, bj)*kapparxb(i, j, k)
              ELSE
                kwzb(i, j, k, bi, bj) = kwzb(i, j, k, bi, bj) + maskinc(
     +            i, j, bi, bj)*kapparxb(i, j, k)
              END IF
      ...
      ELSE
        ...
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) THEN
              kwzb(i, j, karg, bi, bj) = kwzb(i, j, karg, bi, bj) + 
     +          maskinc(i, j, bi, bj)*kapparxb(i, j, k)
            ELSE
              kwzb(i, j, karg, bi, bj) = kwzb(i, j, karg, bi, bj) + 
     +          maskinc(i, j, bi, bj)*kapparxb(i, j, k)
            END IF
