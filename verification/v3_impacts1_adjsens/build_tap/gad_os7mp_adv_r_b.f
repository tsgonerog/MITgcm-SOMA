...
C
C  Differentiation of gad_os7mp_adv_r in reverse (adjoint) mode:
C   gradient     of useful results: wt q wtrans wfld
C   with respect to varying inputs: wt q wtrans wfld
C
C
C
C
...
C
C
      SUBROUTINE GAD_OS7MP_ADV_R_B(bi, bj, k, deltatloc, wtrans, wtransb
     +                             , wfld, wfldb, q, qb, wt, wtb, mythid
     +)
      IMPLICIT NONE
C
C     == GLobal variables ==
...
C     == Local variables ==
      ...
      REAL*8 wlocb, facb, delipb, delib, phib, rp1hb, rp1h_cflb
...
C
          wloc = wfld(i, j)
          IF (wloc*deltatloc*recip_drc(k) .GE. 0.) THEN
            CALL PUSHREAL8(cfl)
            cfl = wloc*deltatloc*recip_drc(k)
      ...
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx,-1
          ...
          IF (branch .EQ. 0) THEN
            ...
            wtransb(i, j) = wtransb(i, j) + (qi+psi*delip)*wtb(i, j)
            tempb = wtrans(i, j)*wtb(i, j)
            wtb(i, j) = 0.D0
            qib = tempb
            psib = delip*tempb
            delipb = psi*tempb
            ...
            END IF
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) rp1h_cflb = rp1h_cflb + 2.d0*x11b
            CALL POPCONTROL1B(branch)
            IF (branch .EQ. 0) phimdb = phimdb + x5b
          ...
          IF (branch .EQ. 0) THEN
            ...
            CALL POPREAL8(mski)
            CALL POPREAL8(mskip)
            qb(i, j, km4) = qb(i, j, km4) + qimmmb
            qb(i, j, km3) = qb(i, j, km3) + qimmb
            qb(i, j, km2) = qb(i, j, km2) + qimb
            CALL POPREAL8(qi)
            qb(i, j, km1) = qb(i, j, km1) + qib
            qb(i, j, k) = qb(i, j, k) + qipb
            qb(i, j, kp1) = qb(i, j, kp1) + qippb
          ELSE IF (branch .EQ. 1) THEN
            ...
            CALL POPREAL8(mski)
            CALL POPREAL8(mskip)
            qb(i, j, kp3) = qb(i, j, kp3) + qimmmb
            qb(i, j, kp2) = qb(i, j, kp2) + qimmb
            qb(i, j, kp1) = qb(i, j, kp1) + qimb
            CALL POPREAL8(qi)
            qb(i, j, k) = qb(i, j, k) + qib
            qb(i, j, km1) = qb(i, j, km1) + qipb
            qb(i, j, km2) = qb(i, j, km2) + qippb
          ELSE
            CALL POPREAL8(mskimmm)
            CALL POPREAL8(mskimm)
            CALL POPREAL8(mskim)
          ...
          IF (branch .EQ. 0) THEN
            CALL POPREAL8(cfl)
            wlocb = deltatloc*recip_drc(k)*cflb
          ELSE
            CALL POPREAL8(cfl)
            wlocb = -(deltatloc*recip_drc(k)*cflb)
          END IF
