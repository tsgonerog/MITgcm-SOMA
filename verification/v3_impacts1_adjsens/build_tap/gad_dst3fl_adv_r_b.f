...
C
C  Differentiation of gad_dst3fl_adv_r in reverse (adjoint) mode:
C   gradient     of useful results: wt tracer rtrans wfld
C   with respect to varying inputs: wt tracer rtrans wfld
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_DST3FL_ADV_R_B(bi, bj, k, dtarg, rtrans, rtransb, 
     +                              wfld, wfldb, tracer, tracerb, wt, 
     +                              wtb, mythid)
      IMPLICIT NONE
C
C     == GLobal variables ==
...
C  wT                :: vertical advective flux
      ...
      REAL*8 wtb(1-olx:snx+olx, 1-oly:sny+oly)
C
C     == Local variables ==
C !LOCAL VARIABLES: ====================================================
C  i,j               :: loop indices
...
C  wCFL              :: Courant-Friedrich-Levy number
      ...
      REAL*8 rjmb, rjb, rjpb, wcflb, d0b, d1b
      REAL*8 psip, psim, thetap, thetam
      REAL*8 psipb, psimb, thetapb, thetamb
      REAL*8 wloc
      REAL*8 wlocb
      REAL*8 thetamax
...
C
      DO j=1-oly,sny+oly
        DO i=1-olx,snx+olx
          rjp = (tracer(i, j, k)-tracer(i, j, kp1))*maskc(i, j, kp1, bi
     +      , bj)
          CALL PUSHREAL8(rj)
          rj = (tracer(i, j, km1)-tracer(i, j, k))*maskc(i, j, k, bi, bj
     +      )*maskc(i, j, km1, bi, bj)
          rjm = (tracer(i, j, km2)-tracer(i, j, km1))*maskc(i, j, km1, 
     +      bi, bj)
C
          wloc = wfld(i, j)
          IF (wloc*dtarg*recip_drc(k) .GE. 0.) THEN
            CALL PUSHREAL8(wcfl)
            wcfl = wloc*dtarg*recip_drc(k)
...
C
          ...
          ELSE
            ...
            CALL PUSHCONTROL1B(1)
          END IF
          IF (x1 .GT. thetap*(1.d0-wcfl)/(wcfl+1.d-20)) THEN
            y1 = thetap*(1.d0-wcfl)/(wcfl+1.d-20)
            CALL PUSHCONTROL1B(0)
          ELSE
            y1 = x1
            CALL PUSHCONTROL1B(1)
          ...
          ELSE
            ...
            CALL PUSHCONTROL1B(1)
          END IF
          IF (x2 .GT. thetam*(1.d0-wcfl)/(wcfl+1.d-20)) THEN
            y2 = thetam*(1.d0-wcfl)/(wcfl+1.d-20)
            CALL PUSHCONTROL1B(0)
          ELSE
            y2 = x2
            CALL PUSHCONTROL1B(1)
      ...
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx,-1
          tempb = (tracer(i, j, k)+psim*rj)*0.5*wtb(i, j)
          tempb0 = (rtrans(i, j)+abs2)*0.5*wtb(i, j)
          tempb1 = (tracer(i, j, km1)-psip*rj)*0.5*wtb(i, j)
          tempb2 = (rtrans(i, j)-abs5)*0.5*wtb(i, j)
          wtb(i, j) = 0.D0
          tracerb(i, j, km1) = tracerb(i, j, km1) + tempb2
          psipb = -(rj*tempb2)
          rjb = psim*tempb0 - psip*tempb2
          rtransb(i, j) = rtransb(i, j) + tempb1 + tempb
          abs5b = -tempb1
          ...
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            tempb = y2b/(wcfl+1.d-20)
            thetamb = (1.d0-wcfl)*tempb
            wcflb = -((thetam+thetam*(1.d0-wcfl)/(wcfl+1.d-20))*tempb)
            x2b = 0.D0
          ELSE
            x2b = y2b
            wcflb = 0.D0
          ...
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            tempb = y1b/(wcfl+1.d-20)
            thetapb = (1.d0-wcfl)*tempb
            wcflb = wcflb - (thetap+thetap*(1.d0-wcfl)/(wcfl+1.d-20))*
     +        tempb
            x1b = 0.D0
          ELSE
            x1b = y1b
     ...
     +      bi, bj)
          ...
          ELSE
            ...
            rjmb = 0.D0
          END IF
          wcflb = wcflb - 2*wcfl*onesixth*d1b - ((1.d0-wcfl)*onesixth+(
     +      2.d0-wcfl)*onesixth)*d0b
          CALL POPCONTROL1B(branch)
          IF (branch .EQ. 0) THEN
            CALL POPREAL8(wcfl)
            wlocb = dtarg*recip_drc(k)*wcflb
          ELSE
            CALL POPREAL8(wcfl)
            wlocb = -(dtarg*recip_drc(k)*wcflb)
          END IF
          wfldb(i, j) = wfldb(i, j) + wlocb
          tempb = maskc(i, j, km1, bi, bj)*rjmb
          tracerb(i, j, km2) = tracerb(i, j, km2) + tempb
          tracerb(i, j, km1) = tracerb(i, j, km1) - tempb
          CALL POPREAL8(rj)
          tempb = maskc(i, j, k, bi, bj)*maskc(i, j, km1, bi, bj)*rjb
          tracerb(i, j, km1) = tracerb(i, j, km1) + tempb
          tracerb(i, j, k) = tracerb(i, j, k) - tempb
          tempb = maskc(i, j, kp1, bi, bj)*rjpb
          tracerb(i, j, k) = tracerb(i, j, k) + tempb
          tracerb(i, j, kp1) = tracerb(i, j, kp1) - tempb
        ENDDO
      ENDDO
      END
