...
C
C  Differentiation of ctrl_swapffields in reverse (adjoint) mode:
C   gradient     of useful results: ffld0
C   with respect to varying inputs: ffld0 ffld1
C
C
C
C
...
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
C
      SUBROUTINE CTRL_SWAPFFIELDS_B(ffld0, ffld0b, ffld1, ffld1b, mythid
     +)
      IMPLICIT NONE
C
C     == global variables ==
...
C     == routine arguments ==
      ...
      REAL*8 ffld0b(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      REAL*8 ffld1(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      REAL*8 ffld1b(1-olx:snx+olx, 1-oly:sny+oly, nsx, nsy)
      INTEGER mythid
C
C     == local variables ==
      INTEGER bi, bj
      ...
      DO bj=mybyhi(mythid),mybylo(mythid),-1
        DO bi=mybxhi(mythid),mybxlo(mythid),-1
          DO j=sny,1,-1
            DO i=snx,1,-1
              ffld1b(i, j, bi, bj) = ffld1b(i, j, bi, bj) + ffld0b(i, j
     +          , bi, bj)
              ffld0b(i, j, bi, bj) = 0.D0
            ENDDO
          ENDDO
        ENDDO
      ENDDO
