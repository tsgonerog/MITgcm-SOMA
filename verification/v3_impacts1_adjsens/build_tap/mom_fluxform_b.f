...
C
C  Differentiation of mom_fluxform in reverse (adjoint) mode:
C   gradient     of useful results: uvel vvel wvel gu gv gvdiss
C                fverukm fverukp fvervkm fvervkp gudiss
C   with respect to varying inputs: uvel vvel wvel gu gv gvdiss
C                fverukm fverukp fvervkm fvervkp gudiss
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_FLUXFORM_B(bi, bj, k, imin, imax, jmin, jmax, 
     +                          kapparu, kapparv, fverukm, fverukmb, 
     +                          fvervkm, fvervkmb, fverukp, fverukpb, 
     +                          fvervkp, fvervkpb, gudiss, gudissb, 
     +                          gvdiss, gvdissb, mytime, myiter, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
C    !INTERFACE:
...
C
     ...
     +surfaceforcingtb, surfaceforcingsb
...
C  myThid               :: my Thread Id number
      ...
      REAL*8 fverukmb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 fvervkm(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 fvervkmb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 fverukp(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 fverukpb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 fvervkp(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 fvervkpb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gudiss(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gudissb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gvdiss(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 gvdissb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 mytime
      INTEGER myiter
      INTEGER mythid
C
...
C  fVrUp,fVrDw          :: vertical viscous fluxes at interface k & k+1
      ...
      REAL*8 vfb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 v4f(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 v4fb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 ucf(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 ucfb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 vcf(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 vcfb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 mt(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 mtb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 fzon(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 fzonb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 fmer(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 fmerb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 fvrup(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 fvrupb(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 fvrdw(1-olx:snx+olx, 1-oly:sny+oly)
      REAL*8 fvrdwb(1-olx:snx+olx, 1-oly:sny+oly)
...
C
     ...
     +    selectbotdragquadr .GE. 0) .OR. bottomdraglinear .NE. 0.)) 
     +THEN
...
C     Calculate tracer cell face open areas
      DO j=1-oly,sny+oly
        DO i=1-olx,snx+olx
          xa(i, j) = dyg(i, j, bi, bj)*deepfacc(k)*drf(k)*hfacw(i, j, k
     +      , bi, bj)
          ya(i, j) = dxg(i, j, bi, bj)*deepfacc(k)*drf(k)*hfacs(i, j, k
     +      , bi, bj)
...
C---  Calculate vertical transports (at k+1) below U & V points :
      IF (momadvection) THEN
        arg1 = k + 1
        CALL PUSHREAL8ARRAY(rtransv, 70**2)
        CALL PUSHREAL8ARRAY(rtransu, 70**2)
        CALL MOM_CALC_RTRANS(arg1, bi, bj, rtransu, rtransv, mytime, 
     +                       myiter, mythid)
        CALL PUSHCONTROL1B(0)
      ELSE
        CALL PUSHCONTROL1B(1)
...
C
      IF (momviscosity) THEN
        ...
        ENDDO
        IF (usevariablevisc) THEN
          CALL PUSHREAL8ARRAY(hdiv, 70**2)
          CALL MOM_CALC_VISC(bi, bj, k, viscah_z, viscah_d, visca4_z, 
     +                       visca4_d, hdiv, vort3, tension, strain, 
     +                       stretching, ke, hfacz, mythid)
...
C-    No-slip BCs impose a drag at bottom
        IF (bottomdragterms) THEN
          CALL PUSHREAL8ARRAY(ke, 70**2)
          CALL MOM_U_BOTDRAG_COEFF(bi, bj, k, .true., ufld, vfld, 
     +                             kapparu, ke, cdrag, myiter, mythid)
          CALL PUSHCONTROL2B(0)
        ELSE
          CALL PUSHCONTROL2B(1)
        END IF
...
C-    No-slip BCs impose a drag at bottom
        IF (bottomdragterms) THEN
          CALL PUSHREAL8ARRAY(cdrag, 70**2)
          CALL PUSHREAL8ARRAY(ke, 70**2)
          CALL MOM_V_BOTDRAG_COEFF(bi, bj, k, .true., ufld, vfld, 
     +                             kapparv, ke, cdrag, myiter, mythid)
          CALL PUSHCONTROL2B(0)
        ELSE
          CALL PUSHCONTROL2B(1)
        END IF
...
C--   3.D Coriolis term (horizontal momentum, Eastward component: -fprime*w)
      ...
      DO j=jmax,jmin,-1
        DO i=imax,imin,-1
          gvdissb(i, j) = masks(i, j, k, bi, bj)*gvdissb(i, j)
          gvb(i, j, k, bi, bj) = masks(i, j, k, bi, bj)*gvb(i, j, k, bi
     +      , bj)
          gudissb(i, j) = maskw(i, j, k, bi, bj)*gudissb(i, j)
          gub(i, j, k, bi, bj) = maskw(i, j, k, bi, bj)*gub(i, j, k, bi
     +      , bj)
        ENDDO
      ENDDO
      ...
      ELSE
        ...
        ELSE
          ...
          DO j=jmax,jmin,-1
            DO i=imax,imin,-1
              vcfb(i, j) = vcfb(i, j) + fvfac*gvb(i, j, k, bi, bj)
            ENDDO
          ENDDO
          CALL MOM_V_CORIOLIS_NH_B(bi, bj, k, wvel, wvelb, vcf, vcfb, 
     +                             mythid)
        END IF
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
        ...
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            ucfb(i, j) = ucfb(i, j) + fufac*gub(i, j, k, bi, bj)
          ENDDO
        ENDDO
        CALL MOM_U_CORIOLIS_NH_B(bi, bj, k, wvel, wvelb, ucf, ucfb, 
     +                           mythid)
      END IF
      ...
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          ...
          ENDDO
        ENDDO
        CALL MOM_V_CORIOLIS_B(bi, bj, k, ufld, ufldb, vcf, vcfb, mythid)
        CALL MOM_U_CORIOLIS_B(bi, bj, k, vfld, vfldb, ucf, ucfb, mythid)
      ELSE
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            vfldb(ii2, ii1) = 0.D0
      ...
      IF (branch .EQ. 0) THEN
        ...
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            mtb(i, j) = mtb(i, j) + mtfacv*gvb(i, j, k, bi, bj)
          ENDDO
        ENDDO
        CALL MOM_V_METRIC_CYLINDER_B(bi, bj, k, ufld, ufldb, vfld, mt, 
     +                               mtb, mythid)
      ELSE
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
      ...
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          ...
          ENDDO
        ENDDO
        CALL MOM_V_METRIC_SPHERE_B(bi, bj, k, ufld, ufldb, mt, mtb, 
     +                             mythid)
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            mtb(i, j) = mtb(i, j) + mtnhfacv*gvb(i, j, k, bi, bj)
          ENDDO
        ENDDO
        CALL MOM_V_METRIC_NH_B(bi, bj, k, vfld, vfldb, wvel, wvelb, mt, 
     +                         mtb, mythid)
      END IF
      CALL POPCONTROL2B(branch)
      IF (branch .EQ. 0) THEN
        ...
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            tempb0 = -(recip_hfacs(i, j, k, bi, bj)*recip_drf(k)*gvdissb
     +        (i, j))
            cdragb(i, j) = cdragb(i, j) + vfld(i, j)*tempb0
            vfldb(i, j) = vfldb(i, j) + cdrag(i, j)*tempb0
          ENDDO
        ENDDO
        CALL POPREAL8ARRAY(ke, 70**2)
        CALL POPREAL8ARRAY(cdrag, 70**2)
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            keb(ii2, ii1) = 0.D0
          ENDDO
        ENDDO
        CALL MOM_V_BOTDRAG_COEFF_B(bi, bj, k, .true., ufld, ufldb, vfld
     +                             , vfldb, kapparv, ke, keb, cdrag, 
     +                             cdragb, myiter, mythid)
      ...
      IF (branch .EQ. 0) THEN
        ...
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            vfb(i, j) = vfb(i, j) + gvdissb(i, j)
          ENDDO
        ENDDO
        CALL MOM_V_SIDEDRAG_B(bi, bj, k, vfld, vfldb, v4f, v4fb, h0facz
     +                        , viscah_z, viscah_zb, visca4_z, visca4_zb
     +                        , useharmonicvisc, usebiharmonicvisc, 
     +                        usevariablevisc, vf, vfb, mythid)
      ELSE
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            v4fb(ii2, ii1) = 0.D0
      ...
      DO j=jmax,jmin,-1
        DO i=imax,imin,-1
          tempb0 = -(recip_hfacs(i, j, k, bi, bj)*recip_ras(i, j, bi, bj
     +      )*recip_drf(k)*recip_deepfac2c(k)*gvdissb(i, j))
          gvdissb(i, j) = 0.D0
          fzonb(i+1, j) = fzonb(i+1, j) + ahdvdxfac*tempb0
          fzonb(i, j) = fzonb(i, j) - ahdvdxfac*tempb0
          fmerb(i, j) = fmerb(i, j) + ahdvdyfac*tempb0
          fmerb(i, j-1) = fmerb(i, j-1) - ahdvdyfac*tempb0
          tempb = rksign*ardvdrfac*recip_rhofacc(k)*tempb0
          fvrdwb(i, j) = fvrdwb(i, j) + tempb
          fvrupb(i, j) = fvrupb(i, j) - tempb
        ENDDO
      ENDDO
      CALL POPCONTROL1B(branch)
      IF (branch .NE. 0) THEN
        CALL MOM_V_RVISCFLUX_B(bi, bj, arg1, vvel, vvelb, kapparv, fvrdw
     +                         , fvrdwb, mythid)
        CALL MOM_V_RVISCFLUX_B(bi, bj, k, vvel, vvelb, kapparv, fvrup, 
     +                         fvrupb, mythid)
      END IF
      CALL MOM_V_YVISCFLUX_B(bi, bj, k, vfld, vfldb, v4f, v4fb, fmer, 
     +                       fmerb, viscah_d, viscah_db, visca4_d, 
     +                       visca4_db, mythid)
      CALL MOM_V_XVISCFLUX_B(bi, bj, k, vfld, vfldb, v4f, v4fb, hfacz, 
     +                       fzon, fzonb, viscah_z, viscah_zb, visca4_z
     +                       , visca4_zb, mythid)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL POPREAL8ARRAY(v4f, 70**2)
        CALL MOM_V_DEL2V_B(bi, bj, k, vfld, vfldb, hfacz, h0facz, v4f, 
     +                     v4fb, mythid)
      END IF
 100  CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            tempb0 = -(recip_hfacs(i, j, k, bi, bj)*recip_drf(k)*
     +        recip_deepfac2c(k)*recip_ras(i, j, bi, bj)*recip_rhofacc(k
     +        )*gvb(i, j, k, bi, bj))
            gvb(i, j, k, bi, bj) = 0.D0
            fzonb(i+1, j) = fzonb(i+1, j) + udvdxfac*tempb0
            fzonb(i, j) = fzonb(i, j) - udvdxfac*tempb0
            fmerb(i, j) = fmerb(i, j) + vdvdyfac*tempb0
            fmerb(i, j-1) = fmerb(i, j-1) - vdvdyfac*tempb0
            tempb = rksign*rveldvdrfac*tempb0
            fvervkpb(i, j) = fvervkpb(i, j) + tempb
            fvervkmb(i, j) = fvervkmb(i, j) - tempb
          ENDDO
        ENDDO
        arg1 = k + 1
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            rtransvb(ii2, ii1) = 0.D0
          ENDDO
        ENDDO
        CALL MOM_V_ADV_WV_B(bi, bj, arg1, vvel, vvelb, wvel, wvelb, 
     +                      rtransv, rtransvb, fvervkp, fvervkpb, mythid
     +                     )
        CALL MOM_V_ADV_VV_B(bi, bj, k, vtrans, vtransb, vfld, vfldb, 
     +                      fmer, fmerb, mythid)
        CALL MOM_V_ADV_UV_B(bi, bj, k, utrans, utransb, vfld, vfldb, 
     +                      fzon, fzonb, mythid)
      ELSE
      ...
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            mtb(i, j) = mtb(i, j) + mtfacu*gub(i, j, k, bi, bj)
          ENDDO
        ENDDO
        CALL MOM_U_METRIC_CYLINDER_B(bi, bj, k, ufld, ufldb, vfld, vfldb
     +                               , mt, mtb, mythid)
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          ...
          ENDDO
        ENDDO
        CALL MOM_U_METRIC_SPHERE_B(bi, bj, k, ufld, ufldb, vfld, vfldb, 
     +                             mt, mtb, mythid)
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            mtb(i, j) = mtb(i, j) + mtnhfacu*gub(i, j, k, bi, bj)
          ENDDO
        ENDDO
        CALL MOM_U_METRIC_NH_B(bi, bj, k, ufld, ufldb, wvel, wvelb, mt, 
     +                         mtb, mythid)
      END IF
      CALL POPCONTROL2B(branch)
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            tempb0 = -(recip_hfacw(i, j, k, bi, bj)*recip_drf(k)*gudissb
     +        (i, j))
            cdragb(i, j) = cdragb(i, j) + ufld(i, j)*tempb0
            ufldb(i, j) = ufldb(i, j) + cdrag(i, j)*tempb0
          ENDDO
        ENDDO
        CALL POPREAL8ARRAY(ke, 70**2)
        CALL MOM_U_BOTDRAG_COEFF_B(bi, bj, k, .true., ufld, ufldb, vfld
     +                             , vfldb, kapparu, ke, keb, cdrag, 
     +                             cdragb, myiter, mythid)
      ...
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            vfb(i, j) = vfb(i, j) + gudissb(i, j)
          ENDDO
        ENDDO
        CALL MOM_U_SIDEDRAG_B(bi, bj, k, ufld, ufldb, v4f, v4fb, h0facz
     +                        , viscah_z, viscah_zb, visca4_z, visca4_zb
     +                        , useharmonicvisc, usebiharmonicvisc, 
     +                        usevariablevisc, vf, vfb, mythid)
      END IF
      DO j=jmax,jmin,-1
        DO i=imax,imin,-1
          tempb0 = -(recip_hfacw(i, j, k, bi, bj)*recip_raw(i, j, bi, bj
     +      )*recip_drf(k)*recip_deepfac2c(k)*gudissb(i, j))
          gudissb(i, j) = 0.D0
          fzonb(i, j) = fzonb(i, j) + ahdudxfac*tempb0
          fzonb(i-1, j) = fzonb(i-1, j) - ahdudxfac*tempb0
          fmerb(i, j+1) = fmerb(i, j+1) + ahdudyfac*tempb0
          fmerb(i, j) = fmerb(i, j) - ahdudyfac*tempb0
          tempb = rksign*ardudrfac*recip_rhofacc(k)*tempb0
          fvrdwb(i, j) = fvrdwb(i, j) + tempb
          fvrupb(i, j) = fvrupb(i, j) - tempb
        ENDDO
      ENDDO
      ...
      IF (branch .NE. 0) THEN
        arg1 = k + 1
        CALL MOM_U_RVISCFLUX_B(bi, bj, arg1, uvel, uvelb, kapparu, fvrdw
     +                         , fvrdwb, mythid)
        CALL MOM_U_RVISCFLUX_B(bi, bj, k, uvel, uvelb, kapparu, fvrup, 
     +                         fvrupb, mythid)
      END IF
      CALL MOM_U_YVISCFLUX_B(bi, bj, k, ufld, ufldb, v4f, v4fb, hfacz, 
     +                       fmer, fmerb, viscah_z, viscah_zb, visca4_z
     +                       , visca4_zb, mythid)
      CALL MOM_U_XVISCFLUX_B(bi, bj, k, ufld, ufldb, v4f, v4fb, fzon, 
     +                       fzonb, viscah_d, viscah_db, visca4_d, 
     +                       visca4_db, mythid)
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) CALL MOM_U_DEL2U_B(bi, bj, k, ufld, ufldb, 
     +                                      hfacz, h0facz, v4f, v4fb, 
     +                                      mythid)
 110  CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        DO j=jmax,jmin,-1
          DO i=imax,imin,-1
            tempb = -(recip_hfacw(i, j, k, bi, bj)*recip_drf(k)*
     +        recip_deepfac2c(k)*recip_raw(i, j, bi, bj)*recip_rhofacc(k
     +        )*gub(i, j, k, bi, bj))
            gub(i, j, k, bi, bj) = 0.D0
            fzonb(i, j) = fzonb(i, j) + ududxfac*tempb
            fzonb(i-1, j) = fzonb(i-1, j) - ududxfac*tempb
            fmerb(i, j+1) = fmerb(i, j+1) + vdudyfac*tempb
            fmerb(i, j) = fmerb(i, j) - vdudyfac*tempb
            tempb0 = rksign*rveldudrfac*tempb
            fverukpb(i, j) = fverukpb(i, j) + tempb0
            fverukmb(i, j) = fverukmb(i, j) - tempb0
          ENDDO
        ENDDO
        arg1 = k + 1
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            rtransub(ii2, ii1) = 0.D0
          ENDDO
        ENDDO
        CALL MOM_U_ADV_WU_B(bi, bj, arg1, uvel, uvelb, wvel, wvelb, 
     +                      rtransu, rtransub, fverukp, fverukpb, mythid
     +                     )
        CALL MOM_U_ADV_VU_B(bi, bj, k, vtrans, vtransb, ufld, ufldb, 
     +                      fmer, fmerb, mythid)
        CALL MOM_U_ADV_UU_B(bi, bj, k, utrans, utransb, ufld, ufldb, 
     +                      fzon, fzonb, mythid)
      ELSE
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
      ...
      IF (branch .EQ. 0) THEN
        ...
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            tensionb(ii2, ii1) = 0.D0
          ENDDO
        ENDDO
        CALL MOM_CALC_VISC_B(bi, bj, k, viscah_z, viscah_zb, viscah_d, 
     +                       viscah_db, visca4_z, visca4_zb, visca4_d, 
     +                       visca4_db, hdiv, hdivb, vort3, vort3b, 
     +                       tension, tensionb, strain, strainb, 
     +                       stretching, ke, keb, hfacz, mythid)
      ELSE IF (branch .EQ. 1) THEN
        DO ii1=1-oly,oly+sny
          DO ii2=1-olx,olx+snx
            hdivb(ii2, ii1) = 0.D0
      ...
      IF (branch .EQ. 0) THEN
        arg1 = k + 1
        CALL POPREAL8ARRAY(rtransu, 70**2)
        CALL POPREAL8ARRAY(rtransv, 70**2)
        CALL MOM_CALC_RTRANS_B(arg1, bi, bj, rtransu, rtransub, rtransv
     +                         , rtransvb, mytime, myiter, mythid)
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        CALL MOM_V_ADV_WV_B(bi, bj, k, vvel, vvelb, wvel, wvelb, rtransv
     +                      , rtransvb, fvervkm, fvervkmb, mythid)
        CALL MOM_U_ADV_WU_B(bi, bj, k, uvel, uvelb, wvel, wvelb, rtransu
     +                      , rtransub, fverukm, fverukmb, mythid)
        CALL MOM_CALC_RTRANS_B(k, bi, bj, rtransu, rtransub, rtransv, 
     +                         rtransvb, mytime, myiter, mythid)
      END IF
      CALL POPCONTROL1B(branch)
      IF (branch .EQ. 0) THEN
        DO j=sny+oly,1-oly,-1
          DO i=snx+olx,1-olx,-1
            CALL POPCONTROL1B(branch)
            IF (branch .NE. 0) THEN
              strainb(i, j) = sidemaskfac*strainb(i, j)
              vort3b(i, j) = sidemaskfac*vort3b(i, j)
            END IF
          ENDDO
        ENDDO
        CALL MOM_CALC_STRAIN_B(bi, bj, k, ufld, ufldb, vfld, vfldb, 
     +                         hfacz, strain, strainb, mythid)
        CALL MOM_CALC_TENSION_B(bi, bj, k, ufld, ufldb, vfld, vfldb, 
     +                          tension, tensionb, mythid)
        CALL MOM_CALC_RELVORT3_B(bi, bj, k, ufld, ufldb, vfld, vfldb, 
     +                           hfacz, vort3, vort3b, mythid)
        CALL MOM_CALC_HDIV_B(bi, bj, k, 2, ufld, ufldb, vfld, vfldb, 
     +                       hdiv, hdivb, mythid)
      END IF
      CALL MOM_CALC_KE_B(bi, bj, k, 2, ufld, ufldb, vfld, vfldb, ke, keb
     +                   , mythid)
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx,-1
          vfldb(i, j) = vfldb(i, j) + ya(i, j)*rhofacc(k)*vtransb(i, j)
          vtransb(i, j) = 0.D0
          ufldb(i, j) = ufldb(i, j) + xa(i, j)*rhofacc(k)*utransb(i, j)
          utransb(i, j) = 0.D0
        ENDDO
      ENDDO
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx,-1
          vvelb(i, j, k, bi, bj) = vvelb(i, j, k, bi, bj) + vfldb(i, j)
          vfldb(i, j) = 0.D0
          uvelb(i, j, k, bi, bj) = uvelb(i, j, k, bi, bj) + ufldb(i, j)
          ufldb(i, j) = 0.D0
        ENDDO
      ENDDO
      DO j=sny+oly,1-oly,-1
