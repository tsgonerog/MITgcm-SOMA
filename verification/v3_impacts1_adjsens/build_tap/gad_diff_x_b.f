...
C
C  Differentiation of gad_diff_x in reverse (adjoint) mode:
C   gradient     of useful results: tracer dfx
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE GAD_DIFF_X_B(bi, bj, k, xa, diffkh, tracer, tracerb, 
     +                        dfx, dfxb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
...
C  i,j                  :: loop indices
      ...
      DO j=sny+oly,1-oly,-1
        DO i=snx+olx,1-olx+1,-1
          tempb = -(xa(i, j)*cosfacu(j, bi, bj)*recip_dxc(i, j, bi, bj)*
     +      diffkh*recip_deepfacc(k)*dfxb(i, j))
          dfxb(i, j) = 0.D0
