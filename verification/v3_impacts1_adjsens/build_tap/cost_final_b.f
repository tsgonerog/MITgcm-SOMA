...
C
C  Differentiation of cost_final in reverse (adjoint) mode:
C   gradient     of useful results: fc
C   with respect to varying inputs: fc /dynvars_r/[3684800,4900000[
C
C
C
C
...
C   ==================================================================
C
      SUBROUTINE COST_FINAL_B(mythid)
      IMPLICIT NONE
C
C     == global variables ==
CBOP
...
C
C
          tile_fc(bi, bj) = tile_fc(bi, bj) + mult_test*objf_test(bi, bj
     +      )
C
C
C
...
C--   Do global summation.
      ...
      DO ii1=1,nsy
        DO ii2=1,nsx
          tile_fcb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      CALL GLOBAL_SUM_TILE_RL_B(tile_fc, tile_fcb, glob_fc, glob_fcb, 
     +                          mythid)
      DO ii1=1,nsy
        DO ii2=1,nsx
          objf_testb(ii2, ii1) = 0.D0
        ENDDO
      ENDDO
      DO bj=mybyhi(mythid),mybylo(mythid),-1
        DO bi=mybxhi(mythid),mybxlo(mythid),-1
          objf_testb(bi, bj) = objf_testb(bi, bj) + mult_test*tile_fcb(
     +      bi, bj)
        ENDDO
      ENDDO
      CALL COST_TEST_B(mythid)
      END
