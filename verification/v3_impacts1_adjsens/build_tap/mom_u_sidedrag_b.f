...
C
C  Differentiation of mom_u_sidedrag in reverse (adjoint) mode:
C   gradient     of useful results: ufld visca4_z del2u udragterms
C                viscah_z
C   with respect to varying inputs: ufld visca4_z del2u viscah_z
C
C
C
C
...
C
C !INTERFACE: ==========================================================
      SUBROUTINE MOM_U_SIDEDRAG_B(bi, bj, k, ufld, ufldb, del2u, del2ub
     +                            , hfacz, viscah_z, viscah_zb, visca4_z
     +                            , visca4_zb, harmonic, biharmonic, 
     +                            usevariableviscosity, udragterms, 
     +                            udragtermsb, mythid)
      IMPLICIT NONE
CBOP
C    !ROUTINE: SIZE.h
...
C  uDragTerms           :: drag term
      ...
      REAL*8 udragtermsb(1-olx:snx+olx, 1-oly:sny+oly)
C
C !LOCAL VARIABLES: ====================================================
C  i,j                  :: loop indices
...
C
        DO j=2-oly,sny+oly-1
          DO i=2-olx,snx+olx-1
            x1 = viscah + viscahgrid*raw(i, j, bi, bj)/deltatmom
            IF (x1 .GT. viscahmax) THEN
              CALL PUSHREAL8(ahtmp)
              ahtmp = viscahmax
            ...
            END IF
            IF (visca4 + visca4grid*raw(i, j, bi, bj)**2/deltatmom .GT. 
     +          visca4max) THEN
            ...
            ELSE
              CALL PUSHREAL8(a4tmp)
              a4tmp = visca4 + visca4grid*raw(i, j, bi, bj)**2/deltatmom
              CALL PUSHCONTROL1B(1)
            END IF
            IF (a4tmp .GT. visca4gridmax*raw(i, j, bi, bj)**2/deltatmom
     +      ) THEN
              a4tmp = visca4gridmax*raw(i, j, bi, bj)**2/deltatmom
            ELSE
              a4tmp = a4tmp
            END IF
            IF (a4tmp .LT. visca4gridmin*raw(i, j, bi, bj)**2/deltatmom
     +      ) THEN
              a4tmp = visca4gridmin*raw(i, j, bi, bj)**2/deltatmom
            ELSE
              a4tmp = a4tmp
            END IF
          ENDDO
        ...
        DO j=sny+oly-1,2-oly,-1
          DO i=snx+olx-1,2-olx,-1
            ...
            tempb = -(recip_drf(k)*2.*drf(k)*recip_hfacw(i, j, k, bi, bj
     +        )*recip_raw(i, j, bi, bj)*(hfaczcloseds*dxv(i, j, bi, bj)*
     +        recip_dyu(i, j, bi, bj)+hfaczclosedn*dxv(i, j+1, bi, bj)*
     +        recip_dyu(i, j+1, bi, bj))*udragtermsb(i, j))
            udragtermsb(i, j) = 0.D0
            ufldb(i, j) = ufldb(i, j) + ahtmp*tempb
            del2ub(i, j) = del2ub(i, j) - a4tmp*tempb
      ...
      ELSE
        DO j=sny+oly-1,2-oly,-1
          DO i=snx+olx-1,2-olx,-1
            hfaczclosedn = hfacw(i, j, k, bi, bj) - hfacz(i, j+1)
            hfaczcloseds = hfacw(i, j, k, bi, bj) - hfacz(i, j)
            tempb = -(recip_hfacw(i, j, k, bi, bj)*recip_raw(i, j, bi, 
     +        bj)*recip_drf(k)*sidedragfactor*drf(k)*udragtermsb(i, j))
            udragtermsb(i, j) = 0.D0
            tempb0 = hfaczcloseds*dxv(i, j, bi, bj)*recip_dyu(i, j, bi, 
     +        bj)*tempb
            tempb1 = hfaczclosedn*dxv(i, j+1, bi, bj)*recip_dyu(i, j+1, 
     +        bi, bj)*tempb
            viscah_zb(i, j+1) = viscah_zb(i, j+1) + ufld(i, j)*tempb1
            ufldb(i, j) = ufldb(i, j) + viscah_z(i, j+1)*tempb1 + 
     +        viscah_z(i, j)*tempb0
            visca4_zb(i, j+1) = visca4_zb(i, j+1) - del2u(i, j)*tempb1
            del2ub(i, j) = del2ub(i, j) - visca4_z(i, j+1)*tempb1 - 
     +        visca4_z(i, j)*tempb0
            viscah_zb(i, j) = viscah_zb(i, j) + ufld(i, j)*tempb0
            visca4_zb(i, j) = visca4_zb(i, j) - del2u(i, j)*tempb0
          ENDDO
        ENDDO
      END IF
      END
