...
C
C  Differentiation of mom_vi_del2uv in reverse (adjoint) mode:
C   gradient     of useful results: hdiv del2u del2v vort3
C   with respect to varying inputs: hdiv vort3
C
C
C
C
...
C
C
      SUBROUTINE MOM_VI_DEL2UV_B(bi, bj, k, hdiv, hdivb, vort3, vort3b, 
     +                           hfacz, del2u, del2ub, del2v, del2vb, 
     +                           mythid)
      IMPLICIT NONE
C
...
Cph-exch2#ifndef ALLOW_AUTODIFF_TAMC
      ...
      DO j=sny+oly-1,2-oly,-1
        DO i=snx+olx-1,2-olx,-1
          tempb1 = masks(i, j, k, bi, bj)*recip_deepfacc(k)*del2vb(i, j)
          del2vb(i, j) = 0.D0
          tempb0 = recip_dyc(i, j, bi, bj)*tempb1
          tempb = recip_hfacs(i, j, k, bi, bj)*recip_dxg(i, j, bi, bj)*
     +      tempb1
          vort3b(i+1, j) = vort3b(i+1, j) + hfacz(i+1, j)*tempb
          vort3b(i, j) = vort3b(i, j) - hfacz(i, j)*tempb
          hdivb(i, j) = hdivb(i, j) + tempb0
          hdivb(i, j-1) = hdivb(i, j-1) - tempb0
        ENDDO
      ENDDO
     ...
     +                                               mythid)
      DO j=sny+oly-1,2-oly,-1
        DO i=snx+olx-1,2-olx,-1
          tempb = maskw(i, j, k, bi, bj)*recip_deepfacc(k)*del2ub(i, j)
          del2ub(i, j) = 0.D0
          tempb0 = recip_dxc(i, j, bi, bj)*tempb
          tempb1 = -(recip_hfacw(i, j, k, bi, bj)*recip_dyg(i, j, bi, bj
     +      )*tempb)
          vort3b(i, j+1) = vort3b(i, j+1) + hfacz(i, j+1)*tempb1
          vort3b(i, j) = vort3b(i, j) - hfacz(i, j)*tempb1
          hdivb(i, j) = hdivb(i, j) + tempb0
          hdivb(i-1, j) = hdivb(i-1, j) - tempb0
        ENDDO
      ENDDO
      CALL POPCONTROL1B(branch)
